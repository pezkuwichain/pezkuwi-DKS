...
/// The registration strategy that will be used for the teyrchain.
#[derive(Debug, Clone, PartialEq)]
pub enum RegistrationStrategy {
	/// The teyrchain will be added to the genesis before spawning.
	InGenesis,
	/// The teyrchain will be registered using an extrinsic after spawning.
	UsingExtrinsic,
	/// The teyrchaing will not be registered and the user can doit after spawning manually.
	Manual,
}
...
/// A teyrchain configuration, composed of collators and fine-grained configuration options.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct TeyrchainConfig {
	id: u32,
	#[serde(skip)]
	// unique_id is internally used to allow multiple teyrchains with the same id
	...
	// with `[teyrchain.collator]` key.
	// NOTE: if the file also contains multiple collators defined in
	// `[[teyrchain.collators]], the single configuration will be added to the bottom.
...
impl TeyrchainConfig {
	/// The teyrchain ID.
	pub fn id(&self) -> u32 {
		self.id
	}

	/// The teyrchain unique ID.
	...
	/// The registration strategy for the teyrchain.
	pub fn registration_strategy(&self) -> Option<&RegistrationStrategy> {
		self.registration_strategy.as_ref()
	}

	/// Whether the teyrchain should be onboarded or stay a parathread
	pub fn onboard_as_teyrchain(&self) -> bool {
		self.onboard_as_teyrchain
	}

	/// Backward compatibility alias for onboard_as_teyrchain().
	...
	/// The location of a pre-existing genesis WASM runtime blob of the teyrchain.
	pub fn genesis_wasm_path(&self) -> Option<&AssetLocation> {
		self.genesis_wasm_path.as_ref()
	}

	/// The generator command used to create the genesis WASM runtime blob of the teyrchain.
	pub fn genesis_wasm_generator(&self) -> Option<&Command> {
		self.genesis_wasm_generator.as_ref()
	}

	/// The location of a pre-existing genesis state of the teyrchain.
	pub fn genesis_state_path(&self) -> Option<&AssetLocation> {
		self.genesis_state_path.as_ref()
	}

	/// The generator command used to create the genesis state of the teyrchain.
	...
	/// The location of a pre-existing chain specification for the teyrchain.
	...
	/// Whether the teyrchain is evm based (e.g frontier).
	...
	/// The collators of the teyrchain.
	...
	/// The grouped collators of the teyrchain.
...
/// A teyrchain configuration builder, used to build a [`TeyrchainConfig`] declaratively with fields validation.
pub struct TeyrchainConfigBuilder<S, C> {
	config: TeyrchainConfig,
...
impl<C: Context> Default for TeyrchainConfigBuilder<Initial, C> {
	fn default() -> Self {
		Self {
			config: TeyrchainConfig {
				id: 100,
...
impl<A, C> TeyrchainConfigBuilder<A, C> {
	fn transition<B>(
		config: TeyrchainConfig,
		validation_context: Rc<RefCell<ValidationContext>>,
		errors: Vec<anyhow::Error>,
	) -> TeyrchainConfigBuilder<B, C> {
...
impl TeyrchainConfigBuilder<Initial, Bootstrap> {
	/// Instantiate a new builder that can be used to build a [`TeyrchainConfig`] during the bootstrap phase.
	pub fn new(
		validation_context: Rc<RefCell<ValidationContext>>,
...
impl TeyrchainConfigBuilder<WithId, Bootstrap> {
	/// Set the registration strategy for the teyrchain, could be Manual (no registered by zombienet) or automatic
...
impl TeyrchainConfigBuilder<WithId, Running> {
	/// Set the registration strategy for the teyrchain, could be Manual (no registered by zombienet) or automatic
...
impl TeyrchainConfigBuilder<Initial, Running> {
...
impl<C: Context> TeyrchainConfigBuilder<Initial, C> {
	/// Set the teyrchain ID and the unique_id (with the suffix `<para_id>-x` if the id is already used)
...
impl<C: Context> TeyrchainConfigBuilder<WithId, C> {
	...
	pub fn onboard_as_parachain(self, choice: bool) -> Self {
		self.onboard_as_teyrchain(choice)
	}
	...
	/// Set the generator command used to create the genesis WASM runtime blob of the teyrchain.
	...
	/// Set the generator command used to create the genesis state of the teyrchain.
	...
	/// Alias for cumulus_based - Set whether the teyrchain is based on pezcumulus/cumulus.
	pub fn pezcumulus_based(self, choice: bool) -> Self {
		self.cumulus_based(choice)
	}
...
impl<C: Context> TeyrchainConfigBuilder<WithAtLeastOneCollator, C> {
	/// Add a new collator using a nested [`NodeConfigBuilder`].
	pub fn with_collator(
		self,
		f: impl FnOnce(NodeConfigBuilder<node::Initial>) -> NodeConfigBuilder<node::Buildable>,
	...
	/// Seals the builder and returns a [`TeyrchainConfig`] if there are no validation errors, else returns errors.
	pub fn build(self) -> Result<TeyrchainConfig, Vec<anyhow::Error>> {
		if !self.errors.is_empty() {
			return Err(self
				.errors
				.into_iter()
				.map(|error| ConfigError::Teyrchain(self.config.id, error).into())
				.collect::<Vec<_>>());
		}
...
mod tests {
	...
	#[test]
	fn teyrchain_config_builder_should_succeeds_and_returns_a_new_teyrchain_config() {
		let teyrchain_config = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
		...
		assert_eq!(teyrchain_config.id(), 1000);
		assert_eq!(teyrchain_config.collators().len(), 2);
		let &collator1 = teyrchain_config.collators().first().unwrap();
		assert_eq!(collator1.name(), "collator1");
		assert_eq!(collator1.command().unwrap().as_str(), "command1");
		assert!(collator1.is_bootnode());
		let &collator2 = teyrchain_config.collators().last().unwrap();
		assert_eq!(collator2.name(), "collator2");
		assert_eq!(collator2.command().unwrap().as_str(), "command2");
		assert!(collator2.is_validator());
		assert_eq!(teyrchain_config.chain().unwrap().as_str(), "mychainname");

		assert_eq!(
			teyrchain_config.registration_strategy().unwrap(),
		...
		assert_eq!(
			teyrchain_config.genesis_state_generator().unwrap().args(),
			&vec![
		...
		assert!(!teyrchain_config.is_cumulus_based());
		...
		assert!(teyrchain_config.no_default_bootnodes());
		assert_eq!(
			teyrchain_config.bootnodes_addresses(),
			bootnodes_addresses.iter().collect::<Vec<_>>()
		);
		assert!(!teyrchain_config.is_evm_based());
		assert!(matches!(
			teyrchain_config.raw_spec_override().unwrap(),
	...
	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_chain_is_invalid() {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
		...
		assert_eq!(
			...
			"teyrchain[1000].chain: 'invalid chain' shouldn't contains whitespace"
		);
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_default_command_is_invalid() {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("chain")
		...
		assert_eq!(
			errors.first().unwrap().to_string(),
			"teyrchain[1000].default_command: 'invalid command' shouldn't contains whitespace"
		);
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_default_image_is_invalid() {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("chain")
		...
		assert_eq!(
			errors.first().unwrap().to_string(),
			r"teyrchain[1000].default_image: 'invalid image' doesn't match regex '^([ip]|[hostname]/)?[tag_name]:[tag_version]?$'"
		);
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_default_resources_are_invalid()
	{
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("chain")
		...
		assert_eq!(
			errors.first().unwrap().to_string(),
			r"teyrchain[1000].default_resources.request_cpu: 'invalid' doesn't match regex '^\d+(.\d+)?(m|K|M|G|T|P|E|Ki|Mi|Gi|Ti|Pi|Ei)?$'"
		);
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_genesis_wasm_generator_is_invalid(
	) {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
		...
		assert_eq!(
            errors.first().unwrap().to_string(),
            "teyrchain[2000].genesis_wasm_generator: 'invalid command' shouldn't contains whitespace"
        );
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_bootnodes_addresses_are_invalid(
	) {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
		...
		assert_eq!(
            errors.first().unwrap().to_string(),
            "teyrchain[2000].bootnodes_addresses[0]: '/ip4//tcp/45421' failed to parse: invalid IPv4 address syntax"
        );
		assert_eq!(
            errors.get(1).unwrap().to_string(),
            "teyrchain[2000].bootnodes_addresses[1]: '//10.42.153.10/tcp/43111' unknown protocol string: "
        );
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_an_error_if_first_collator_is_invalid() {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("myteyrchain")
			.with_collator(|collator| {
		...
		assert_eq!(
            errors.first().unwrap().to_string(),
            "teyrchain[1000].collators['collator'].command: 'invalid command' shouldn't contains whitespace"
        );
	}

	#[test]
	fn teyrchain_config_builder_with_at_least_one_collator_should_fails_and_returns_an_error_if_second_collator_is_invalid(
	) {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
		...
		assert_eq!(
            errors.first().unwrap().to_string(),
            "teyrchain[2000].collators['collator2'].command: 'invalid command' shouldn't contains whitespace"
        );
	}

	#[test]
	fn teyrchain_config_builder_should_fails_and_returns_multiple_errors_if_multiple_fields_are_invalid(
	) {
		let errors = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
		...
		assert_eq!(
            ...
            "teyrchain[2000].collators['collator1'].command: 'invalid command' shouldn't contains whitespace"
        );
		assert_eq!(
			errors.get(3).unwrap().to_string(),
			r"teyrchain[2000].collators['collator1'].resources.limit_cpu: 'invalid' doesn't match regex '^\d+(.\d+)?(m|K|M|G|T|P|E|Ki|Mi|Gi|Ti|Pi|Ei)?$'",
		);
		assert_eq!(
            errors.get(4).unwrap().to_string(),
            "teyrchain[2000].collators['collator2'].image: 'invalid.image' doesn't match regex '^([ip]|[hostname]/)?[tag_name]:[tag_version]?$'"
        );
	}
	...
	fn import_toml_registration_strategy_should_deserialize() {
		...
		for teyrchain in load_from_toml.teyrchains().iter() {
			if teyrchain.id() == 1000 {
				assert_eq!(
					teyrchain.registration_strategy(),
					Some(&RegistrationStrategy::UsingExtrinsic)
				);
			}
			if teyrchain.id() == 2000 {
				assert_eq!(
					teyrchain.registration_strategy(),
		...
		let load_from_toml_small = NetworkConfig::load_from_toml(
			"./testing/snapshots/0003-small-network_w_teyrchain.toml",
		)
		.unwrap();

		let teyrchain = load_from_toml_small.teyrchains()[0];
		let teyrchain_evm = load_from_toml_small.teyrchains()[1];

		assert_eq!(teyrchain.registration_strategy(), None);
		assert!(!teyrchain.is_evm_based());
		assert_eq!(teyrchain.collators().len(), 1);
		assert!(teyrchain_evm.is_evm_based());
	}

	#[test]
	fn onboard_as_teyrchain_should_default_to_true() {
		let config = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
		...
		assert!(config.onboard_as_teyrchain());
	}
	...
	fn build_config_in_running_context() {
		let config = TeyrchainConfigBuilder::new_with_running(Default::default())
			.with_id(2000)
			.with_chain("myteyrchain")
			.with_collator(|collator| collator.with_name("collator"))
			.build()
	...
	#[test]
	fn teyrchain_config_builder_should_works_with_chain_spec_command() {
	...
	#[test]
	fn teyrchain_config_builder_should_works_with_chain_spec_command_and_local() {
		const CMD_TPL: &str = "./bin/chain-spec-generator {% raw %} {{chainName}} {% endraw %}";
		let config = TeyrchainConfigBuilder::new(Default::default())
			.with_id(2000)
			.with_chain("some-chain")
	...
	#[test]
	fn teyrchain_with_group_config_builder_should_succeeds_and_returns_a_new_teyrchain_config() {
		let teyrchain_config = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("mychainname")
			.with_registration_strategy(RegistrationStrategy::UsingExtrinsic)
		...
		assert_eq!(teyrchain_config.id(), 1000);
		assert_eq!(teyrchain_config.collators().len(), 1);
		assert_eq!(teyrchain_config.group_collators_configs().len(), 2);

		let group_collator1 = teyrchain_config.group_collators_configs()[0].clone();
		...
		let group_collator2 = teyrchain_config.group_collators_configs()[1].clone();
	...
	#[test]
	fn teyrchain_with_group_count_0_config_builder_should_fail() {
		let teyrchain_config = TeyrchainConfigBuilder::new(Default::default())
			.with_id(1000)
			.with_chain("mychainname")
			.with_registration_strategy(RegistrationStrategy::UsingExtrinsic)
		...
		let errors: Vec<anyhow::Error> = match teyrchain_config {
			Ok(_) => vec![],
			Err(errs) => errs,
		};
		...
		assert_eq!(
			errors.first().unwrap().to_string(),
			"teyrchain[1000].collators['collator_group2'].Count cannot be zero"
		);
	}
}
