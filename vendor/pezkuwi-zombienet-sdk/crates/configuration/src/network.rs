...
impl NetworkConfig {
	...
	/// Backward compatibility alias for teyrchains().
	...
	pub fn load_from_toml(path: &str) -> Result<NetworkConfig, anyhow::Error> {
		...
		network_config.teyrchains().iter().for_each(|parachain| {
...
impl NetworkConfigBuilder<WithRelaychain> {
	...
	/// Backward compatibility alias for with_teyrchain().
...
mod tests {
	...
	fn network_config_builder_should_succeeds_and_returns_a_network_config() {
		...
		// parachain1
		let &parachain1 = network_config.teyrchains().first().unwrap();
		...
		// parachain2
		let &parachain2 = network_config.teyrchains().last().unwrap();
	...
	fn raw_spec_override_in_toml_should_work() {
		...
		assert_eq!(
			load_from_toml.teyrchains()[0].raw_spec_override(),
			expected.teyrchains()[0].raw_spec_override()
		);
	}
}
