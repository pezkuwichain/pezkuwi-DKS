...
mod tests {
	...
	fn insufficient_extrinsic_bytes() {
		...
		let result = Extrinsics::<BizinikiwiConfig>::decode_from(vec![vec![]], metadata);
		assert_matches!(
			result.err(),
	...
	fn tx_hashes_line_up() {
		let metadata = metadata();
		let hasher = <BizinikiwiConfig as Config>::Hasher::new(&metadata);
		...
		let tx_encoded = crate::tx::create_v4_unsigned::<BizinikiwiConfig, _>(&tx, &metadata)
			.expect("Valid dynamic parameters are provided");

		// Extrinsic details ready to decode.
		let extrinsics = Extrinsics::<BizinikiwiConfig>::decode_from(
