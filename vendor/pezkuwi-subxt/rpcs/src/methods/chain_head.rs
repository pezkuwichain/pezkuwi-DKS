...
impl<T: RpcConfig> ChainHeadRpcMethods<T> {
	...
	pub fn new(client: RpcClient) -> Self {
		ChainHeadRpcMethods { client, _marker: std::marker::PhantomData }
	}
	...
	/// block's storage ([`chainHead_v1_storage`](ChainHeadRpcMethods::chainhead_v1_storage)) and
	/// submitting runtime API calls at this block
	/// ([`chainHead_v1_call`](ChainHeadRpcMethods::chainhead_v1_call)).
	///
	/// # Note
	///
	...
	pub async fn chainhead_v1_continue(
		...
		self.client
			.request("chainHead_v1_continue", rpc_params![follow_subscription, operation_id])
			.await
	}
	...
	pub async fn chainhead_v1_stop_operation(
		...
		self.client
			.request("chainHead_v1_stopOperation", rpc_params![follow_subscription, operation_id])
			.await
	}
	...
	/// Call the `chainHead_v1_storage` method and return an operation ID to obtain the block's
	/// storage.
	///
	/// The response events are provided on the `chainHead_follow` subscription and identified by
	/// the returned operation ID.
	...
	pub async fn chainhead_v1_storage(
		...
		let items: Vec<StorageQuery<String>> = items
			.into_iter()
			.map(|item| StorageQuery { key: to_hex(item.key), query_type: item.query_type })
			.collect();
	...
	/// Call the `chainHead_v1_call` method and return an operation ID to obtain the runtime API
	/// result.
	///
	/// The response events are provided on the `chainHead_follow` subscription and identified by
	/// the returned operation ID.
	...
	pub async fn chainspec_v1_genesis_hash(&self) -> Result<T::Hash, Error> {
		self.client.request("chainSpec_v1_genesisHash", rpc_params![]).await
	}
	...
	pub async fn chainspec_v1_chain_name(&self) -> Result<String, Error> {
		self.client.request("chainSpec_v1_chainName", rpc_params![]).await
	}
	...
	pub async fn chainspec_v1_properties<Props: serde::de::DeserializeOwned>(
		...
		self.client.request("chainSpec_v1_properties", rpc_params![]).await
	}
	...
	pub async fn transaction_v1_broadcast(&self, tx: &[u8]) -> Result<Option<String>, Error> {
		self.client.request("transaction_v1_broadcast", rpc_params![to_hex(tx)]).await
	}
	...
	///
	/// Returns an error if the operation ID does not correspond to any active transaction for this
	/// connection.
	pub async fn transaction_v1_stop(&self, operation_id: &str) -> Result<(), Error> {
		self.client.request("transaction_v1_stop", rpc_params![operation_id]).await
	}
	...
	pub async fn archive_v1_body(&self, block_hash: T::Hash) -> Result<Option<Vec<Bytes>>, Error> {
		self.client.request("archive_v1_body", rpc_params![block_hash]).await
	}
	...
	pub async fn archive_v1_call(
		...
		let res: Response = self
			.client
			.request("archive_v1_call", rpc_params![block_hash, function, to_hex(call_parameters)])
			.await?;
	...
	pub async fn archive_v1_finalized_height(&self) -> Result<usize, Error> {
		self.client.request("archive_v1_finalizedHeight", rpc_params![]).await
	}

	/// Return the genesis hash.
	pub async fn archive_v1_genesis_hash(&self) -> Result<T::Hash, Error> {
		self.client.request("archive_v1_genesisHash", rpc_params![]).await
	}

	/// Given a block height, return the hashes of the zero or more blocks at that height.
	/// For blocks older than the latest finalized block, only one entry will be returned. For
	/// blocks newer than the latest finalized block, it's possible to have 0, 1 or multiple blocks
	/// at that height given that forks could occur.
	pub async fn archive_v1_hash_by_height(&self, height: usize) -> Result<Vec<T::Hash>, Error> {
		self.client.request("archive_v1_hashByHeight", rpc_params![height]).await
	}

	/// Fetch the header for a block with the given hash, or `None` if no block with that hash
	/// exists.
	pub async fn archive_v1_header(&self, block_hash: T::Hash) -> Result<Option<T::Header>, Error> {
		let maybe_encoded_header: Option<Bytes> =
			self.client.request("archive_v1_header", rpc_params![block_hash]).await?;
	...
	/// Query the node storage and return a subscription which streams corresponding storage events
	/// back.
	...
	pub async fn archive_unstable_body(
		...
		self.client.request("archive_unstable_body", rpc_params![block_hash]).await
	}
	...
	pub async fn archive_unstable_finalized_height(&self) -> Result<usize, Error> {
		self.client.request("archive_unstable_finalizedHeight", rpc_params![]).await
	}

	/// Return the genesis hash.
	pub async fn archive_unstable_genesis_hash(&self) -> Result<T::Hash, Error> {
		self.client.request("archive_unstable_genesisHash", rpc_params![]).await
	}
	...
	pub async fn archive_unstable_hash_by_height(
		...
		self.client.request("archive_unstable_hashByHeight", rpc_params![height]).await
	}
	...
	pub async fn archive_unstable_header(
		...
		let maybe_encoded_header: Option<Bytes> =
			self.client.request("archive_unstable_header", rpc_params![block_hash]).await?;
...
/// This contains the runtime version information necessary to make transactions, and is obtained
/// from the "initialized" event of `chainHead_follow` if the `withRuntime` flag is set.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RuntimeSpec {
	...
	/// Opaque integer. The JSON-RPC client can assume that the Runtime API call to
	/// `Metadata_metadata` will always produce the same output as long as the specVersion is the
	/// same.
	pub spec_version: u32,

	/// Opaque integer. Whenever the runtime code changes in a backwards-compatible way, the
	/// implVersion is modified while the specVersion is left untouched.
	pub impl_version: u32,

	/// Opaque integer. Necessary when building the bytes of a transaction. Transactions that have
	/// been generated with a different `transaction_version` are incompatible.
	pub transaction_version: u32,

	/// Object containing a list of "entry point APIs" supported by the runtime. Each key is an
	/// opaque string indicating the API, and each value is an integer version number. Before
...
/// Something went wrong during the [`ChainHeadRpcMethods::archive_unstable_storage()`]
/// subscription.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// A storage item returned from the [`ChainHeadRpcMethods::archive_unstable_storage()`]
/// subscription.
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ArchiveStorageEventItem<Hash> {
	...
	/// Returned when the request type is [`StorageQueryType::Value`] or
	/// [`StorageQueryType::DescendantsValues`].
	pub value: Option<Bytes>,
	/// String containing the hexadecimal-encoded hash of the storage entry.
	/// Returned when the request type is [`StorageQueryType::Hash`] or
	/// [`StorageQueryType::DescendantsHashes`].
	pub hash: Option<Hash>,
	/// String containing the hexadecimal-encoded Merkle value of the closest descendant of key
	/// (including branch nodes). Returned when the request type is
	/// [`StorageQueryType::ClosestDescendantMerkleValue`].
	pub closest_descendant_merkle_value: Option<Bytes>,
	/// String containing the hexadecimal-encoded key of the child trie of the "default" namespace
	/// if the storage entry is part of a child trie. If the storage entry is part of the main
	/// trie, this field is not present.
...
mod test {
	...
	fn chain_head_initialized() {
		...
		assert_eq!(decoded.finalized_block_hashes, vec!["0x1".to_string(), "0x2".to_string()]);
