...
impl<T: Config + Send + Sync + 'static> Backend<T> for LegacyBackend<T> {
	async fn storage_fetch_values(
		...
		let s = stream::iter(iter)
			// Resolve the future
			.then(|fut| fut)
			// Filter any Options out (ie if we didn't find a value at some key we return nothing
			// for it).
	...
	async fn stream_finalized_block_headers(
		...
		let retry_sub = retry_stream(move || {
			...
			Box::pin(async move {
				...
				// Get the last finalized block immediately so that the stream will emit every
				// finalized block after this.
				...
				// Fill in any missing blocks, because the backend may not emit every finalized
				// block; just the latest ones which are finalized each time.
