...
/// A `Stream` which builds on `FollowStream`, and handles pinning. It replaces any block hash seen
/// in the follow events with a `BlockRef` which, when all clones are dropped, will lead to an
/// "unpin" call for that block hash being queued. It will also automatically unpin any blocks that
/// exceed a given max age, to try and prevent the underlying stream from ending (and _all_ blocks
/// from being unpinned as a result). Put simply, it tries to keep every block pinned as long as
/// possible until the block is no longer used anywhere.
#[derive(Debug)]
...
impl<H> std::fmt::Debug for UnpinMethodHolder<H> {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(f, "UnpinMethodHolder(Box<dyn FnMut(Hash, Arc<str>) -> UnpinFut>)")
	}
}
		...
		loop {
			...
			let ev = match ev {
				...
				FollowStreamMsg::Event(FollowEvent::Initialized(details)) => {
					...
					// Pin each of the finalized blocks. None of them will show up again (except as
					// a parent block), and so they can all be unpinned immediately at any
					// time. Increment the block age for each one, so that older finalized
					// blocks are pruned first.
				...
				FollowStreamMsg::Event(FollowEvent::Finalized(details)) => {
					let finalized_block_refs: Vec<_> = details
						...
						.enumerate()
						.map(|(idx, hash)| {
							// These blocks _should_ exist already and so will have a known block
							// num, but if they don't, we just increment the num from the
							// last finalized block we saw, which should be accurate.
							//
							// `pin_unpinnable_block_at` indicates that the block will not show up
							// in future events (They will show up as a parent block, but we
					...
					let pruned_block_refs: Vec<_> = details
						...
						.into_iter()
						.map(|hash| {
							// We should know about these, too, and if not we set their age to
							// last_finalized + 1.
							//
							// `pin_unpinnable_block_at` indicates that the block will not show up
							// in future events.
					...
					// At this point, we also check to see which blocks we should submit unpin
					// events for. We will unpin:
					// - Any block that's older than the max age.
					// - Any block that has no references left (ie has been dropped) that _also_ has
					//   showed up in the pruned list in a finalized event (so it will never be in
					//   another event).
...
impl<H: Hash> FollowStreamUnpin<H> {
	...
	/// Pin a block, or return the reference to an already-pinned block. If the block has been
	/// registered to be unpinned, we'll clear those flags, so that it won't be unpinned. If the
	/// unpin request has already been sent though, then the block will be unpinned.
	...
	///
	/// This is the same as [`Self::pin_block_at`], except that it also marks the block as being
	/// unpinnable now, which should be done for any block that will no longer be seen in future
	/// events.
	...
	fn pin_block_at_setting_unpinnable_flag(
		...
		let entry = self
			...
			.or_insert_with(|| PinnedDetails {
				rel_block_age,
				block_ref: BlockRef {
					inner: Arc::new(BlockRefInner { hash, unpin_flags: self.unpin_flags.clone() }),
				},
				can_be_unpinned,
			});
...
impl<H: Hash> BlockRef<H> {
	...
	pub fn new(hash: H) -> Self {
		BlockRef { inner: Arc::new(BlockRefInner { hash, unpin_flags: Default::default() }) }
	}
...
mod test {
	...
	async fn dropped_new_blocks_should_not_get_unpinned_until_finalization() {
		...
		// After block 2 finalized, block 1 can be unpinned finally, but block 2 needs to wait one
		// more event.
		assert!(!follow_unpin.is_pinned(&H256::from_low_u64_le(1)));
		assert!(follow_unpin.is_pinned(&H256::from_low_u64_le(2)));
		assert_from_unpin_rx(&unpin_rx, [H256::from_low_u64_le(1)]);
	...
	async fn dropped_new_blocks_should_not_get_unpinned_until_pruned() {
		...
		assert!(!follow_unpin.is_pinned(&H256::from_low_u64_le(2)));
		assert!(!follow_unpin.is_pinned(&H256::from_low_u64_le(3)));
		assert_from_unpin_rx(&unpin_rx, [H256::from_low_u64_le(2), H256::from_low_u64_le(3)]);
	}

	#[tokio::test]
	async fn never_unpin_new_block_before_finalized() {
		// Ensure that if we drop a new block; the pinning is still active until the block is
		// finalized.
		let (mut follow_unpin, unpin_rx) = test_unpin_stream_getter(
			|| {
				[
