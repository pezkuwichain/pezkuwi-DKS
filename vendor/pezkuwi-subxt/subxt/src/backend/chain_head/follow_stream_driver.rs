...
/// A `Stream` which builds on `FollowStreamDriver`, and allows multiple subscribers to obtain
/// events from the single underlying subscription (each being provided an `Initialized` message and
/// all new blocks since then, as if they were each creating a unique `chainHead_follow`
/// subscription). This is the "top" layer of our follow stream subscriptions, and the one that's
/// interacted with elsewhere.
#[derive(Debug)]
...
impl<H: Hash> FollowStreamDriver<H> {
	...
	pub fn new(follow_unpin: FollowStreamUnpin<H>) -> Self {
		Self { inner: follow_unpin, shared: Shared::default() }
	}
...
impl<H: Hash> Shared<H> {
	...
	pub fn push_item(&self, item: FollowStreamMsg<BlockRef<H>>) {
		...
		match item {
			...
			FollowStreamMsg::Event(FollowEvent::Finalized(finalized_ev)) => {
				// Update the init message that we'll hand out to new subscriptions. If the init
				// message is `None` for some reason, we just ignore this step.
				...
				// the state at the head of the chain, therefore it is correct to remove those as
				// well. Idem for the pruned hashes; they will never be reported again and we
				// remove them from the window of events.
			...
			FollowStreamMsg::Event(FollowEvent::Stop) => {
				// On a stop event, clear everything. Wait for resubscription and new
				// ready/initialised events.
				shared.block_events_for_new_subscriptions.clear();
	...
	pub fn subscribe(&self) -> FollowStreamDriverSubscription<H> {
		...
		shared
			.subscribers
			.insert(id, SubscriberDetails { items: VecDeque::new(), waker: None });
		...
		if let Some(init_msg) = &shared.current_init_message {
			local_items
				.push_back(FollowStreamMsg::Event(FollowEvent::Initialized(init_msg.clone())));
		}
		...
		FollowStreamDriverSubscription { id, done: false, shared: self.clone(), local_items }
	}
}
	...
	F: Fn(FollowEvent<BlockRef<H>>) -> Vec<BlockRef<H>>,
{
	pub fn new(stream: FollowStreamDriverSubscription<H>, f: F) -> Self {
		Self { stream, sub_id: None, last_seen_block: None, f, is_done: false }
	}
}
		...
		loop {
			...
			let block_refs = match ev {
				...
				FollowStreamMsg::Event(FollowEvent::Initialized(mut init)) => {
					...
					if let Some(p) = prev {
						let Some(pos) =
							init.finalized_block_hashes.iter().position(|b| b.hash() == p.hash())
						else {
			...
			let sub_id =
				self.sub_id.clone().expect("Ready is always emitted before any other event");
...
mod test {
	...
	async fn subscribe_finalized_blocks_restart_works() {
		...
		let expected = vec![
			("sub_id_0".to_string(), vec![BlockRef::new(H256::from_low_u64_le(0))]),
			("sub_id_0".to_string(), vec![BlockRef::new(H256::from_low_u64_le(1))]),
			("sub_id_5".to_string(), vec![BlockRef::new(H256::from_low_u64_le(2))]),
		];
		assert_eq!(evs, expected);
	}
	...
	async fn subscribe_finalized_blocks_restart_with_missed_blocks() {
		...
		let evs: Vec<_> = FollowStreamFinalizedHeads::new(handle.subscribe(), f).collect().await;
		...
		assert_eq!(
			evs[2].as_ref().unwrap(),
			&("sub_id_2".to_string(), vec![BlockRef::new(H256::from_low_u64_le(14))])
		);
	}
}
