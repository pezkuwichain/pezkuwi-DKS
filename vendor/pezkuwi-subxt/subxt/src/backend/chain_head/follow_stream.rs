...
/// A `Stream` whose goal is to remain subscribed to `chainHead_follow`. It will re-subscribe if the
/// subscription is ended for any reason, and it will return the current `subscription_id` as an
/// event, along with the other follow events.
...
impl<Hash> FollowStream<Hash> {
	...
	pub fn new(stream_getter: FollowEventStreamGetter<Hash>) -> Self {
		Self { stream_getter, stream: InnerStreamState::New }
	}
...
pub(super) mod test_utils {
	...
	pub fn ev_finalized(
		...
		FollowEvent::Finalized(Finalized {
			finalized_block_hashes: finalized_ns.into_iter().map(H256::from_low_u64_le).collect(),
