	...
	Client: OnlineClientT<T>,
{
	...
	/// trying to validate it (ie the View Function in question do not exist at all)
	pub fn validate<Call: Payload>(&self, payload: Call) -> Result<(), ViewFunctionError> {
		pezkuwi_subxt_core::view_functions::validate(payload, &self.client.metadata())
			.map_err(Into::into)
	}
	...
	pub fn call<Call: Payload>(
		...
		async move {
			...
			// Decode the response.
			let value = pezkuwi_subxt_core::view_functions::decode_value(
				&mut &*bytes,
				&payload,
				&metadata,
			)?;
