...
impl<T: Config, Client: OfflineClientT<T>> CustomValuesClient<T, Client> {
	...
	/// back a dynamic value, or a static address from the generated static interface to get a
	/// value of a static type returned.
	...
	/// validation hash). Returns an error if the address was not valid (wrong name, type or raw
	/// bytes)
