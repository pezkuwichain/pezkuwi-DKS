	...
	Client: OfflineClientT<T>,
{
	...
	/// if the address is valid (or if it's not possible to check since the address has no
	/// validation hash). Return an error if the address was not valid or something went wrong
	/// trying to validate it (ie the pallet or storage entry in question do not exist at all).
