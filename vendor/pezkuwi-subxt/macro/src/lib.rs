...
// Note: docs for this are in the subxt library; don't add further docs here as they will be
// appended.
#[allow(missing_docs)]
...
fn subxt_inner(args: TokenStream, item_mod: syn::ItemMod) -> Result<TokenStream, TokenStream> {
	...
	codegen.set_additional_global_derives(
		args.derive_for_all_types.unwrap_or_default().into_iter().collect(),
	);
	...
	let code = codegen.generate(metadata).map_err(|e| e.into_compile_error())?;

	Ok(code.into())
}

/// Checks that a type is present in the type registry. If it is not found, abort with a
/// helpful error message, showing the user alternative types, that have the same name, but are at
/// different locations in the metadata.
...
fn fetch_metadata(
	args: &RuntimeMetadataArgs,
) -> Result<pezkuwi_subxt_codegen::Metadata, TokenStream> {
