// Copyright 2024-2026 Dijital Kurdistan Tech Institute
...
fn call_and_decode(wasm_file: Vec<u8>) -> WasmMetadataResult<Metadata> {
	...
	if let Ok(versions) = executor.versions() {
		let version = versions.into_iter().max().expect("This is checked earlier and can't fail.");
...
impl Executor {
	fn new(wasm_file: &[u8]) -> WasmMetadataResult<Self> {
		...
		Ok(Self { runtime_blob, executor, externalities })
	}
	...
	fn versions(&mut self) -> WasmMetadataResult<Vec<u32>> {
		...
		let versions =
			<Vec<u32>>::decode(&mut &version[..]).map_err(CodegenError::Decode).map(|x| {
				x.into_iter()
