	...
	match ty_def {
		TypeDef::Composite(composite) => {
			...
			let composite_field_bytes =
				composite.fields.iter().fold([0u8; HASH_LEN], |bytes, field| {
					// With EncodeAsType and DecodeAsType we no longer care which order the fields
					// are in, as long as all of the names+types are there. XOR to not care
					// about ordering.
					xor(bytes, get_field_hash(registry, field, cache))
				});
...
fn get_type_hash_recurse(
	...
	//    if the type has not been seen yet, mark it with `CachedHash::Recursive` in the cache and
	// proceed to `get_type_def_hash()`.        -> During the execution of get_type_def_hash() we
	// might get into get_type_hash(id) again for the original id            -> in this case the
	// `CachedHash::Recursive` provokes an early return.        -> Once we return from
	// `get_type_def_hash()` we need to update the cache entry:            -> We set the cache
	// value to `CachedHash::Hash(type_hash)`, where `type_hash` was returned from
	// `get_type_def_hash()`            -> It makes sure, that different types end up with
	...
	//         -> `CachedHash::Recursive`  = We have seen the type but hash calculation for it
	// hasn't finished yet.         -> `CachedHash::Hash(hash)` = Hash calculation for the type was
	// completed.
...
fn get_extrinsic_hash(registry: &PortableRegistry, extrinsic: &ExtrinsicMetadata) -> Hash {
	...
	// Supported versions are just u8s and we will likely never have more than 32 of these, so put
	// them into an array of u8s and panic if more than 32.
	...
	let mut bytes = concat_and_hash3(&address_hash, &signature_hash, &supported_extrinsic_versions);
...
fn get_custom_metadata_hash(custom_metadata: &CustomMetadata) -> Hash {
	custom_metadata.iter().fold([0u8; HASH_LEN], |bytes, custom_value| {
...
pub fn get_custom_value_hash(custom_value: &CustomValueMetadata) -> Hash {
	...
	if custom_value.types.resolve(custom_value.type_id()).is_none() {
		...
		concat_and_hash2(&name_hash, &get_type_hash(custom_value.types, custom_value.type_id()))
	}
}
...
pub fn get_runtime_apis_hash(trait_metadata: RuntimeApiMetadata) -> Hash {
	...
	trait_metadata.methods().fold([0u8; HASH_LEN], |bytes, method_metadata| {
...
impl<'a> MetadataHasher<'a> {
	...
	pub fn hash(&self) -> Hash {
		...
		let pallet_hash = metadata.pallets().fold([0u8; HASH_LEN], |bytes, pallet| {
			// If specific pallets are given, only include this pallet if it is in the specific
			// pallets.
		...
		let apis_hash = metadata.runtime_api_traits().fold([0u8; HASH_LEN], |bytes, api| {
			// If specific runtime APIs are given, only include this pallet if it is in the specific
			// runtime APIs.
			let should_hash = self
...
mod tests {
	...
	fn build_default_pallets() -> Vec<v15::PalletMetadata> {
		vec![
			v15::PalletMetadata {
				name: "First",
				calls: Some(v15::PalletCallMetadata { ty: meta_type::<MetadataTestType>() }),
				..default_pallet()
			},
			v15::PalletMetadata {
				name: "Second",
	...
	fn recursive_type() {
		...
		pallet.calls = Some(v15::PalletCallMetadata { ty: meta_type::<A>() });
	...
	fn recursive_types_different_order() {
		...
		pallets[0].calls = Some(v15::PalletCallMetadata { ty: meta_type::<A>() });
		pallets[1].calls = Some(v15::PalletCallMetadata { ty: meta_type::<B>() });
		pallets[1].index = 1;
		let mut pallets_swap = pallets.clone();
		let metadata = pallets_to_metadata(pallets);
	...
	fn pallet_hash_correctness() {
		...
		// }
		// ```
		pallet.calls = Some(v15::PalletCallMetadata { ty: meta_type::<Call>() });
		compare_pallets_hash(&pallet_lhs, &pallet);

		let pallet_lhs = pallet.clone();
		// Events are similar to Calls.
		pallet.event = Some(v15::PalletEventMetadata { ty: meta_type::<Call>() });
		compare_pallets_hash(&pallet_lhs, &pallet);
		...
		let pallet_lhs = pallet.clone();
		pallet.error = Some(v15::PalletErrorMetadata { ty: meta_type::<MetadataTestType>() });
		compare_pallets_hash(&pallet_lhs, &pallet);
	}
	...
	fn metadata_per_pallet_hash_correctness() {
		...
		let hash = MetadataHasher::new(&metadata_one)
			...
			.hash();
		let hash_rhs = MetadataHasher::new(&metadata_one).only_these_pallets(&["First"]).hash();
		...
		let hash_second = MetadataHasher::new(&metadata_both).only_these_pallets(&["First"]).hash();
		assert_eq!(hash_second, hash, "hashing one pallet should ignore the others");
	...
	fn field_semantic_changes() {
		...
		assert_eq!(to_hash(meta_type::<EnumA1>()), to_hash(meta_type::<EnumA2>()));
		...
		assert_eq!(to_hash(meta_type::<StructB1>()), to_hash(meta_type::<StructB2>()));
		...
		assert_ne!(to_hash(meta_type::<EnumC1>()), to_hash(meta_type::<EnumC2>()));
		...
		assert_ne!(to_hash(meta_type::<EnumD1>()), to_hash(meta_type::<EnumD2>()));
		...
		assert_ne!(to_hash(meta_type::<StructE1>()), to_hash(meta_type::<StructE2>()));
	}
	...
	fn metadata_with_pallet_events() -> v15::RuntimeMetadataV15 {
		...
		let pallets = vec![
			v15::PalletMetadata {
				name: "First",
				index: 0,
				calls: Some(v15::PalletCallMetadata { ty: meta_type::<u8>() }),
				storage: Some(PalletStorageMetadata {
					...
					entries: vec![StorageEntryMetadata {
						...
						// The Events type will be trimmed down and this trimming needs to be
						// reflected when the hash of this storage item is computed.
						ty: frame_metadata::v14::StorageEntryType::Plain(meta_type::<Vec<Events>>()),
						default: vec![],
						docs: vec![],
					}],
				}),
				event: Some(PalletEventMetadata { ty: meta_type::<FirstEvent>() }),
				constants: vec![],
				error: None,
				docs: vec![],
			},
			v15::PalletMetadata {
				name: "Second",
				index: 1,
				calls: Some(v15::PalletCallMetadata { ty: meta_type::<u64>() }),
				storage: None,
				event: Some(PalletEventMetadata { ty: meta_type::<SecondEvent>() }),
				constants: vec![],
				error: None,
				docs: vec![],
			},
	...
	fn hash_comparison_trimmed_metadata() {
		...
		let hash = MetadataHasher::new(&metadata).only_these_pallets(&["First"]).hash();
