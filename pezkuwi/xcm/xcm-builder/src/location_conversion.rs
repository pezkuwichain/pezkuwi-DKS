...
/// Converts a location which is a top-level teyrchain (i.e. a teyrchain held on a
...
/// teyrchain index under the same Relay-chain, regardless of the relative security of
/// this Relay-chain compared to the local chain.
///
/// Note: No distinction is made when the local chain happens to be the teyrchain in
...
mod tests {
	...
	fn inverter_works_in_tree() {
		parameter_types! {
			pub UniversalLocation: InteriorLocation = [Teyrchain(1), account20(), account20()].into();
		}

		let input = Location::new(3, [Teyrchain(2), account32()]);
		let inverted = UniversalLocation::get().invert_target(&input).unwrap();
		assert_eq!(inverted, Location::new(2, [Teyrchain(1), account20(), account20()]));
	}
	...
	fn inverter_errors_when_location_is_too_large() {
		...
		let input = Location { parents: 99, interior: [Teyrchain(88)].into() };
	...
	fn global_consensus_converts_for_works() {
		parameter_types! {
			pub UniversalLocationInNetwork1: InteriorLocation = [GlobalConsensus(ByGenesis([1; 32])), Teyrchain(1234)].into();
			pub UniversalLocationInNetwork2: InteriorLocation = [GlobalConsensus(ByGenesis([2; 32])), Teyrchain(1234)].into();
		}
		...
		let test_data = vec![
			...
			(Location::new(0, [GlobalConsensus(network_2), Teyrchain(1000)]), false),
			(Location::new(1, [GlobalConsensus(network_2), Teyrchain(1000)]), false),
			(Location::new(2, [GlobalConsensus(network_2), Teyrchain(1000)]), false),
		];
	...
	#[test]
	fn global_consensus_teyrchain_converts_for_works() {
		parameter_types! {
			pub UniversalLocation: InteriorLocation = [GlobalConsensus(ByGenesis([9; 32])), Teyrchain(1234)].into();
		}

		let test_data = vec![
			(Location::parent(), false),
			(Location::new(0, [Teyrchain(1000)]), false),
			(Location::new(1, [Teyrchain(1000)]), false),
			(
				Location::new(
					2,
					[
			...
			(Location::new(0, [GlobalConsensus(ByGenesis([0; 32])), Teyrchain(1000)]), false),
			(Location::new(1, [GlobalConsensus(ByGenesis([0; 32])), Teyrchain(1000)]), false),
			(Location::new(2, [GlobalConsensus(ByGenesis([0; 32])), Teyrchain(1000)]), true),
			(Location::new(3, [GlobalConsensus(ByGenesis([0; 32])), Teyrchain(1000)]), false),
			(Location::new(9, [GlobalConsensus(ByGenesis([0; 32])), Teyrchain(1000)]), false),
		];
	...
	fn remote_account_convert_on_para_sending_para_32() {
		let mul = Location {
			parents: 1,
			interior: [Teyrchain(1), AccountId32 { network: None, id: [0u8; 32] }].into(),
		};
		...
		let mul = Location {
			parents: 1,
			interior: [Teyrchain(2), AccountId32 { network: None, id: [0u8; 32] }].into(),
		};
	...
	fn remote_account_convert_on_para_sending_para_20() {
		let mul = Location {
			parents: 1,
			interior: [Teyrchain(1), AccountKey20 { network: None, key: [0u8; 20] }].into(),
		};
		let rem_1 = ForeignChainAliasAccount::<[u8; 32]>::convert_location(&mul).unwrap();
		...
		let mul = Location {
			parents: 1,
			interior: [Teyrchain(2), AccountKey20 { network: None, key: [0u8; 20] }].into(),
		};
		let rem_2 = ForeignChainAliasAccount::<[u8; 32]>::convert_location(&mul).unwrap();
	...
	fn remote_account_convert_on_para_sending_from_remote_para_treasury() {
		...
		let para_to_para_treasury_location = Location::new(
			1,
			[Teyrchain(1001), Plurality { id: BodyId::Treasury, part: BodyPart::Voice }],
		);
		let actual_description = ForeignChainAliasTreasuryAccount::<[u8; 32]>::convert_location(
