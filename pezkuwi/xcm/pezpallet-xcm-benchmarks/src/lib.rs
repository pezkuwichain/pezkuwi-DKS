...
use xcm_executor::{traits::ConvertLocation, AssetsInHolding, Config as XcmConfig};
...
/// A base trait for all individual pezpallets
...
pub fn generate_holding_assets(max_assets: u32) -> AssetsInHolding {
	...
	for i in 0..holding_fungibles {
		let asset_id = AssetId(GeneralIndex(i as u128).into());
		let amount = fungibles_amount * (i + 1) as u128;
		holding.fungible.insert(asset_id, alloc::boxed::Box::new(MockCredit(amount)));
	}
	...
	for i in 0..holding_non_fungibles {
		let asset_id = AssetId(GeneralIndex(i as u128).into());
		let instance = asset_instance_from(i);
		holding.non_fungible.insert((asset_id, instance));
	}
