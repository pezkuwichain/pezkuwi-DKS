...
mod tests {
	...
	fn reanchor_preserves_sorting() {
		...
		let reanchor_context = X1(Teyrchain(2000));
		...
		assert_eq!(
			asset_1_reanchored,
			(MultiLocation::new(0, X3(Teyrchain(2000), PalletInstance(50), GeneralIndex(1))), 10)
				.into()
		);
		...
		let asset_3: MultiAsset = (MultiLocation::new(1, X1(Teyrchain(1000))), 10).into();
		let mut asset_3_reanchored = asset_3.clone();
		assert!(asset_3_reanchored.reanchor(&dest, reanchor_context).is_ok());
		assert_eq!(asset_3_reanchored, (MultiLocation::new(0, X1(Teyrchain(1000))), 10).into());
	...
	fn prepend_preserves_sorting() {
		...
		let prefix = MultiLocation::new(0, X1(Teyrchain(1000)));
		...
		assert_eq!(
			asset_1_prepended,
			(MultiLocation::new(0, X3(Teyrchain(1000), PalletInstance(50), GeneralIndex(1))), 10)
				.into()
		);
