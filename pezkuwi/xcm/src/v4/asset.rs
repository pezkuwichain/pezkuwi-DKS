...
mod tests {
	...
	fn reanchor_preserves_sorting() {
		...
		let reanchor_context: Junctions = Teyrchain(2000).into();
		...
		assert_eq!(
			asset_1_reanchored,
			(Location::new(0, [Teyrchain(2000), PalletInstance(50), GeneralIndex(1)]), 10).into()
		);
		...
		let asset_3: Asset = (Location::new(1, [Teyrchain(1000)]), 10).into();
		let mut asset_3_reanchored = asset_3.clone();
		assert!(asset_3_reanchored.reanchor(&dest, &reanchor_context).is_ok());
		assert_eq!(asset_3_reanchored, (Location::new(0, [Teyrchain(1000)]), 10).into());
	...
	fn prepend_preserves_sorting() {
		...
		let prefix = Location::new(0, [Teyrchain(1000)]);
		...
		assert_eq!(
			asset_1_prepended,
			(Location::new(0, [Teyrchain(1000), PalletInstance(50), GeneralIndex(1)]), 10).into()
		);
