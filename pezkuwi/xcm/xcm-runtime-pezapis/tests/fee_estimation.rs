	...
	new_test_ext_with_balances_and_assets(balances, assets).execute_with(|| {
		...
		assert_eq!(
			...
			vec![
				...
				RuntimeEvent::AssetsPallet(pezpallet_assets::Event::Withdrawn {
					asset_id: 1,
					who: 1,
					amount: 20
				}),
				RuntimeEvent::AssetsPallet(pezpallet_assets::Event::BurnedCredit {
					asset_id: 1,
					amount: 20
				}),
				RuntimeEvent::Balances(pezpallet_balances::Event::Withdraw { who: 1, amount: 100 }),
				RuntimeEvent::Balances(pezpallet_balances::Event::BurnedDebt { amount: 100 }),
				RuntimeEvent::XcmPallet(pezpallet_xcm::Event::Attempted {
					outcome: Outcome::Complete { used: Weight::from_parts(400, 40) },
				}),
				RuntimeEvent::Balances(pezpallet_balances::Event::Withdraw { who: 1, amount: 20 }),
				RuntimeEvent::Balances(pezpallet_balances::Event::BurnedDebt { amount: 20 }),
...
fn dry_run_xcm_common(xcm_version: XcmVersion) {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		assert_eq!(
			dry_run_effects.emitted_events,
			vec![
				RuntimeEvent::Balances(pezpallet_balances::Event::Withdraw { who: 1, amount: 540 }),
