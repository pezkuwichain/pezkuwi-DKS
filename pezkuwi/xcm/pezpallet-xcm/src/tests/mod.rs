...
fn report_outcome_notify_works() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		XcmPallet::report_outcome_notify(
			&mut message,
			Teyrchain(OTHER_PARA_ID).into_location(),
			notify,
			100,
		)
		.unwrap();
		...
		let status = QueryStatus::Pending {
			responder: Location::from(Teyrchain(OTHER_PARA_ID)).into(),
...
fn report_outcome_works() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		XcmPallet::report_outcome(&mut message, Teyrchain(OTHER_PARA_ID).into_location(), 100)
			.unwrap();
		assert_eq!(
			message,
			Xcm(vec![
...
fn authorized_aliases_work() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		for i in 1..MaxAuthorizedAliases::get() {
			let alias = Location::new(0, [Teyrchain(OTHER_PARA_ID), GeneralIndex(i as u128)]);
			assert_ok!(XcmPallet::add_authorized_alias(
				RuntimeOrigin::signed(who.clone()),
				Box::new(alias.clone().into()),
				None
		...
		let alias = Location::new(
			0,
			[Teyrchain(OTHER_PARA_ID), GeneralIndex(MaxAuthorizedAliases::get() as u128 + 100)],
		);
		assert_eq!(
			XcmPallet::add_authorized_alias(
				RuntimeOrigin::signed(who.clone()),
...
fn fake_latest_versioned_location_works() {
	use codec::Encode;
	let remote: Location = Teyrchain(1000).into();
...
fn subscriptions_increment_id() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		let remote2: Location = Teyrchain(1001).into();
		assert_ok!(XcmPallet::force_subscribe_version_notify(
			RuntimeOrigin::root(),
...
/// Teyrchain 1000 is asking us for a version subscription.
#[test]
...
fn subscription_side_upgrades_work_with_notify() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		let v3_location = VersionedLocation::V3(xcm::v3::Junction::Teyrchain(1001).into());
		VersionNotifyTargets::<Test>::insert(3, v3_location, (70, Weight::zero(), 3));
		let v4_location = Teyrchain(1003).into_versioned();
		...
		assert_eq!(
			sent,
			vec![
				(Teyrchain(1001).into(), Xcm(vec![instr1])),
				(Teyrchain(1003).into(), Xcm(vec![instr3])),
			]
		);
		...
		assert_eq!(
			contents,
			vec![
				(XCM_VERSION, Teyrchain(1001).into_versioned(), (70, Weight::zero(), 4)),
				(XCM_VERSION, Teyrchain(1003).into_versioned(), (72, Weight::zero(), 4)),
			]
		);
	});
}
...
fn auto_subscription_works() {
	new_test_ext_with_balances_and_xcm_version(vec![], None, vec![]).execute_with(|| {
		let remote_v3: Location = Teyrchain(1000).into();
		let remote_v4: Location = Teyrchain(1001).into();
...
fn subscription_side_upgrades_work_with_multistage_notify() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		let v3_location = VersionedLocation::V3(xcm::v3::Junction::Teyrchain(1001).into());
		VersionNotifyTargets::<Test>::insert(3, v3_location, (70, Weight::zero(), 3));
		let v3_location = VersionedLocation::V3(xcm::v3::Junction::Teyrchain(1002).into());
		...
		assert_eq!(
			...
			vec![
				(Teyrchain(1001).into(), Xcm(vec![instr1])),
				(Teyrchain(1002).into(), Xcm(vec![instr2])),
				(Teyrchain(1003).into(), Xcm(vec![instr3])),
			]
		);
		...
		assert_eq!(
			...
			vec![
				(XCM_VERSION, Teyrchain(1001).into_versioned(), (70, Weight::zero(), 4)),
				(XCM_VERSION, Teyrchain(1002).into_versioned(), (71, Weight::zero(), 4)),
				(XCM_VERSION, Teyrchain(1003).into_versioned(), (72, Weight::zero(), 4)),
			]
		);
	});
...
fn get_and_wrap_version_works() {
	let remote_a: Location = Teyrchain(1000).into();
	let remote_b: Location = Teyrchain(1001).into();
	let remote_c: Location = Teyrchain(1002).into();
	let remote_d: Location = Teyrchain(1003).into();
...
fn multistage_migration_works() {
	new_test_ext_with_balances(vec![]).execute_with(|| {
		...
		assert_eq!(
			...
			vec![(XCM_VERSION, Teyrchain(1001).into_versioned(), v3_version),]
		);
		assert_eq!(
			VersionNotifiers::<Test>::iter().collect::<Vec<_>>(),
			vec![(XCM_VERSION, Teyrchain(1001).into_versioned(), 1),]
		);
		assert_eq!(
			VersionNotifyTargets::<Test>::iter().collect::<Vec<_>>(),
			vec![(XCM_VERSION, Teyrchain(1001).into_versioned(), (70, Weight::zero(), 4)),]
		);
