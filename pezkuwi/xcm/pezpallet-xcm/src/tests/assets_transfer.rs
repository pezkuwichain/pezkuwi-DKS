...
fn reserve_transfer_assets_with_paid_router_works() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// Destination account (teyrchain account) has amount
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// Destination account (teyrchain account) added native reserve used as fee to balances
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		assert_eq!(Balances::free_balance(ALICE), INITIAL_BALANCE - SEND_AMOUNT);
		// Sovereign account of dest teyrchain holds `SEND_AMOUNT` native asset in local reserve
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// transfer destination is some other teyrchain != fee reserve location (no teleport trust)
		let dest = RelayLocation::get().pushed_with_interior(Teyrchain(OTHER_PARA_ID)).unwrap();
		...
		assert_eq!(Balances::free_balance(ALICE), INITIAL_BALANCE);
		// Sovereign account of reserve teyrchain is unchanged
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// transfer destination is some other teyrchain
		let dest = RelayLocation::get().pushed_with_interior(Teyrchain(OTHER_PARA_ID)).unwrap();
		...
		// Alice's native asset balance is untouched
		assert_eq!(Balances::free_balance(ALICE), INITIAL_BALANCE);
		// Destination account (teyrchain account) has expected (same) balances
		assert_eq!(Balances::free_balance(usdc_chain_sovereign_account.clone()), 0);
		assert_eq!(
			AssetsPallet::balance(usdc_id_location.clone(), usdc_chain_sovereign_account),
			0
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// Verify balances of USDT reserve teyrchain
		assert_eq!(Balances::free_balance(usdt_chain_sovereign_account.clone()), 0);
		assert_eq!(
			AssetsPallet::balance(usdt_id_location.clone(), usdt_chain_sovereign_account),
			0
		);
		// Verify balances of transferred-asset reserve teyrchain
		assert_eq!(Balances::free_balance(dest_sovereign_account.clone()), 0);
		assert_eq!(
			AssetsPallet::balance(foreign_asset_id_location.clone(), dest_sovereign_account),
			0
...
fn reserve_transfer_assets_with_teleportable_asset_disallowed() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// No USDT moved to sovereign account of reserve teyrchain
		assert_eq!(
			AssetsPallet::balance(usdt_id_location.clone(), usdt_chain_sovereign_account),
			0
		);
...
fn intermediary_error_reverts_side_effects() {
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		assert_eq!(Balances::free_balance(ALICE), INITIAL_BALANCE);
		// Destination account (teyrchain account) no changes
		assert_eq!(Balances::free_balance(usdc_chain_sovereign_account.clone()), 0);
		assert_eq!(
			AssetsPallet::balance(usdc_id_location.clone(), usdc_chain_sovereign_account),
			0
	...
	) -> DispatchResult,
{
	...
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// Alice used native asset for fees
		assert_eq!(Balances::free_balance(ALICE), INITIAL_BALANCE - FEE_AMOUNT);
		// Destination account (teyrchain account) added native reserve to balances
