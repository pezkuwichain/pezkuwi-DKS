...
//! A module that is responsible for migration of storage for the configuration pezpallet.
//! v12 -> v13:
...
/// This is identical to `pezkuwi_primitives::v9::SchedulerParams`.
pub type V12SchedulerParams<BlockNumber> = pezkuwi_primitives::v9::SchedulerParams<BlockNumber>;
...
pub struct V12HostConfiguration<BlockNumber> {
	...
	pub async_backing_params: pezkuwi_primitives::AsyncBackingParams,
	...
	pub executor_params: pezkuwi_primitives::ExecutorParams,
	...
	pub node_features: pezkuwi_primitives::NodeFeatures,
	pub approval_voting_params: pezkuwi_primitives::ApprovalVotingParams,
...
impl<BlockNumber: Default + From<u32>> Default for V12HostConfiguration<BlockNumber> {
	fn default() -> Self {
		Self {
			async_backing_params: pezkuwi_primitives::AsyncBackingParams {
			...
			max_code_size: pezkuwi_primitives::MAX_CODE_SIZE,
			...
			approval_voting_params: pezkuwi_primitives::ApprovalVotingParams {
				max_approval_coalesce_count: 1,
			},
			minimum_backing_votes: pezkuwi_primitives::LEGACY_MIN_BACKING_VOTES,
...
mod v13 {
	...
	#[pezframe_support::storage_alias]
	pub(crate) type ActiveConfig<T: Config> =
		StorageValue<Pezpallet<T>, V13HostConfiguration<BlockNumberFor<T>>, OptionQuery>;
...
impl<T: Config> UncheckedOnRuntimeUpgrade for UncheckedMigrateToV13<T> {
	...
	fn post_upgrade(_state: Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		log::trace!(target: crate::configuration::LOG_TARGET, "Running post_upgrade() for HostConfiguration MigrateToV13");
			...
			StorageVersion::get::<Pezpallet<T>>() >= 13,
...
mod tests {
	...
	fn test_migrate_to_v13() {
		let v12 = V12HostConfiguration::<pezkuwi_primitives::BlockNumber> {
		...
		new_test_ext(Default::default()).execute_with(|| {
			...
			for (_, v13) in configs_to_check {
				...
				#[allow(deprecated)]
				{
					...
					assert_eq!(v12.hrmp_max_teyrchain_outbound_channels     , v13.hrmp_max_teyrchain_outbound_channels);
					...
					assert_eq!(v12.hrmp_max_teyrchain_inbound_channels      , v13.hrmp_max_teyrchain_inbound_channels);
	...
	fn test_migrate_to_v13_no_pending() {
		let v12 = V12HostConfiguration::<pezkuwi_primitives::BlockNumber>::default();
