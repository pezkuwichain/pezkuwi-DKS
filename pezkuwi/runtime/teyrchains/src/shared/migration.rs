...
use pezkuwi_primitives::vstaging::RelayParentInfo;
...
pub mod v1 {
	...
	pub(crate) type AllowedRelayParents<T: Config> = StorageValue<
		Pezpallet<T>,
		AllowedRelayParentsTracker<<T as pezframe_system::Config>::Hash, BlockNumberFor<T>>,
		ValueQuery,
	>;
	...
	impl<T: Config> UncheckedOnRuntimeUpgrade for VersionUncheckedMigrateToV2<T> {
		...
		fn post_upgrade(state: Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
			log::trace!(target: LOG_TARGET, "Running post_upgrade() for shared MigrateToV2");
				...
				Pezpallet::<T>::on_chain_storage_version() >= StorageVersion::new(2),
...
pub type MigrateToV2<T> = pezframe_support::migrations::VersionedMigration<
	1,
	2,
