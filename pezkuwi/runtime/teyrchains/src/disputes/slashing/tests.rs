...
fn insert_session_info(session: SessionIndex, validators: Vec<ValidatorId>) {
	use pezkuwi_primitives::{AuthorityDiscoveryId, IndexedVec, SessionInfo};
...
fn ext() -> pezsp_io::TestExternalities {
...
fn validate_unsigned_rejects_proof_from_different_session() {
	ext().execute_with(|| {
		...
		let outcome = Pezpallet::<Test>::validate_unsigned(TransactionSource::Local, &call);
...
fn validate_unsigned_rejects_external_source() {
	ext().execute_with(|| {
		...
		let outcome: TransactionValidity =
			Pezpallet::<Test>::validate_unsigned(TransactionSource::External, &call);
...
fn validate_unsigned_accepts_in_block_source() {
	ext().execute_with(|| {
		...
		assert!(Pezpallet::<Test>::validate_unsigned(TransactionSource::InBlock, &call).is_ok());
	});
}
...
fn validate_unsigned_uses_kind_specific_tag_prefix() {
	ext().execute_with(|| {
		...
		for (kind, expected) in [
			...
			let valid = Pezpallet::<Test>::validate_unsigned(TransactionSource::Local, &call)
...
fn pre_dispatch_replays_validation() {
	ext().execute_with(|| {
		...
		assert_eq!(
			Pezpallet::<Test>::pre_dispatch(&call),
...
type Handler = SlashValidatorsForDisputes<Pezpallet<Test>>;
...
fn initializer_on_new_session_prunes_old_unapplied_slashes() {
	ext().execute_with(|| {
		...
		Pezpallet::<Test>::initializer_on_new_session(new_session);
...
fn initializer_on_new_session_is_noop_for_early_sessions() {
	ext().execute_with(|| {
		...
		Pezpallet::<Test>::initializer_on_new_session(dispute_period + 1);
