...
mod enter {
		...
		BlockLength::set(
			limits::BlockLength::builder()
				...
				.modify_max_length_for_class(
					...
					|m| *m = Perbill::from_percent(75) * 600,
				)
				.build(),
		);
	...
	fn candidates_are_size_limited() {
		BlockLength::set(
			limits::BlockLength::builder()
				...
				.modify_max_length_for_class(
					pezframe_support::dispatch::DispatchClass::Normal,
					|m| *m = Perbill::from_percent(75) * 1_300,
				)
				.build(),
		);
		// Virtually no time based limit:
	...
	// Ensure that overweight teyrchain inherents are always rejected by the runtime.
	#[rstest]
	#[case(true)]
	#[case(false)]
	...
	fn v3_cross_session_candidate_accepted_through_enter() {
		...
		new_test_ext(config).execute_with(|| {
			...
			let old_relay_parent = pezsp_core::H256::repeat_byte(0xCC);
			...
			let scenario = make_inherent_data(TestConfig {
				...
				candidate_modifier: Some(|mut candidate| {
					if candidate.descriptor.para_id() == ParaId::from(0) {
						candidate.descriptor.set_relay_parent(pezsp_core::H256::repeat_byte(0xCC));
			...
			let current_relay_parent_number: u32 =
				(pezframe_system::Pezpallet::<Test>::block_number() - 1).try_into().unwrap_or(0);
			...
			Pezpallet::<Test>::enter(pezframe_system::RawOrigin::None.into(), para_inherent_data)
				.unwrap();
	...
	fn v2_cross_session_candidate_rejected() {
		...
		new_test_ext(config).execute_with(|| {
			...
			Pezpallet::<Test>::enter(pezframe_system::RawOrigin::None.into(), filtered).unwrap();
	...
	// Test when teyrchain runtime is upgraded to support the new commitments
...
mod sanitizers {
	...
	mod candidates {
		...
		// Backed candidates and scheduled teyrchains used for `sanitize_backed_candidates` testing
		struct TestData {
		...
		fn get_test_data_one_core_per_para(backing_kind: BackingKind) -> TestData {
			...
			// Add the scheduling parent to `shared` pezpallet. Otherwise some code (e.g. filtering
			// backing votes) won't behave correctly
			shared::Pezpallet::<Test>::add_allowed_scheduling_parent(
				default_header().hash(),
				Default::default(),
				RELAY_PARENT_NUM,
				1,
			...
			.unwrap();
			on_demand::Pezpallet::<Test>::push_back_order(1.into());
			Scheduler::assign_core(
				CoreIndex(1),
				0,
				vec![(CoreAssignment::Pool, PartsOf57600::FULL)],
			...
			.unwrap();
			on_demand::Pezpallet::<Test>::push_back_order(2.into());
			...
			// One backed candidate from each teyrchain
