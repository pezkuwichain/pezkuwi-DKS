...
pub mod pezpallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pezpallet<T> {
		...
		fn on_finalize(_: BlockNumberFor<T>) {
			if Included::<T>::take().is_none() {
				panic!("TeyrchainInherent was not executed in this block. This is a bug. Please report this at https://github.com/paritytech/polkadot-sdk/issues.");
			}
		}
	}
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn enter(
			origin: OriginFor<T>,
			data: TeyrchainsInherentData<HeaderFor<T>>,
...
impl<T: Config> Pezpallet<T> {
	...
	/// unit tested.
	fn create_inherent_inner(data: &InherentData) -> Option<TeyrchainsInherentData<HeaderFor<T>>> {
		let teyrchains_inherent_data = match data.get_data(&Self::INHERENT_IDENTIFIER) {
			Ok(Some(d)) => d,
			Ok(None) => return None,
			Err(_) => {
				log::warn!(target: LOG_TARGET, "TeyrchainsInherentData failed to decode");
				return None;
			},
		};
		match Self::process_inherent_data(teyrchains_inherent_data) {
	...
	) -> Result<(TeyrchainsInherentData<HeaderFor<T>>, PostDispatchInfo), DispatchErrorWithPostInfo>
	{
		...
		if T::DisputesHandler::is_frozen() {
			// Relay chain freeze, at this point we will not include any teyrchain blocks.
			...
			let disputes = checked_disputes_sets
				...
				.collect::<Vec<_>>();
			let processed = TeyrchainsInherentData {
				bitfields: Vec::new(),
			...
			// The relay chain we are currently on is invalid. Proceed no further on teyrchains.
	...
	> {
		...
		let scheduled = scheduler::Pezpallet::<T>::advance_claim_queue(is_blocked);
...
/// teyrchains their chances of inclusion become slim. All backed candidates  are checked
...
fn check_descriptor_version_and_signals<T: crate::inclusion::Config>(
	...
	if shared::Pezpallet::<T>::get_relay_parent_info(session_index, relay_parent).is_none() {
		log::debug!(
			target: LOG_TARGET,
	...
	let Some((pezsp_info, _)) = allowed_scheduling_parents.acquire_info(scheduling_parent) else {
		log::debug!(
			target: LOG_TARGET,
	...
	if let Err(err) = candidate.candidate().parse_ump_signals(&pezsp_info.claim_queue) {
		log::debug!(
			target: LOG_TARGET,
...
>(
	...
	// the validator group assigned to the teyrchain. To obtain this group we need:
	// 1. Core index assigned to the teyrchain which has produced the candidate
