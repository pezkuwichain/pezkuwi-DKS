...
fn assignments_stay_stable_when_pushed_back() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		for para in paras.clone() {
			on_demand::Pezpallet::<Test>::push_back_order(para);
		}
		...
		let claim_queue_initial = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let popped = scheduler::Pezpallet::<Test>::advance_claim_queue(|core_idx| {
		...
		let claim_queue_after_block = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let mut claim_queue_snapshot = scheduler::Pezpallet::<Test>::claim_queue();

		// Advance several times and verify each popped assignment matches what we expected
		loop {
			System::set_block_number(System::block_number() + 1);
			let popped = scheduler::Pezpallet::<Test>::advance_claim_queue(|_| false);
...
fn advance_claim_queue_doubles_assignment_only_if_empty() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		{
			on_demand::Pezpallet::<Test>::push_back_order(para_a);
			on_demand::Pezpallet::<Test>::push_back_order(para_c);
			on_demand::Pezpallet::<Test>::push_back_order(para_b);

			let mut claim_queue = scheduler::Pezpallet::<Test>::claim_queue();
...
fn advance_claim_queue_except_for() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		on_demand::Pezpallet::<Test>::push_back_order(para_b);
		on_demand::Pezpallet::<Test>::push_back_order(para_d);
		on_demand::Pezpallet::<Test>::push_back_order(para_e);
...
fn peek_claim_queue_predicts_scheduling() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		let peeked_first = scheduler::Pezpallet::<Test>::claim_queue();
		let peeked_second = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let claim_queue_after_pop = scheduler::Pezpallet::<Test>::claim_queue();
...
fn on_demand_order_on_empty_core_appears_in_next_two_blocks() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		let claim_queue_before = scheduler::Pezpallet::<Test>::claim_queue();
		...
		scheduler::Pezpallet::<Test>::advance_claim_queue(|_| false);

		// After advancing, we should have 1 para_a left (the duplicate)
		let claim_queue_after_first_advance = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let claim_queue_after = scheduler::Pezpallet::<Test>::claim_queue();
...
fn assign_core_within_claim_queue_maintains_stable_claim_queue() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		let claim_queue_after_test1 = scheduler::Pezpallet::<Test>::claim_queue();
		assert_eq!(
		...
		let claim_queue_after_test2 = scheduler::Pezpallet::<Test>::claim_queue();
		assert_eq!(
		...
		let mut claim_queue_after_test3 = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let claim_queue_before_test4 = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let claim_queue_after_test4 = scheduler::Pezpallet::<Test>::claim_queue();
		...
		let claim_queue_block_4 = scheduler::Pezpallet::<Test>::claim_queue();
