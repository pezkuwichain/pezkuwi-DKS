...
//! assignments it relies on the separate on-demand pezpallet, where it forwards requests
//! to.
//!
...
pub use pezpallet_broker::CoreAssignment;
	...
	fn pop_assignment_for_ondemand_cores(
		...
		match self {
			Self::Peek { on_demand_orders } => on_demand_orders
				...
				.collect::<Vec<_>>(),
			Self::Pop => on_demand::Pezpallet::<T>::pop_assignment_for_cores(now, num_cores)
				.collect::<Vec<_>>(),
		}
		.into_iter()
	}
...
pub(super) fn peek_next_block<T: super::Config>(
	...
	let now = pezframe_system::Pezpallet::<T>::block_number().saturating_plus_one();
...
pub(super) fn advance_assignments<T: Config, F: Fn(CoreIndex) -> bool>(
	...
	for blocked in assignments.pool_assignments.iter().filter_map(|(core_idx, para_id)| {
		...
		on_demand::Pezpallet::<T>::push_back_order(blocked);
	}
	...
	let mut on_demand_orders = on_demand::Pezpallet::<T>::peek_order_queue();
...
pub(super) fn assign_core<T: Config>(
	...
	super::CoreDescriptors::<T>::mutate(|core_descriptors| {
		...
		if assignments_exist && begin < claim_queue_end {
			log::debug!(
				target: "runtime::teyrchains::assigner-coretime",
