...
pub(super) mod v3 {
	use super::*;
	use pezframe_support::pezpallet_prelude::{OptionQuery, Twox256};
	...
	/// Storage alias for the old CoreSchedules storage in the AssignerCoretime pezpallet.
	///
	/// NOTE: The pezpallet name must match the name used in the runtime's `construct_runtime!` macro.
	/// In production runtimes (Pezkuwi/Dicle), this is named "CoretimeAssignmentProvider".
	///
	...
	/// Storage alias for the old CoreDescriptors storage in the AssignerCoretime pezpallet.
	///
	/// NOTE: The pezpallet name must match the name used in the runtime's `construct_runtime!` macro.
	/// In production runtimes (Pezkuwi/Dicle), this is named "CoretimeAssignmentProvider".
	#[storage_alias]
...
/// Migration for consolidating coretime assignment scheduling into the Scheduler pezpallet.
///
/// V4 completes the transition to the new coretime scheduling model by moving all coretime-related
/// storage from the deprecated top-level AssignerCoretime pezpallet into the Scheduler pezpallet.
///
/// Major changes in V4:
...
///   pezpallet to Scheduler pezpallet storage, following linked-list structure via `next_schedule`
///   pointers.
/// - **Migrates CoreDescriptors**: Moves core metadata (queue descriptors and current work state)
///   from the deprecated pezpallet to Scheduler pezpallet storage.
...
///   the on-demand pezpallet queue to ensure no orders are lost.
...
///
/// After this migration, the deprecated AssignerCoretime pezpallet becomes an empty stub that can be
...
impl<T: Config> UncheckedMigrateToV4<T> {
	pub fn pre_upgrade() -> Result<Vec<u8>, pezsp_runtime::DispatchError> {
		...
		// If we find nothing, the storage alias pezpallet name is likely wrong
		if descriptor_count == 0 && schedule_count == 0 {
			log::error!(
				target: super::LOG_TARGET,
				"Migration found no data to migrate! This likely means the storage alias pezpallet name \
				(CoretimeAssignmentProvider) doesn't match the actual pezpallet name in construct_runtime. \
				Check the runtime's construct_runtime macro for the correct pezpallet name."
			);
			return Err("No data found to migrate - wrong pezpallet name in storage alias?".into());
		}

		Ok((schedule_count, descriptor_count, total_assignments, pool_assignments).encode())
	}

	pub fn post_upgrade(state: Vec<u8>) -> Result<(), pezsp_runtime::DispatchError> {
		...
		for (_core_idx, assignments) in old_claim_queue.iter() {
			for assignment in assignments {
				...
				if let v3::Assignment::Pool { para_id, .. } = assignment {
					// Push the on-demand order back to the on-demand pezpallet.
					// This ensures user-paid orders are not lost.
					on_demand::Pezpallet::<T>::push_back_order(*para_id);
...
mod v4_tests {
	...
	use pezpallet_broker::CoreAssignment as BrokerCoreAssignment;
	...
	fn basic_migration_works() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			StorageVersion::new(3).put::<super::Pezpallet<Test>>();
	...
	fn claim_queue_migration_works() {
		...
		// - Pool assignments should be pushed to the on-demand pezpallet queue
		...
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			let claim_queue_before = super::Pezpallet::<Test>::claim_queue();
			...
			// The migration calls `on_demand::Pezpallet::<T>::push_back_order` for each pool
			...
			let mut on_demand_queue = on_demand::Pezpallet::<Test>::peek_order_queue();
			...
			pezframe_system::Pezpallet::<Test>::set_block_number(block_number);
	...
	fn empty_storage_migration_works() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			// pre_upgrade should fail if no data is found (sanity check for wrong pezpallet name)
