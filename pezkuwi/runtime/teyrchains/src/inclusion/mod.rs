...
impl<T: Config> Pezpallet<T> {
	...
	fn check_backing_votes(
		...
		let maybe_amount_validated = pezkuwi_primitives::check_candidate_backing(
	...
	pub(crate) fn check_validation_outputs_for_runtime_api(
		...
		validation_outputs: pezkuwi_primitives::CandidateCommitments,
	) -> bool {
		...
		if let Err(err) = check_ctx.check_validation_outputs(
			...
			log::debug!(
				target: LOG_TARGET,
				"Validation outputs checking for teyrchain `{}` failed, error: {:?}",
...
impl<T: Config> CandidateCheckContext<T> {
	...
	pub(crate) fn verify_backed_candidate(
		...
		let validation_code_hash = paras::CurrentCodeHash::<T>::get(para_id)
			// A candidate for a teyrchain without current validation code is not scheduled.
		...
		if let Err(err) = self.check_validation_outputs(
			...
			log::debug!(
				target: LOG_TARGET,
				"Validation outputs checking during inclusion of a candidate {:?} for teyrchain `{}` failed, error: {:?}",
	...
	/// by the teyrchain block.
	///
	/// Limits are checked against the current state. The teyrchain block must be acceptable
	...
	/// considering the information available at the relay-parent of the teyrchain block.
	fn check_validation_outputs(
		&self,
		para_id: ParaId,
		relay_parent_number: BlockNumberFor<T>,
		head_data: &HeadData,
		new_validation_code: &Option<pezkuwi_primitives::ValidationCode>,
		processed_downward_messages: u32,
		upward_messages: &[pezkuwi_primitives::UpwardMessage],
		hrmp_watermark: BlockNumberFor<T>,
		horizontal_messages: &[pezkuwi_primitives::OutboundHrmpMessage<ParaId>],
		...
		.map_err(|e| {
			log::debug!(
				target: LOG_TARGET,
				"Check processed downward messages for teyrchain `{}` on relay parent number `{:?}` failed, error: {:?}",
				u32::from(para_id),
				relay_parent_number,
				e
			);
		...
		Pezpallet::<T>::check_upward_messages(&self.config, para_id, upward_messages).map_err(
			|e| {
				log::debug!(
					target: LOG_TARGET,
					"Check upward messages for teyrchain `{}` failed, error: {:?}",
					u32::from(para_id),
					e
				);
				e
		...
		hrmp::Pezpallet::<T>::check_hrmp_watermark(para_id, relay_parent_number, hrmp_watermark)
			.map_err(|e| {
				log::debug!(
					target: LOG_TARGET,
					"Check hrmp watermark for teyrchain `{}` on relay parent number `{:?}` failed, error: {:?}",
					u32::from(para_id),
					relay_parent_number,
					e
				);
		...
		hrmp::Pezpallet::<T>::check_outbound_hrmp(&self.config, para_id, horizontal_messages)
			.map_err(|e| {
				log::debug!(
					target: LOG_TARGET,
					"Check outbound hrmp for teyrchain `{}` failed, error: {:?}",
					u32::from(para_id),
					e
				);
				e
