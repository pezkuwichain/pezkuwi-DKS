...
fn open_channel_works() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// We need both A & B to be registered and alive teyrchains.
...
fn force_open_channel_works() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// We need both A & B to be registered and live teyrchains.
		register_teyrchain(para_a);
		register_teyrchain(para_b);
...
fn force_open_channel_without_free_balance_works() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// We need both A & B to be registered and live teyrchains, but they should not have any
...
fn verify_externally_accessible() {
	...
	new_test_ext(GenesisConfigBuilder::default().build()).execute_with(|| {
		// Register two teyrchains, wait until a session change, then initiate channel open
...
fn refund_deposit_on_normal_closure() {
	...
	new_test_ext(genesis.build()).execute_with(|| {
		// Register two teyrchains funded with different amounts of funds and arrange a channel.
...
fn refund_deposit_on_offboarding() {
	...
	genesis.hrmp_recipient_deposit = 15;
	new_test_ext(genesis.build()).execute_with(|| {
		// Register two teyrchains and open a channel between them.
		register_teyrchain_with_balance(para_a, 100);
		register_teyrchain_with_balance(para_b, 110);
		run_to_block(5, Some(vec![4, 5]));
		Hrmp::init_open_channel(para_a, para_b, 2, 8).unwrap();
...
fn no_dangling_open_requests() {
	...
	new_test_ext(genesis.build()).execute_with(|| {
		...
		// Then deregister one teyrchain, but don't wait two sessions until it takes effect.
