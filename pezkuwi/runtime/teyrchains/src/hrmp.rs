...
pub mod pezpallet {
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn establish_channel_with_system(
			...
			let sender = ensure_teyrchain(<T as Config>::RuntimeOrigin::from(origin))?;
...
impl<T: Config> Pezpallet<T> {
	...
	fn wrap_notification(
	...
	) -> impl FnOnce(ParaId) -> pezkuwi_primitives::DownwardMessage {
		use xcm::{
			opaque::VersionedXcm,
		...
		move |dest| {
			// Attempt to wrap the notification for the destination teyrchain.
			T::VersionWrapper::wrap_version(
				&Location::new(0, [Junction::Teyrchain(dest.into())]),
				notification(),
			)
			.unwrap_or_else(|_| {
	...
	/// Sends/enqueues notification to the destination teyrchain.
	fn send_to_para(
		log_label: &str,
		config: &HostConfiguration<BlockNumberFor<T>>,
		dest: ParaId,
		notification_bytes_for: impl FnOnce(ParaId) -> pezkuwi_primitives::DownwardMessage,
	) {
