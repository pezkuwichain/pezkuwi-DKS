...
impl<T: Config> Pezpallet<T> {
	...
	fn proceed_with_upgrade(
		...
		// teyrchain will have some time until the upgrade finally takes place.
		...
		for cause in causes {
			...
			match cause {
				PvfCheckCause::Onboarding(id) => {
					...
					// with exception that before actual onboarding the teyrchain did not have a
	...
	pub(crate) fn schedule_para_initialize(
		...
		// Make sure teyrchain isn't already in our system and that the onboarding parameters are
		// valid.
		...
		//   as long we do not allow registering teyrchains with empty code. At the moment of
	...
	///
	/// - para is not a stable teyrchain (i.e. [`ParaLifecycle::is_stable`] is `false`)
	...
	///
	/// Noop if `ParaLifecycle` is not `Teyrchain`.
	...
	/// Schedule a future code upgrade of the given teyrchain.
	///
	...
	fn kick_off_pvf_check(
		...
		// We increase the code RC here in any case. Intuitively the teyrchain that requested this
	...
	/// Set the current code for the given teyrchain.
	// `at` for para-triggered replacement is the block number of the relay-chain
	// block in whose context the parablock was executed
	// (i.e. number of `relay_parent` in the receipt)
	...
	/// Force set the current code for the given teyrchain.
	...
	/// Whether a para ID corresponds to any live lease holding teyrchain.
	///
	/// Includes lease holding teyrchains which will downgrade to a on-demand teyrchains in the
	/// future.
	...
	/// Whether a para ID corresponds to any live parathread (on-demand teyrchain).
	///
	/// Includes on-demand teyrchains which will upgrade to lease holding teyrchains in the future.
	...
	/// If a candidate from the specified teyrchain were submitted at the current block, this
...
/// An overlay over the `Teyrchains` storage entry that provides a convenient interface for adding
/// or removing teyrchains in bulk.
pub(crate) struct TeyrchainsCache<T: Config> {
	// `None` here means the teyrchains list has not been accessed yet, nevermind modified.
...
impl<T: Config> TeyrchainsCache<T> {
	...
	/// Removes the given para id from the list of teyrchains. Does nothing if the id is not in the
	/// list.
