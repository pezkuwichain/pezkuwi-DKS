...
fn upgrade_strategy_apply_at_expected_block_works() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		let expected_at = 1 + validation_upgrade_delay;
		let next_possible_upgrade_at = 1 + validation_upgrade_cooldown;
		// `set_go_ahead` parameter set to `false` which prevents signaling the teyrchain
...
fn upgrade_restriction_elapsed_doesnt_mean_can_upgrade() {
	// Situation: teyrchain scheduled upgrade but it doesn't produce any candidate after
	// `expected_at`. When `validation_upgrade_cooldown` elapsed the teyrchain produces a
...
#[test]
fn full_teyrchain_cleanup_storage() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		// cleaning up the teyrchain should place the current teyrchain code
...
fn para_incoming_at_session() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		assert_eq!(paras::Teyrchains::<Test>::get(), Vec::new());
		assert_eq!(ActionsQueue::<Test>::get(Paras::scheduled_session()), vec![c, b, a],);
		...
		assert_eq!(paras::Teyrchains::<Test>::get(), vec![c, b]);
		...
		assert_eq!(ParaLifecycles::<Test>::get(&b), Some(ParaLifecycle::Teyrchain));
		assert_eq!(ParaLifecycles::<Test>::get(&c), Some(ParaLifecycle::Teyrchain));
...
fn add_trusted_validation_code_insta_approval() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		// Then some teyrchain upgrades it's code with the relay-parent 1.
		run_to_block(2, None);
		Paras::schedule_code_upgrade(
			para_id,
			validation_code.clone(),
...
fn add_trusted_validation_code_enacts_existing_pvf_vote() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		// First, some teyrchain upgrades it's code with the relay-parent 1.
		run_to_block(2, None);
		Paras::schedule_code_upgrade(
			para_id,
			validation_code.clone(),
...
fn most_recent_context() {
	...
	new_test_ext(genesis_config).execute_with(|| {
		...
		assert_eq!(ParaLifecycles::<Test>::get(&para_id), Some(ParaLifecycle::Teyrchain));
...
fn parakind_encodes_decodes_to_bool_serde() {
	let chain = ParaKind::Teyrchain;
	let ser_chain = serde_json::to_string(&ParaKind::Teyrchain).unwrap();
...
#[test]
fn teyrchains_cache_is_set() {
	new_test_ext(MockGenesisConfig::default()).execute_with(|| {
		let a = ParaId::from(111);

		let mut teyrchains_cache: TeyrchainsCache<Test> = TeyrchainsCache::new();

		// Add element twice
		teyrchains_cache.add(a);
		teyrchains_cache.add(a);

		// Flush cache to storage
		drop(teyrchains_cache);

		// In order after addition
		assert_eq!(Teyrchains::<Test>::get(), vec![a]);

		let mut teyrchains_cache: TeyrchainsCache<Test> = TeyrchainsCache::new();

		// Remove element twice
		teyrchains_cache.remove(a);
		teyrchains_cache.remove(a);
		...
		assert_eq!(Teyrchains::<Test>::get(), vec![]);

		let mut teyrchains_cache: TeyrchainsCache<Test> = TeyrchainsCache::new();

		// Remove nonexisting element
		teyrchains_cache.remove(a);
		assert_storage_noop!(drop(teyrchains_cache));
		assert_eq!(Teyrchains::<Test>::get(), vec![]);
	});
}

#[test]
fn teyrchains_cache_preserves_order() {
	new_test_ext(MockGenesisConfig::default()).execute_with(|| {
		let a = ParaId::from(111);
		let b = ParaId::from(222);
		let c = ParaId::from(333);
		...
		// Add in mixed order
		teyrchains_cache.add(b);
		teyrchains_cache.add(c);
		teyrchains_cache.add(a);
		teyrchains_cache.add(d);

		// Flush cache to storage
		drop(teyrchains_cache);

		// In order after addition
		assert_eq!(Teyrchains::<Test>::get(), vec![a, b, c, d]);

		let mut teyrchains_cache: TeyrchainsCache<Test> = TeyrchainsCache::new();

		// Remove 2 elements
		teyrchains_cache.remove(b);
		teyrchains_cache.remove(d);

		// Flush cache to storage
		drop(teyrchains_cache);

		// In order after removal
		assert_eq!(Teyrchains::<Test>::get(), vec![a, c]);
	});
}
...
fn authorize_force_set_current_code_hash_works() {
	new_test_ext(MockGenesisConfig::default()).execute_with(|| {
		...
		ParaLifecycles::<Test>::insert(&para_a, ParaLifecycle::Teyrchain);
		assert!(Paras::is_valid_para(para_a));
		...
		ParaLifecycles::<Test>::insert(&para_b, ParaLifecycle::Teyrchain);
