...
use pezkuwi_primitives::{Id as ParaId, ON_DEMAND_MAX_QUEUE_MAX_SIZE};
use pezsp_runtime::{
...
struct EnqueuedOrder<N> {
	/// The teyrchain the order was placed for.
	para_id: ParaId,
...
pub mod pezpallet {
	...
	impl<T: Config> Pezpallet<T> {
		...
		#[pezpallet::weight(<T as Config>::WeightInfo::place_order_allow_death())]
		#[allow(deprecated)]
		...
		#[pezpallet::weight(<T as Config>::WeightInfo::place_order_keep_alive())]
		#[allow(deprecated)]
		#[deprecated(note = "This will be removed in favor of using `place_order_with_credits`")]
		...
		#[pezpallet::weight(<T as Config>::WeightInfo::place_order_with_credits())]
	...
	BalanceOf<T>: FixedPointOperand,
{
	...
	pub fn pop_assignment_for_cores(
		...
		pezpallet::OrderStatus::<T>::mutate(|order_status| {
	...
	pub fn peek_order_queue() -> OrderQueue<BlockNumberFor<T>> {
		pezpallet::OrderStatus::<T>::get().queue
	}
	...
	fn set_order_status(new_status: OrderStatus<BlockNumberFor<T>>) {
		pezpallet::OrderStatus::<T>::set(new_status);
	}

	#[cfg(test)]
	fn get_order_status() -> OrderStatus<BlockNumberFor<T>> {
		pezpallet::OrderStatus::<T>::get()
	}
