...
mod v1 {
	...
	pub(super) type ParaIdAffinity<T: Config> =
		StorageMap<Pezpallet<T>, Twox64Concat, ParaId, CoreAffinityCount, OptionQuery>;

	#[storage_alias]
	pub(super) type QueueStatus<T: Config> =
		StorageValue<Pezpallet<T>, OldQueueStatus, OptionQuery>;

	#[storage_alias]
	pub(super) type FreeEntries<T: Config> =
		StorageValue<Pezpallet<T>, BinaryHeap<OldEnqueuedOrder>, OptionQuery>;
...
impl<T: Config> UncheckedMigrateToV2<T> {
	pub fn pre_upgrade() -> Result<alloc::vec::Vec<u8>, pezsp_runtime::TryRuntimeError> {
		...
		if total_orders > pezkuwi_primitives::ON_DEMAND_MAX_QUEUE_MAX_SIZE as usize {
			log::error!(
				target: LOG_TARGET,
				"Migration would lose orders: {} total orders exceeds V2 capacity of {}",
				total_orders,
				pezkuwi_primitives::ON_DEMAND_MAX_QUEUE_MAX_SIZE
			);
	...
	pub fn post_upgrade(state: alloc::vec::Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		...
		// Verify new storage
		let new_order_status = super::pezpallet::OrderStatus::<T>::get();
...
impl<T: Config> UncheckedOnRuntimeUpgrade for UncheckedMigrateToV2<T> {
	fn on_runtime_upgrade() -> Weight {
		...
		super::pezpallet::OrderStatus::<T>::mutate(|order_status| {
...
mod tests {
	...
	fn affinity_queue_merging_works() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			...
			StorageVersion::new(1).put::<on_demand::Pezpallet<Test>>();
			...
			// Verify new storage
			let new_status = on_demand::pezpallet::OrderStatus::<Test>::get();
	...
	fn queue_full_handling() {
		new_test_ext(MockGenesisConfig::default()).execute_with(|| {
			let _now = pezframe_system::Pezpallet::<Test>::block_number();
