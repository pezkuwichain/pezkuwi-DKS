...
//! they contributed; the root of this can then be used by the teyrchain to allow contributors to
...
//! can be dissolved.
//!
//! Funds may accept contributions at any point before their success or end. When a teyrchain
//! slot auction enters its ending period, then teyrchains will each place a bid; the bid will be
//! raised once per block if the teyrchain had additional funds contributed since the last bid.
//!
//! Successful funds remain tracked (in the `Funds` storage item and the associated child trie) as
//! long as the teyrchain remains active. Users can withdraw their funds once the slot is completed
...
/// Information on a funding effort for a pre-existing teyrchain. We assume that the teyrchain ID
...
pub mod pezpallet {
	...
	pub enum Event<T: Config> {
		...
		/// A teyrchain has been moved to `NewRaise`
	...
	pub enum Error<T> {
		...
		InvalidOrigin,
		/// This crowdloan does not correspond to a teyrchain.
		NotTeyrchain,
		/// This teyrchain lease is still active and retirement cannot yet begin.
		LeaseActive,
		/// This teyrchain's bid or lease is still active and withdraw cannot yet begin.
		BidOrLeaseActive,
