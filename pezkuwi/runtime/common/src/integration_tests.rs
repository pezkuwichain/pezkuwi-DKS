...
fn basic_end_to_end_works() {
	for offset in [0u32, 50, 100, 200].iter() {
		...
		new_test_ext().execute_with(|| {
			...
			// 2 sessions later they are parathreads (on-demand teyrchains)
			...
			// User 10 will bid directly for teyrchain 1
			...
			// User 2 will be a contribute to crowdloan for teyrchain 2
			Balances::make_free_balance_be(&account_id(2), 1_000_000_000);
			...
			// First slot, Para 1 should be transitioning to lease holding Teyrchain
			assert_eq!(
...
fn competing_slots() {
	...
	new_test_ext().execute_with(|| {
		...
		for n in 1..=max_bids {
			...
			// Users will bid directly for teyrchain
			assert_ok!(Auctions::bid(
				signed(n * 10),
				para_id + n - 1,
				1,                                // Auction Index
	...
	new_test_ext().execute_with(|| {
		...
		for n in 1..=9 {
			...
			if n % 2 == 0 {
				...
				// User 20 will be a contribute to crowdloan for teyrchain 2
...
#[test]
fn basic_swap_works() {
	// This test will test a swap between a lease holding teyrchain and on-demand teyrchain works
	// successfully.
	new_test_ext().execute_with(|| {
		...
		Balances::make_free_balance_be(&account_id(1), 1_000_000_000);
		Balances::make_free_balance_be(&account_id(2), 1_000_000_000);
		// First register 2 on-demand teyrchains with different data
		...
		// 2 sessions later they are on-demand teyrchains
		run_to_session(START_SESSION_INDEX + 2);
		...
		// 2 sessions later it is a teyrchain
		run_to_block(lease_start_block + 20);
		...
		// Deregister on-demand teyrchain
...
fn teyrchain_swap_works() {
	// This test will test a swap between two teyrchains works successfully.
	new_test_ext().execute_with(|| {
		assert!(System::block_number().is_one()); /* So events are emitted */
		...
		// Let's repeat the process now for another teyrchain.
		...
		// This means that teyrchain 1 should only have 6 slots left, and teyrchain 2 has all 8.
...
// This test verifies that if a teyrchain already has won some lease periods, that it cannot bid for
...
fn cant_bid_on_existing_lease_periods() {
	new_test_ext().execute_with(|| {
		...
		Balances::make_free_balance_be(&account_id(1), 1_000_000_000);
		// First register an on-demand teyrchain
		let validation_code = test_validation_code(10);
		assert_ok!(Registrar::reserve(signed(1)));
		assert_ok!(Registrar::register(
			signed(1),
