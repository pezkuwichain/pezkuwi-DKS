...
//! `cargo build -r -F fast-runtime -p pezkuwi-teyrchain-bin && \`
//! `cargo build -r -F fast-runtime --bin pezkuwi --bin pezkuwi-execute-worker --bin
//! pezkuwi-prepare-worker`
//!
//! Running with normal runtimes is possible but would take ages. Running fast relay runtime with
//! normal teyrchain runtime WILL mess things up.
...
type CoretimeUtilityCall = coretime_api::runtime_types::pezpallet_utility::pezpallet::Call;
type CoretimeBrokerCall = coretime_api::runtime_types::pezpallet_broker::pezpallet::Call;
	...
	<C::Header as pezkuwi_subxt::config::Header>::Number: Display,
{
	...
	log::debug!("Starting teyrchain watcher");
	while let Some(block) = blocks_sub.next().await {
		let block = block.unwrap();
		log::debug!("Finalized teyrchain block {}", block.number());
...
async fn coretime_revenue_test() -> Result<(), anyhow::Error> {
	...
	relay_client
		...
		.sign_and_submit_default(
			&pezkuwichain::tx().xcm_pallet().teleport_assets(
				VersionedLocation::V4(Location {
					parents: 0,
					interior: Junctions::X1([Junction::Teyrchain(1005)]),
				}),
				VersionedLocation::V4(Location {
					parents: 0,
	...
	let order = r
		.find_first::<pezkuwichain::on_demand_assignment_provider::events::OnDemandOrderPlaced>()?
		.unwrap();
