...
// Test if teyrchains progress when group is getting spammed by statement distribution requests.
...
async fn spam_statement_distribution_requests_test() -> Result<(), anyhow::Error> {
		...
		.with_relaychain(|r| {
			r.with_chain("pezkuwichain-local")
				...
				.with_default_args(vec![
					// teyrchain::statement-distribution=trace to find
					// "Peer already being served, dropping request"
					("-lteyrchain=debug,teyrchain::statement-distribution=trace").into(),
				])
				...
				.with_validator(|node| {
					node.with_name("malus")
						...
						.with_args(vec![
							...
							"-lMALUS=trace,teyrchain=debug".into(),
						])
				})
		})
		.with_teyrchain(|p| {
	...
	// Ensure teyrchains are registered.
	...
	// Ensure teyrchains made progress.
	assert_para_throughput(
		&relay_client,
		10,
	...
	honest
		.assert(
			"pezkuwi_teyrchain_approval_checking_finality_lag{chain=\"pezkuwichain_local_testnet\"}",
			0.0,
		)
		.await?;
