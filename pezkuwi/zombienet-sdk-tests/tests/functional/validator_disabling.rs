...
async fn validator_disabling_test() -> Result<(), anyhow::Error> {
	...
	while let Some(block) = best_blocks.next().await {
		...
		let disputes = Vec::<(SessionIndex, CandidateHash, DisputeState<BlockNumber>)>::decode(
			&mut &relay_client
				.runtime_api()
				.at(current_hash)
				.call_raw("TeyrchainHost_disputes", None)
				.await?[..],
		)?;
	...
	let concluded_dispute_metric =
		"pezkuwi_teyrchain_candidate_dispute_concluded{validity=\"invalid\"}";
	let teyrchain_candidate_dispute_metric = "teyrchain_candidate_disputes_total";
	// Check that we have at least one dispute
	honest_validator
		.wait_metric_with_timeout(teyrchain_candidate_dispute_metric, |d| d >= 1.0, 600_u64)
		.await?;
	...
	let disabled_validators = Vec::<ValidatorIndex>::decode(
		&mut &relay_client
			...
			.call_raw("TeyrchainHost_disabled_validators", None)
			.await?[..],
	)?;
	...
	let session_validators = Vec::<ValidatorId>::decode(
		&mut &relay_client
			.runtime_api()
			.at(block_hash.unwrap())
			.call_raw("TeyrchainHost_validators", None)
			.await?[..],
	)?;
