...
// Test that a teyrchain that uses a single slot-based collator with elastic scaling can use 12
		...
		.with_teyrchain(|p| {
			p.with_id(2300)
				...
				.with_default_args(vec![
					...
					("-lteyrchain=debug,aura=debug,teyrchain::collator-protocol=trace").into(),
				])
