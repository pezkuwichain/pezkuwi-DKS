...
type VirtualOverseer =
	pezkuwi_node_subsystem_test_helpers::TestSubsystemContextHandle<AvailabilityStoreMessage>;

use pezkuwi_node_clock::MockClock;
	...
	// ValidatorIndex->ChunkIndex mapping should be 1:1 for all core indices.
	{
		...
		for core_index in 0..n_cores {
			...
			test_harness(test_state.clone(), store.clone(), |mut virtual_overseer| async move {
				...
				let chunks =
					pezkuwi_erasure_coding::obtain_chunks_v1(n_validators as _, &available_data)
						.unwrap();
	...
	test_harness(test_state.clone(), store.clone(), |mut virtual_overseer| async move {
...

		{
			let chunks_expected =
				pezkuwi_erasure_coding::obtain_chunks_v1(n_validators as _, &available_data)
					.unwrap();
			let branches = pezkuwi_erasure_coding::branches(chunks_expected.as_ref());
			let (tx, rx) = oneshot::channel();
			let block_msg = AvailabilityStoreMessage::StoreAvailableData {
...
fn stored_but_not_included_data_is_pruned() {
	...
	test_harness(test_state.clone(), store.clone(), |mut virtual_overseer| async move {
		...
		let (tx, rx) = oneshot::channel();
		let chunks =
			pezkuwi_erasure_coding::obtain_chunks_v1(n_validators as _, &available_data).unwrap();
		let branches = pezkuwi_erasure_coding::branches(chunks.as_ref());
