...
use super::*;
use bitvec::bitvec;
use pezkuwi_primitives::{MutateDescriptorV2, OccupiedCore, ScheduledCore};
use pezkuwi_primitives_test_helpers::{dummy_candidate_descriptor_v2, dummy_hash};
...
pub(crate) mod common {
	...
	pub fn test_harness<OverseerFactory, Overseer, TestFactory, Test>(
		...
		Test: Future<Output = ()>,
	{
		let (tx, rx) = pezkuwi_node_subsystem_test_helpers::sender_receiver();
...
mod select_candidates {
	...
	// For testing only one core assigned to a teyrchain, we return this set of availability cores:
	//
	//   [
	//      0: Free,
	//      1: Scheduled(default),
