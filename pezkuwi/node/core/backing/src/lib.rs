...
struct PerSchedulingParentState {
	/// The hash of the scheduling parent on top of which this job is doing it's work.
	parent: Hash,
	/// The node features.
	node_features: NodeFeatures,
	/// The `CoreIndex` assigned to the local validator at this scheduling parent.
...
struct State {
	...
	/// State tracked for all scheduling-parents backing work is ongoing for. This includes
	/// all active leaves.
	per_scheduling_parent: HashMap<Hash, PerSchedulingParentState>,
	/// State tracked for all candidates relevant to the implicit view.
	///
	/// This is guaranteed to have an entry for each candidate with a scheduling parent in the
	/// implicit or explicit view for which a `Seconded` statement has been successfully imported.
	...
	loop {
		futures::select!(
			validated_command = background_validation_rx.next().fuse() => {
				if let Some((scheduling_parent, command)) = validated_command {
...
struct BackgroundValidationParams<S: overseer::CandidateBackingSenderTrait, F> {
	...
	candidate: CandidateReceipt,
	/// The scheduling parent hash. Used as context for runtime API queries and as
...
async fn validate_and_make_available(
	...
	tx_command
		.send((scheduling_parent, make_command(res)))
		.await
		.map_err(Into::into)
}
...
async fn handle_active_leaves_update<Context>(
	...
	// clean up `per_scheduling_parent` according to ancestry
	...
	// as a result.
	{
		...
		state.per_scheduling_parent.retain(|r, _| remaining.contains(&r));
	}
	...
	// add entries in `per_scheduling_parent`. for all new relay-parents.
	for maybe_new in fresh_relay_parents {
		if state.per_scheduling_parent.contains_key(&maybe_new) {
			continue;
		}

		// construct a `PerSchedulingParent` from the runtime API
		// and insert it.
		let per = construct_per_scheduling_parent_state(
			ctx,
			maybe_new,
			&state.keystore,
...
fn core_index_from_statement(
	pezsp_state: &PerSchedulingParentState,
	...
	gum::trace!(
		target: LOG_TARGET,
		group_rotation_info = ?pezsp_state.group_rotation_info,
		?statement,
		validator_to_group = ?pezsp_state.validator_to_group,
		n_cores = pezsp_state.n_cores,
		?candidate_hash,
		"Extracting core index from statement"
	);

	let statement_validator_index = statement.validator_index();
	let Some(Some(group_index)) = pezsp_state.validator_to_group.get(statement_validator_index)
	else {
		gum::debug!(
			target: LOG_TARGET,
			group_rotation_info = ?pezsp_state.group_rotation_info,
	...
	let core_index = pezsp_state
		.group_rotation_info
		.core_for_group(*group_index, pezsp_state.n_cores as _);

	if core_index.0 > pezsp_state.n_cores {
		gum::warn!(target: LOG_TARGET, ?candidate_hash, ?core_index, n_cores = pezsp_state.n_cores, "Invalid CoreIndex");
		return None;
	}

	if let StatementWithPVD::Seconded(candidate, _pvd) = statement.payload() {
		let candidate_para_id = candidate.descriptor.para_id();
		let mut assigned_paras = pezsp_state.claim_queue.iter_claims_for_core(&core_index);
		...
		if !assigned_paras.any(|id| id == &candidate_para_id) {
			gum::debug!(
				...
				?candidate_hash,
				?core_index,
				assigned_paras = ?pezsp_state.claim_queue.iter_claims_for_core(&core_index).collect::<Vec<_>>(),
				?candidate_para_id,
...
#[overseer::contextbounds(CandidateBacking, prefix = self::overseer)]
async fn construct_per_scheduling_parent_state<Context>(
	ctx: &mut Context,
	relay_parent: Hash,
	keystore: &KeystorePtr,
	per_session_cache: &mut PerSessionCache,
) -> Result<Option<PerSchedulingParentState>, Error> {
	...
	Ok(Some(PerSchedulingParentState {
		parent,
		node_features,
		assigned_core,
		backed: HashSet::new(),
	...
	while let Some(response) = responses.next().await {
		match response {
			Err(oneshot::Canceled) => {
				gum::warn!(
					target: LOG_TARGET,
					"Failed to reach prospective teyrchains subsystem for hypothetical membership",
				);
...
async fn handle_validated_candidate_command<Context>(
	...
	match state.per_scheduling_parent.get_mut(&scheduling_parent) {
		Some(pezsp_state) => {
			...
			pezsp_state.awaiting_validation.remove(&candidate_hash);
			...
			match command {
				ValidatedCandidateCommand::Second(res) => match res {
					Ok(outputs) => {
						...
						if pezsp_state.issued_statements.contains(&candidate_hash) {
							return Ok(());
						}
						...
						if let Err(Error::RejectedByProspectiveTeyrchains) = res {
							...
							gum::debug!(
								...
								?candidate_hash,
								"Attempted to second candidate but was rejected by prospective teyrchains",
							);
						...
						if let Some(stmt) = res? {
							...
							pezsp_state.issued_statements.insert(candidate_hash);
				...
				ValidatedCandidateCommand::Attest(res) => {
					...
					pezsp_state.fallbacks.remove(&candidate_hash);
					// sanity check.
					if !pezsp_state.issued_statements.contains(&candidate_hash) {
						if res.is_ok() {
				...
				ValidatedCandidateCommand::AttestNoPoV(candidate_hash) => {
					if let Some(attesting) = pezsp_state.fallbacks.get_mut(&candidate_hash) {
						if let Some(index) = attesting.backing.pop() {
							...
							//
							// If it is not, then no point in validating it anyway.
...
async fn import_statement<Context>(
	ctx: &mut Context,
	pezsp_state: &mut PerSchedulingParentState,
	...
	if let StatementWithPVD::Seconded(candidate, pvd) = statement.payload() {
		if !per_candidate.contains_key(&candidate_hash) {
			...
			// Only save the candidate if it was approved by prospective teyrchains.
	...
	let core =
		core_index_from_statement(pezsp_state, statement).ok_or(Error::CoreIndexUnavailable)?;

	Ok(pezsp_state.table.import_statement(&pezsp_state.table_context, core, stmt))
}
...
async fn post_import_statement_actions<Context>(
	...
	if let Some(attested) = summary.as_ref().and_then(|s| {
		pezsp_state.table.attested_candidate(
			&s.candidate,
			&pezsp_state.table_context,
			pezsp_state.minimum_backing_votes,
		)
	}) {
		let candidate_hash = attested.candidate.hash();

		// `HashSet::insert` returns true if the thing wasn't in there already.
		if pezsp_state.backed.insert(candidate_hash) {
			if let Some(backed) = table_attested_to_backed(attested, &pezsp_state.table_context) {
				let para_id = backed.candidate().descriptor.para_id();
				gum::debug!(
					target: LOG_TARGET,
					candidate_hash = ?candidate_hash,
					scheduling_parent = ?pezsp_state.parent,
					%para_id,
					"Candidate backed",
				);
	...
	issue_new_misbehaviors(ctx, pezsp_state.parent, &mut pezsp_state.table);
}
...
async fn sign_import_and_distribute_statement<Context>(
	...
	if let Some(signed_statement) = sign_statement(&*pezsp_state, statement, keystore, metrics) {
		let summary = import_statement(ctx, pezsp_state, per_candidate, &signed_statement).await?;

		// `Share` must always be sent before `Backed`. We send the latter in
		// `post_import_statement_action` below.
		let smsg =
			StatementDistributionMessage::Share(pezsp_state.parent, signed_statement.clone());
		ctx.send_unbounded_message(smsg);

		post_import_statement_actions(ctx, pezsp_state, summary.as_ref()).await;
...
async fn background_validate_and_make_available<Context>(
	...
	let Some(core_index) = pezsp_state.assigned_core else { return Ok(()) };
	if pezsp_state.awaiting_validation.insert(candidate_hash) {
...
async fn kick_off_validation_work<Context>(
	...
	match pezsp_state.table_context.local_validator_is_disabled() {
		Some(true) => {
	...
	background_validate_and_make_available(
		...
		BackgroundValidationParams {
			...
			scheduling_parent,
			session_index: pezsp_state.session_index,
			node_features: pezsp_state.node_features.clone(),
			persisted_validation_data,
			pov,
			n_validators: pezsp_state.table_context.validators.len(),
...
async fn maybe_validate_and_import<Context>(
	...
	let pezsp_state = match state.per_scheduling_parent.get_mut(&scheduling_parent) {
		Some(r) => r,
		None => {
			gum::trace!(
				target: LOG_TARGET,
				?scheduling_parent,
				"Received statement for unknown scheduling parent"
			);
	...
	if pezsp_state.table_context.validator_is_disabled(&statement.validator_index()) {
		gum::debug!(
			target: LOG_TARGET,
	...
	if let Err(Error::RejectedByProspectiveTeyrchains) = res {
		gum::debug!(
			target: LOG_TARGET,
			?scheduling_parent,
			"Statement rejected by prospective teyrchains."
		);
	...
	if let Some(summary) = summary {
		...
		// prospective teyrchains subsystem. At this point, the candidate
		// has already been accepted by the subsystem.

		let candidate_hash = summary.candidate;

		if Some(summary.group_id) != pezsp_state.assigned_core {
			return Ok(());
		}
		...
		let attesting = match statement.payload() {
			StatementWithPVD::Seconded(receipt, _) => {
				...
				pezsp_state.fallbacks.insert(summary.candidate, attesting.clone());
				attesting
			},
			StatementWithPVD::Valid(candidate_hash) => {
				if let Some(attesting) = pezsp_state.fallbacks.get_mut(candidate_hash) {
					let our_index = pezsp_state.table_context.validator.as_ref().map(|v| v.index());
					if our_index == Some(statement.validator_index()) {
						return Ok(());
					}

					if pezsp_state.awaiting_validation.contains(candidate_hash) {
						// Job already running:
...
async fn handle_second_message<Context>(
	...
	let pezsp_state = match state.per_scheduling_parent.get_mut(&scheduling_parent) {
		None => {
			gum::trace!(
				...
				?scheduling_parent,
				?candidate_hash,
				"Candidate has scheduling_parent outside of our view."
			);
	...
	// Get the scheduling info (assigned_core and claim_queue) from the scheduling_parent state
	let assigned_core = pezsp_state.assigned_core;
	let claim_queue = &pezsp_state.claim_queue;
	...
	if pezsp_state.table_context.local_validator_is_disabled().unwrap_or(false) {
		gum::warn!(target: LOG_TARGET, "Local validator is disabled. Don't validate and second");
		return Ok(());
	}

	// For V3, use scheduling info from scheduling_parent (claim queue determines assignments)
	let assigned_paras = assigned_core.and_then(|core| claim_queue.0.get(&core));

	// Sanity check that candidate is from our assignment.
	if !matches!(assigned_paras, Some(paras) if paras.contains(&candidate.descriptor().para_id())) {
		gum::debug!(
			target: LOG_TARGET,
			our_assignment_core = ?assigned_core,
...
async fn handle_statement_message<Context>(
	...
	match maybe_validate_and_import(ctx, state, scheduling_parent, statement).await {
...
fn handle_get_backable_candidates_message(
	...
	for (para_id, para_candidates) in requested_candidates {
		for candidate_ref in para_candidates.iter() {
			...
			let pezsp_state = match state.per_scheduling_parent.get(&scheduling_parent) {
				Some(pezsp_state) => pezsp_state,
				None => {
					gum::debug!(
						target: LOG_TARGET,
						?scheduling_parent,
			...
			let maybe_backed_candidate = pezsp_state
				.table
				.attested_candidate(
					&candidate_hash,
					&pezsp_state.table_context,
				...
				.and_then(|attested| {
					table_attested_to_backed(attested, &pezsp_state.table_context)
				});
