...
type VirtualOverseer =
	pezkuwi_node_subsystem_test_helpers::TestSubsystemContextHandle<CandidateBackingMessage>;
...
fn make_erasure_root(test: &TestState, pov: PoV, validation_data: PersistedValidationData) -> Hash {
	...
	let chunks =
		pezkuwi_erasure_coding::obtain_chunks_v1(test.validators.len(), &available_data).unwrap();
...
impl TestCandidateBuilder {
	fn build(self) -> CommittedCandidateReceipt {
		CommittedCandidateReceipt {
			descriptor: pezkuwi_primitives::CandidateDescriptorV2::new(
				self.para_id,
				self.relay_parent,
				CoreIndex(0),
...
fn extract_core_index_from_statement_works() {
	...
	let pezsp_state = PerSchedulingParentState {
	...
	let core_index_1 = core_index_from_statement(&pezsp_state, &signed_statement_1).unwrap();

	assert_eq!(core_index_1, CoreIndex(0));

	let core_index_2 = core_index_from_statement(&pezsp_state, &signed_statement_2);

	// Must be none, para_id in descriptor is different than para assigned to core
	assert_eq!(core_index_2, None);

	let core_index_3 = core_index_from_statement(&pezsp_state, &signed_statement_3).unwrap();
...
fn candidate_backing_reorders_votes() {
	...
	let fake_attestation = |idx: u32| {
		...
		pezkuwi_statement_table::generic::ValidityAttestation::Implicit(sig)
	};
...
// Test that a seconded candidate which is not approved by prospective teyrchains
// subsystem doesn't change the view.
#[test]
fn prospective_teyrchains_reject_candidate() {
	let mut test_state = TestState::default();
	test_harness(test_state.keystore.clone(), |mut virtual_overseer| async move {
		// Candidate is seconded in a parent of the activated `leaf_a`.
		const LEAF_A_BLOCK_NUMBER: BlockNumber = 100;
...
// Tests that validators start work on consecutive prospective teyrchain blocks.
#[test]
...
fn occupied_core_assignment() {
	...
	test_harness(test_state.keystore.clone(), |mut virtual_overseer| async move {
		...
		let mut candidate_descriptor =
			pezkuwi_primitives_test_helpers::dummy_candidate_descriptor(Hash::zero());
...
fn version_acceptance_before_and_after_v3_activation_on_statement() {
	...
	test_harness(test_state.keystore.clone(), |mut virtual_overseer| async move {
		...
		// prospective teyrchains for import — this is the first step of the
