...
//! Utility for managing teyrchain fragments not referenced by the relay-chain.
//!
//! # Overview
//!
...
//! - A [`SchedulingScope`] shared across all teyrchains, containing the scheduling parent block and
...
//!   sessions old.
//!
//! - A [`Scope`] per teyrchain, containing the base constraints derived from the latest included
//!   candidate, the candidates pending availability, and the maximum chain length.
//!
//! Each teyrchain has a single `FragmentChain` for each scheduling parent block in the view where
//! it is scheduled.
//!
...
//! number of claim queue slots this teyrchain occupies across all assigned cores. Only backed
...
//! teyrchain forks before a winner is determined.
//!
//! We accept teyrchain forks only if the fork selection rule allows for it. In other words, if we
...
//! to not accept teyrchain forks, candidates could end up getting only half of the backing votes or
//! even less (for forks of larger arity). This would affect the validator rewards. Still, we don't
//! guarantee that a fork-producing teyrchain will be able to fully use elastic scaling.
//!
...
//! # Teyrchain cycles
//!
//! Teyrchains can create cycles, because:
//!   1. There's no requirement that head-data is unique for a teyrchain. Furthermore, a teyrchain
...
//!
//! On the other hand, enforcing that a teyrchain will NEVER be acyclic would be very complicated
//! (looping through the entire teyrchain's history on every new candidate or changing the candidate
//! receipt to reference the parent's candidate hash).
//!
//! Therefore, we don't provide a guarantee that a cycle-producing teyrchain will work (although in
...
//! Our assumption is that the amount of candidates and teyrchains we consider will be reasonably
...
use pezkuwi_node_subsystem::messages::{Ancestors, BackableCandidateRef};
...
///
/// Represents the scheduling parent blocks that teyrchain candidates can be anchored in.
