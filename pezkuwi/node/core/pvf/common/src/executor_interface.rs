...
pub unsafe fn create_runtime_from_artifact_bytes(
	...
	let ecc_hf_enabled = executor_params.iter().any(|p| {
		p == &ExecutorParam::EnabledHostFunction(
			pezkuwi_primitives::ExecutorHostFunction::EccRfc163,
		)
	});

	if ecc_hf_enabled {
		pezsc_executor_wasmtime::create_runtime_from_artifact_bytes::<HostFunctionsWithEcc>(
...
type HostFunctionsWithEcc = (HostFunctions, pezsp_crypto_ec_utils::HostFunctionsRfc163);
