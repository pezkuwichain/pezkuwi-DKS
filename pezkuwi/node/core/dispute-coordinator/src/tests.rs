...
use pezkuwi_node_clock::{Clock, MockClock};
...
fn make_v3_candidate_receipt(
	...
	use pezkuwi_primitives::CandidateDescriptorV2;
	let descriptor = CandidateDescriptorV2::new_from_raw(
		...
		pezkuwi_primitives_test_helpers::dummy_validation_code().hash(), // validation_code_hash
	);
	...
	test_harness(|mut test_state, mut virtual_overseer| {
		Box::pin(async move {
			...
			let supermajority_threshold =
				pezkuwi_primitives::supermajority_threshold(test_state.validators.len());
...
// When a dispute has concluded against a teyrchain block candidate we want to notify
	...
	test_harness(|mut test_state, mut virtual_overseer| {
		Box::pin(async move {
			...
			let byzantine_threshold =
				pezkuwi_primitives::byzantine_threshold(test_state.validators.len());
