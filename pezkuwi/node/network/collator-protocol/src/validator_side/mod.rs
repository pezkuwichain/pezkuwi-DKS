...
struct State {
	...
	/// [`pezkuwi_node_network_protocol::View`] and can be dropped once the transition
	...
	/// Teyrchains we're currently assigned to. With async backing enabled
...
{
	let validators = pezkuwi_node_subsystem_util::request_validators(relay_parent, sender)
		.await
		.await
		.map_err(Error::CancelledActiveValidators)??;

	let (groups, rotation_info) =
		pezkuwi_node_subsystem_util::request_validator_groups(relay_parent, sender)
			.await
			.await
