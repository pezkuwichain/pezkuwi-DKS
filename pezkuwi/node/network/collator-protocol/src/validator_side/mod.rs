...
//! - **Function:** `can_second()` → backing subsystem → prospective-teyrchain
//! - **Checks:**
...
struct State {
	...
	///
	/// Relay-chain blocks which don't support prospective teyrchains are
	...
	/// ancestry of some active leaf, then it does support prospective teyrchains.
	implicit_view: ImplicitView,
	...
	/// prospective-teyrchains. Mapped from the paraid and parent_head_hash to the fetched
...
impl State {
	...
	fn seconded_and_pending_for_para(&self, scheduling_parent: &Hash, para_id: &ParaId) -> usize {
		...
		let pending_fetch =
			self.per_scheduling_parent.get(scheduling_parent).map_or(0, |pezsp_state| {
				match pezsp_state.collations.status {
...
async fn construct_per_scheduling_parent<Sender>(
	sender: &mut Sender,
...
{
	let validators = pezkuwi_node_subsystem_util::request_validators(scheduling_parent, sender)
		.await
		.await
		.map_err(Error::CancelledActiveValidators)??;

	let (groups, rotation_info) =
		pezkuwi_node_subsystem_util::request_validator_groups(scheduling_parent, sender)
			.await
			.await
			.map_err(Error::CancelledValidatorGroups)??;

	let core_now = if let Some(group) =
		pezkuwi_node_subsystem_util::signing_key_and_index(&validators, keystore).and_then(
			|(_, index)| pezkuwi_node_subsystem_util::find_validator_group(&groups, index),
		) {
		rotation_info.core_for_group(group, groups.len())
	} else {
		gum::trace!(target: LOG_TARGET, ?scheduling_parent, "Not a validator");
		return Ok(None);
	};
		...
		if let Some(para_id) = peer_data.collating_para() {
			...
			if !para_still_assigned {
				gum::trace!(
					...
					?peer_id,
					?para_id,
					"Disconnecting peer on view change (not current teyrchain id)"
				);
...
async fn handle_network_msg<Context>(
	...
	match bridge_message {
		PeerConnected(peer_id, observed_role, protocol_version, _) => {
			...
			// Since the AssetHub system teyrchain is very important for the operation of the
			// relay chain we need to ensure that we always accept connections from the invulnerable
			// AssetHub collators.
			// However there is a connection limit in `pezkuwi-node-network-protocol` of the number
...
async fn process_msg<Context>(
	...
	match msg {
		...
		Seconded(parent, stmt) => {
			...
			let fetched_collation = FetchedCollation::new(&receipt.to_plain());
		...
		Invalid(parent, candidate_receipt) => {
			...
			let fetched_collation = FetchedCollation::new(&candidate_receipt);
	...
	Sender: CollatorProtocolSenderTrait,
{
	...
	pezkuwi_node_subsystem_util::request_persisted_validation_data(
		relay_parent,
		para_id,
	...
	if let Entry::Vacant(entry) = state.fetched_candidates.entry(fetched_collation) {
		...
		let pvd = match (maybe_pvd, maybe_parent_head.clone(), maybe_parent_head_hash) {
			...
			(None, None, Some(parent_head_data_hash)) => {
				...
				// prospective-teyrchains. We add this to the blocked_from_seconding collection
