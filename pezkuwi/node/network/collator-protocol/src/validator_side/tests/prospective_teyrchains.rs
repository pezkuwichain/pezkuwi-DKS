...
//! Tests for the validator side with enabled prospective teyrchains.
		...
		assert_matches!(
			...
			AllMessages::ChainApi(ChainApiMessage::BlockHeader(hash, tx)) if hash == leaf_hash => {
				let slot = leaf_slot.unwrap_or_else(|| Slot::from_timestamp(
					...
					pezsp_consensus_slots::SlotDuration::from_millis(RELAY_CHAIN_SLOT_DURATION_MILLIS),
				));
...
fn v3_scheduling_parent_finished_slot_accepts_leaf() {
	...
	test_harness(ReputationAggregator::new(|_| true), HashSet::new(), |test_harness| async move {
		...
		let finished_slot = Slot::from_timestamp(
			pezsp_timestamp::Timestamp::new(
				*pezsp_timestamp::Timestamp::current() - RELAY_CHAIN_SLOT_DURATION_MILLIS,
			),
			pezsp_consensus_slots::SlotDuration::from_millis(RELAY_CHAIN_SLOT_DURATION_MILLIS),
		);
...
fn v1_descriptor_version_detection_with_v3_enabled() {
	...
	test_harness(ReputationAggregator::new(|_| true), HashSet::new(), |test_harness| async move {
		...
		let v1_descriptor = CandidateDescriptor {
			...
			relay_parent: head_b,
			collator: CollatorId::from(pezsp_core::sr25519::Public::from_raw(collator_bytes)),
			persisted_validation_data_hash: dummy_pvd().hash(),
			pov_hash: Hash::zero(),
			erasure_root: Hash::zero(),
			signature: CollatorSignature::from(pezsp_core::sr25519::Signature::from_raw(
				signature_bytes,
			)),
			para_head: Hash::zero(),
		...
		// NOT ProspectiveTeyrchains::GetProspectiveValidationData.
