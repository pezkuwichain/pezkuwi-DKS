...
use pezkuwi_node_subsystem::CollatorProtocolSenderTrait;
use pezkuwi_node_subsystem_util::{database::Database, reputation::ReputationAggregator};
...
use pezkuwi_primitives::{CollatorPair, Hash, RELAY_CHAIN_SLOT_DURATION_MILLIS};
...
struct LeafSchedulingInfo {
	...
	slot: pezsp_consensus_slots::Slot,
}
...
pub(crate) fn is_scheduling_parent_valid(
	...
	let current_slot = pezsp_consensus_slots::Slot::from_timestamp(
		pezsp_timestamp::Timestamp::new(clock.duration_since_epoch().as_millis() as u64),
		slot_duration,
	);
