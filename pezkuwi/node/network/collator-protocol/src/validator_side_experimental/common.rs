...
/// many collators we can have per teyrchain.
///
/// The combination of `VALID_INCLUDED_CANDIDATE_BUMP` and `INACTIVITY_DECAY` determines the maximum
/// number of collators for the teyrchain. We assume that each collator has got an equal chance in
...
///   know the number of collators in the teyrchain and actually know when a collator skips its
///   slot.
...
/// The value is optimal for lookahead=5. In that case we can have no more than 5 teyrchains in the
...
/// less than the maximum score for the teyrchain.
