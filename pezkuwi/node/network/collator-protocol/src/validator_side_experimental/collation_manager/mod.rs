...
pub struct CollationManager {
	...
	// prospective-teyrchains. Mapped from the para_id and parent_head_hash to the fetched
...
impl CollationManager {
	...
	fn build_leaf_core_cqs(&self) -> Vec<LeafCoreCq> {
		...
		for leaf in leaves {
			for &core in &cores {
				...
				for (depth, pezsp_hash) in
					sps_by_depth.iter().enumerate().filter_map(|(i, x)| x.map(|h| (i, h)))
				{
					let Some(per_sp) = self.per_scheduling_parent.get(&pezsp_hash) else {
						continue;
					};
					...
					let in_flight = self
						.fetching
						.iter()
						.filter(|adv| adv.scheduling_parent == pezsp_hash)
						.map(|adv| adv.para_id);
	...
	async fn get_session_info<Sender: CollatorProtocolSenderTrait>(
		...
		if self.per_session.get(&index).is_none() {
			...
			let our_group =
				pezkuwi_node_subsystem_util::signing_key_and_index(&validators, &self.keystore)
					.and_then(|(_, index)| {
						pezkuwi_node_subsystem_util::find_validator_group(&groups, index)
					});
...
struct FetchedCollation {
	...
	/// Proof of validity.
	pub pov: PoV,
	/// Optional teyrchain parent head data. This is needed for elastic scaling to work.
	...
	fn pick_best_advertisement_works() {
		...
		let new_collation_manager_instance = || CollationManager {
			...
			per_scheduling_parent: HashMap::from([(
				scheduling_parent,
				PerSchedulingParent::new(0, CoreIndex(0), &*pezkuwi_node_clock::system_clock()),
			)]),
			blocked_from_seconding: HashMap::new(),
			per_session: LruMap::new(ByLength::new(2)),
			fetching: PendingRequests::default(),
			keystore: Arc::new(pezsc_keystore::LocalKeystore::in_memory()),
			leaf_scheduling_info: HashMap::default(),
