...
use pezsc_network::{OutboundFailure, RequestFailure};
use pezsc_network_types::multihash::Multihash;
use pezsp_consensus_babe::digests::{PreDigest, SecondaryPlainPreDigest};
			...
			match msg {
				AllMessages::RuntimeApi(RuntimeApiMessage::Request(
					...
					let events = self
						...
						.iter()
						.map(|ccr| {
							pezkuwi_primitives::CandidateEvent::CandidateIncluded(
								ccr.to_plain(),
								Default::default(),
								Default::default(),
								Default::default(),
...
async fn v3_advertisement_accepted_when_sp_is_finished_slot_leaf() {
	...
	let slot_duration = pezsp_consensus_slots::SlotDuration::from_millis(
		pezkuwi_primitives::RELAY_CHAIN_SLOT_DURATION_MILLIS,
	);
	let current_slot = pezsp_consensus_slots::Slot::from_timestamp(
		pezsp_timestamp::Timestamp::current(),
		slot_duration,
	);
	test_state
		.slot_overrides
		.insert(get_hash(10), pezsp_consensus_slots::Slot::from(*current_slot - 1));
...
// Short claim queues (e.g. on-demand teyrchains where not every CQ position is filled) must
...
// teyrchain with only the next slot scheduled). Advertise para 100 at sp=8 (depth 2). Slot
...
async fn short_claim_queue_does_not_reject_ancestor_advertisements() {
	...
	// Advertise at sp=8 (depth 2 from leaf=10). leaf-CQ position 0 maps to pezsp-position 3,
