...
async fn handle_active_leaf_update<Context>(
	...
	let disabled_validators: HashSet<_> =
		pezkuwi_node_subsystem_util::request_disabled_validators(new_relay_parent, ctx.sender())
			.await
			.await
	...
	let group_rotation_info =
		pezkuwi_node_subsystem_util::request_validator_groups(new_relay_parent, ctx.sender())
			.await
			.await
			.map_err(JfyiError::RuntimeApiUnavailable)?
	...
	let claim_queue = ClaimQueueSnapshot(
		pezkuwi_node_subsystem_util::request_claim_queue(new_relay_parent, ctx.sender())
			.await
			.await
			.map_err(JfyiError::RuntimeApiUnavailable)?
