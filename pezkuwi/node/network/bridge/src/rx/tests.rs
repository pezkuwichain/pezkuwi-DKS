...
use pezkuwi_node_network_protocol::{self as net_protocol, OurView};
use pezkuwi_node_subsystem::messages::NetworkBridgeEvent;

use assert_matches::assert_matches;
use async_trait::async_trait;
use parking_lot::Mutex;
use pezkuwi_overseer::TimeoutExt;
use std::{
	collections::HashSet,
...
fn new_test_network(
	...
	let (validation_tx, validation_rx) = pezkuwi_node_subsystem_test_helpers::single_item_sink();
	let (collation_tx, collation_rx) = pezkuwi_node_subsystem_test_helpers::single_item_sink();
...
impl TestNetworkHandle {
	...
	async fn connect_peer(
		...
		let negotiated_fallback = match (protocol_version.into(), peer_set) {
			(1, PeerSet::Collation) => Some(ProtocolName::from("/pezkuwi/collation/1")),
...
fn network_new_topology_update() {
	...
	test_harness(Box::new(oracle), |test_harness| async move {
		...
		virtual_overseer
			.send(pezkuwi_overseer::FromOrchestra::Communication {
