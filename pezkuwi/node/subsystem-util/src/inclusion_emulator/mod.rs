...
/// teyrchain blocks in order to build prospective teyrchains that are produced ahead of the
...
/// exhaustively define the set of valid inputs and outputs to teyrchain execution. A
/// [`Fragment`] indicates a teyrchain block, anchored to the relay-chain at a particular
...
/// that block, assuming there is no intermediate teyrchain block pending availability.
///
...
/// ## Fragment Chains
///
/// For the sake of this module, we don't care how higher-level code is managing teyrchain
/// fragments, whether or not they're kept as a chain or tree. In reality,
/// prospective-teyrchains is maintaining for every active leaf, a chain of the "best" backable
...
/// teyrchain) and code upgrades are fairly rare. So what's likely to happen around code
...
use pezkuwi_node_subsystem::messages::HypotheticalCandidate;
...
/// A teyrchain fragment, representing another prospective teyrchain block.
///
