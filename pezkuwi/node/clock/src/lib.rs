...
//! `clippy.toml` (see `pezkuwi-collator-protocol` for an example).
//!
//! Note: not every subsystem currently routes all of its time reads through this
//! trait. In particular, `pezkuwi-node-core-chain-selection`,
//! `pezkuwi-node-core-av-store`, and `pezkuwi-node-core-dispute-coordinator`
