...
pub struct ApprovalTestState {
	...
	approval_voting_parallel_metrics: pezkuwi_node_core_approval_voting_parallel::Metrics,
	...
	fn new(
		...
		let state = ApprovalTestState {
			...
			approval_voting_parallel_metrics:
				pezkuwi_node_core_approval_voting_parallel::Metrics::try_register(
...
fn build_overseer(
	...
	let db: pezkuwi_node_subsystem_util::database::kvdb_impl::DbAdapter<kvdb_memorydb::InMemory> =
		pezkuwi_node_subsystem_util::database::kvdb_impl::DbAdapter::new(db, &[]);
...
pub async fn bench_approvals_run(
	...
	gum::info!("Awaiting pezkuwi_teyrchain_subsystem_bounded_received to tells us the messages have been processed");
