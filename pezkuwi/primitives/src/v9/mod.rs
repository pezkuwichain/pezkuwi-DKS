...
pub const POLKADOT_ENGINE_ID: pezsp_runtime::ConsensusEngineId = *b"POL1";
...
impl ConsensusLog {
	...
	pub fn from_digest_item(
		...
		match digest_item {
			pezsp_runtime::DigestItem::Consensus(id, encoded) if id == &POLKADOT_ENGINE_ID => {
...
/// An either implicit or explicit attestation to the validity of a teyrchain
/// candidate.
...
/// Statements that can be made about teyrchain candidates. These are the
/// actual values that are signed.
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Debug)]
#[cfg_attr(feature = "std", derive(Hash))]
pub enum CompactStatement {
	/// Proposal of a teyrchain candidate.
	Seconded(CandidateHash),
	/// State that a teyrchain candidate is valid.
	Valid(CandidateHash),
}
...
pub struct SessionInfo {
	...
	/// [`max_validators`](https://github.com/paritytech/polkadot/blob/a52dca2be7840b23c19c153cf7e110b1e3e475f8/runtime/teyrchains/src/configuration.rs#L148).
	///
...
pub struct SchedulerParams<BlockNumber> {
	/// How often teyrchain groups should be rotated across teyrchains.
	///
	/// Must be non-zero.
...
impl<H> CandidateDescriptorV2<H> {
	fn v3_version(&self) -> CandidateDescriptorVersion {
		...
		// drop a teyrchain block, which is not a big deal on v1, where we are
...
/// Signals that a teyrchain can send to the relay chain via the UMP queue.
#[derive(PartialEq, Eq, Clone, Encode, Decode, TypeInfo, Debug)]
pub enum UMPSignal {
	/// A message sent by a teyrchain to select the core the candidate is committed to.
	...
	/// A message sent by a teyrchain to promote the reputation of a given peerid.
	ApprovedPeer(ApprovedPeerId),
}

/// The default claim queue offset to be used if it's not configured/accessible in the teyrchain
/// runtime
...
pub enum CommittedCandidateReceiptError {
	...
	UmpSignalDecode,
	/// The teyrchain is not assigned to any core at specified claim queue offset.
	#[cfg_attr(
		feature = "std",
		error("The teyrchain is not assigned to any core at specified claim queue offset")
	)]
	NoAssignment,
	/// Unknown version.
	...
	DuplicateUMPSignal,
	/// If the teyrchain runtime started sending ump signals, v1 descriptors are no longer
	/// allowed.
	...
	pub fn parse_ump_signals(
		...
		match self.descriptor.version() {
			CandidateDescriptorVersion::V1 => {
				// If the teyrchain runtime started sending ump signals, v1 descriptors are no
				// longer allowed.
				if !signals.is_empty() {
...
/// The claim queue mapped by teyrchain id.
