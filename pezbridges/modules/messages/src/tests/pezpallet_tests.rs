...
fn send_regular_message(lane_id: TestLaneIdType) {
	...
	let valid_message = Pezpallet::<TestRuntime, ()>::validate_message(lane_id, &REGULAR_PAYLOAD)
		.expect("validate_message has failed");
	let artifacts = Pezpallet::<TestRuntime, ()>::send_message(valid_message);
...
fn receive_messages_delivery_proof() {
	...
	assert_ok!(Pezpallet::<TestRuntime>::receive_messages_delivery_proof(
...
fn pezpallet_rejects_transactions_if_halted() {
	run_test(|| {
		...
		assert_noop!(
			Pezpallet::<TestRuntime>::receive_messages_delivery_proof(
				RuntimeOrigin::signed(1),
				delivery_proof,
				UnrewardedRelayersState {
					unrewarded_relayer_entries: 1,
...
fn pezpallet_rejects_new_messages_in_rejecting_outbound_messages_operating_mode() {
	run_test(|| {
		...
		assert_ok!(Pezpallet::<TestRuntime>::receive_messages_proof(
			RuntimeOrigin::signed(1),
			TEST_RELAYER_A,
...
fn send_message_rejects_too_large_message() {
	run_test(|| {
		...
		let valid_message =
			Pezpallet::<TestRuntime, ()>::validate_message(test_lane_id(), &message_payload)
				.expect("validate_message has failed");
		Pezpallet::<TestRuntime, ()>::send_message(valid_message);
	})
}
...
fn receive_messages_delivery_proof_rewards_relayers() {
	run_test(|| {
		...
		let result = Pezpallet::<TestRuntime>::receive_messages_delivery_proof(
			RuntimeOrigin::signed(1),
	...
	run_test(|| {
		fn submit_with_unspent_weight(
			...
			*payload.dispatch_result.unspent_weight.ref_time_mut() = unspent_weight;
			...
			let result = Pezpallet::<TestRuntime>::receive_messages_proof(
				RuntimeOrigin::signed(1),
				TEST_RELAYER_A,
				proof,
				messages_count,
...
fn proof_size_refund_from_receive_messages_proof_works() {
	run_test(|| {
		...
		let post_dispatch_weight = Pezpallet::<TestRuntime>::receive_messages_proof(
			RuntimeOrigin::signed(1),
			TEST_RELAYER_A,
			proof.clone(),
			messages_count,
...
fn inbound_message_details_works() {
	run_test(|| {
		assert_eq!(
			Pezpallet::<TestRuntime>::inbound_message_data(
				test_lane_id(),
...
fn send_messages_fails_if_outbound_lane_is_not_opened() {
	run_test(|| {
		assert_noop!(
			Pezpallet::<TestRuntime, ()>::validate_message(unknown_lane_id(), &REGULAR_PAYLOAD),
			Error::<TestRuntime, ()>::LanesManager(LanesManagerError::UnknownOutboundLane),
		);

		assert_noop!(
			Pezpallet::<TestRuntime, ()>::validate_message(closed_lane_id(), &REGULAR_PAYLOAD),
...
fn do_try_state_for_outbound_lanes_works() {
	run_test(|| {
		...
		assert_err!(
			Pezpallet::<TestRuntime>::do_try_state(),
