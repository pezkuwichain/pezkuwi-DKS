...
//! Pezpallet that may be used instead of `SovereignPaidRemoteExporter` or `UnpaidRemoteExporter`
//! in the XCM router configuration. The main thing that the pezpallet offers is the dynamic
...
//!
//! The pezpallet is configurable to use either paid or unpaid execution on the bridge hub
...
//! system teyrchains where the bridge hub waives fees).
//!
...
///
/// This doesn't match the pattern used by other bridge pezpallets (`runtime::bridge-*`). But this
/// pezpallet has significant differences with those pezpallets. The main one is that is intended to
/// be deployed at sending chains. Other bridge pezpallets are likely to be deployed at the separate
...
pub mod pezpallet {
	...
	pub trait Config<I: 'static = ()>: pezframe_system::Config {
		...
		/// Set to `ConstBool<true>` for system teyrchains where the bridge hub waives fees,
...
// This pezpallet acts as the `ExporterFor` for the inner exporter to compute
