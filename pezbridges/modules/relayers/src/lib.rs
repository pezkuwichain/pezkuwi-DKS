...
pub mod pezpallet {
		...
		BeneficiaryOf<T, I>: From<<T as pezframe_system::Config>::AccountId>,
	{
		...
		pub fn register(origin: OriginFor<T>, valid_till: BlockNumberFor<T>) -> DispatchResult {
			...
			let lease = valid_till.saturating_sub(pezframe_system::Pezpallet::<T>::block_number());
			ensure!(
			...
			RegisteredRelayers::<T, I>::try_mutate(
				...
				|maybe_registration| -> DispatchResult {
					...
					// we can't deregister until `valid_till + 1`
						...
						registration.valid_till < pezframe_system::Pezpallet::<T>::block_number(),
	...
	impl<T: Config<I>, I: 'static> Pezpallet<T, I> {
		...
		pub fn is_registration_active(relayer: &T::AccountId) -> bool {
			...
			let remaining_lease = registration
				.valid_till
				.saturating_sub(pezframe_system::Pezpallet::<T>::block_number());
...
impl<T: Config<I>, I: 'static, Reward, RewardBalance>
	RewardLedger<T::AccountId, Reward, RewardBalance> for Pezpallet<T, I>
where
...
mod tests {
	...
	fn register_relayer_reward_emit_event() {
		run_test(|| {
			...
			Pezpallet::<TestRuntime>::register_relayer_reward(
	...
	fn slash_and_deregister_works() {
		run_test(|| {
			...
			// register
			assert_ok!(Pezpallet::<TestRuntime>::register(
				RuntimeOrigin::signed(REGISTER_RELAYER),
				150,
			));
			// check if registered
			let registration =
				Pezpallet::<TestRuntime>::registered_relayer(&REGISTER_RELAYER).unwrap();
	...
	fn root_cant_claim_anything() {
		run_test(|| {
			assert_noop!(
				Pezpallet::<TestRuntime>::claim_rewards(
					RuntimeOrigin::root(),
					test_reward_account_param()
				),
	...
	fn relayer_can_claim_reward() {
		run_test(|| {
			...
			assert_ok!(Pezpallet::<TestRuntime>::claim_rewards(
				RuntimeOrigin::signed(REGULAR_RELAYER),
				test_reward_account_param()
			));
			assert_eq!(
	...
	fn relayer_can_claim_reward_to() {
		run_test(|| {
			...
			assert_ok!(Pezpallet::<TestRuntime>::claim_rewards_to(
				RuntimeOrigin::signed(REGULAR_RELAYER),
				test_reward_account_param(),
				REGULAR_RELAYER2,
			));
	...
	fn register_fails_if_valid_till_is_a_past_block() {
		run_test(|| {
			...
			assert_noop!(
				Pezpallet::<TestRuntime>::register(RuntimeOrigin::signed(REGISTER_RELAYER), 50),
	...
	fn register_fails_if_valid_till_lease_is_less_than_required() {
		run_test(|| {
			...
			assert_noop!(
				Pezpallet::<TestRuntime>::register(
					RuntimeOrigin::signed(REGISTER_RELAYER),
					99 + Lease::get()
				),
				Error::<TestRuntime>::InvalidRegistrationLease,
	...
	fn register_works() {
		run_test(|| {
			...
			assert_eq!(
				Pezpallet::<TestRuntime>::registered_relayer(&REGISTER_RELAYER),
	...
	fn register_fails_if_new_valid_till_is_lesser_than_previous() {
		run_test(|| {
			...
			assert_noop!(
				Pezpallet::<TestRuntime>::register(RuntimeOrigin::signed(REGISTER_RELAYER), 125),
	...
	fn register_fails_if_it_cant_unreserve_some_balance_if_required_stake_decreases() {
		run_test(|| {
			...
			assert_noop!(
				Pezpallet::<TestRuntime>::register(RuntimeOrigin::signed(REGISTER_RELAYER), 150),
	...
	fn deregister_fails_if_not_registered() {
		run_test(|| {
			assert_noop!(
				Pezpallet::<TestRuntime>::deregister(RuntimeOrigin::signed(REGISTER_RELAYER)),
	...
	fn is_registration_active_is_false_for_unregistered_relayer() {
		run_test(|| {
			assert!(!Pezpallet::<TestRuntime>::is_registration_active(&REGISTER_RELAYER));
		});
	}
