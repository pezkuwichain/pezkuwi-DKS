...
//!   by registering corresponding reward in the pezpallet;
//!
	...
	C::LaneId: From<LaneIdOf<R, C::BridgeMessagesPalletInstance>>,
{
	...
	///
	/// - with reasonable bundled messages that may be accepted by the messages pezpallet.
	///
...
/// Verify that the messages pezpallet call, supported by extension has succeeded.
...
mod tests {
	...
	fn expected_delivery_reward() -> RewardBalance {
		...
		pezpallet_transaction_payment::Pezpallet::<TestRuntime>::compute_actual_fee(
			1024,
			&dispatch_info(),
			&post_dispatch_info,
			Zero::zero(),
