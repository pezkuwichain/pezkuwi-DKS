...
//!
//! There is another `pezpallet_bridge_messages` pezpallet that handles inbound/outbound lanes for
...
//!
//! The `pezpallet_xcm_bridge_hub` pezpallet has the extrinsic `fn open_bridge` and an important
...
//! messages pezpallet. Assuming that this pezpallet is deployed at the bridge hub
//! teyrchain and there's a similar pezpallet at the bridged network, the dynamic
...
//!
//! The pezpallet doesn't provide any mechanism for graceful closure, because it always involves
...
pub mod pezpallet {
		...
		BridgeMessagesConfig<Self::BridgeMessagesPalletInstance>
	{
		...
		/// Associated messages pezpallet instance that bridges us with the
		...
		T: pezframe_system::Config<AccountId = AccountIdOf<ThisChainOf<T, I>>>,
	{
		fn build(&self) {
			...
			{
				let locations = Pezpallet::<T, I>::bridge_locations(
	...
	pub enum Error<T, I = ()> {
		...
		/// The bridge is already registered in this pezpallet.
		BridgeAlreadyExists,
