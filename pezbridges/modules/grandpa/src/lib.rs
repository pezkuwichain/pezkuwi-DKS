...
//!
//! This pezpallet achieves this by trustlessly verifying GRANDPA finality proofs on-chain. Once
...
//! bug causing resulting in an equivocation. Such events are outside the scope of this pezpallet.
...
/// Module, containing weights for this pezpallet.
pub mod weights;
pub mod weights_ext;
...
/// Bridged chain from the pezpallet configuration.
...
/// Header data of the bridged chain that is stored at this chain by this pezpallet.
...
pub mod pezpallet {
	...
	pub trait Config<I: 'static = ()>: pezframe_system::Config {
		...
		///
		/// To be able to track the bridged chain, the pezpallet requires all headers that are
		/// changing GRANDPA authorities set at the bridged chain (we call them mandatory).
		/// So it is a common good deed to submit mandatory headers to the pezpallet.
		///
		/// The pezpallet may be configured (see `[Self::FreeHeadersInterval]`) to import some
	...
	impl<T: Config<I>, I: 'static> Pezpallet<T, I> {
		...
		#[pezpallet::weight(T::WeightInfo::submit_finality_proof_weight(
		...
		#[pezpallet::weight((T::DbWeight::get().reads_writes(2, 5), DispatchClass::Operational))]
		pub fn initialize(
			origin: OriginFor<T>,
			...
			tracing::info!(
				...
				"Pezpallet has been initialized"
			);
		...
		///
		/// - the pezpallet knows better header than the `finality_target`;
		///
		/// - the id of best GRANDPA authority set, known to the pezpallet is not equal to the
		///   `current_set_id`;
		///
		...
		pub fn submit_finality_proof_ex(
			...
			// mandatory header is a header that changes authorities set. The pezpallet can't go
		...
		///
		/// - the call origin is not a root or a pezpallet owner;
		///
		...
		pub fn force_set_pallet_state(
			...
			// known to the pezpallet - it changes nothing (except for the fact that previously
	...
	/// Hash of the header used to bootstrap the pezpallet.
	#[pezpallet::storage]
	...
	pub enum Error<T, I = ()> {
		...
		/// The pezpallet is not yet initialized.
		NotInitialized,
		...
		/// the id of the current set, known to the pezpallet.
	...
	/// this transaction is considered as common good deed w.r.t to pezpallet configuration.
	...
	///
	/// Will use the GRANDPA current authorities known to the pezpallet.
	///
	...
	/// Initialize pezpallet so that it is ready for inserting new header.
	///
...
/// Bridge GRANDPA pezpallet as header chain.
pub type GrandpaChainHeaders<T, I> = Pezpallet<T, I>;
...
mod tests {
	...
	> {
		...
		Pezpallet::<TestRuntime>::initialize(origin, init_data.clone()).map(|_| init_data)
	}
	...
	fn submit_finality_proof(header: u8) -> pezframe_support::dispatch::DispatchResultWithPostInfo {
		...
		Pezpallet::<TestRuntime>::submit_finality_proof_ex(
	...
	fn init_root_or_owner_origin_can_initialize_pallet() {
		run_test(|| {
			...
			// Reset storage so we can initialize the pezpallet again
	...
	fn init_storage_entries_are_correctly_initialized() {
		run_test(|| {
			...
			assert_eq!(Pezpallet::<TestRuntime>::best_finalized(), None);
	...
	fn init_fails_if_there_are_too_many_authorities_in_the_set() {
		run_test(|| {
			...
			assert_noop!(
				Pezpallet::<TestRuntime>::initialize(RuntimeOrigin::root(), init_data),
	...
	fn successfully_imports_header_with_valid_finality() {
		run_test(|| {
			...
			assert_eq!(
				Pezpallet::<TestRuntime>::synced_headers_grandpa_info(),
	...
	fn disallows_invalid_authority_set() {
		run_test(|| {
			...
			assert_ok!(Pezpallet::<TestRuntime>::initialize(RuntimeOrigin::root(), init_data));
	...
	fn importing_header_enacts_new_authority_set() {
		run_test(|| {
			...
			let result = Pezpallet::<TestRuntime>::submit_finality_proof_ex(
				RuntimeOrigin::signed(1),
				Box::new(header.clone()),
				justification.clone(),
				TEST_GRANDPA_SET_ID,
	...
	fn verify_storage_proof_rejects_unknown_header() {
		run_test(|| {
			assert_noop!(
				Pezpallet::<TestRuntime>::verify_storage_proof(
					Default::default(),
					Default::default(),
				)
				.map(|_| ()),
	...
	fn should_prune_headers_over_headers_to_keep_parameter() {
		run_test(|| {
			initialize_bizinikiwi_bridge();
			assert_ok!(submit_finality_proof(1));
			let first_header_hash = Pezpallet::<TestRuntime>::best_finalized().unwrap().hash();
			next_block();
