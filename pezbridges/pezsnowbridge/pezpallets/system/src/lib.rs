...
pub mod pezpallet {
	...
	impl<T: Config> Pezpallet<T> {
		...
		#[pezpallet::weight((T::WeightInfo::upgrade(), DispatchClass::Operational))]
		pub fn upgrade(
			origin: OriginFor<T>,
			impl_address: H160,
			impl_code_hash: H256,
		...
		#[pezpallet::weight((T::WeightInfo::set_operating_mode(), DispatchClass::Operational))]
		...
		#[pezpallet::weight((T::WeightInfo::set_pricing_parameters(), DispatchClass::Operational))]
		...
		#[pezpallet::weight(T::WeightInfo::register_token())]
	...
	impl<T: Config> Pezpallet<T> {
		...
		/// Checks if the pezpallet has been initialized.
	...
	impl<T: Config> Contains<ChannelId> for Pezpallet<T> {
		fn contains(channel_id: &ChannelId) -> bool {
			Channels::<T>::get(channel_id).is_some()
		}
	}

	impl<T: Config> Get<PricingParametersOf<T>> for Pezpallet<T> {
