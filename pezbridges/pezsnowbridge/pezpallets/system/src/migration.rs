...
pub mod v0 {
		...
		AssetHubParaId: Get<u32>,
	{
		fn on_runtime_upgrade() -> Weight {
			if !Pezpallet::<T>::is_initialized() {
				Pezpallet::<T>::initialize(
...
pub mod v1 {
	...
	{
		fn on_runtime_upgrade() -> Weight {
			let mut params = Pezpallet::<T>::parameters();
		...
		fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
			...
			let params = Pezpallet::<T>::parameters();
...
/// Run the migration of the gas price and increment the pezpallet version so it cannot be re-run.
