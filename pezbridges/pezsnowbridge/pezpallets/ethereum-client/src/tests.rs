...
fn signing_root_uses_previous_slot_for_fork_version() {
	new_tester().execute_with(|| {
		...
		// The pezpallet's signing_root should use the previous slot's fork version (per spec)
		let pezpallet_signing_root =
			EthereumBeaconClient::signing_root(&header, validators_root, signature_slot).unwrap();

		assert_eq!(
			pezpallet_signing_root, signing_root_with_previous_slot,
