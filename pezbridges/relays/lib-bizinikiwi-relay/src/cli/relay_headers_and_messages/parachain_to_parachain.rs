...
pub struct TeyrchainToParachainBridge<
	...
	($left_teyrchain:ident, $left_chain:ident, $right_teyrchain:ident, $right_chain:ident) => {
		pezbp_runtime::paste::item! {
			...
			impl [<$left_teyrchain $right_teyrchain HeadersAndMessages>] {
				...
				>(
					self,
				) -> anyhow::Result<$crate::cli::relay_headers_and_messages::teyrchain_to_teyrchain::TeyrchainToParachainBridge<L2R, R2L>> {
					Ok($crate::cli::relay_headers_and_messages::teyrchain_to_teyrchain::TeyrchainToParachainBridge {
	...
	AccountIdOf<Right>: From<<AccountKeyPairOf<Right> as Pair>::Public>,
{
	...
	async fn start_on_demand_headers_relayers(
		...
		let left_to_right_on_demand_teyrchains = OnDemandParachainsRelay::<
		...
		let right_to_left_on_demand_teyrchains = OnDemandParachainsRelay::<
