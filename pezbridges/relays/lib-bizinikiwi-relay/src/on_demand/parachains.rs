...
use crate::{
	...
	teyrchains::{
		...
		SubmitParachainHeadsCallBuilder, BizinikiwiParachainsPipeline,
	},
	TransactionParams,
};
...
#[derive(Clone)]
pub struct OnDemandParachainsRelay<P: BizinikiwiParachainsPipeline, SourceRelayClnt, TargetClnt> {
	/// Relay task name.
	...
	> OnDemandParachainsRelay<P, SourceRelayClnt, TargetClnt>
{
			...
			From<<AccountKeyPairOf<P::TargetChain> as pezsp_core::Pair>::Public>,
	{
		...
		let this = OnDemandParachainsRelay {
			relay_task_name: on_demand_teyrchains_relay_name::<P::SourceParachain, P::TargetChain>(
			),
...
#[async_trait]
impl<P: BizinikiwiParachainsPipeline, SourceRelayClnt, TargetClnt>
	...
	TargetClnt: Client<P::TargetChain>,
{
	...
	async fn prove_header(
		&self,
		required_teyrchain_header: BlockNumberOf<P::SourceParachain>,
	) -> Result<(HeaderIdOf<P::SourceParachain>, Vec<CallOf<P::TargetChain>>), BizinikiwiError> {
		...
		if proved_relay_block != selected_relay_block {
			proved_teyrchain_block = teyrchains_source
				...
				.ok_or_else(|| {
					BizinikiwiError::MissingRequiredParachainHead(
						para_id,
		...
		calls.push(P::SubmitParachainHeadsCallBuilder::build_submit_teyrchain_heads_call(
			proved_relay_block,
...
async fn background_task<P: BizinikiwiParachainsPipeline>(
	...
	on_demand_source_relay_to_target_headers: Arc<
		dyn OnDemandRelay<P::SourceRelayChain, P::TargetChain>,
	>,
	required_teyrchain_header_number_receiver: Receiver<BlockNumberOf<P::SourceParachain>>,
) where
	P::SourceParachain: Chain<Hash = ParaHash>,
	P::SourceRelayChain:
		Chain<BlockNumber = RelayBlockNumber, Hash = RelayBlockHash, Hasher = RelayBlockHasher>,
...
{
	let relay_task_name = on_demand_teyrchains_relay_name::<P::SourceParachain, P::TargetChain>();
...
async fn read_relay_data<P: BizinikiwiParachainsPipeline, SourceRelayClnt, TargetClnt>(
		...
		TargetClient<TeyrchainsPipelineAdapter<P>> + RelayClient<Error = BizinikiwiError>,
{
	let map_target_err = |e| {
		tracing::error!(
			target: "bridge",
			error=?e,
			relay_name=%on_demand_teyrchains_relay_name::<P::SourceParachain, P::TargetChain>(),
	...
	.await;
	// if there are no teyrchain heads at the target (`NoParachainHeadAtTarget`), we'll need to
...
#[async_trait]
impl<'a, P: BizinikiwiParachainsPipeline, SourceRelayClnt, TargetClnt>
	...
	PBN: BlockNumberBase,
{
	...
	let best_possible_teyrchain_block = env
		...
		.ok_or(BizinikiwiError::MissingRequiredParachainHead(
			env.teyrchain_id(),
...
mod tests {
	...
	async fn select_headers_to_prove_returns_err_if_required_para_block_is_missing_at_source() {
		assert!(matches!(
			...
			Err(BizinikiwiError::MissingRequiredParachainHead(ParaId(0), 300_u64)),
		));
	}
