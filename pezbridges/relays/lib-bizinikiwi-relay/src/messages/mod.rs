...
use crate::{
	messages::{
		source::{BizinikiwiMessagesProof, BizinikiwiMessagesSource},
		target::{BizinikiwiMessagesDeliveryProof, BizinikiwiMessagesTarget},
	},
...
/// Bizinikiwi -> Bizinikiwi messages synchronization pipeline.
pub trait BizinikiwiMessageLane: 'static + Clone + Debug + Send + Sync {
...
/// Adapter that allows all `BizinikiwiMessageLane` to act as `MessageLane`.
#[derive(Clone, Debug)]
pub struct MessageLaneAdapter<P: BizinikiwiMessageLane> {
	_phantom: PhantomData<P>,
}

impl<P: BizinikiwiMessageLane> MessageLane for MessageLaneAdapter<P> {
	const SOURCE_NAME: &'static str = P::SourceChain::NAME;
	const TARGET_NAME: &'static str = P::TargetChain::NAME;

	type LaneId = P::LaneId;

	type MessagesProof = BizinikiwiMessagesProof<P::SourceChain, P::LaneId>;
	type MessagesReceivingProof = BizinikiwiMessagesDeliveryProof<P::TargetChain, P::LaneId>;
...
/// Bizinikiwi <-> Bizinikiwi messages relay parameters.
pub struct MessagesRelayParams<P: BizinikiwiMessageLane, SourceClnt, TargetClnt> {
	...
	/// using `TransactionPayment` pezpallet runtime API.
	...
	BatchProofTransaction<SC, TC, B>
{
	...
	) -> Result<Option<Self>, BizinikiwiError> {
...
/// Run Bizinikiwi-to-Bizinikiwi messages sync loop.
	...
	BalanceOf<P::SourceChain>: TryFrom<BalanceOf<P::TargetChain>>,
{
	...
	pez_messages_relay::message_lane_loop::run(
		...
		BizinikiwiMessagesTarget::<P, _, _>::new(
			target_client,
			source_client,
			params.lane_id,
			relayer_id_at_source,
...
/// Relay messages delivery confirmation of Bizinikiwi-to-Bizinikiwi messages.
/// No checks are made to ensure that transaction will succeed.
pub async fn relay_messages_delivery_confirmation<P: BizinikiwiMessageLane>(
	source_client: impl Client<P::SourceChain>,
	target_client: impl Client<P::TargetChain>,
	source_transaction_params: TransactionParams<AccountKeyPairOf<P::SourceChain>>,
...
pub trait ReceiveMessagesProofCallBuilder<P: BizinikiwiMessageLane> {
...
macro_rules! generate_receive_message_proof_call_builder {
	($pipeline:ident, $mocked_builder:ident, $bridge_messages:path, $receive_messages_proof:path) => {
		...
		{
			fn build_receive_messages_proof_call(
				relayer_id_at_source: relay_bizinikiwi_client::AccountIdOf<
					<$pipeline as $crate::messages::BizinikiwiMessageLane>::SourceChain
				>,
				proof: $crate::messages::source::BizinikiwiMessagesProof<
					<$pipeline as $crate::messages::BizinikiwiMessageLane>::SourceChain,
...
pub trait ReceiveMessagesDeliveryProofCallBuilder<P: BizinikiwiMessageLane> {
	...
	fn build_receive_messages_delivery_proof_call(
		proof: BizinikiwiMessagesDeliveryProof<P::TargetChain, P::LaneId>,
		trace_call: bool,
...
macro_rules! generate_receive_message_delivery_proof_call_builder {
	($pipeline:ident, $mocked_builder:ident, $bridge_messages:path, $receive_messages_delivery_proof:path) => {
		...
		{
			fn build_receive_messages_delivery_proof_call(
				proof: $crate::messages::target::BizinikiwiMessagesDeliveryProof<
					<$pipeline as $crate::messages::BizinikiwiMessageLane>::TargetChain,
					<$pipeline as $crate::messages::BizinikiwiMessageLane>::LaneId
				>,
				_trace_call: bool,
...
fn dummy_messages_delivery_transaction<P: BizinikiwiMessageLane, SourceClnt, TargetClnt>(
	params: &MessagesRelayParams<P, SourceClnt, TargetClnt>,
	messages: u32,
...
mod tests {
	...
	fn ensure_macro_compatibility_for_generate_receive_message_proof_call_builder() {
		...
		// construct pezpallet Call directly
		let pezpallet_receive_messages_proof =
			pezpallet_bridge_messages::Call::<mock::TestRuntime>::receive_messages_proof {
		...
		assert_eq!(
			pezpallet_receive_messages_proof.encode(),
		...
		match relayer_call_builder_receive_messages_proof {
			RuntimeCall::BridgeMessages(call) => match call {
				call @ CodegenBridgeMessagesCall::receive_messages_proof { .. } => {
					assert_eq!(pezpallet_receive_messages_proof.encode(), call.encode())
				},
	...
	fn ensure_macro_compatibility_for_generate_receive_message_delivery_proof_call_builder() {
		...
		// construct pezpallet Call directly
		let pezpallet_receive_messages_delivery_proof =
			pezpallet_bridge_messages::Call::<mock::TestRuntime>::receive_messages_delivery_proof {
		...
		// ensure they are all equal
		assert_eq!(
			pezpallet_receive_messages_delivery_proof.encode(),
		...
		match relayer_call_builder_receive_messages_delivery_proof {
			RuntimeCall::BridgeMessages(call) => match call {
				call @ CodegenBridgeMessagesCall::receive_messages_delivery_proof { .. } => {
					assert_eq!(pezpallet_receive_messages_delivery_proof.encode(), call.encode())
				},
				_ => panic!("Unexpected CodegenBridgeMessagesCall type"),
			},
			_ => panic!("Unexpected RuntimeCall type"),
	...
	// mock runtime with `pezpallet_bridge_messages`
	...
	mod mock {
		...
		use pezsp_core::H256;
		use pezsp_runtime::{
			generic, testing::Header as BizinikiwiHeader, traits::BlakeTwo256, StateVersion,
		};
		...
		pezframe_support::construct_runtime! {
			pub enum TestRuntime
			{
				...
				Messages: pezpallet_bridge_messages,
			}
		}
	...
	mod relayer {
		...
		impl relay_bizinikiwi_client::Chain for ThisChain {
		...
		impl relay_bizinikiwi_client::ChainWithTransactions for ThisChain {
		...
		impl relay_bizinikiwi_client::ChainWithMessages for ThisChain {
			const TO_CHAIN_MESSAGE_DETAILS_METHOD: &'static str = "";
			const FROM_CHAIN_MESSAGE_DETAILS_METHOD: &'static str = "";
		}
		impl relay_bizinikiwi_client::ChainWithUtilityPallet for ThisChain {
		...
		impl relay_bizinikiwi_client::Chain for BridgedChain {
			const NAME: &'static str = "";
			const BEST_FINALIZED_HEADER_ID_METHOD: &'static str = "";
			const FREE_HEADERS_INTERVAL_METHOD: &'static str = "";
			const AVERAGE_BLOCK_INTERVAL: Duration = Duration::from_millis(0);
		...
		impl relay_bizinikiwi_client::ChainWithTransactions for BridgedChain {
			type AccountKeyPair = pezsp_core::sr25519::Pair;
			type SignedTransaction = ();
		...
		impl relay_bizinikiwi_client::ChainWithMessages for BridgedChain {
			const TO_CHAIN_MESSAGE_DETAILS_METHOD: &'static str = "";
			const FROM_CHAIN_MESSAGE_DETAILS_METHOD: &'static str = "";
		}
		impl relay_bizinikiwi_client::ChainWithUtilityPallet for BridgedChain {
			type UtilityPallet = MockedRuntimeUtilityPallet<RuntimeCall>;
		}

		#[derive(Clone, Debug)]
		pub struct ThisChainToBridgedChainMessageLane;
		impl BizinikiwiMessageLane for ThisChainToBridgedChainMessageLane {
