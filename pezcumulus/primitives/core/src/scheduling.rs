...
//! V3 scheduling types for low-latency teyrchain block production.
//!
...
use pezkuwi_primitives::{ApprovedPeerId, CoreSelector, Header as RelayChainHeader};
use pezsp_runtime::traits::{BlakeTwo256, Hash as HashT};
...
pub struct SchedulingInfoPayload {
	/// Which core to use (indexes into the teyrchain's assigned cores).
	...
	pub internal_scheduling_parent: pezkuwi_primitives::Hash,
}
...
impl SchedulingInfoPayload {
	...
	pub fn new(
		...
		internal_scheduling_parent: pezkuwi_primitives::Hash,
	) -> Self {
...
pub struct SchedulingProof {
	...
	/// Length is defined by the teyrchain runtime config (RelayParentOffset).
	...
	///   selection comes from the teyrchain block's UMP signals.
	///
...
impl SchedulingProof {
	...
	/// scheduling parent when the teyrchain runs with `relay_parent_offset = 0`).
	pub fn scheduling_parent(&self) -> pezkuwi_primitives::Hash {
		self.header_chain
			.first()
...
///
/// Reports whether V3 scheduling is enabled for the teyrchain (via
