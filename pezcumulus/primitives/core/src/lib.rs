...
impl CoreInfo {
	/// Puts this into a [`CumulusDigestItem::CoreInfo`] and then encodes it as a Bizinikiwi
	/// [`DigestItem`].
...
impl BlockBundleInfo {
	/// Puts this into a [`CumulusDigestItem::BlockBundleInfo`] and then encodes it as a Bizinikiwi
	/// [`DigestItem`].
	pub fn to_digest_item(&self) -> DigestItem {
...
pezsp_api::decl_runtime_apis! {
	...
	///
	/// This API allows teyrchains to request both top-level relay chain storage keys
	...
	pub trait KeyToIncludeInRelayProof {
		...
		///
		/// The collator will include them in the relay chain proof that is passed alongside the teyrchain inherent into the runtime.
