...
use pezcumulus_primitives_core::CumulusDigestItem;
...
///
/// With block bundling, teyrchains are running with block weights that may not allow certain
...
///    [`WeightsPerClass::max_extrinsic`](pezframe_system::limits::WeightsPerClass::max_extrinsic) is
///    set, after this
///    [`WeightsPerClass::max_total`](pezframe_system::limits::WeightsPerClass::max_total) and if both
...
/// [`BlockWeight`](pezframe_system::BlockWeight). Otherwise the weight tracking is not working
/// properly. Normally this is done by [`CheckWeight`](pezframe_system::CheckWeight).
///
/// # Generic parameters
///
/// - `Config`: The [`Config`](crate::Config) trait of this pezpallet.
///
/// - `Inner`: The inner transaction extensions aka the other transaction extensions to be used by
///   the runtime.
///
/// - `TargetBlockRate`: The target block rate the teyrchain should be running with. Or in other
///   words, the number of blocks the teyrchain should produce in `6s`(relay chain slot duration).
///
	...
	TargetBlockRate: Get<u32>,
{
	...
	fn pre_validate_extrinsic(
		...
		let is_not_inherent = pezframe_system::Pezpallet::<Config>::inherents_applied();
		let extrinsic_index =
			pezframe_system::Pezpallet::<Config>::extrinsic_index().unwrap_or_default();
				...
				BlockWeightMode::<Config>::FractionOfCore { first_transaction_index, .. } => {
					...
					if block_weight_over_limit {
						*mode = Some(BlockWeightMode::<Config>::full_core());
						...
						pezframe_system::Pezpallet::<Config>::deposit_log(
						...
						if !is_first_block_in_core_with_digest(&digest).unwrap_or(false) {
							...
							// extrinsics.
							pezframe_system::Pezpallet::<Config>::register_extra_weight_unchecked(
								FULL_CORE_WEIGHT,
		...
		crate::BlockWeightMode::<Config>::mutate(|weight_mode| {
			...
			match mode {
				...
				BlockWeightMode::<Config>::PotentialFullCore {
					...
					let block_weight = pezframe_system::BlockWeight::<Config>::get();
	...
	Config::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	fn metadata() -> Vec<pezsp_runtime::traits::TransactionExtensionMetadata> {
		let mut inner = Inner::metadata();
		inner.push(pezsp_runtime::traits::TransactionExtensionMetadata {
