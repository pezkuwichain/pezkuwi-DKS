...
use pezframe_system::{CheckWeight, RawOrigin as SystemOrigin};
use pezkuwi_primitives::MAX_POV_SIZE;
...
fn tx_extension_sets_fraction_of_core_mode() {
	use pezframe_support::dispatch::{DispatchClass, DispatchInfo};
		...
		TestExtBuilder::new()
			...
			.execute_with(|| {
				...
				if !first_block_in_core {
					...
					let final_remaining =
						pezframe_system::Pezpallet::<Runtime>::remaining_block_weight();
...
fn pre_inherents_hook_non_first_block_over_limit() {
	TestExtBuilder::new()
		...
		.execute_with(|| {
			...
			// Should have registered FULL_CORE_WEIGHT to prevent more transactions
			let final_remaining = pezframe_system::Pezpallet::<Runtime>::remaining_block_weight();
			assert!(final_remaining.consumed().all_gte(FULL_CORE_WEIGHT));
		});
}
...
fn ongoing_mbm_requests_full_core() {
	TestExtBuilder::new()
		...
		.execute_with(|| {
			...
			assert_eq!(
				FULL_CORE_WEIGHT,
				<RuntimeOnlyOperational as pezframe_system::Config>::BlockWeights::get().max_block
			);
...
fn full_core_weight_in_inherent_context() {
	TestExtBuilder::new()
		...
		.execute_with(|| {
			...
			assert!(!pezframe_system::Pezpallet::<RuntimeOnlyOperational>::inherents_applied());
...
fn executive_validate_transaction_respects_dispatch_class_max_block_size() {
	...
	for signed in [true, false] {
		TestExtBuilder::new().previous_core_count(4).build().execute_with(|| {
			assert!(<RuntimeOnlyOperational as pezframe_system::Config>::BlockWeights::get()
