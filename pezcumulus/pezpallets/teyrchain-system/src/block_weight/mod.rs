...
//! Provides functionality to dynamically calculate the block weight for a teyrchain.
//!
//! With block bundling, teyrchains are relatively free to choose whatever block interval they want.
//! The block interval is the time between individual blocks. The available resources per block (max
//! block weight) depend on the number of cores allocated to the teyrchain on the relay chain. Each
...
use pezkuwi_primitives::{executor_params::DEFAULT_BACKING_EXECUTION_TIMEOUT, MAX_POV_SIZE};
use pezsp_core::Get;
use pezsp_runtime::Digest;
...
const LOG_TARGET: &str = "runtime::teyrchain-system::block-weight";
...
impl<T: Config> BlockWeightMode<T> {
	...
	fn is_stale(&self) -> bool {
		...
		context < pezframe_system::Pezpallet::<T>::block_number()
	}
	...
	pub(crate) fn full_core() -> Self {
		Self::FullCore { context: pezframe_system::Pezpallet::<T>::block_number() }
	}
...
/// Calculates the maximum block weight for a teyrchain.
///
	...
	fn get() -> Weight {
		...
		match crate::BlockWeightMode::<Config>::get().filter(|m| !m.is_stale()) {
			...
			Some(BlockWeightMode::<Config>::FractionOfCore { first_transaction_index, .. }) => {
				let is_phase_finalization = pezframe_system::Pezpallet::<Config>::execution_phase()
					.map_or(false, |p| matches!(p, pezframe_system::Phase::Finalization));
				let inherents_applied = pezframe_system::Pezpallet::<Config>::inherents_applied();
