...
use pezcumulus_primitives_core::{
	...
	ClaimQueueOffset, CoreSelector, CumulusDigestItem, PersistedValidationData, TeyrchainBlockData,
	...
	<B::Extrinsic as ExtrinsicCall>::Call: IsSubType<crate::Call<PSC>>,
{
	...
	let _guard = (
		...
		pezsp_io::transaction_index::host_index
			.replace_implementation(host_transaction_index_index),
		#[cfg(feature = "transaction-index")]
		pezsp_io::transaction_index::host_renew
			.replace_implementation(host_transaction_index_renew),
	);
	...
	for (block_index, mut block) in blocks.into_iter().enumerate() {
		...
		let code_upgrade_detected =
			if <PSC as pezframe_system::Config>::Version::get().system_version >= 3 {
...
/// Teyrchain validation does not require maintaining a transaction index,
/// and indexing transactions does **not** contribute to the teyrchain state.
...
fn host_transaction_index_index(_extrinsic: u32, _size: u32, _context_hash: [u8; 32]) {
	// No-op host function used during teyrchain validation.
}
