...
use pezcumulus_primitives_core::SchedulingProof;
use pezkuwi_teyrchain_primitives::primitives::ValidationParamsExtension;
use pezsp_runtime::traits::Header as HeaderT;

/// Hash type for relay chain.
pub type RelayHash = pezsp_core::H256;
...
pub fn validate_v3_scheduling(
	...
	match (v3_enabled, extension) {
		...
		(false, Some(_)) => {
			...
			// The relay chain should not send V3 candidates to teyrchains that have not enabled it
			panic!(
		...
		(true, Some(ValidationParamsExtension::V3 { relay_parent, scheduling_parent })) => {
			...
			let scheduling_proof = scheduling_proof
				.expect("V3 candidates require TeyrchainBlockData::V2 with scheduling_proof");
...
mod tests {
	...
	use pezsp_runtime::{generic::Header, traits::BlakeTwo256};
	...
	#[test]
	#[should_panic(expected = "V3 candidates require TeyrchainBlockData::V2 with scheduling_proof")]
