...
use pezframe_system::{limits::BlockWeights, pezpallet_prelude::BlockNumberFor, RawOrigin};
use pezsp_core::ConstU32;
use pezsp_runtime::{traits::BlakeTwo256, BuildStorage};
use pezsp_version::RuntimeVersion;
use std::cell::RefCell;

use crate as teyrchain_system;
...
parameter_types! {
	...
	pub const TeyrchainId: ParaId = ParaId::new(200);
...
impl pezframe_system::Config for Test {
	...
	type OnSetCode = TeyrchainSetCode<Self>;
}
...
pub fn wasm_ext() -> pezsp_io::TestExternalities {
	...
	ext.register_extension(pezsp_core::traits::ReadRuntimeVersionExt::new(ReadRuntimeVersion(
		version.encode(),
	)));
	ext
}
...
pub struct BlockTests {
	...
	inherent_data_hook:
		Option<Box<dyn Fn(&BlockTests, RelayChainBlockNumber, &mut TeyrchainInherentData)>>,
...
impl BlockTests {
	...
	where
		F: 'static + Fn(&BlockTests, RelayChainBlockNumber, &mut TeyrchainInherentData),
	{
	...
	pub fn run_without_ext(&mut self) {
		...
		for BlockTest { n, within_block, after_block } in self.tests.iter() {
			...
			let inherent_data = {
				...
				let mut system_inherent_data = TeyrchainInherentData {
			...
			// execute the block
			TeyrchainSystem::on_initialize(*n);
			TeyrchainSystem::create_inherent(&inherent_data)
