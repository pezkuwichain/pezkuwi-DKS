...
/// A snapshot of some messaging related state of relay chain pertaining to the current teyrchain.
///
/// This data is essential for making sure that the teyrchain is aware of current resource use on
/// the relay chain and that the candidates produced for this teyrchain do not exceed any of these
/// limits.
...
pub struct MessagingStateSnapshot {
	...
	/// The current capacity of the upward message queue of the current teyrchain on the relay
	/// chain.
	...
	/// of the channel. Obviously, the recipient is the current teyrchain.
	///
	...
	/// of the channel. Obviously, the sender is the current teyrchain.
	///
...
pub enum Error {
	...
	/// The latest included teyrchain head cannot be extracted.
...
impl RelayChainStateProof {
	...
	/// Read latest included teyrchain [head data](`relay_chain::HeadData`) from the relay chain
	/// state proof.
	///
	/// Returns an error if anything failed at reading or decoding.
	...
	pub fn read_next_authorities(
		...
		Option<Vec<(pezsp_consensus_babe::AuthorityId, pezsp_consensus_babe::BabeAuthorityWeight)>>,
		Error,
	> {
	...
	///
	/// The go-ahead specifies whether the teyrchain can apply the upgrade or should abort it. If
	...
	///
	/// If the upgrade restriction is not `None`, then the teyrchain cannot signal an upgrade at
	/// this block.
	///
	/// Returns an error if anything failed at reading or decoding.
