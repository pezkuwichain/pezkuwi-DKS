...
/// Register the `validate_block` function that is used by teyrchains to validate blocks on a
/// validator.
///
...
///
/// Each Teyrchain block is built in the context of a relay chain block, this trait allows us
/// to validate the given relay chain block number. With async backing it is legal to build
/// multiple Teyrchain blocks per relay chain parent. With this trait it is possible for the
/// Teyrchain to ensure that still only one Teyrchain block is build per relay chain parent.
///
...
///
/// It will ensure that the associated relay block number strictly increases between Teyrchain
/// blocks. This should be used by production Teyrchains when in doubt.
...
impl CheckAssociatedRelayNumber for RelayNumberStrictlyIncreases {
	fn check_associated_relay_number(
		...
		if current <= previous {
			panic!("Relay chain block number needs to strictly increase between Teyrchain blocks!")
		}
	}
}
...
/// Provides an implementation of [`CheckAssociatedRelayNumber`].
///
/// It will ensure that the associated relay block number monotonically increases between Teyrchain
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config<OnSetCode = TeyrchainSetCode<Self>> {
		...
		/// An entry-point for higher-level logic to manage the backlog of unincluded teyrchain
		...
		/// when authoring a teyrchain block.
		///
		...
		/// teyrchain blocks are authored at the correct offset.
		///
		/// While this helps to reduce forks on the teyrchain side, it increases the delay for
	...
	impl<T: Config> ProvideInherent for Pezpallet<T> {
		...
		fn create_inherent(data: &InherentData) -> Option<Self::Call> {
			let data = match data
				.get_data::<TeyrchainInherentData>(&Self::INHERENT_IDENTIFIER)
				.ok()
				.flatten()
			{
				None => {
...
impl<T: Config> Pezpallet<T> {
	...
	/// the teyrchain from the relay chain doesn't exceed the block size. Currently each message
	...
	fn do_create_inherent(data: TeyrchainInherentData) -> Call<T> {
		let (data, mut downward_messages, mut horizontal_messages) =
			deconstruct_teyrchain_inherent_data(data);
	...
	///            a para which has no open channel to this teyrchain or if after processing
	...
	fn maybe_drop_included_ancestors(
		...
		// If the unincluded segment length is nonzero, then the teyrchain head must be present.
		let para_head =
	...
	///
	/// Returns `None` if the relay chain teyrchain host configuration hasn't been submitted yet.
	pub fn max_code_size() -> Option<u32> {
		<HostConfiguration<T>>::get().map(|cfg| cfg.max_code_size)
	}

	/// The implementation of the runtime upgrade functionality for teyrchains.
	...
	pub fn open_custom_outbound_hrmp_channel_for_benchmarks_or_tests(
		...
		RelevantMessagingState::<T>::put(MessagingStateSnapshot {
			...
			relay_dispatch_queue_remaining_capacity: Default::default(),
			ingress_channels: Default::default(),
			egress_channels: vec![(target_teyrchain, channel)],
		})
	}
	...
	pub fn initialize_for_set_code_benchmark(max_code_size: u32) {
		...
		let vfp = PersistedValidationData {
			parent_head: pezkuwi_teyrchain_primitives::primitives::HeadData(Default::default()),
...
impl<T: Config> Pezpallet<T> {
	...
	pub fn send_upward_message(message: UpwardMessage) -> Result<(u32, XcmHash), MessageSendError> {
		...
		let hash = pezsp_io::hashing::blake2_256(&message);
...
/// called when the teyrchain inherent is executed that contains the validation data.
...
///
/// Enables teyrchains to read relay chain state via state proofs.
