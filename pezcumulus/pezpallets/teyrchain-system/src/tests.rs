...
fn test_inherent_compatibility() {
	...
	let mut valid_inherent_data_v1 = pezsp_inherents::InherentData::new();
	...
	let mut valid_inherent_data_full_compatibility = pezsp_inherents::InherentData::new();
	...
	wasm_ext().execute_with(|| {
		assert!(
			TeyrchainSystem::create_inherent(&valid_inherent_data_v1).is_some(),
			"V1 inherent was not accepted"
		);
		assert!(
			TeyrchainSystem::create_inherent(&valid_inherent_data_legacy).is_some(),
			"Legacy inherent was not accepted"
		);

		assert!(
			TeyrchainSystem::create_inherent(&valid_inherent_data_full_compatibility).is_some(),
...
fn hrmp_ingress_channels_are_checked() {
	...
	let mut test = BlockTests::new()
		...
		.with_relay_sproof_builder(move |_, relay_block_num, sproof| match relay_block_num {
			// Let's open a channel only with teyrchain 1000.
			1 => {
				let mqc_head =
		...
		.with_inherent_data(move |_, relay_block_num, data| match relay_block_num {
			// Simulate receiving a message from teyrchain 1000 at block 1. This should work.
			1 => {
	...
	let mut test = test
		...
		.with_inherent_data(move |_, relay_block_num, data| match relay_block_num {
			// Simulate receiving a message from teyrchain 2000 at block 2. This should lead to a
			// panic.
			2 => {
	...
	BlockTests::new()
		...
		.add_with_post_test(
			...
			|| {
				...
				assert_eq!(
					events[0].event,
					RuntimeEvent::TeyrchainSystem(crate::Event::ValidationFunctionStored)
				);
			},
		)
		.add_with_post_test(
			...
			|| {
				...
				assert_eq!(
					events[1].event,
					RuntimeEvent::TeyrchainSystem(crate::Event::ValidationFunctionApplied {
				...
				assert!(System::digest()
					.logs()
					.iter()
					.any(|d| *d == pezsp_runtime::generic::DigestItem::RuntimeEnvironmentUpdated));
			},
		);
}
	...
	BlockTests::new()
		...
		.add_with_post_test(
			...
			|| {
				...
				assert_eq!(
					events[0].event,
					RuntimeEvent::TeyrchainSystem(crate::Event::ValidationFunctionDiscarded)
				);
			},
		);
}
...
fn send_upward_message_num_per_candidate() {
	BlockTests::new()
		...
		.add_with_post_test(
			1,
			|| {
				TeyrchainSystem::send_upward_message(b"Mr F was here".to_vec()).unwrap();
				TeyrchainSystem::send_upward_message(b"message 2".to_vec()).unwrap();
			},
			|| {
...
fn send_upward_message_relay_bottleneck() {
	BlockTests::new()
		...
		.add_with_post_test(
			1,
			|| {
				TeyrchainSystem::send_upward_message(vec![0u8; 8]).unwrap();
			},
			|| {
...
fn send_upwards_message_checks_size_on_validate() {
	BlockTests::new()
		...
		.add(1, || {
			assert_eq!(
				TeyrchainSystem::can_send_upward_message(vec![0u8; 129].as_ref()),
...
fn send_upward_message_check_size() {
	BlockTests::new()
		...
		.add(1, || {
			assert_eq!(
				TeyrchainSystem::send_upward_message(vec![0u8; 129]),
				Err(MessageSendError::TooBig)
			);
		});
}
...
fn upgrade_version_checks_should_work() {
	...
	let test_data = vec![
		("test", 0, 1, pezframe_system::Error::<Test>::SpecVersionNeedsToIncrease),
		("test", 1, 0, pezframe_system::Error::<Test>::SpecVersionNeedsToIncrease),
		("test", 1, 1, pezframe_system::Error::<Test>::SpecVersionNeedsToIncrease),
		("test", 1, 2, pezframe_system::Error::<Test>::SpecVersionNeedsToIncrease),
		("test2", 1, 1, pezframe_system::Error::<Test>::InvalidSpecName),
	];
	...
	for (spec_name, spec_version, impl_version, expected) in test_data.into_iter() {
		...
		ext.execute_with(|| {
			...
			let new_code_hash = H256(pezsp_crypto_hashing::blake2_256(&new_code));
			...
			System::assert_last_event(
				pezframe_system::Event::RejectedInvalidAuthorizedUpgrade {
...
fn deposits_relay_parent_storage_root() {
	BlockTests::new().add_with_post_test(
		...
		|| {
			...
			assert!(pezcumulus_primitives_core::rpsr_digest::extract_relay_parent_storage_root(
				&digest
			)
			.is_some());
		},
	...
	BlockTests::new()
		...
		.add_with_post_test(
			...
			|| {
				...
				TeyrchainSystem::send_upward_message(b"Test".to_vec()).unwrap();
				assert_eq!(UpwardDeliveryFeeFactor::<Test>::get(), FixedU128::from_u32(1));

				TeyrchainSystem::send_upward_message(
