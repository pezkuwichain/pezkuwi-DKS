...
impl pezframe_system::ExtensionsWeightInfo for MockWeightInfo {
...
impl pezframe_system::WeightInfo for MockWeightInfo {
...
impl pezframe_system::Config for Test {
	type Block = Block;
	type AccountData = ();
	type MaxConsumers = pezframe_support::traits::ConstU32<3>;
...
pub fn setup_test_ext_default() -> pezsp_io::TestExternalities {
	setup_test_externalities(&[0; 32])
}

fn set_current_storage_weight(new_weight: u64) {
	pezframe_system::BlockWeight::<Test>::mutate(|current_weight| {
		current_weight.set(Weight::from_parts(0, new_weight), DispatchClass::Normal);
	});
}

fn get_storage_weight() -> Weight {
	*pezframe_system::BlockWeight::<Test>::get().get(DispatchClass::Normal)
}

const CALL: &<Test as pezframe_system::Config>::RuntimeCall =
	&RuntimeCall::System(pezframe_system::Call::set_heap_pages { pages: 0u64 });
const ALICE_ORIGIN: pezframe_system::Origin<Test> = pezframe_system::Origin::<Test>::Signed(99);
const LEN: usize = 150;

fn new_tx_ext() -> Tx {
	Tx::new((pezframe_system::CheckWeight::new(), MockExtensionWithRefund))
}
...
fn new_extrinsic() -> generic::CheckedExtrinsic<AccountId, RuntimeCall, Tx> {
	generic::CheckedExtrinsic {
		...
		function: RuntimeCall::System(pezframe_system::Call::remark { remark: vec![] }),
	}
}
...
mod doc {
	...
	type Tx = StorageWeightReclaim<
		...
		(
			...
			// No need for `pezframe_system::WeightReclaim` as the reclaim.
		),
	>;
}
...
fn test_series() {
	...
	let base_extrinsic = <<Test as pezframe_system::Config>::BlockWeights as Get<
		pezframe_system::limits::BlockWeights,
	>>::get()
	.per_class
...
fn test_pov_missing_from_node_reclaim() {
	...
	test_ext.execute_with(|| {
		...
		let tx_ext = StorageWeightReclaim::<Test, pezframe_system::CheckWeight<Test>>::new(
		...
		assert_eq!(
			pezframe_system::ExtrinsicWeightReclaimed::<Test>::get().proof_size(),
...
fn test_ref_time_weight_reclaim() {
	...
	test_ext.execute_with(|| {
		...
		type InnerTxExt = (pezframe_system::CheckWeight<Test>, MockExtensionWithRefund);
		// Execute the transaction.
...
fn test_metadata() {
	assert_eq!(
		StorageWeightReclaim::<Test, pezframe_system::CheckWeight<Test>>::metadata()
			.iter()
			.map(|m| m.identifier)
			.collect::<Vec<_>>(),
