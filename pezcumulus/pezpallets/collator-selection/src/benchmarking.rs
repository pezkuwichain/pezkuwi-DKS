...
pub type BalanceOf<T> = <<T as super::Config>::Currency as Currency<
...
fn register_validators<T: Config + session_benchmarking::Config>(count: u32) -> Vec<T::AccountId> {
	...
	for (who, keys, proof) in validators.clone() {
		...
		<session::Pezpallet<T>>::set_keys(RawOrigin::Signed(who).into(), keys, proof).unwrap();
	}
...
#[benchmarks(where T: pezpallet_authorship::Config + session_benchmarking::Config)]
mod benchmarks {
	use super::*;
	...
	fn register_as_candidate(c: Linear<1, { T::MaxCandidates::get() - 1 }>) {
		...
		<session::Pezpallet<T>>::set_keys(RawOrigin::Signed(caller.clone()).into(), keys, proof)
			.unwrap();
