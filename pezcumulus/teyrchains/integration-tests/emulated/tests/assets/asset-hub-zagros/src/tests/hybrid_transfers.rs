...
fn para_to_para_assethub_hop_assertions(mut t: ParaToParaThroughAHTest) {
	...
	let sov_penpal_b_on_ah = AssetHubZagros::sovereign_account_id_of(
	...
	t.insert_unique_topic_id("AssetHubZagros", mq_prc_id);
	let msg_sent_id = find_xcm_sent_message_id::<AssetHubZagros>().expect("Missing Sent Event");
	t.insert_unique_topic_id("AssetHubZagros", msg_sent_id.into());
}
...
fn transfer_foreign_assets_from_asset_hub_to_para() {
	...
	let native_amount_to_send: Balance = ASSET_HUB_ZAGROS_ED * 1000;
	...
	PenpalA::execute_with(|| {
		assert_ok!(<PenpalA as Chain>::System::set_storage(
			...
			vec![(
				...
				Location::new(2, [GlobalConsensus(ByGenesis(PEZKUWICHAIN_GENESIS_HASH))]).encode(),
			)],
		));
	});
	...
	AssetHubZagros::mint_foreign_asset(
		<AssetHubZagros as Chain>::RuntimeOrigin::signed(assets_owner),
		roc_at_zagros_teyrchains.clone().try_into().unwrap(),
		sender.clone(),
	...
	let assets: Vec<Asset> = vec![
		...
		(roc_at_zagros_teyrchains.clone(), foreign_amount_to_send).into(),
	];
	...
	let sender_rocs_before = AssetHubZagros::execute_with(|| {
	...
	let receiver_rocs_before = PenpalA::execute_with(|| {
		type ForeignAssets = <PenpalA as PenpalAPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(roc_at_zagros_teyrchains.clone(), &receiver)
	});
	...
	test.set_dispatchable::<AssetHubZagros>(ah_to_para_transfer_assets);
	test.assert();

	// Query final balances
	let sender_balance_after = test.sender.balance;
	let sender_rocs_after = AssetHubZagros::execute_with(|| {
		type ForeignAssets = <AssetHubZagros as AssetHubZagrosPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
			roc_at_zagros_teyrchains.clone().try_into().unwrap(),
			&sender,
	...
	let receiver_rocs_after = PenpalA::execute_with(|| {
		type ForeignAssets = <PenpalA as PenpalAPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(roc_at_zagros_teyrchains, &receiver)
	});
...
fn transfer_foreign_assets_from_para_to_asset_hub() {
	...
	let native_amount_to_send: Balance = ASSET_HUB_ZAGROS_ED * 10000;
	let native_asset_location = RelayLocation::get();
	let assets_owner = PenpalAssetOwner::get();
...
fn transfer_foreign_assets_from_para_to_para_through_asset_hub() {
	...
	let sender = PenpalASender::get();
	let wnd_to_send: Balance = ZAGROS_ED * 10000;
	let assets_owner = PenpalAssetOwner::get();
	let wnd_location = RelayLocation::get();
	let sender_as_seen_by_ah = AssetHubZagros::sibling_location_of(PenpalA::para_id());
	let sov_of_sender_on_ah = AssetHubZagros::sovereign_account_id_of(sender_as_seen_by_ah);
	let receiver_as_seen_by_ah = AssetHubZagros::sibling_location_of(PenpalB::para_id());
	...
	AssetHubZagros::fund_accounts(vec![(sov_of_sender_on_ah.clone().into(), wnd_to_send * 2)]);
	AssetHubZagros::mint_foreign_asset(
		<AssetHubZagros as Chain>::RuntimeOrigin::signed(assets_owner),
		roc_at_zagros_teyrchains.clone().try_into().unwrap(),
	...
	let assets: Vec<Asset> = vec![
		...
		(roc_at_zagros_teyrchains.clone(), roc_to_send).into(),
	];
	...
	let wnds_in_sender_reserve_on_ah_before =
		<AssetHubZagros as Chain>::account_data_of(sov_of_sender_on_ah.clone()).free;
	let rocs_in_sender_reserve_on_ah_before = AssetHubZagros::execute_with(|| {
	...
	let wnds_in_receiver_reserve_on_ah_before =
		<AssetHubZagros as Chain>::account_data_of(sov_of_receiver_on_ah.clone()).free;
	let rocs_in_receiver_reserve_on_ah_before = AssetHubZagros::execute_with(|| {
		type Assets = <AssetHubZagros as AssetHubZagrosPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_zagros_teyrchains.clone().try_into().unwrap(),
	...
	test.set_assertion::<AssetHubZagros>(para_to_para_assethub_hop_assertions);
	...
	let rocs_in_sender_reserve_on_ah_after = AssetHubZagros::execute_with(|| {
		type Assets = <AssetHubZagros as AssetHubZagrosPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_zagros_teyrchains.clone().try_into().unwrap(),
			&sov_of_sender_on_ah,
	...
	let wnds_in_sender_reserve_on_ah_after =
		<AssetHubZagros as Chain>::account_data_of(sov_of_sender_on_ah).free;
	let rocs_in_receiver_reserve_on_ah_after = AssetHubZagros::execute_with(|| {
		type Assets = <AssetHubZagros as AssetHubZagrosPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_zagros_teyrchains.clone().try_into().unwrap(),
			&sov_of_receiver_on_ah,
	...
	let wnds_in_receiver_reserve_on_ah_after =
		<AssetHubZagros as Chain>::account_data_of(sov_of_receiver_on_ah).free;
...
// ===============================================================
/// Transfers of native asset Relay to Penpal (using AssetHub reserve). Teyrchains want to avoid
...
fn transfer_native_asset_from_relay_to_penpal_through_asset_hub() {
	...
	let amount_to_send: Balance = ZAGROS_ED * 100;
	...
	let sov_penpal_on_ah = AssetHubZagros::sovereign_account_id_of(
		AssetHubZagros::sibling_location_of(PenpalA::para_id()),
	);
	// Query initial balances
	let sender_balance_before = test.sender.balance;
	...
	fn transfer_assets_dispatchable(t: RelayToParaThroughAHTest) -> DispatchResult {
		let fee: Asset = t
			...
			.cloned()
			.unwrap();
		let asset_hub_location = Zagros::child_location_of(AssetHubZagros::para_id());
		let context = ZagrosUniversalLocation::get();
	...
	// Set assertions and dispatchables
	test.set_assertion::<Zagros>(relay_assertions);
	test.set_assertion::<AssetHubZagros>(asset_hub_assertions);
	test.set_assertion::<PenpalA>(penpal_assertions);
	test.set_dispatchable::<Zagros>(transfer_assets_dispatchable);
	test.assert();

	// Query final balances
	let sender_balance_after = test.sender.balance;
	let sov_penpal_on_ah_after = AssetHubZagros::execute_with(|| {
		<AssetHubZagros as AssetHubZagrosPallet>::Balances::free_balance(sov_penpal_on_ah)
	});
	let receiver_assets_after = PenpalA::execute_with(|| {
		type ForeignAssets = <PenpalA as PenpalAPallet>::ForeignAssets;
...
// ===============================================================
/// Transfers of native asset Penpal to Relay (using AssetHub reserve). Teyrchains want to avoid
/// managing SAs on all system chains, thus want all their HEZ-in-reserve to be held in their
/// Sovereign Account on Asset Hub.
#[test]
fn transfer_native_asset_from_penpal_to_relay_through_asset_hub() {
	...
	fn transfer_assets_dispatchable(t: PenpalToRelayThroughAHTest) -> DispatchResult {
		...
		// reanchor fees to the view of destination (Zagros Relay)
		let mut remote_fees = fee.clone().reanchored(&t.args.dest, &context).unwrap();
		if let Fungible(ref mut amount) = remote_fees.fun {
			// we already spent some fees along the way, just use half of what we started with
			*amount = *amount / 2;
		...
		// reanchor final dest (Zagros Relay) to the view of hop (Asset Hub)
		let mut dest = t.args.dest.clone();
		dest.reanchor(&asset_hub_location, &context).unwrap();
		// on Asset Hub
...
fn bidirectional_transfer_multiple_assets_between_penpal_and_asset_hub() {
	fn execute_xcm_penpal_to_asset_hub(t: ParaToSystemParaTest) -> DispatchResult {
		...
		// TODO(https://github.com/pezkuwichain/pezkuwi-DKS/issues/290): dry-run to get exact fees.
