...
use pezsp_runtime::MultiAddress;
...
fn register_token_v2() {
	...
	BridgeHubZagros::execute_with(|| {
		...
		let message = Message {
			...
			origin,
			assets: vec![],
			payload: Payload::CreateAsset { token, network: Network::Pezkuwi },
	...
	AssetHubZagros::execute_with(|| {
		...
		assert_expected_events!(
			...
			vec![
				...
				RuntimeEvent::ForeignAssets(pezpallet_assets::Event::Created { asset_id, owner, .. }) => {
...
fn send_token_v2() {
	...
	set_up_eth_and_hez_pool();
	let topic_id = BridgeHubZagros::execute_with(|| {
		type RuntimeEvent = <BridgeHubZagros as Chain>::RuntimeEvent;
	...
	AssetHubZagros::execute_with(|| {
		...
		assert_expected_events!(
			...
			vec![
				// message processed successfully
				RuntimeEvent::MessageQueue(
					pezpallet_message_queue::Event::Processed { success: true, id, .. }
				) => {
					id: *id == topic_id.into(),
				},
...
fn send_token_to_penpal_v2() {
	...
	set_up_eth_and_hez_pool();
	set_up_eth_and_hez_pool_on_penpal();
	...
	BridgeHubZagros::execute_with(|| {
		...
		let instructions = vec![
			...
			InitiateTransfer {
				// Penpal
				destination: Location::new(1, [Teyrchain(PARA_ID_B)]),
	...
	let penpal_sov_on_ah = AssetHubZagros::sovereign_account_id_of(Location::new(
		1,
...
fn invalid_xcm_traps_funds_on_ah() {
	...
	AssetHubZagros::execute_with(|| {
		...
		assert_expected_events!(
			AssetHubZagros,
			vec![RuntimeEvent::PezkuwiXcm(pezpallet_xcm::Event::AssetsTrapped { .. }) => {},]
		);
	});
}
