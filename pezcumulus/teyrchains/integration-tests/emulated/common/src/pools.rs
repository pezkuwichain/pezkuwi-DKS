		...
		$crate::pools::paste::paste! {
			<$chain>::execute_with(|| {
				...
				assert_expected_events!(
					$chain,
					vec![
						RuntimeEvent::AssetConversion($crate::pools::pezpallet_asset_conversion::Event::PoolCreated { .. }) => {},
					]
				);
				...
				assert_expected_events!(
					$chain,
					vec![
						RuntimeEvent::AssetConversion($crate::pools::pezpallet_asset_conversion::Event::LiquidityAdded { .. }) => {},
					]
				);
			});
		}
...
macro_rules! create_foreign_pool_with_parent_native_on {
	// default amounts and pezpallet name
	( $chain:ident, $asset_id:expr, $asset_owner:expr ) => {
	...
	// custom amounts, default pezpallet name
	( $chain:ident, $asset_id:expr, $asset_owner:expr, $wnd_amount:expr, $asset_amount:expr ) => {
		$crate::create_foreign_pool_with_parent_native_on!(
			$chain,
			ForeignAssets,
