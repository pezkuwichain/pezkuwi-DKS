...
//! # Bridge Hub Pezkuwichain Runtime
//!
//! This runtime currently supports bridging between:
//! - Pezkuwichain <> Zagros
//! - Pezkuwichain <> Pezkuwichain Bulletin
...
pub mod bridge_to_zagros_config;
...
pub type Migrations = (
	...
	>,
	bridge_to_zagros_config::migration::FixMessagesV1Migration<
		Runtime,
		bridge_to_zagros_config::WithBridgeHubZagrosMessagesInstance,
	>,
	bridge_to_zagros_config::migration::StaticToDynamicLanes,
	pezframe_support::migrations::RemoveStorage<
		BridgeZagrosMessagesPalletName,
...
parameter_types! {
	pub const BridgeZagrosMessagesPalletName: &'static str = "BridgeZagrosMessages";
...
construct_runtime!(
	pub enum Runtime
	{
		...
		// With-Zagros teyrchain bridge module.
		...
		//
		// we can't use `BridgePezkuwichainBulletinGrandpa` name here, because the same Bulletin runtime
		...
		BridgePezkuwiBulletinGrandpa: pezpallet_bridge_grandpa::<Instance4> = 60,
		// With-Pezkuwichain Bulletin messaging bridge module.
		//
		// we can't use `BridgePezkuwichainBulletinMessages` name here, because the same Bulletin runtime
		// will be used for both Pezkuwichain and Pezkuwi Bulletin chains AND this name affects runtime
...
pezbridge_runtime_common::generate_bridge_reject_obsolete_headers_and_messages! {
	...
	CheckAndBoostBridgeGrandpaTransactions<
		...
		bridge_to_zagros_config::PriorityBoostPerRelayHeader,
	...
	>,
	// Messages
	BridgeZagrosMessages,
	BridgePezkuwichainBulletinMessages
}
...
mod benches {
	pezframe_benchmarking::define_benchmarks!(
		...
		// Bridge pallets
		[pezpallet_bridge_grandpa, ZagrosFinality]
		[pezpallet_bridge_teyrchains, WithinZagros]
		[pezpallet_bridge_messages, PezkuwichainToZagros]
		[pezpallet_bridge_messages, PezkuwichainToPezkuwichainBulletin]
...
impl_runtime_apis! {
	...
	impl pezframe_benchmarking::Benchmark<Block> for Runtime {
		...
		fn dispatch_benchmark(
			...
			impl pezpallet_xcm_benchmarks::generic::Config for Runtime {
				...
				fn export_message_origin_and_destination(
					...
					let _ = PezkuwiXcm::force_xcm_version(
						RuntimeOrigin::root(),
						Box::new(bridge_to_zagros_config::BridgeHubZagrosLocation::get()),
						XCM_VERSION,
					).map_err(|e| {
						tracing::error!(
							...
							location=?bridge_to_zagros_config::BridgeHubZagrosLocation::get(),
							version=?XCM_VERSION,
					...
					pezframe_support::assert_ok!(
						Balances::mint_into(
							...
							bridge_to_zagros_config::BridgeDeposit::get()
					...
					// open bridge
					let bridge_destination_universal_location: InteriorLocation = [GlobalConsensus(NetworkId::ByGenesis(ZAGROS_GENESIS_HASH)), Teyrchain(8765)].into();
					let locations = XcmOverBridgeHubZagros::bridge_locations(
					...
					XcmOverBridgeHubZagros::do_open_bridge(
						...
						tracing::error!(
							target: "bridges::benchmark",
							error=?e,
							locations=?sibling_teyrchain_location,
							lane_id=?bridge_destination_universal_location,
							"Failed to `XcmOverBridgeHubZagros::open_bridge`"
						);
			...
			impl BridgeMessagesConfig<bridge_to_zagros_config::WithBridgeHubZagrosMessagesInstance> for Runtime {
				...
				fn prepare_message_proof(
					...
					let universal_source = bridge_to_zagros_config::open_bridge_for_benchmarks::<
						Runtime,
						bridge_to_zagros_config::XcmOverBridgeHubZagrosInstance,
						xcm_config::LocationToAccountId,
					>(params.lane, 42);
					prepare_message_proof_from_teyrchain::<
						Runtime,
				...
				fn prepare_message_delivery_proof(
					params: MessageDeliveryProofParams<AccountId, LaneIdOf<Runtime, bridge_to_zagros_config::WithBridgeHubZagrosMessagesInstance>>,
				) -> bridge_to_zagros_config::ToZagrosBridgeHubMessagesDeliveryProof<bridge_to_zagros_config::WithBridgeHubZagrosMessagesInstance> {
					let _ = bridge_to_zagros_config::open_bridge_for_benchmarks::<
						Runtime,
						bridge_to_zagros_config::XcmOverBridgeHubZagrosInstance,
						xcm_config::LocationToAccountId,
					>(params.lane, 42);
			...
			impl BridgeMessagesConfig<bridge_to_bulletin_config::WithPezkuwichainBulletinMessagesInstance> for Runtime {
				...
				fn prepare_message_delivery_proof(
					params: MessageDeliveryProofParams<AccountId, LaneIdOf<Runtime, bridge_to_bulletin_config::WithPezkuwichainBulletinMessagesInstance>>,
				) -> bridge_to_bulletin_config::ToPezkuwichainBulletinMessagesDeliveryProof<bridge_to_bulletin_config::WithPezkuwichainBulletinMessagesInstance> {
