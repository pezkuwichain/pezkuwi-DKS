	...
	LaneId: Copy + Encode,
{
	...
	let (relay_chain_header, justification, bridged_para_head, teyrchain_heads, para_heads_proof) =
		make_complex_bridged_teyrchain_heads_proof::<BridgedRelayChain, BridgedParachain>(
			para_state_root,
			para_header_number,
			relay_header_number,
			bridged_para_id,
...
pub fn make_complex_bridged_teyrchain_heads_proof<BridgedRelayChain, BridgedParachain>(
	...
	BridgedParachain: bp_runtime::Chain<Hash = ParaHash> + Teyrchain,
{
	...
	let (relay_state_root, para_heads_proof, teyrchain_heads) =
		prepare_teyrchain_heads_proof::<HeaderOf<BridgedParachain>>(vec![(
			bridged_para_id,
