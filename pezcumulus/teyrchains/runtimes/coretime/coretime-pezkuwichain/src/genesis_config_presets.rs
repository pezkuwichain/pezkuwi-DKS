...
//! # Coretime Pezkuwichain Runtime genesis config presets
...
const CORETIME_PEZKUWICHAIN_ED: Balance = ExistentialDeposit::get();
pub const CORETIME_PARA_ID: ParaId = ParaId::new(1005);

fn coretime_pezkuwichain_genesis(
	...
	build_struct_json_patch!(RuntimeGenesisConfig {
		...
		collator_selection: CollatorSelectionConfig {
			...
			candidacy_bond: CORETIME_PEZKUWICHAIN_ED * 16,
		},
...
pub fn get_preset(id: &PresetId) -> Option<Vec<u8>> {
	let patch = match id.as_ref() {
		pezsp_genesis_builder::LOCAL_TESTNET_RUNTIME_PRESET => coretime_pezkuwichain_genesis(
			// initial collators.
			vec![
