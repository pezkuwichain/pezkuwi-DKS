...
/// from the teyrchain context. That is, the teyrchain provides a market (broker) for the sale of
...
impl CoretimeInterface for CoretimeAllocator {
	...
	fn request_core_count(count: CoreIndex) {
		...
		// Weight for `request_core_count` from zagros benchmarks:
	...
	fn assign_core(
		...
		// Weight for `assign_core` from zagros benchmarks:
...
impl MaybeConvert<TaskId, AccountId> for SovereignAccountOf {
	fn maybe_convert(id: TaskId) -> Option<AccountId> {
		// Currently all tasks are teyrchains.
		let location = Location::new(1, [Teyrchain(id)]);
...
impl pezpallet_broker::Config for Runtime {
	...
	type ConvertBalance = pezsp_runtime::traits::Identity;
	type WeightInfo = weights::pezpallet_broker::WeightInfo<Runtime>;
	type PalletId = BrokerPalletId;
	type AdminOrigin = EnsureRoot<AccountId>;
	type SovereignAccountOf = SovereignAccountOf;
	type MaxAutoRenewals = ConstU32<20>;
	type PriceAdapter = pezpallet_broker::MinimumPrice<Balance, MinimumEndPrice>;
