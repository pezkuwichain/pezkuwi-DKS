...
//! Penpal Teyrchain Runtime genesis config presets
...
mod tests {
	...
	// Duplicated from `pezsc_chain_spec::json_patch::merge` which lives in a node-side crate
	// with heavy transitive deps (pezsc-executor, pezsc-network, etc.), making it unsuitable as a
	...
	fn assert_genesis_preset_valid(preset_id: &pezsp_genesis_builder::PresetId) {
	...
	fn dev_genesis_preset_is_valid() {
		assert_genesis_preset_valid(&pezsp_genesis_builder::DEV_RUNTIME_PRESET.into());
	}

	#[test]
	fn local_testnet_genesis_preset_is_valid() {
		assert_genesis_preset_valid(&pezsp_genesis_builder::LOCAL_TESTNET_RUNTIME_PRESET.into());
	}
}
