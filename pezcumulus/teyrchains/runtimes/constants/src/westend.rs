...
pub mod account {
	...
	/// Westend treasury pezpallet id, used to convert into AccountId - in Westend as a destination for
	/// slashed funds.
...
pub mod fee {
	...
	impl WeightToFeePolynomial for RefTimeToFee {
		...
		fn polynomial() -> WeightToFeeCoefficients<Self::Balance> {
			...
			// The standard system teyrchain configuration is 1/10 of that, as in 1/100 CENT.
...
pub mod snowbridge {
	...
	parameter_types! {
		...
		pub AssetHubParaId: ParaId = ParaId::from(westend_runtime_constants::system_teyrchain::ASSET_HUB_ID);
	}
}
...
/// DAP-related constants, shared across all Westend system teyrchain runtimes.
pub mod dap {
	pub use westend_runtime_constants::system_teyrchain::dap::*;
}

/// Accumulate-and-forward constants, shared across all Westend system teyrchain runtimes.
pub mod accumulate_forward {
	pub use westend_runtime_constants::system_teyrchain::accumulate_forward::*;
}

pub mod locations {
	use pezframe_support::parameter_types;
	pub use westend_runtime_constants::system_teyrchain::{AssetHubParaId, PeopleParaId};
	use xcm::latest::prelude::{Location, Teyrchain};

	parameter_types! {
		pub AssetHubLocation: Location = Location::new(1, Teyrchain(westend_runtime_constants::system_teyrchain::ASSET_HUB_ID));
		pub PeopleLocation: Location = Location::new(1, Teyrchain(westend_runtime_constants::system_teyrchain::PEOPLE_ID));
	}
