...
// Implement Convert trait for use with pezpallet_multi_asset_bounties
/// Converter from `AccountId32` to `VersionedLocatableAccount` for use with
/// `pezpallet_multi_asset_bounties`.
///
/// # Example
///
/// ,ignore
...
impl pezsp_runtime::traits::Convert<pezsp_runtime::AccountId32, VersionedLocatableAccount>
	for AccountIdToLocalLocation
{
	...
	/// This conversion fills in default values (location = "here", network = None) which may
	/// be incorrect if the account is from another chain or network.
	fn convert(account_id: pezsp_runtime::AccountId32) -> VersionedLocatableAccount {
...
pub mod benchmarks {
	...
	use pezpallet_multi_asset_bounties::ArgumentsFactory as MultiAssetBountiesArgumentsFactory;
