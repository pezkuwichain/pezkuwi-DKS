...
/// Generic Glutton Zagros Config for all currently used setups.
pub fn glutton_zagros_config(
	para_id: ParaId,
	...
	GenericChainSpec::builder(
		glutton_zagros_runtime::WASM_BINARY.expect("WASM binary was not built, please build it!"),
...
pub fn chain_id(para_id: ParaId, chain_type: &ChainType) -> String {
	match chain_type {
		ChainType::Development => format!("glutton-zagros-dev-{}", para_id),
		ChainType::Local => format!("glutton-zagros-local-{}", para_id),
		ChainType::Live => format!("glutton-zagros-{}", para_id),
		ChainType::Custom(_) => format!("glutton-zagros-custom-{}", para_id),
	}
}
