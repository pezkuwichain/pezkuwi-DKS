...
use pezsp_runtime::{generic::BlockId, traits::Header};
...
impl RelayChainInterface for TestRelayClient {
	...
	async fn prove_child_read(
		&self,
		_: RelayHash,
		_: &pezcumulus_relay_chain_interface::ChildInfo,
		_: &[Vec<u8>],
	) -> RelayChainResult<pezsc_client_api::StorageProof> {
		unimplemented!("Not needed for test")
	}
...
pub fn babe_epoch_change_digest_item() -> pezsp_runtime::generic::DigestItem {
	...
	pezsp_runtime::generic::DigestItem::Consensus(BABE_ENGINE_ID, log.encode())
}
...
pub fn relay_header_with_slot(number: u32, parent_hash: RelayHash, slot: u64) -> RelayHeader {
	use pezsc_consensus_babe::{CompatibleDigestItem, PreDigest, SecondaryPlainPreDigest};
