...
use pezcumulus_client_consensus_common::{
	self as consensus_common, get_relay_slot, TeyrchainBlockImportMarker,
};
use pezcumulus_client_proof_size_recording::prepare_proof_size_recording_aux_data;
...
use pezkuwi_primitives::{Block as RelayBlock, CoreIndex, Header as RelayHeader, Id as ParaId};
...
use pezsp_api::{ApiExt, ProofRecorder, ProvideRuntimeApi, StorageProof};
...
where
	...
	Client::Api: AuraApi<Block, P::Public>
		...
		+ BlockBuilder<Block>
		+ pezcumulus_primitives_core::KeyToIncludeInRelayProof<Block>
	...
	P::Signature: TryFrom<Vec<u8>> + Member + Codec,
{
	async move {
		...
		loop {
			...
			// Query scheduling parameters at the teyrchain best head. This assumes
			...
			for blocks_per_core in blocks_per_cores {
				...
				match build_collation_for_core(BuildCollationParams {
					...
					blocks_per_core,
					time_for_core,
					is_last_core_in_teyrchain_slot: cores.is_last_core()
						&& slot_time.is_teyrchain_slot_ending(para_slot_duration.as_duration()),
					collator_peer_id,
...
> {
	...
	collator_sender: &'a pezsc_utils::mpsc::TracingUnboundedSender<CollatorMessage<Block>>,
	...
	is_last_core_in_teyrchain_slot: bool,
	collator_peer_id: PeerId,
	relay_parent_data: RelayParentData,
	total_number_of_blocks: u32,
	included_header_hash: Block::Hash,
	relay_slot: pezcumulus_primitives_aura::Slot,
	para_slot: pezcumulus_primitives_aura::Slot,
...
>(
	BuildCollationParams {
		...
		is_last_core_in_teyrchain_slot,
		collator_peer_id,
		...
		+ pezcumulus_primitives_core::KeyToIncludeInRelayProof<Block>,
{
	...
	for block_index in 0..blocks_per_core {
		...
		let runtime_upgrade_digest = parent_header
			...
			.logs
			.iter()
			.any(|it| matches!(it, pezsp_runtime::DigestItem::RuntimeEnvironmentUpdated));
	...
	RelayClient: RelayChainInterface + 'static,
{
	...
	loop {
		...
		if pezsc_consensus_babe::contains_epoch_change::<RelayBlock>(&current_relay_header) {
...
enum SlotHandoverAdjustment {
	...
	/// producing the last block in each teyrchain slot entirely.
	Skip,
}
...
impl BlockProductionSchedule {
	...
	///
	/// In Bundling mode, we skip the last block in the teyrchain slot
	...
	fn should_skip_production(&self) -> bool {
		self.mode.skips_last_block()
			...
			&& self.is_last_core_in_teyrchain_slot
	}
	...
	fn authoring_duration(&self, time_left: Duration, block_time: Duration) -> Duration {
		let adjusted = match &self.mode {
			SlotHandoverAdjustment::Shorten { time_factor }
				if self.is_last_core_in_teyrchain_slot =>
			{
...
mod block_production_schedule_tests {
	...
	mod schedule_tests {
		...
		fn skip_production_only_in_fast_mode_last_core_last_block() {
			...
			// Should NOT skip: not last core in teyrchain slot
		...
		fn effective_last_block_includes_second_to_last() {
			...
			// (original logic doesn't gate on is_last_core_in_teyrchain_slot)
