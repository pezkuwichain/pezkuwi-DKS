...
use pezcumulus_client_consensus_common::get_relay_slot;
use pezcumulus_primitives_aura::Slot;
use pezcumulus_relay_chain_interface::RelayChainInterface;
use pezkuwi_node_subsystem::gen::{stream::Stream, FutureExt};
use pezkuwi_primitives::Block as RelayBlock;
...
/// V3 is enabled on both the teyrchain (runtime API) and the relay chain
...
impl<RelayClient: RelayChainInterface + 'static> SchedulingInfo<RelayClient> {
	...
	pub async fn wait_for_scheduling_parent(
			...
			>= get_current_relay_slot(Duration::ZERO, self.relay_slot_duration)
		{
			...
			// relay block.
			if pezsc_consensus_babe::contains_epoch_change::<RelayBlock>(
...
mod tests {
	...
	use pezkuwi_primitives::{node_features::FeatureIndex, NodeFeatures};
