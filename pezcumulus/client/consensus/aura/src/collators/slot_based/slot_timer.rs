...
impl SlotTime {
	...
	/// Check if the next relay chain slot would be in a different teyrchain slot.
	pub fn is_teyrchain_slot_ending(&self, teyrchain_slot_duration: Duration) -> bool {
		...
		// Calculate current teyrchain slot
		let current_teyrchain_slot = now.as_millis() / teyrchain_slot_duration.as_millis();

		// Calculate teyrchain slot for next relay slot
		let next_teyrchain_slot =
			next_relay_slot_start_time.as_millis() / teyrchain_slot_duration.as_millis() as u128;

		current_teyrchain_slot != next_teyrchain_slot
	}
}
