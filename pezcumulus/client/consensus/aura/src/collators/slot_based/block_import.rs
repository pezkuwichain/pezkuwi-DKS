...
use pezsp_runtime::traits::{Block as BlockT, HashingFor, Header as _};
use pezsp_trie::proof_size_extension::{ProofSizeExt, RecordingProofSizeProvider};
use std::sync::Arc;
...
fn ignored_nodes_key<H: Encode>(block_hash: H) -> Vec<u8> {
	(b"pezcumulus_slot_based_nodes_to_ignore", block_hash).encode()
}
...
impl<Block: BlockT, BI, Client> SlotBasedBlockImport<Block, BI, Client> {
	...
	fn execute_block_and_collect_storage_proof(
		&self,
		params: &mut pezsc_consensus::BlockImportParams<Block>,
	) -> Result<(), pezsp_consensus::Error>
	where
