...
use pezcumulus_primitives_core::{
	relay_chain::Hash as PHash, DigestItem, PersistedValidationData, RelayProofRequest,
	TeyrchainBlockData,
};
...
use pezsp_consensus::{BlockOrigin, Environment, ProposeArgs, Proposer};
...
pub struct BuildBlockAndImportParams<'a, Block: BlockT, P: Pair> {
	...
	/// Teyrchain-specific inherent data.
	pub teyrchain_inherent_data: TeyrchainInherentData,
...
impl<Block: BlockT> From<BuiltBlock<Block>> for TeyrchainCandidate<Block> {
	...
	P::Signature: TryFrom<Vec<u8>> + Member + Codec,
{
	...
	/// Build and import a teyrchain block using the given parameters.
	...
	/// Build a teyrchain block using the given parameters.
	...
	> {
		...
		// Prepare inherent data - merge teyrchain inherent data with other inherent data
	...
	pub async fn collate(
		...
		max_pov_size: usize,
		scheduling_proof: Option<pezcumulus_primitives_core::SchedulingProof>,
		...
		let maybe_candidate = self
			.build_block_and_import(BuildBlockAndImportParams {
				...
				teyrchain_inherent_data: inherent_data.0,
