...
use pezcumulus_primitives_core::{
	CollationInfo, CollectCollationInfo, SchedulingProof, TeyrchainBlockData,
};

use pezkuwi_primitives::UMP_SEPARATOR;
use pezsc_client_api::BlockBackend;
use pezsp_api::{ApiExt, ProvideRuntimeApi, StorageProof};
...
pub trait ServiceInterface<Block: BlockT> {
	...
	///
	/// `scheduling_proof` is `Some` for V3 candidates (produces [`TeyrchainBlockData::V2`])
	/// and `None` for legacy candidates (produces [`TeyrchainBlockData::V1`]).
	///
