...
use pezsp_trie::proof_size_extension::RecordedProofSizeEstimations;
use std::sync::Arc;

const PROOF_SIZE_RECORDING_VERSION: &[u8] = b"pezcumulus_proof_size_recording_version";
...
fn proof_size_recording_key<H: Encode>(block_hash: H) -> Vec<u8> {
	(b"pezcumulus_proof_size_recording", block_hash).encode()
}
