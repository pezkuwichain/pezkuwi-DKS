	...
	fn runtime(&self, chain_spec: &dyn ChainSpec) -> pezsc_cli::Result<Runtime> {
		...
		let block_number = metadata_inspector.block_number().unwrap_or_else(|| {
			log::warn!(
                ...
                https://paritytech.github.io/pezkuwi-sdk/master/pezkuwi_sdk_docs/reference_docs/omni_node/index.html#runtime-conventions.
			...
			None => {
				log::warn!(
					r#"⚠️  The Aura authority ID type was not found in the runtime metadata.
					   This can be expected if the runtime does not include `pezpallet-aura`,
...
impl MetadataInspector {
	...
	fn aura_consensus_id(&self) -> Option<AuraConsensusId> {
		...
		// 2. (Robust Fallback) Check the "Authorities" storage item in the Aura pezpallet.
	...
	fn fetch_metadata(chain_spec: &dyn ChainSpec) -> Result<(Metadata, u32), pezsc_cli::Error> {
		let mut storage = chain_spec.build_storage()?;
		let code_bytes = storage
			.top
			.remove(pezsp_storage::well_known_keys::CODE)
			.ok_or("chain spec genesis does not contain code")?;
		let executor = WasmExecutor::<TeyrchainHostFunctions>::builder()
	...
	fn fetch_metadata_from_bytes(mut encoded: &[u8]) -> Result<(Metadata, u32), pezsc_cli::Error> {
		let prefixed = RuntimeMetadataPrefixed::decode(&mut encoded).map_err(|e| {
			pezsc_cli::Error::Input(format!("failed to decode prefixed metadata: {e}").into())
		})?;
		...
		let metadata = Metadata::decode(&mut &encoded[..]).map_err(|e| {
			pezsc_cli::Error::Input(format!("failed to decode subxt metadata: {e}").into())
		})?;
...
mod tests {
	...
	fn pezcumulus_test_runtime_inspector() -> MetadataInspector {
		let opaque_metadata = fetch_latest_metadata_from_code_blob(
