		...
		pezsc_consensus::BlockImport<Block, Error = pezsp_consensus::Error> + 'static,
{
	...
	fn start_dev_node(
		...
		if let (Some(pool), Some(hop)) = (hop_pool.as_ref(), hop.as_ref()) {
			let task = pezsc_hop::build_maintenance_task::<Block, _, _>(
				&client,
				&transaction_pool,
				pool.clone(),
	...
	AuraId: AuraIdT + Sync,
{
	...
	> + Send
		...
		move |block: Hash, ()| {
			...
			let timestamp_provider = pezsp_timestamp::InherentDataProvider::new(timestamp.into());
	...
	<AuraId as AppCrypto>::Pair: Send + Sync,
{
	...
	fn launch_slot_based_collator<CIDP, CHP, Proposer, CS, Spawner>(
		...
		CHP: pezcumulus_client_consensus_common::ValidationCodeHashProvider<Hash>
			+ Send
			+ Sync
			+ 'static,
