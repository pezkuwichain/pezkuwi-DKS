...
use pezsp_api::{ApiExt, ProofRecorder, ProofRecorderIgnoredNodes, ProvideRuntimeApi};
...
fn init_block_builder(
	...
	let mut block_builder = pezsc_block_builder::BlockBuilderBuilder::new(client)
		.on_parent_block(at)
...
impl<'a> BuildTeyrchainBlockData for pezsc_block_builder::BlockBuilder<'a, Block, Client> {
	fn build_teyrchain_block(self, parent_state_root: Hash) -> TeyrchainBlockData<Block> {
		...
		TeyrchainBlockData::new(vec![built_block.block], storage_proof, None)
	}
}
