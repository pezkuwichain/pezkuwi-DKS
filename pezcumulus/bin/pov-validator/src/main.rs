...
fn main() -> anyhow::Result<()> {
	...
	let validation_code =
		pezsp_maybe_compressed_blob::decompress(&validation_code, VALIDATION_CODE_BOMB_LIMIT)
			.map_err(|error| {
	...
	let executor = WasmExecutor::<pezsp_io::BizinikiwiHostFunctions>::builder()
	...
	let pov = pezsp_maybe_compressed_blob::decompress(&pov.block_data.0, POV_BOMB_LIMIT).map_err(
		|error| {
