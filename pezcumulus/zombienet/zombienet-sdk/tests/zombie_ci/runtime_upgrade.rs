...
use anyhow::anyhow;
use pezcumulus_test_runtime::wasm_spec_version_incremented::WASM_BINARY as WASM_RUNTIME_UPGRADE;
...
use pezkuwi_primitives::{Id as ParaId, MAX_CODE_SIZE};
...
async fn big_message_client(ws_uri: &str) -> Result<OnlineClient<PezkuwiConfig>, anyhow::Error> {
	let rpc = ReconnectingRpcClient::builder()
		.max_request_size(25 * 1024 * 1024)
		.max_response_size(25 * 1024 * 1024)
		.build(ws_uri.to_string())
		.await?;
	Ok(OnlineClient::<PezkuwiConfig>::from_rpc_client(rpc).await?)
}
...
async fn runtime_upgrade() -> Result<(), anyhow::Error> {
	...
	let _: OnlineClient<PezkuwiConfig> = charlie.wait_client().await?;
	...
	log::info!("Measuring teyrchain throughput before runtime upgrade...");
	...
	log::info!("Waiting for teyrchain runtime upgrade to version {}", expected_spec_version);
	...
	log::info!("Measuring teyrchain throughput after runtime upgrade...");
	assert_para_throughput(&relay_client, 15, [(ParaId::from(PARA_ID), 14..17)], []).await?;
