...
fn update_root_cargo_toml(
	...
	deps.values()
		...
		.for_each(|(name, dep)| {
			...
			workspace_dependencies[name]["git"] = value(BIZINIKIWI_GIT_URL);
...
fn main() {
	...
	tar.append_dir_all("bizinikiwi-node-template", node_template_path)
		.expect("Writes bizinikiwi-node-template archive");
}
...
pezframe-system = { version = "4.0.0-dev", default-features = false, path = "../../../../frame/system" }
"#;
		...
		assert_eq!(
			actual_deps.get("dependencies").unwrap().get("pezframe-system").unwrap(),
			&Dependency {
				name: "pezframe-system".into(),
