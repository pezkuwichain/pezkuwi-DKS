...
pub use pezsp_application_crypto::key_types::BEEFY as KEY_TYPE;
...
pub trait BeefyAuthorityId: RuntimeAppPublic {
	...
	fn try_sign_with_store(
		...
		store: KeystorePtr,
		msg: &[u8],
	) -> Result<Option<impl AsRef<[u8]> + Debug>, pezsp_keystore::Error>;
...
pub mod ecdsa_crypto {
	...
	impl BeefyAuthorityId for AuthorityId {
		...
		fn try_sign_with_store(
			&self,
			store: pezsp_keystore::KeystorePtr,
			msg: &[u8],
		) -> Result<Option<impl AsRef<[u8]> + Debug>, pezsp_keystore::Error> {
...
pub mod bls_crypto {
	...
	use pezsp_core::{bls381::Pair as BlsPair, crypto::Wraps, ByteArray, Pair as _};
	#[cfg(feature = "std")]
	use pezsp_keystore::KeystorePtr;
...
pub mod ecdsa_bls_crypto {
	...
	use pezsp_core::{crypto::Wraps, ecdsa_bls381::Pair as EcdsaBlsPair, ByteArray};
	#[cfg(feature = "std")]
	use pezsp_keystore::KeystorePtr;
