...
fn function_std_impl(
	...
	let args = get_function_arguments(&method.sig)
		...
		.chain(
			...
			iter::from_fn(|| {
				if is_wasm_only {
					Some(parse_quote!(
						mut __function_context__: &mut dyn #crate_::pezsp_wasm_interface::FunctionContext
					))
				} else {
					None
				}
	...
	let call_to_trait = if !tracing {
		...
		parse_quote!(
			#crate_::pezsp_tracing::within_span! { #crate_::pezsp_tracing::trace_span!(#function_name_str);
				#call_to_trait
			}
		)
	};
...
fn generate_call_to_trait(
	...
	if takes_self_argument(&method.sig) {
		...
		let impl_trait_name = if is_wasm_only {
			quote!( #crate_::pezsp_wasm_interface::FunctionContext )
		} else {
