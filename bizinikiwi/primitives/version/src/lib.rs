...
//! Bizinikiwi runtime version
//!
//! Each runtime that should be executed by a Bizinikiwi based node needs to have a runtime version.
...
//!
//! Bizinikiwi will fetch the runtime version from a `wasm` blob by first checking the
...
/// };
///
/// # const RUNTIME_API_VERSIONS: pezsp_version::ApisVec = pezsp_version::create_apis_vec!([]);
/// ```
///
...
pub use pezsp_version_proc_macro::runtime_version;
...
pub fn core_version_from_apis(apis: &ApisVec) -> Option<u32> {
	let id = pezsp_crypto_hashing_proc_macro::blake2b_64!(b"Core");
