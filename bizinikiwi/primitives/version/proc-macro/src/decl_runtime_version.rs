...
/// This is a duplicate of `pezsp_version::RuntimeVersion`. We cannot unfortunately use the original
/// declaration, because if we directly depend on `pezsp_version` from this proc-macro cargo will
...
impl ParseRuntimeVersion {
	...
	fn parse_str_literal(expr: &Expr) -> Result<String> {
		match expr {
			// TODO: Remove this branch when `pezsp_runtime::create_runtime_str` is removed
