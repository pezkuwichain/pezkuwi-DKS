...
/// The Bizinikiwi format implementation of `NodeCodec`.
...
/// The Bizinikiwi format implementation of `TrieStream`.
	...
	H: Hasher,
{
	const USE_EXTENSION: bool = false;
	const ALLOW_EMPTY: bool = true;
	const MAX_INLINE_VALUE: Option<u32> = Some(pezsp_core::storage::TRIE_VALUE_NODE_THRESHOLD);
...
mod tests {
	...
	fn generate_storage_root_with_proof_works_independently_from_the_delta_order() {
		...
		let storage_root =
			pezsp_core::H256::decode(&mut &include_bytes!("../test-res/storage_root")[..]).unwrap();
