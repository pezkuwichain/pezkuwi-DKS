...
use pezsp_core::{self, storage::StateVersion, Hasher, TypeId, U256};
#[doc(hidden)]
pub use pezsp_core::{
...
impl IdentifyAccount for pezsp_core::ed25519::Public {
	type AccountId = Self;
...
impl IdentifyAccount for pezsp_core::sr25519::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl IdentifyAccount for pezsp_core::ecdsa::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl IdentifyAccount for pezsp_core::ecdsa_bls381::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl Verify for pezsp_core::ed25519::Signature {
	type Signer = pezsp_core::ed25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &pezsp_core::ed25519::Public) -> bool {
		pezsp_io::crypto::ed25519_verify(self, msg.get(), signer)
	}
}

impl Verify for pezsp_core::sr25519::Signature {
	type Signer = pezsp_core::sr25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &pezsp_core::sr25519::Public) -> bool {
		pezsp_io::crypto::sr25519_verify(self, msg.get(), signer)
	}
}

impl Verify for pezsp_core::ecdsa::Signature {
	type Signer = pezsp_core::ecdsa::Public;
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &pezsp_core::ecdsa::Public) -> bool {
		match pezsp_io::crypto::secp256k1_ecdsa_recover_compressed(
			self.as_ref(),
			&pezsp_io::hashing::blake2_256(msg.get()),
		) {
...
#[cfg(feature = "bls-experimental")]
impl Verify for pezsp_core::ecdsa_bls381::Signature {
	type Signer = pezsp_core::ecdsa_bls381::Public;
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &pezsp_core::ecdsa_bls381::Public) -> bool {
		<pezsp_core::ecdsa_bls381::Pair as pezsp_core::Pair>::verify(self, msg.get(), signer)
	}
}
...
where
	...
	<<T as AppCrypto>::Public as pezsp_application_crypto::AppPublic>::Generic: IdentifyAccount<
		AccountId = <<T as AppCrypto>::Public as pezsp_application_crypto::AppPublic>::Generic,
	>,
{
	type AccountId = <T as AppCrypto>::Public;
	fn verify<L: Lazy<[u8]>>(&self, msg: L, signer: &<T as AppCrypto>::Public) -> bool {
		use pezsp_application_crypto::IsWrappedBy;
		let inner: &S = self.as_ref();
		let inner_pubkey =
			<<T as AppCrypto>::Public as pezsp_application_crypto::AppPublic>::Generic::from_ref(
				signer,
			);
...
///
/// ```
/// # use pezsp_runtime::{morph_types, traits::{Morph, TryMorph, TypedGet, ConstU32}};
/// # use pezsp_arithmetic::traits::CheckedSub;
///
/// morph_types! {
...
impl Hasher for BlakeTwo256 {
	type Out = pezsp_core::H256;
	type StdHasher = hash256_std_hasher::Hash256StdHasher;
	const LENGTH: usize = 32;

	fn hash(s: &[u8]) -> Self::Out {
		pezsp_io::hashing::blake2_256(s).into()
	}
}

impl Hash for BlakeTwo256 {
	type Output = pezsp_core::H256;

	fn ordered_trie_root(input: Vec<Vec<u8>>, version: StateVersion) -> Self::Output {
		pezsp_io::trie::blake2_256_ordered_root(input, version)
	}

	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>, version: StateVersion) -> Self::Output {
		pezsp_io::trie::blake2_256_root(input, version)
	}
}
...
impl Hasher for Keccak256 {
	...
	fn hash(s: &[u8]) -> Self::Out {
		pezsp_io::hashing::keccak_256(s).into()
	}
}
...
impl Hash for Keccak256 {
	...
	fn ordered_trie_root(input: Vec<Vec<u8>>, version: StateVersion) -> Self::Output {
		pezsp_io::trie::keccak_256_ordered_root(input, version)
	}

	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>, version: StateVersion) -> Self::Output {
		pezsp_io::trie::keccak_256_root(input, version)
	}
}
...
impl CheckEqual for pezsp_core::H256 {
	#[cfg(feature = "std")]
	fn check_equal(&self, other: &Self) {
...
/// Something which fulfills the abstract idea of a Bizinikiwi header. It has types for a `Number`,
...
/// A Bizinikiwi block that allows us to lazily decode its extrinsics.
...
/// Something which fulfills the abstract idea of a Bizinikiwi block. It has types for
...
/// A "checkable" piece of information, used by the standard Bizinikiwi Executive in order to
...
pub trait RefundWeight {
	...
	fn refund(&mut self, weight: pezsp_weights::Weight);
}
...
impl RefundWeight for () {
	fn refund(&mut self, _weight: pezsp_weights::Weight) {}
}
...
pub trait Dispatchable {
	...
	/// down the line in a runtime. The current Bizinikiwi runtime uses a struct with the same name
...
/// An "executable" piece of information, used by the standard Bizinikiwi Executive in order to
...
macro_rules! impl_opaque_keys_inner {
	(
		...
		impl $name {
			...
			pub fn generate(
				owner: &[u8],
				seed: Option<$crate::pezsp_std::vec::Vec<u8>>,
...
impl Printable for u64 {
	fn print(&self) {
		pezsp_io::misc::print_num(*self);
	}
}

impl Printable for &[u8] {
	fn print(&self) {
		pezsp_io::misc::print_hex(self);
	}
}
...
impl Printable for &str {
	fn print(&self) {
		pezsp_io::misc::print_utf8(self.as_bytes());
	}
}
...
impl Printable for pezsp_weights::Weight {
	fn print(&self) {
		self.ref_time().print()
	}
}
...
mod tests {
	...
	impl crate::BoundToRuntimeAppPublic for Sr25519Key {
		type Public = pezsp_application_crypto::sr25519::AppPublic;
	}

	pub struct Ed25519Key;
	impl crate::BoundToRuntimeAppPublic for Ed25519Key {
		type Public = pezsp_application_crypto::ed25519::AppPublic;
	}

	pub struct EcdsaKey;
	impl crate::BoundToRuntimeAppPublic for EcdsaKey {
		type Public = pezsp_application_crypto::ecdsa::AppPublic;
	}
	...
	fn opaque_keys_ownership_proof_works() {
		let mut sr25519 = pezsp_core::sr25519::Pair::generate().0;
		let mut ed25519 = pezsp_core::ed25519::Pair::generate().0;
		let mut ecdsa = pezsp_core::ecdsa::Pair::generate().0;
		...
		let mut ext = pezsp_io::TestExternalities::default();
		ext.register_extension(pezsp_keystore::KeystoreExt(Arc::new(
