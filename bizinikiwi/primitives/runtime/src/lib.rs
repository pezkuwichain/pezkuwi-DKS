...
//! the Bizinikiwi runtime, but are not particularly `FRAME`-oriented.
//!
...
pub use paste;
#[doc(hidden)]
pub use pezsp_arithmetic::traits::Saturating;

#[doc(hidden)]
pub use pezsp_application_crypto as app_crypto;

pub use pezsp_core::storage::StateVersion;
#[cfg(feature = "std")]
pub use pezsp_core::storage::{Storage, StorageChild};
...
pub use pezsp_application_crypto::{BoundToRuntimeAppPublic, RuntimeAppPublic};
...
pub use pezsp_arithmetic::helpers_128bit;
...
pub trait BuildStorage {
	...
	fn build_storage(&self) -> Result<pezsp_core::storage::Storage, String> {
...
#[cfg(feature = "std")]
impl BuildStorage for pezsp_core::storage::Storage {
	fn assimilate_storage(&self, storage: &mut pezsp_core::storage::Storage) -> Result<(), String> {
...
impl BuildStorage for () {
	fn assimilate_storage(&self, _: &mut pezsp_core::storage::Storage) -> Result<(), String> {
...
pub enum MultiSigner {
	...
	/// `pezpallet_revive`. This means that the same public key controls two accounts. But
	/// this is already the case due to `pezpallet_revive`'s address mapping.
...
impl traits::IdentifyAccount for MultiSigner {
	...
	fn into_account(self) -> AccountId32 {
		match self {
			...
			Self::Ecdsa(who) => pezsp_io::hashing::blake2_256(who.as_ref()).into(),
			Self::Eth(who) => {
				...
				// of an eth account.
				let eth_address = &pezsp_io::hashing::keccak_256(who.as_ref())[12..];
				// This is by convention: `pezpallet_revive` maps eth addresses to account ids
...
impl Verify for MultiSignature {
	...
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &AccountId32) -> bool {
		...
		match self {
			...
			Self::Ecdsa(sig) => {
				let m = pezsp_io::hashing::blake2_256(msg.get());
				pezsp_io::crypto::secp256k1_ecdsa_recover_compressed(sig.as_ref(), &m)
					.map_or(false, |pubkey| pezsp_io::hashing::blake2_256(&pubkey) == who)
			},
			Self::Eth(sig) => {
				let m = pezsp_io::hashing::keccak_256(msg.get());
				pezsp_io::crypto::secp256k1_ecdsa_recover_compressed(sig.as_ref(), &m)
					.map_or(false, |pubkey| {
...
/// ```rust
/// # fn main() {
/// pezsp_runtime::assert_eq_error_rate!(10, 10, 0);
/// pezsp_runtime::assert_eq_error_rate!(10, 11, 1);
/// pezsp_runtime::assert_eq_error_rate!(12, 10, 2);
/// # }
/// ```
///
/// ```rust,should_panic
/// # fn main() {
/// pezsp_runtime::assert_eq_error_rate!(12, 10, 1);
/// # }
/// ```
#[macro_export]
#[cfg(feature = "std")]
...
impl core::fmt::Debug for OpaqueExtrinsic {
	...
	fn fmt(&self, fmt: &mut core::fmt::Formatter) -> core::fmt::Result {
		write!(fmt, "{}", pezsp_core::hexdisplay::HexDisplay::from(&self.0.as_ref()))
	}
...
impl ::serde::Serialize for OpaqueExtrinsic {
	...
	where
		S: ::serde::Serializer,
	{
		codec::Encode::using_encoded(&self.0, |bytes| ::pezsp_core::bytes::serialize(bytes, seq))
	}
}
...
mod tests {
	...
	use pezsp_core::{crypto::Pair, hex2array};
	use pezsp_io::TestExternalities;
	use pezsp_state_machine::create_proof_check_backend;
	...
	fn execute_and_generate_proof_works() {
		...
		let (_, proof) = ext.execute_and_prove(|| {
			pezsp_io::storage::get(b"a");
			pezsp_io::storage::get(b"b");
			pezsp_io::storage::get(b"v");
			pezsp_io::storage::get(b"d");
		});
		...
		let _ = ext.execute_and_prove(|| {
			pezsp_io::storage::set(b"a", &vec![1u8; 44]);
		});

		// ensure that these changes are propagated to the backend.

		ext.execute_with(|| {
			assert_eq!(pezsp_io::storage::get(b"a").unwrap(), vec![1u8; 44]);
			assert_eq!(pezsp_io::storage::get(b"b").unwrap(), vec![2u8; 33]);
		});
	}
}

// NOTE: we have to test the pezsp_core stuff also from a different crate to check that the macro
