...
//!
//! `pezsp-io` crate exposes a low level methods to make and control HTTP requests
...
mod header {
	...
	impl Header {
		...
		pub fn name(&self) -> &str {
			...
			// we don't store them as `Strings` to avoid bringing `alloc::String` to pezsp-std
			// or here.
