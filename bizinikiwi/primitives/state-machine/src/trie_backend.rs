...
use hash_db::HashDB;
use hash_db::Hasher;
use pezsp_core::storage::{ChildInfo, StateVersion};
#[cfg(feature = "std")]
use pezsp_trie::{
...
#[cfg(not(feature = "std"))]
use pezsp_trie::{Error, NodeCodec};
use pezsp_trie::{MerkleValue, PrefixedMemoryDB, StorageProof, TrieRecorderProvider};
...
pub trait TrieCacheProvider<H: Hasher> {
	...
	type Cache<'a>: TrieCacheT<pezsp_trie::NodeCodec<H>> + 'a
	where
		Self: 'a;
...
pub struct UnimplementedRecorderProvider<H> {
	// Not strictly necessary, but the H bound allows to use this as a drop-in
	// replacement for the [`pezsp_trie::recorder::Recorder`] in no-std contexts.
	_phantom: core::marker::PhantomData<H>,
}
...
#[cfg(feature = "std")]
type DefaultRecorder<H> = pezsp_trie::recorder::Recorder<H>;
...
pub mod tests {
	...
	type Recorder = pezsp_trie::recorder::Recorder<BlakeTwo256>;
	...
	pub(crate) fn test_db(state_version: StateVersion) -> (PrefixedMemoryDB<BlakeTwo256>, H256) {
...

		{
			...
			fn build<L: pezsp_trie::TrieLayout>(
				mut trie: pezsp_trie::TrieDBMut<L>,
				child_info: &ChildInfo,
				sub_root: &[u8],
			) {
	...
	fn child_proof_recording_with_edge_cases_works_inner(state_version: StateVersion) {
		...
		let nodes = {
			...
			for node in proof.into_iter_nodes() {
				...
				if hash != value_hash {
					let node = pezsp_trie::NodeCodec::<BlakeTwo256>::decode(&node)
						.unwrap()
						.to_owned_node::<pezsp_trie::LayoutV1<BlakeTwo256>>()
						.unwrap();
