...
#[cfg(feature = "std")]
use pezsp_externalities::TransactionType;
use pezsp_externalities::{Extension, ExtensionStore, Externalities, MultiRemovalResults};
...
#[cfg(feature = "std")]
fn guard() -> pezsp_panic_handler::AbortGuard {
	pezsp_panic_handler::AbortGuard::force_abort()
}
	...
	B: Backend<H>,
{
	...
	pub fn new(
		overlay: &'a mut OverlayedChanges<H>,
		backend: &'a B,
		extensions: Option<&'a mut pezsp_externalities::Extensions>,
	) -> Self {
		Self {
			overlay,
			backend,
	...
	B: Backend<H>,
{
	...
	fn clear_prefix(
		...
		if pezsp_core::storage::well_known_keys::starts_with_child_storage_key(prefix) {
			warn!(
				target: "trie",
	...
	B: Backend<H>,
{
	...
	fn register_extension_with_type_id(
		...
		Err(pezsp_externalities::Error::ExtensionsAreNotSupported)
	}
