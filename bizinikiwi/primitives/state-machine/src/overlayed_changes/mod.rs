...
#[cfg(feature = "std")]
use pezsp_externalities::{Extension, Extensions, TransactionType};
use pezsp_trie::{empty_child_trie_root, LayoutV1};
...
impl<'a> OverlayedExtensions<'a> {
	...
	pub fn register(
		...
		match self.extensions.entry(type_id) {
			...
			MapEntry::Occupied(_) => Err(pezsp_externalities::Error::ExtensionAlreadyRegistered),
		}
	}
...
mod tests {
	...
	use pezsp_core::{traits::Externalities, Blake2Hasher};
