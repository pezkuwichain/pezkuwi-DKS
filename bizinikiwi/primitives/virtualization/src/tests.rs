...
//!
//! - **Runtime-side**, compiled into `pezsc-runtime-test` and dispatched through the wasm executor —
//!   this exercises the full host-function FFI round-trip.
//! - **Host-side**, invoked as a regular `#[test]` from `pezsc-virtualization` — this exercises the
...
//! in `pezsc-virtualization` rather than here.
...
/// - the runtime (compiled into `pezsc-runtime-test`) — exercises the wasm/forwarder path end-to-end,
///   including the host-function FFI;
/// - native host tests (in `pezsc-virtualization`) — exercises the host-side dispatch directly.
///
/// Tests that need pre-populated externalities (storage fallback for `compile_from_storage_key`)
/// can't run from a runtime context and live as standalone `#[test]`s in `pezsc-virtualization`.
///
/// The `program` needs to be set to `pezsp_virtualization_test_fixture::binary()`. It can't be
