...
//! This crate is intended for use by runtime code (e.g. pezpallet-contracts) to spawn PolkaVM
...
//! in the `pezsc-virtualization` client crate, which registers a `VirtManager` via [`VirtManagerExt`]
...
//! **not available on Pezkuwi** (or any other production relay/teyrchain) until they are. Using
...
/// (e.g. [`pezsp_io::BizinikiwiHostFunctions`]). At runtime, register a [`VirtManagerExt`]
