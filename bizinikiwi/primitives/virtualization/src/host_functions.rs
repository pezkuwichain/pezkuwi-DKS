...
///
/// These host functions are **not available on Pezkuwi** (or any other production
/// relay/teyrchain) until the API has been stabilized. If you use them in a production
...
pub trait Virtualization {
	...
	fn compile_from_bytes(
		...
		i64,
	> {
		use pezsp_externalities::ExternalitiesExt as _;
		use std::sync::Once;
	...
	> {
		...
		let code = if child_trie.is_empty() {
			...
			let child_info = pezsp_storage::ChildInfo::new_default(child_trie);
