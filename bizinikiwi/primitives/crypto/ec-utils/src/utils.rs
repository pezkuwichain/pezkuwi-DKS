...
use pezsp_runtime_interface::RIType;
...
impl pezsp_runtime_interface::host::IntoFFIValue for HostcallResult {
	fn into_ffi_value(
		value: Self::Inner,
		_context: &mut dyn pezsp_runtime_interface::pezsp_wasm_interface::FunctionContext,
	) -> pezsp_runtime_interface::pezsp_wasm_interface::Result<Self::FFIType> {
		Ok(match value {
			Ok(()) => 0,
			Err(e) => e as u32,
		})
