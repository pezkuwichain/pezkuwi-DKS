...
/// Statement topic.
///
/// A 32-byte topic identifier that serializes as a hex string (like `pezsp_core::Bytes`).
#[derive(
	Clone,
	Copy,
	Debug,
...
impl serde::Serialize for Topic {
	...
	where
		S: serde::Serializer,
	{
		pezsp_core::bytes::serialize(&self.0, serializer)
	}
}
...
impl<'de> serde::Deserialize<'de> for Topic {
	...
	{
		let mut arr = [0u8; 32];
		pezsp_core::bytes::deserialize_check_len(
			deserializer,
			pezsp_core::bytes::ExpectedLen::Exact(&mut arr[..]),
		)?;
		Ok(Topic(arr))
	}
}
