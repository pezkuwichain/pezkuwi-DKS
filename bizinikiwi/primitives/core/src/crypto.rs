...
/// as a password.
/// - If `string` begins with a `/` character it is prefixed with the Bizinikiwi public `DEV_PHRASE`
...
///
/// ```
/// # use pezsp_core::crypto::{SecretUri, DeriveJunction, DEV_PHRASE, ExposeSecret};
/// # use std::str::FromStr;
...
pub trait Pair: CryptoType + Sized {
	...
	fn from_phrase(
		...
		let big_seed =
			bizinikiwi_bip39::seed_from_entropy(&entropy[0..entropy_len], password.unwrap_or(""))
	...
	///   - the path may be followed by `///`, in which case everything after the `///` is treated
	/// as a password.
	/// - If `s` begins with a `/` character it is prefixed with the Bizinikiwi public `DEV_PHRASE`
	///   and interpreted as above.
	///
	/// In this case they are interpreted as HDKD junctions; purely numeric items are interpreted as
	/// integers, non-numeric items as strings. Junctions prefixed with `/` are interpreted as soft
...
pub mod key_types {
	...
	/// Key type for controlling an account in a Bizinikiwi runtime, built-in. Identified as `acco`.
