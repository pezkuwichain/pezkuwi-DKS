...
pub fn create_rpc_runtime(max_connections: u32) -> std::io::Result<tokio::runtime::Runtime> {
	tokio::runtime::Builder::new_multi_thread()
		...
		.on_thread_start(|| {
			pezsc_utils::metrics::TOKIO_THREADS_ALIVE.inc();
			pezsc_utils::metrics::TOKIO_THREADS_TOTAL.inc();
			middleware::RPC_THREADS_ALIVE.inc();
			middleware::RPC_THREADS_TOTAL.inc();
		})
		.on_thread_stop(|| {
			pezsc_utils::metrics::TOKIO_THREADS_ALIVE.dec();
...
impl pezsp_core::traits::SpawnNamed for RpcSpawnHandle {
	fn spawn_blocking(
		&self,
		_name: &'static str,
...
impl Server {
	...
	pub fn spawn_handle(&self) -> Arc<dyn pezsp_core::traits::SpawnNamed> {
