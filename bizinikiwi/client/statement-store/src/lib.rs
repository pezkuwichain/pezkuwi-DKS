...
use pezsc_client_api::{backend::StorageProvider, Backend, StorageKey};
...
pub use pezsp_statement_store::{Error, StatementStore, MAX_TOPICS};
...
pub const MAX_STATEMENT_SIZE: usize =
	pezsc_network_statement::config::MAX_STATEMENT_NOTIFICATION_SIZE as usize - 1;
...
pub use pezsc_network_statement::config::DEFAULT_STATEMENTS_PER_SECOND as DEFAULT_RATE_LIMIT;
	...
	Client: HeaderBackend<Block> + StorageProvider<Block, BE> + Send + Sync + 'static,
{
	fn read_allowance(
		...
		use pezsp_statement_store::{statement_allowance_key, StatementAllowance};
		...
		self.collect_statements(
			...
			|statement| {
				if let (Some(key), Some(_)) = (statement.decryption_key(), statement.data()) {
					let public: pezsp_core::ed25519::Public = UncheckedFrom::unchecked_from(key);
					let public: pezsp_statement_store::ed25519::Public = public.into();
					match self.keystore.key_pair::<pezsp_statement_store::ed25519::Pair>(&public) {
						Err(e) => {
							log::debug!(
								target: LOG_TARGET,
...
mod tests {
	...
	use crate::{col, Store};
	use pezsc_keystore::Keystore;
	use pezsp_core::{Decode, Encode, Pair};
	use pezsp_statement_store::{
	...
	pub(crate) type TestBackend = pezsc_client_api::in_mem::Backend<Block>;

	impl pezsc_client_api::StorageProvider<Block, TestBackend> for TestClient {
		fn storage(
			&self,
			_hash: Hash,
			key: &pezsc_client_api::StorageKey,
		) -> pezsp_blockchain::Result<Option<pezsc_client_api::StorageData>> {
			use pezsp_statement_store::StatementAllowance;
			...
			Ok(Some(pezsc_client_api::StorageData(allowance.encode())))
		}

		fn storage_hash(
			&self,
			_hash: Hash,
			_key: &pezsc_client_api::StorageKey,
		...
		fn storage_keys(
			&self,
			_hash: Hash,
			_prefix: Option<&pezsc_client_api::StorageKey>,
			_start_key: Option<&pezsc_client_api::StorageKey>,
		) -> pezsp_blockchain::Result<
			pezsc_client_api::backend::KeysIter<
				<TestBackend as pezsc_client_api::Backend<Block>>::State,
				Block,
			>,
		> {
			unimplemented!()
		...
		fn storage_pairs(
			...
			pezsc_client_api::backend::PairsIter<
				<TestBackend as pezsc_client_api::Backend<Block>>::State,
				Block,
			>,
		> {
		...
		fn child_storage(
			&self,
			_hash: Hash,
			_child_info: &pezsc_client_api::ChildInfo,
			_key: &pezsc_client_api::StorageKey,
		) -> pezsp_blockchain::Result<Option<pezsc_client_api::StorageData>> {
			unimplemented!()
		}
		...
		fn closest_merkle_value(
			...
			_hash: Hash,
			_key: &pezsc_client_api::StorageKey,
		) -> pezsp_blockchain::Result<Option<pezsc_client_api::MerkleValue<Hash>>> {
			unimplemented!()
		}
	...
	fn test_store() -> (Store, tempfile::TempDir) {
		...
		path.push("db");
		let keystore = std::sync::Arc::new(pezsc_keystore::LocalKeystore::in_memory());
		let store = Store::new::<Block, TestClient, TestBackend>(
			...
			keystore,
			None,
			Box::new(pezsp_core::testing::TaskExecutor::new()),
		)
		.unwrap();
	...
	fn signed_statement_with_topics(
		...
		let kp = pezsp_core::ed25519::Pair::from_string("//Alice", None).unwrap();
	...
	fn account_keypair(seed: u64) -> pezsp_core::ed25519::Pair {
		pezsp_core::ed25519::Pair::from_string(&format!("//StatementAccount{seed}"), None)
	...
	fn posted_clear_decrypts() {
		...
		let public = store
			.keystore
			.ed25519_generate_new(pezsp_core::crypto::key_types::STATEMENT, None)
			.unwrap();
	...
	fn remove_by_covers_various_situations() {
		use pezsp_statement_store::{StatementSource, StatementStore, SubmitResult};
	...
	fn submit_rejects_malformed_statements() {
		...
		let ed_kp = pezsp_core::ed25519::Pair::from_string("//Alice", None).unwrap();
		let sr_kp = pezsp_core::sr25519::Pair::from_string("//Alice", None).unwrap();
		let ecdsa_kp = pezsp_core::ecdsa::Pair::from_string("//Alice", None).unwrap();
		...
		let bob_kp = pezsp_core::ed25519::Pair::from_string("//Bob", None).unwrap();
	...
	fn subscription_reconnect_receives_current_state() {
		...
		use pezsp_statement_store::OptimizedTopicFilter;
