...
use pezsc_mixnet::{PostErr, RemoteErr, TopologyErr};

/// Mixnet RPC error type.
pub struct Error(pub pezsc_mixnet::Error);
...
impl From<Error> for ErrorObjectOwned {
	fn from(err: Error) -> Self {
		let code = match err.0 {
			pezsc_mixnet::Error::ServiceUnavailable => BASE_ERROR + 1,
			pezsc_mixnet::Error::NoReply => BASE_ERROR + 2,
			pezsc_mixnet::Error::BadReply => BASE_ERROR + 3,
			...
			pezsc_mixnet::Error::Remote(RemoteErr::Decode(_)) => BASE_ERROR + 201,
		};
