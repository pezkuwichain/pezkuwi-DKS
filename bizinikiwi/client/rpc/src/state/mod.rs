...
use pezsc_rpc_api::{check_if_safe, DenyUnsafe};
use pezsc_tracing::block::TracingExecuteBlock;
use pezsp_api::{CallApiAt, Metadata, ProvideRuntimeApi};
...
pub use pezsc_rpc_api::{child_state::*, state::*};
	...
	Client: Send + Sync + 'static,
{
	...
	) -> Result<pezsp_rpc::tracing::TraceBlockResponse, Error>;
	...
	Client: Send + Sync + 'static,
{
	...
	async fn storage_size(
		...
		let deny_unsafe = ext
			.get::<DenyUnsafe>()
			.cloned()
			.expect("DenyUnsafe extension is always set by the bizinikiwi rpc server; qed");
...
fn client_err(err: pezsp_blockchain::Error) -> Error {
