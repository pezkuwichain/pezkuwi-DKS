...
use pezsc_rpc_api::state::ReadProof;
	...
	Client::Api: Metadata<Block>,
{
	...
	) -> std::result::Result<pezsp_rpc::tracing::TraceBlockResponse, Error> {
		pezsc_tracing::block::BlockExecutor::new(
			self.client.clone(),
			block,
			targets,
			storage_keys,
	...
	Client::Api: Metadata<Block>,
{
	...
	fn storage_entries(
			...
			ChildType::from_prefixed_key(&storage_key)
		{
			...
			return Err(client_err(pezsp_blockchain::Error::InvalidChildStorageKey));
		};
