...
use pezsc_rpc_api::system::helpers::PeerInfo;
		...
		futures::executor::block_on(rx.for_each(move |request| {
			match request {
				...
				Request::NetworkState(sender) => {
					let _ = sender.send(
						serde_json::to_value(&pezsc_network::network_state::NetworkState {
							peer_id: String::new(),
...
async fn system_network_state() {
	use pezsc_network::network_state::NetworkState;
...
fn test_add_reset_log_filter() {
	...
	if std::env::var("TEST_LOG_FILTER").is_ok() {
		let mut builder = pezsc_tracing::logging::LoggerBuilder::new("test_before_add=debug");
