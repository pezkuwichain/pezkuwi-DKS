...
use pezsp_statement_store::{statement_allowance_key, Statement, StatementAllowance, Topic};
use std::sync::Arc;
...
fn generate_statements() -> Vec<Statement> {
	...
	statement.set_proof(pezsp_statement_store::Proof::Ed25519 {
		signature: [0u8; 64],
		signer: [0u8; 32],
	});
...
async fn check_submitted(
	mut expected: Vec<pezsp_statement_store::Statement>,
	...
	while !expected.is_empty() {
		...
		for result in result {
			let new_statement = pezsp_statement_store::Statement::decode(&mut &result.0[..])
