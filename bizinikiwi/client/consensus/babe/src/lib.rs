...
use pezsp_consensus_babe::{inherents::BabeInherentData, SlotDuration};
...
const AUTHORING_SCORE_VRF_CONTEXT: &[u8] = b"bizinikiwi-babe-vrf";
...
pub struct Epoch(pezsp_consensus_babe::Epoch);

impl Deref for Epoch {
	type Target = pezsp_consensus_babe::Epoch;
...
impl From<pezsp_consensus_babe::Epoch> for Epoch {
	fn from(epoch: pezsp_consensus_babe::Epoch) -> Self {
		Epoch(epoch)
	}
}
...
pub enum Error<B: BlockT> {
	...
	FetchParentHeader(pezsp_blockchain::Error),
	...
	CheckInherentsUnhandled(pezsp_inherents::InherentIdentifier),
...
where
	...
	L: pezsc_consensus::JustificationSyncLink<B> + 'static,
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	let slot_worker = pezsc_consensus_slots::start_slot_worker(
		babe_link.config.slot_duration(),
		select_chain,
		pezsc_consensus_slots::SimpleSlotWorkerToSlotWorker(worker),
		sync_oracle,
		create_inherent_data_providers,
	);
...
impl<B, C, E, I, Error, SO, L, BS> pezsc_consensus_slots::SimpleSlotWorker<B>
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	fn pre_digest_data(&self, _slot: Slot, claim: &Self::Claim) -> Vec<pezsp_runtime::DigestItem> {
...
where
	...
	CIDP::InherentDataProviders: InherentDataProviderExt + Send,
	SC: pezsp_consensus::SelectChain<Block> + 'static,
{
...
where
	Block: BlockT,
	Client: HeaderBackend<Block> + HeaderMetadata<Block, Error = pezsp_blockchain::Error>,
{
	let info = client.info();
	...
	Backend: BackendT<Block>,
{
	...
	let leaves = backend.blockchain().leaves()?.into_iter().filter(|&leaf| {
		pezsp_blockchain::tree_route(&*client, revert_up_to_hash, leaf)
