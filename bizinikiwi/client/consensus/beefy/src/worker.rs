...
use pezsc_utils::notification::NotificationReceiver;
use pezsp_api::ProvideRuntimeApi;
use pezsp_arithmetic::traits::{AtLeast32Bit, Saturating};
...
impl<B: Block, AuthorityId: AuthorityIdBound> PersistedState<B, AuthorityId> {
	...
	pub fn pezpallet_genesis(&self) -> NumberFor<B> {
		self.pezpallet_genesis
	}
	...
	pub fn init_session_at(
		...
		if is_authority && key_store.public_keys().map_or(false, |k| k.is_empty()) {
			error!(
				...
				(https://wiki.network.pezkuwichain.io/docs/maintain-guides-how-to-validate-polkadot#generating-the-session-keys)",
				new_session_start,
			);
	...
	AuthorityId: AuthorityIdBound,
{
	...
	fn handle_finality_notification(
		...
		match self.runtime.runtime_api().beefy_genesis(notification.hash) {
			Ok(Some(genesis)) if genesis != self.persisted_state.pezpallet_genesis => {
				debug!(target: LOG_TARGET, "🥩 ConsensusReset detected. Expected genesis: {}, found genesis: {}", self.persisted_state.pezpallet_genesis, genesis);
...
pub(crate) mod tests {
	...
	use pezsc_network_gossip::GossipEngine;
	use pezsc_network_sync::SyncingService;
	use pezsc_network_test::TestNetFactory;
	use pezsp_blockchain::Backend as BlockchainBackendT;
	use pezsp_consensus_beefy::{
