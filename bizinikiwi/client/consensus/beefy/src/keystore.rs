...
use pezsp_application_crypto::{key_types::BEEFY as BEEFY_KEY_TYPE, RuntimeAppPublic};

use pezsp_keystore::KeystorePtr;
use std::marker::PhantomData;

use pezsp_consensus_beefy::{AuthorityIdBound, BeefyAuthorityId};
...
pub mod tests {
	...
	use pezsp_core::{ecdsa, Pair as PairT};
	...
	fn sign_error_for_ecdsa() {
		sign_error::<ecdsa_crypto::AuthorityId>(
			"pezsp_consensus_beefy::ecdsa_crypto::Public::try_sign_with_store() failed",
		);
	}
	...
	fn sign_error_for_ecdsa_n_bls() {
		sign_error::<ecdsa_bls_crypto::AuthorityId>(
			"pezsp_consensus_beefy::ecdsa_bls_crypto::Public::try_sign_with_store() failed",
		);
	}
