...
use pezsc_telemetry::{telemetry, TelemetryHandle, CONSENSUS_DEBUG};
...
impl<Block: BlockT> Inner<Block> {
	...
	fn validate_round_message(
		...
		if !pezsp_consensus_grandpa::check_message_signature(
	...
	pub(crate) fn register(
		...
		Ok(Self {
			messages_validated: register(
				CounterVec::new(
					Opts::new(
						"bizinikiwi_finality_grandpa_communication_gossip_validator_messages",
...
impl<Block: BlockT> pezsc_network_gossip::Validator<Block> for GossipValidator<Block> {
	fn new_peer(
		&self,
	...
	fn validate(
		...
		who: &PeerId,
		data: &[u8],
	) -> pezsc_network_gossip::ValidationResult<Block::Hash> {
		...
		match action {
			Action::Keep(topic, cb) => {
				...
				pezsc_network_gossip::ValidationResult::ProcessAndKeep(topic)
			},
			Action::ProcessAndDiscard(topic, cb) => {
				self.report(*who, cb);
				pezsc_network_gossip::ValidationResult::ProcessAndDiscard(topic)
			},
			Action::Discard(cb) => {
				self.report(*who, cb);
