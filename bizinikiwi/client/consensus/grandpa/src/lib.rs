...
use pezsc_network::{types::ProtocolName, NetworkBackend, NotificationService};
...
use pezsp_core::{crypto::ByteArray, traits::CallContext};
...
pub struct Config {
	...
	pub local_role: pezsc_network::config::Role,
...
pub trait ClientForGrandpa<Block, BE>:
	...
	+ BlockImport<Block, Error = pezsp_consensus::Error>
...
pub fn grandpa_peers_set_config<B: BlockT, N: NetworkBackend<B, <B as BlockT>::Hash>>(
	...
	peer_store_handle: Arc<dyn pezsc_network::peer_store::PeerStoreProvider>,
	...
	N::notification_config(
		...
		pezsc_network::config::SetConfig {
			...
			non_reserved_mode: pezsc_network::config::NonReservedPeerMode::Deny,
		},
		metrics,
		peer_store_handle,
	)
