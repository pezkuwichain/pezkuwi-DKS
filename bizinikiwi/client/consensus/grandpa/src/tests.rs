...
use pezsc_transaction_pool_api::RejectAllTxPool;
...
use pezsp_application_crypto::key_types::GRANDPA;

type TestLinkHalf =
	LinkHalf<Block, PeersFullClient, LongestChain<bizinikiwi_test_runtime_client::Backend, Block>>;
...
type GrandpaBlockImport = crate::GrandpaBlockImport<
	...
	Block,
	PeersFullClient,
	LongestChain<bizinikiwi_test_runtime_client::Backend, Block>,
>;
...
impl GenesisAuthoritySetProvider<Block> for TestApi {
	fn get(&self) -> pezsp_blockchain::Result<AuthorityList> {
...
fn add_scheduled_change(builder: &mut impl BlockBuilderExt, change: ScheduledChange<BlockNumber>) {
	builder
		.push_deposit_log_digest_item(DigestItem::Consensus(
			GRANDPA_ENGINE_ID,
			pezsp_consensus_grandpa::ConsensusLog::ScheduledChange(change).encode(),
		))
		.unwrap();
}
...
fn add_forced_change(
	...
	builder
		.push_deposit_log_digest_item(DigestItem::Consensus(
			GRANDPA_ENGINE_ID,
			pezsp_consensus_grandpa::ConsensusLog::ForcedChange(median_last_finalized, change)
				.encode(),
		))
		.unwrap();
}
...
type TestEnvironment<N, S, SC, VR> =
	Environment<bizinikiwi_test_runtime_client::Backend, Block, TestClient, N, S, SC, VR>;
...
fn test_environment<N, S, VR>(
	...
	notification_service: Box<dyn NotificationService>,
	voting_rule: VR,
) -> TestEnvironment<N, S, LongestChain<bizinikiwi_test_runtime_client::Backend, Block>, VR>
where
	N: NetworkT<Block>,
	S: SyncingT<Block>,
	VR: VotingRule<Block, TestClient>,
...
async fn grandpa_environment_uses_round_base_block_for_voting_if_finality_target_errors() {
	...
	let link = peer.data.lock().take().unwrap();
	let client = peer.client().as_client().clone();
	let select_chain = pezsc_consensus::LongestChain::new(peer.client().as_backend());
...
async fn grandpa_environment_doesnt_send_equivocation_reports_for_itself() {
	...
	let equivocation_proof = pezsp_consensus_grandpa::Equivocation::Prevote(equivocation.clone());
	...
	let equivocation_proof = pezsp_consensus_grandpa::Equivocation::Prevote(equivocation);
