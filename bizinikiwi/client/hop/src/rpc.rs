...
mod tests {
	...
	use codec::Encode;
	use pezsp_api::{ApiError, CallApiAtParams};
	use pezsp_blockchain::{self, Info};
	use pezsp_core::{crypto::Pair, ed25519};
	use pezsp_runtime::{
		traits::{HashingFor, NumberFor},
		MultiSigner,
	};
	use pezsp_state_machine::InMemoryBackend;
	...
	impl HeaderBackend<Block> for MockClient {
		...
		fn status(
			&self,
			_hash: <Block as BlockT>::Hash,
		) -> pezsp_blockchain::Result<pezsp_blockchain::BlockStatus> {
			Ok(pezsp_blockchain::BlockStatus::Unknown)
		}
	...
	impl CallApiAt<Block> for MockClient {
		...
		fn runtime_version_at(
			...
			_call_context: pezsp_api::CallContext,
		) -> Result<pezsp_version::RuntimeVersion, ApiError> {
		...
		fn initialize_extensions(
			...
			_extensions: &mut pezsp_externalities::Extensions,
		) -> Result<(), ApiError> {
			Ok(())
		}
	}
