...
//!   dispatch (see [`crate::runtime_api`]) plus [`pezsc_transaction_pool_api::LocalTransactionPool`].
...
pub trait HopPromoter: Send + Sync + 'static {
	...
	/// carried into the unsigned promotion extrinsic so the runtime pezpallet can
...
where
	...
	P: pezsc_transaction_pool_api::LocalTransactionPool<Block = Block> + 'static,
{
	...
	P: pezsc_transaction_pool_api::LocalTransactionPool<Block = Block> + 'static,
{
	...
	match client
		.runtime_api()
		.has_api_with::<dyn pezsp_hop::HopRuntimeApi<Block, AccountId32>, _>(best_hash, |v| v >= 1)
	{
		Ok(true) => {
	...
	pub fn tick(&self) {
		...
		if let Some(ref promoter) = self.promoter {
			...
			for hash in hashes {
				...
				// pezpallet does not deduplicate.
