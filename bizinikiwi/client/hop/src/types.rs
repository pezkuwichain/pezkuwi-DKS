...
pub struct HopEntryMeta {
	...
	pub promoted: bool,
	/// `MultiSigner` of the account that signed the submission. The runtime pezpallet
...
pub enum HopError {
	...
	RuntimeApiError(#[from] pezsp_api::ApiError),
...
///
/// The runtime pezpallet re-derives this exact byte sequence to verify the
/// signature on-chain, so the construction must remain byte-identical to the
/// pezpallet's `signing_payload(data, submit_timestamp)`:
