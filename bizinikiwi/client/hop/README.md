...
mean; both are delegated to the runtime via the `pezsp_hop::HopRuntimeApi`.
...
Companion runtime crate: [`pezsp-hop`](../../primitives/hop/) — defines the
...
```rust,ignore
use pezsc_hop::{build_maintenance_task, HopApiServer, HopRpcServer};
...
If the runtime doesn't implement `pezsp_hop::HopRuntimeApi`, `try_build_promoter`
