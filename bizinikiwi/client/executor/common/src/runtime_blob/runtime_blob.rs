...
/// A program blob containing a Bizinikiwi runtime.
#[derive(Clone)]
...
impl RuntimeBlob {
	...
	///
	/// See [`pezsp_maybe_compressed_blob`] for details about decompression.
	pub fn uncompress_if_needed(wasm_code: &[u8]) -> Result<Self, WasmError> {
		use pezsp_maybe_compressed_blob::CODE_BLOB_BOMB_LIMIT;
		let wasm_code = pezsp_maybe_compressed_blob::decompress(wasm_code, CODE_BLOB_BOMB_LIMIT)
