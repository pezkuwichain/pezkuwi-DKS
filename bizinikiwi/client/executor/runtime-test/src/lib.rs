...
#[cfg(not(feature = "std"))]
use pezsp_core::{ed25519, sr25519};
#[cfg(not(feature = "std"))]
use pezsp_io::{
...
pezsp_core::wasm_export_functions! {
	...
	fn test_ordered_trie_root() -> Vec<u8> {
		BlakeTwo256::ordered_trie_root(
			...
			pezsp_core::storage::StateVersion::V1,
		).as_ref().to_vec()
	}

	fn test_offchain_index_set() {
		pezsp_io::offchain_index::set(b"k", b"v");
	}

	fn test_offchain_local_storage() -> bool {
		let kind = pezsp_core::offchain::StorageKind::PERSISTENT;
		assert_eq!(pezsp_io::offchain::local_storage_get(kind, b"test"), None);
		pezsp_io::offchain::local_storage_set(kind, b"test", b"asd");
		assert_eq!(pezsp_io::offchain::local_storage_get(kind, b"test"), Some(b"asd".to_vec()));

		let res = pezsp_io::offchain::local_storage_compare_and_set(
			kind,
			b"test",
			Some(b"asd".to_vec()),
			b"",
		);
		assert_eq!(pezsp_io::offchain::local_storage_get(kind, b"test"), Some(b"".to_vec()));
		res
	}
	...
	fn test_offchain_local_storage_with_none() {
		...
		let res = pezsp_io::offchain::local_storage_compare_and_set(kind, b"test", None, b"value");
		assert_eq!(res, true);
		assert_eq!(pezsp_io::offchain::local_storage_get(kind, b"test"), Some(b"value".to_vec()));
	}

	fn test_offchain_http() -> bool {
		use pezsp_core::offchain::HttpRequestStatus;
		let run = || -> Option<()> {
			let id = pezsp_io::offchain::http_request_start(
				"POST",
				"http://localhost:12345",
				&[],
			).ok()?;
			pezsp_io::offchain::http_request_add_header(id, "X-Auth", "test").ok()?;
			pezsp_io::offchain::http_request_write_body(id, &[1, 2, 3, 4], None).ok()?;
			pezsp_io::offchain::http_request_write_body(id, &[], None).ok()?;
			let status = pezsp_io::offchain::http_response_wait(&[id], None);
			assert!(status == vec![HttpRequestStatus::Finished(200)], "Expected Finished(200) status.");
			let headers = pezsp_io::offchain::http_response_headers(id);
			assert_eq!(headers, vec![(b"X-Auth".to_vec(), b"hello".to_vec())]);
			let mut buffer = vec![0; 64];
			let read = pezsp_io::offchain::http_response_read_body(id, &mut buffer, None).ok()?;
			assert_eq!(read, 3);
	...
	fn test_abort_on_panic() {
		pezsp_io::panic_handler::abort_on_panic("test_abort_on_panic called");
	}
	...
	fn test_virtualization(test_fixture: Vec<u8>) {
		pezsp_virtualization::run_tests(test_fixture.as_ref());
	}
}
...
mod output_validity {
	...
	#[cfg(not(feature = "std"))]
	use pezsp_runtime_interface::pack_ptr_and_len;
