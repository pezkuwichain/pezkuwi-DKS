...
//
// See https://github.com/paritytech/bizinikiwi/issues/2967 for details
...
fn test_virtualization(wasm_method: WasmExecutionMethod) {
	let mut ext = TestExternalities::default();
	ext.register_extension(pezsp_virtualization::VirtManagerExt::new(
		pezsc_virtualization::VirtManager::default(),
	));
	let mut ext = ext.ext();
	let fixture = pezsp_virtualization_test_fixture::binary().encode();
...
fn import_doubles_heap_strategy(wasm_method: WasmExecutionMethod) {
	use pezsp_core::traits::{CallContext, CodeExecutor, FetchRuntimeCode, RuntimeCode};
