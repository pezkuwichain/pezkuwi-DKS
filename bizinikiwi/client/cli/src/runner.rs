...
use log::info;
use pezsc_service::{Configuration, Error as ServiceError, TaskManager};
use pezsc_utils::metrics::{TOKIO_THREADS_ALIVE, TOKIO_THREADS_TOTAL};
...
/// A Bizinikiwi CLI runtime that can be used to run a node or a command
pub struct Runner<C: BizinikiwiCli> {
...
impl<C: BizinikiwiCli> Runner<C> {
	...
	/// 2020-06-03 16:14:21 ⛓  Native runtime: node-251 (bizinikiwi-node-1.tx1.au10)
	/// ```
...
/// Log information about the node itself.
pub fn print_node_infos<C: BizinikiwiCli>(config: &Configuration) {
	info!("{}", C::impl_name());
	...
	fn create_runner() -> Runner<Cli> {
		...
		let runner = Runner::new(
			Configuration {
				impl_name: "spec".into(),
				impl_version: "3".into(),
				role: pezsc_service::Role::Authority,
				tokio_handle: runtime.handle().clone(),
				transaction_pool: Default::default(),
				network: NetworkConfiguration::new_memory(),
				keystore: pezsc_service::config::KeystoreConfig::InMemory,
				database: pezsc_client_db::DatabaseSource::ParityDb { path: root.clone() },
				...
				rpc: RpcConfiguration {
					...
					port: 9944,
					batch_config: pezsc_service::config::RpcBatchRequestConfig::Unlimited,
					rate_limit: None,
				...
				announce_block: true,
				base_path: pezsc_service::BasePath::new(root.clone()),
				data_path: root,
			},
			runtime,
			Signals::dummy(),
	...
	fn ensure_run_until_exit_informs_tasks_to_end() {
		...
		runner
			.run_node_until_exit(move |cfg| async move {
				...
				Ok::<_, pezsc_service::Error>(task_manager)
			})
			.unwrap_err();
