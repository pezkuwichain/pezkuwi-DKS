...
//! Host-side PolkaVM backend for the [`pezsp_virtualization`] host functions.
//!
...
//! [`pezsp_virtualization::VirtManagerExt::new`].

use pezsp_virtualization::{
...
mod tests {
	...
	fn cache_does_not_leak_between_instances() {
		let program = pezsp_virtualization_test_fixture::binary();
