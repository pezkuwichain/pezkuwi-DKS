...
pub struct SubscriptionsInner<Block: BlockT, BE: Backend<Block>> {
	...
	///
	/// The `Arc` is handled one level-above, but bizinikiwi exposes the backend as Arc<T>.
	backend: Arc<BE>,
}
...
mod tests {
	...
	fn init_backend() -> (
		Arc<pezsc_client_api::in_mem::Backend<Block>>,
		Arc<Client<pezsc_client_api::in_mem::Backend<Block>>>,
	) {
	...
	fn produce_blocks(
		client: Arc<Client<pezsc_client_api::in_mem::Backend<Block>>>,
		num_blocks: usize,
