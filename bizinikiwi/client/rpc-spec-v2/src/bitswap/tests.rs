...
type Block = bizinikiwi_test_runtime::Block;
...
impl pezsc_client_api::BlockBackend<Block> for MockClient {
	fn block_body(
		&self,
	...
	fn block(
		&self,
		_hash: <Block as BlockT>::Hash,
	) -> pezsp_blockchain::Result<Option<pezsp_runtime::generic::SignedBlock<Block>>> {
		unimplemented!()
	}
	...
	fn justifications(
		&self,
		_hash: <Block as BlockT>::Hash,
	) -> pezsp_blockchain::Result<Option<pezsp_runtime::Justifications>> {
		unimplemented!()
	}

	fn block_hash(
		&self,
		_number: pezsp_runtime::traits::NumberFor<Block>,
...
impl pezsp_consensus::SyncOracle for MockSyncOracle {
...
async fn valid_cid_data_found_sha256() {
	...
	let digest = pezsp_crypto_hashing::sha2_256(&data);
...
async fn valid_cid_data_found_blake2b() {
	...
	let data = vec![1u8, 2, 3, 4, 5];
	let digest = pezsp_crypto_hashing::blake2_256(&data);
	mock_client.insert_transaction(H256::from(digest), data.clone());
...
async fn valid_cid_data_found_keccak256() {
	...
	let data = vec![1u8, 2, 3, 4, 5];
	let digest = pezsp_crypto_hashing::keccak_256(&data);
	mock_client.insert_transaction(H256::from(digest), data.clone());
