...
use pezsc_executor::RuntimeVersion;
...
use pezsp_trie::{proof_size_extension::ProofSizeExt, CompactProof, MerkleValue, StorageProof};
use std::{
...
use pezsp_core::traits::CodeExecutor;
...
enum PrepareStorageChangesResult<Block: BlockT> {
	...
	Import(Option<pezsc_consensus::StorageChanges<Block>>),
}
...
pub fn new_with_backend<B, E, Block, G, RA>(
	...
	telemetry: Option<TelemetryHandle>,
	config: ClientConfig<Block>,
) -> pezsp_blockchain::Result<Client<B, LocalCallExecutor<Block, B, E>, Block, RA>>
where
	E: CodeExecutor + pezsc_executor::RuntimeVersionOf,
	G: BuildGenesisBlock<
		Block,
...
where
	...
	Block::Header: Clone,
{
	/// Creates new Bizinikiwi Client with given blockchain and code executor.
	pub fn new<G>(
		backend: Arc<B>,
		executor: E,
		spawn_handle: Box<dyn SpawnNamed>,
	...
	pub fn state_at(&self, hash: Block::Hash) -> pezsp_blockchain::Result<B::State> {
	...
	pub fn code_at(&self, hash: Block::Hash) -> pezsp_blockchain::Result<Vec<u8>> {
	...
	fn apply_block(
		...
		storage_changes: Option<pezsc_consensus::StorageChanges<Block>>,
	) -> pezsp_blockchain::Result<ImportResult>
	where
	...
	) -> pezsp_blockchain::Result<PrepareStorageChangesResult<Block>>
	where
		Self: ProvideRuntimeApi<Block>,
		<Self as ProvideRuntimeApi<Block>>::Api: CoreApi<Block> + ApiExt<Block>,
	{
		...
		let storage_changes = match (enact_state, storage_changes, &import_block.body) {
			...
			(true, None, Some(ref body)) => {
				...
				Some(pezsc_consensus::StorageChanges::Changes(gen_storage_changes))
			},
	...
	fn apply_finality_with_block_hash(
		...
		let route_from_finalized =
			pezsp_blockchain::tree_route(self.backend.blockchain(), info.finalized_hash, hash)?;
		...
		if self.backend.blockchain().leaves()?.len() > 1 || info.best_number < block_number {
			let route_from_best =
				pezsp_blockchain::tree_route(self.backend.blockchain(), info.best_hash, hash)?;
	...
	pub fn revert(&self, n: NumberFor<Block>) -> pezsp_blockchain::Result<NumberFor<Block>> {
	...
	pub fn unsafe_revert(
		...
		n: NumberFor<Block>,
		blacklist: bool,
	) -> pezsp_blockchain::Result<NumberFor<Block>> {
	...
	/// Get block status.
	pub fn block_status(&self, hash: Block::Hash) -> pezsp_blockchain::Result<BlockStatus> {
	...
	) -> pezsp_blockchain::Result<Vec<Block::Hash>> {
		let load_header = |hash: Block::Hash| -> pezsp_blockchain::Result<Block::Header> {
			self.backend
				.blockchain()
				.header(hash)?
	...
	Block: BlockT,
{
	...
	fn read_proof_collection(
		...
		let proof = proof
			...
			.map_err(|e| pezsp_blockchain::Error::from_state(Box::new(e)))?;
		Ok((proof, count))
	}
	...
	fn verify_range_proof(
		...
		let mut db = pezsp_state_machine::MemoryDB::<HashingFor<Block>>::new(&[]);
		// Compact encoding
		pezsp_trie::decode_compact::<pezsp_state_machine::LayoutV0<HashingFor<Block>>, _, _>(
			&mut db,
			proof.iter_compact_encoded_nodes(),
			Some(&root),
		)
		.map_err(|e| pezsp_blockchain::Error::from_state(Box::new(e)))?;
		let proving_backend = pezsp_state_machine::TrieBackendBuilder::new(db, root).build();
...
where
	...
	RA: Send + Sync,
{
	fn header(&self, hash: Block::Hash) -> pezsp_blockchain::Result<Option<Block::Header>> {
		self.backend.blockchain().header(hash)
	}
	...
	fn status(&self, hash: Block::Hash) -> pezsp_blockchain::Result<blockchain::BlockStatus> {
	...
	fn hash(&self, number: NumberFor<Block>) -> pezsp_blockchain::Result<Option<Block::Hash>> {
	...
	RA: Send + Sync,
{
	...
	fn to_hash(&self, block_id: &BlockId<Block>) -> pezsp_blockchain::Result<Option<Block::Hash>> {
	...
	) -> pezsp_blockchain::Result<Option<NumberFor<Block>>> {
...
impl<B, E, Block, RA> pezsc_consensus::BlockImport<Block> for &Client<B, E, Block, RA>
where
	B: backend::Backend<Block>,
	E: CallExecutor<Block> + Send + Sync,
	Block: BlockT,
	...
	Block: BlockT,
{
	...
	fn block_status(&self, hash: Block::Hash) -> pezsp_blockchain::Result<BlockStatus> {
...
impl<BE, E, B, RA> pezsp_consensus::block_validation::Chain<B> for Client<BE, E, B, RA>
where
...
impl<BE, E, B, RA> pezsp_transaction_storage_proof::IndexedBody<B> for Client<BE, E, B, RA>
where
	BE: backend::Backend<B>,
	E: CallExecutor<B>,
	B: BlockT,
{
	fn block_indexed_body(
		&self,
		number: NumberFor<B>,
	) -> Result<Option<Vec<Vec<u8>>>, pezsp_transaction_storage_proof::Error> {
		let hash = match self
			.backend
			.blockchain()
			.block_hash_from_id(&BlockId::Number(number))
			.map_err(|e| pezsp_transaction_storage_proof::Error::Application(Box::new(e)))?
		{
			Some(hash) => hash,
			None => return Ok(None),
		};
	...
	fn number(
		&self,
		hash: B::Hash,
	) -> Result<Option<NumberFor<B>>, pezsp_transaction_storage_proof::Error> {
		self.backend
			.blockchain()
			.number(hash)
			.map_err(|e| pezsp_transaction_storage_proof::Error::Application(Box::new(e)))
