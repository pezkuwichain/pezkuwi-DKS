...
impl DatabaseSource {
	...
	pub fn path(&self) -> Option<&Path> {
		match self {
			// as per https://github.com/paritytech/bizinikiwi/pull/9500#discussion_r684312550
			//
			...
			Ok(Some(hashes)) => Ok(Some(
				hashes
					.map(|hash| match self.db.get(columns::TRANSACTION, hash.as_ref()) {
						Some(t) => Ok(t),
						None => Err(pezsp_blockchain::Error::Backend(format!(
							"Missing indexed transaction {hash:?}",
						))),
					})
					.collect::<Result<_, _>>()?,
...
pub(crate) mod tests {
	...
	fn multi_renew_transaction_storage() {
		...
		let x1_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&x1[1..]);
		let x2_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&x2[1..]);
	...
	fn multi_renew_mixed_duplicates_and_uniques() {
		...
		let w_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&w[1..]);
		let x_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&x[1..]);
		let y_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&y[1..]);
		let z_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&z[1..]);
	...
	fn block_indexed_body_preserves_renew_op_submission_order() {
		...
		let payloads: Vec<Vec<u8>> = (0..5)
			...
			.collect();
		let hashes: Vec<<HashingFor<Block> as pezsp_core::Hasher>::Out> = payloads
			.iter()
			.map(|p| <HashingFor<Block> as pezsp_core::Hasher>::hash(&p[1..]))
			.collect();
	...
	mod indexed_transaction_tests {
		...
		use pezsp_database::Transaction as DbTransaction;
		use rstest::rstest;
		...
		impl DbFactory {
			fn new(kind: BackendKind) -> Self {
				match kind {
					BackendKind::KvdbMemdb => Self::Persistent(pezsp_database::as_database(
		...
		fn prefetched_multi_renew_same_hash_balanced_lifecycle(#[case] kind: BackendKind) {
			...
			let payload_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&payload);
		...
		fn same_block_insert_and_renew_different_indices_with_prefetch(#[case] kind: BackendKind) {
			...
			let x = x_xt[1..].to_vec();
			let x_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&x);
		...
		fn synthetic_insert_extracts_tail_from_body() {
			...
			let tail_hash = <HashingFor<Block> as pezsp_core::Hasher>::hash(&expected_tail);
		...
		fn multiple_synthetic_ops_per_block_apply_in_order() {
			...
			let hash_a = <HashingFor<Block> as pezsp_core::Hasher>::hash(&payload_a);
			let hash_b = <HashingFor<Block> as pezsp_core::Hasher>::hash(&payload_b);
