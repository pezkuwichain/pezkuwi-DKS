...
pub use pezsp_runtime::transaction_validity::{
...
/// Bizinikiwi validates the transaction before it enters the pool.
///
...
/// See `pezsp_runtime::ValidTransaction` for details about every field.
...
pub enum ChainEvent<B: BlockT> {
	...
	NewBestBlock {
		...
		tree_route: Option<Arc<pezsp_blockchain::TreeRoute<B>>>,
	},
...
/// Wraps a `pool` and `block_hash` to implement [`pezsp_core::offchain::TransactionPool`].
struct OffchainTransactionPool<Block: BlockT> {
	block_hash: Block::Hash,
	pool: Arc<dyn OffchainSubmitTransaction<Block>>,
}

impl<Block: BlockT> pezsp_core::offchain::TransactionPool for OffchainTransactionPool<Block> {
