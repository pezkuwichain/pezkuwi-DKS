...
//! See <https://github.com/pezkuwichain/pezkuwi-sdk/issues/8912> for some more information. The implementation of the
	...
	<Block as BlockT>::Hash: Unpin,
{
	...
	) -> Result<InsertionInfo<ExtrinsicHash<ChainApi>>, pezsc_transaction_pool_api::error::Error> {
	...
	pub(super) async fn try_insert_with_replacement(
		...
		if new_tx.bytes > self.max_transactions_total_bytes {
			return Err(pezsc_transaction_pool_api::error::Error::ImmediatelyDropped);
		}

		let mut transactions = self.transactions.write().await;

		if transactions.contains_key(&hash) {
			return Err(pezsc_transaction_pool_api::error::Error::AlreadyImported(Box::new(hash)));
		}
		...
		match insertion_result {
			None => Err(pezsc_transaction_pool_api::error::Error::ImmediatelyDropped),
	...
	) -> Vec<Result<InsertionInfo<ExtrinsicHash<ChainApi>>, pezsc_transaction_pool_api::error::Error>>
	{
...
mod tx_mem_pool_tests {
	...
	async fn extend_unwatched_obeys_limit() {
		...
		assert!(matches!(
			...
			pezsc_transaction_pool_api::error::Error::ImmediatelyDropped
		));
	}
	...
	async fn extend_unwatched_detects_already_imported() {
		...
		assert!(matches!(
			results.into_iter().last().unwrap().unwrap_err(),
			pezsc_transaction_pool_api::error::Error::AlreadyImported(_)
		));
	}
