...
impl<A, Block, C, PR> pezsp_consensus::Environment<Block> for ProposerFactory<A, C, PR>
where
	A: TransactionPool<Block = Block> + 'static,
	Block: BlockT,
...
impl<A, Block, C, PR> pezsp_consensus::Proposer<Block> for Proposer<Block, C, A, PR>
where
	A: TransactionPool<Block = Block> + 'static,
	Block: BlockT,
	C: HeaderBackend<Block> + ProvideRuntimeApi<Block> + CallApiAt<Block> + Send + Sync + 'static,
	...
	PR: ProofRecording,
{
	...
	) -> Result<Proposal<Block, PR::Proof>, pezsp_blockchain::Error> {
