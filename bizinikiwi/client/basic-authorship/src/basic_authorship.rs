...
// FIXME #1021 move this into pezsp-consensus
...
use pezsp_consensus::{Proposal, ProposeArgs};
...
impl<A, Block, C> pezsp_consensus::Environment<Block> for ProposerFactory<A, C>
where
	A: TransactionPool<Block = Block> + 'static,
	Block: BlockT,
...
impl<A, Block, C> pezsp_consensus::Proposer<Block> for Proposer<Block, C, A>
where
	A: TransactionPool<Block = Block> + 'static,
	Block: BlockT,
	C: HeaderBackend<Block> + ProvideRuntimeApi<Block> + CallApiAt<Block> + Send + Sync + 'static,
	...
	C::Api: ApiExt<Block> + BlockBuilderApi<Block>,
{
	...
	) -> Result<Proposal<Block>, pezsp_blockchain::Error> {
	...
	) -> Result<EndProposingReason, pezsp_blockchain::Error> {
