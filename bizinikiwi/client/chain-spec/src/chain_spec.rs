	...
	fn assimilate_storage(&self, storage: &mut Storage) -> Result<(), String> {
		match self.genesis.resolve()? {
			...
			Genesis::RuntimeGenesis(RuntimeGenesisInner {
				...
				storage
					.top
					.insert(pezsp_core::storage::well_known_keys::CODE.to_vec(), code.clone());
			},
			Genesis::RuntimeGenesis(RuntimeGenesisInner {
...
impl From<pezsp_core::storage::Storage> for RawGenesis {
	fn from(value: pezsp_core::storage::Storage) -> Self {
		Self {
	...
	fn json_container(&self, raw: bool) -> Result<ChainSpecJsonContainer<E>, String> {
		let raw_genesis = match (raw, self.genesis.resolve()?) {
			(
				...
				storage.top.insert(pezsp_core::storage::well_known_keys::CODE.to_vec(), code);
...
pub fn update_code_in_json_chain_spec(chain_spec: &mut json::Value, code: &[u8]) -> bool {
	...
	if json_contains_path(&chain_spec, &mut path) {
		...
		#[derive(Serialize)]
		struct Container<'a> {
			#[serde(with = "pezsp_core::bytes", rename = "0x3a636f6465")]
			code: &'a [u8],
		}
...
pub fn set_code_substitute_in_json_chain_spec(
	...
	let substitutes = json::json!({"codeSubstitutes":{ &block_height.to_string(): pezsp_core::bytes::to_hex(code, false) }});
...
mod tests {
	...
	fn build_chain_spec_with_patch_works() {
		let output = ChainSpec::<()>::builder(
			bizinikiwi_test_runtime::wasm_binary_unwrap().into(),
			Default::default(),
		)
		.with_name("TestName")
		.with_id("test_id")
	...
	fn generate_chain_spec_with_patch_works() {
		...
		let expected =
			from_str::<Value>(include_str!("../res/bizinikiwi_test_runtime_from_patch.json"))
				.unwrap();
		let expected_raw =
			from_str::<Value>(include_str!("../res/bizinikiwi_test_runtime_from_patch_raw.json"))
				.unwrap();
	...
	fn generate_chain_spec_with_full_config_works() {
		...
		let expected =
			from_str::<Value>(include_str!("../res/bizinikiwi_test_runtime_from_config.json"))
				.unwrap();
		let expected_raw =
			from_str::<Value>(include_str!("../res/bizinikiwi_test_runtime_from_config_raw.json"))
				.unwrap();
	...
	fn chain_spec_as_json_fails_with_invalid_config() {
		...
		let re = regex::Regex::new(concat!(
			...
			r"`bizinikiwiTest`, `balances` at line \d+ column \d+ for blob:$"
		))
		.unwrap();
	...
	fn chain_spec_as_json_fails_with_invalid_patch() {
		...
		assert!(output.as_json(true).unwrap_err().contains("Invalid JSON blob: unknown field `invalid_pallet`, expected one of `system`, `babe`, `bizinikiwiTest`, `balances`"));
	}
