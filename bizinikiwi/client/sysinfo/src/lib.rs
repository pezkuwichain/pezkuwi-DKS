...
pub fn print_sysinfo(sysinfo: &pezsc_telemetry::SysInfo) {
...
pub fn initialize_hwbench_telemetry(
	telemetry_handle: pezsc_telemetry::TelemetryHandle,
	hwbench: HwBench,
	...
	async move {
		...
		while connect_stream.next().await.is_some() {
			telemetry_handle.send_telemetry(pezsc_telemetry::BIZINIKIWI_INFO, payload.clone());
		}
	}
}
