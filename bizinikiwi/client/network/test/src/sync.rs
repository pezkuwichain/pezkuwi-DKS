...
async fn syncs_state() {
	...
	for skip_proofs in &[false, true] {
		let mut net = TestNet::new(0);
		let mut genesis_storage: pezsp_core::storage::Storage = Default::default();
		...
		let child1 = pezsp_core::storage::StorageChild {
			data: child_data.clone(),
			child_info: pezsp_core::storage::ChildInfo::new_default(b"child1"),
		};
		let child3 = pezsp_core::storage::StorageChild {
			data: child_data.clone(),
			child_info: pezsp_core::storage::ChildInfo::new_default(b"child3"),
		};
		for i in 22u8..33 {
			child_data.insert(vec![i; 5], vec![i; 33]);
		}
		let child2 = pezsp_core::storage::StorageChild {
			data: child_data.clone(),
			child_info: pezsp_core::storage::ChildInfo::new_default(b"child2"),
		};
		genesis_storage
			.children_default
...
async fn syncs_indexed_blocks() {
	...
	let indexed_key = pezsp_runtime::traits::BlakeTwo256::hash(&42u64.to_le_bytes());
	assert!(net
		.peer(0)
		.client()
		.as_client()
...
async fn syncs_blocks_with_large_headers() {
	use pezsc_consensus::ForkChoiceStrategy;
	use pezsp_runtime::{
