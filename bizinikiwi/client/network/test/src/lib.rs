...
use pezsc_network_types::{build_multiaddr, multiaddr::Multiaddr};
...
pub type PeersFullClient = Client<
	...
	Block,
	bizinikiwi_test_runtime_client::runtime::RuntimeApi,
>;

#[derive(Clone)]
pub struct PeersClient {
	client: Arc<PeersFullClient>,
	backend: Arc<bizinikiwi_test_runtime_client::Backend>,
}
...
impl PeersClient {
	...
	pub fn as_backend(&self) -> Arc<bizinikiwi_test_runtime_client::Backend> {
		self.backend.clone()
	}
...
pub struct Peer<D, BlockImport> {
	...
	select_chain: Option<LongestChain<bizinikiwi_test_runtime_client::Backend, Block>>,
	backend: Option<Arc<bizinikiwi_test_runtime_client::Backend>>,
	...
	B: BlockImport<Block, Error = ConsensusError> + Send + Sync,
{
	...
	pub fn select_chain(
		&self,
	) -> Option<LongestChain<bizinikiwi_test_runtime_client::Backend, Block>> {
...
pub struct FullPeerConfig {
	...
	pub extra_storage: Option<pezsp_core::storage::Storage>,
...
pub trait TestNetFactory: Default + Sized + Send {
	...
	fn add_full_peer_with_config(&mut self, config: FullPeerConfig) {
		...
		let metrics = <NetworkWorker<_, _> as pezsc_network::NetworkBackend<
			Block,
		...
		let (engine, sync_service, block_announce_config) =
			pezsc_network_sync::engine::SyncingEngine::new(
		...
		let network = NetworkWorker::new(pezsc_network::config::Params {
