...
mod tests {
	...
	#[derive(Clone)]
	struct TestNetwork {
		reported_peers: Arc<Mutex<Vec<(PeerId, pezsc_network::ReputationChange)>>>,
		disconnected_peers: Arc<Mutex<Vec<PeerId>>>,
		/// Role to return from `peer_role`. Default: `Full`.
		default_role: pezsc_network::ObservedRole,
		added_reserved: Arc<Mutex<Vec<HashSet<pezsc_network::Multiaddr>>>>,
	...
	impl TestNetwork {
		fn new() -> Self {
			Self {
				...
				default_role: pezsc_network::ObservedRole::Full,
		...
		fn new_light() -> Self {
			Self {
				reported_peers: Arc::new(Mutex::new(Vec::new())),
				disconnected_peers: Arc::new(Mutex::new(Vec::new())),
				default_role: pezsc_network::ObservedRole::Light,
				added_reserved: Arc::new(Mutex::new(Vec::new())),
				removed_reserved: Arc::new(Mutex::new(Vec::new())),
			}
		}

		fn get_reports(&self) -> Vec<(PeerId, pezsc_network::ReputationChange)> {
		...
		fn get_added_reserved(&self) -> Vec<HashSet<pezsc_network::Multiaddr>> {
	...
	impl NetworkPeers for TestNetwork {
		...
		fn report_peer(&self, peer_id: PeerId, cost_benefit: pezsc_network::ReputationChange) {
		...
		fn disconnect_peer(&self, peer: PeerId, _: pezsc_network::ProtocolName) {
		...
		fn add_peers_to_reserved_set(
			&self,
			_: pezsc_network::ProtocolName,
			addrs: std::collections::HashSet<pezsc_network::Multiaddr>,
		) -> Result<(), String> {
	...
	impl SyncEventStream for TestSync {
		fn event_stream(
			&self,
			_name: &'static str,
		) -> Pin<Box<dyn Stream<Item = pezsc_network_sync::types::SyncEvent> + Send>> {
			Box::pin(futures::stream::pending())
		}
	}

	impl pezsp_consensus::SyncOracle for TestSync {
	...
	impl NetworkEventStream for TestNetwork {
		fn event_stream(
			&self,
			_name: &'static str,
		) -> Pin<Box<dyn Stream<Item = pezsc_network::Event> + Send>> {
			unimplemented!()
		}
	}
	...
	impl NotificationService for TestNotificationService {
		...
		fn protocol(&self) -> &pezsc_network::types::ProtocolName {
			unimplemented!()
		}
		...
		fn message_sink(
			&self,
			_peer: &PeerId,
		) -> Option<Box<dyn pezsc_network::service::traits::MessageSink>> {
			unimplemented!()
		}
	}
	...
	impl StatementStore for TestStatementStore {
		fn statements(
			&self,
		) -> pezsp_statement_store::Result<
			Vec<(pezsp_statement_store::Hash, pezsp_statement_store::Statement)>,
		> {
		...
		fn statement(
			&self,
			_hash: &pezsp_statement_store::Hash,
		) -> pezsp_statement_store::Result<Option<pezsp_statement_store::Statement>> {
			unimplemented!()
		}

		fn has_statement(&self, hash: &pezsp_statement_store::Hash) -> bool {
			self.statements.lock().unwrap().contains_key(hash)
		}

		fn statement_hashes(&self) -> Vec<pezsp_statement_store::Hash> {
			self.statements.lock().unwrap().keys().cloned().collect()
		}

		fn statements_by_hashes(
			&self,
			hashes: &[pezsp_statement_store::Hash],
			filter: &mut dyn FnMut(
				&pezsp_statement_store::Hash,
				&[u8],
				&pezsp_statement_store::Statement,
			) -> FilterDecision,
		) -> pezsp_statement_store::Result<(
			Vec<(pezsp_statement_store::Hash, pezsp_statement_store::Statement)>,
			usize,
		...
		fn broadcasts(
			&self,
			_match_all_topics: &[pezsp_statement_store::Topic],
		) -> pezsp_statement_store::Result<Vec<Vec<u8>>> {
			unimplemented!()
		}
		...
		fn submit(
			...
			_source: pezsp_statement_store::StatementSource,
		) -> pezsp_statement_store::SubmitResult {
			unimplemented!()
		}

		fn remove(&self, _hash: &pezsp_statement_store::Hash) -> pezsp_statement_store::Result<()> {
			unimplemented!()
		}

		fn remove_by(&self, _who: [u8; 32]) -> pezsp_statement_store::Result<()> {
			unimplemented!()
		}
	}
	...
	fn build_handler(
		...
		let handler = StatementHandler {
			...
			sync_event_stream: (Box::pin(futures::stream::pending())
				as Pin<Box<dyn Stream<Item = pezsc_network_sync::types::SyncEvent> + Send>>)
				.fuse(),
			peers,
	...
	fn get_peer_hashes(
		sent: &[(PeerId, Vec<u8>)],
		peer: PeerId,
	) -> Vec<pezsp_statement_store::Hash> {
		sent.iter()
			.filter(|(p, _)| *p == peer)
	...
	async fn test_initial_sync_burst_single_peer() {
		...
		handler
			.handle_notification_event(NotificationEvent::NotificationStreamOpened {
				peer: peer_id,
				direction: pezsc_network::service::traits::Direction::Inbound,
				handshake: vec![],
	...
	fn deferred_peers_flushed_on_sync_end_without_remove() {
		...
		let expected_addr1: pezsc_network::Multiaddr =
			iter::once(multiaddr::Protocol::P2p(peer1.into())).collect();
		let expected_addr2: pezsc_network::Multiaddr =
	...
	async fn sync_recovery_gated_by_dropped_statements_flag() {
		...
		let make_handler = |network: TestNetwork,
		                    dropped: bool|
		 -> StatementHandler<TestNetwork, TestSync> {
