...
fn is_descendent_of<Block, T>(
	...
	base: &Block::Hash,
	block: &Block::Hash,
) -> pezsp_blockchain::Result<bool>
where
	Block: BlockT,
	T: HeaderMetadata<Block, Error = pezsp_blockchain::Error> + ?Sized,
{
	if base == block {
		return Ok(false);
	}

	let ancestor = pezsp_blockchain::lowest_common_ancestor(client, *block, *base)?;
