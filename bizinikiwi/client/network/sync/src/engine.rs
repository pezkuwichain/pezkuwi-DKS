...
//! `SyncingEngine` is the actor responsible for syncing Bizinikiwi chain
...
use pezsc_client_api::{BlockBackend, HeaderBackend, ProofProvider};
...
use pezsp_blockchain::{Error as ClientError, HeaderMetadata};
use pezsp_consensus::{block_validation::BlockAnnounceValidator, BlockOrigin};
use pezsp_runtime::{
	...
	fn process_service_command(&mut self, command: ToServiceCommand<B>) {
		match command {
			...
			ToServiceCommand::JustificationImported(peer_id, hash, number, import_result) => {
				let success =
					matches!(import_result, pezsc_consensus::JustificationImportResult::Success);
				self.strategy.on_justification_import(hash, number, success);

				match import_result {
					pezsc_consensus::JustificationImportResult::OutdatedJustification => {
						log::info!(
							target: LOG_TARGET,
							"💔 Outdated justification provided by {peer_id} for #{hash}",
						);
					},
					pezsc_consensus::JustificationImportResult::Failure => {
						log::info!(
							target: LOG_TARGET,
					...
					pezsc_consensus::JustificationImportResult::Success => {
						log::debug!(
							target: LOG_TARGET,
