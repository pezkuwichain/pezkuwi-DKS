...
fn hash_for_multihash_code(multihash_code: u64, data: &[u8]) -> Option<[u8; 32]> {
	match multihash_code {
		BLAKE2B_256_MULTIHASH_CODE => Some(pezsp_crypto_hashing::blake2_256(data)),
		SHA2_256_MULTIHASH_CODE => Some(pezsp_crypto_hashing::sha2_256(data)),
		KECCAK_256_MULTIHASH_CODE => Some(pezsp_crypto_hashing::keccak_256(data)),
		_ => None,
	}
}
...
mod tests {
	...
	async fn request_bitswap_blocks_unverified_accepts_bytes_without_hash_recompute() {
		...
		let cid = cid_for_digest(BLAKE2B_256_MULTIHASH_CODE, pezsp_crypto_hashing::sha2_256(&data));
	...
	async fn request_bitswap_blocks_unverified_dont_have_returned_as_missing() {
		let cid = cid_for_digest(
			...
			pezsp_crypto_hashing::sha2_256(b"pruned-unverified-payload"),
		);
	...
	async fn request_bitswap_blocks_unverified_multi_want_all_served_in_request_order() {
		...
		let cid_a =
			cid_for_digest(BLAKE2B_256_MULTIHASH_CODE, pezsp_crypto_hashing::sha2_256(&data_a));
		let cid_b =
			cid_for_digest(BLAKE2B_256_MULTIHASH_CODE, pezsp_crypto_hashing::keccak_256(&data_b));
		let cid_c = cid_for_data(BLAKE2B_256_MULTIHASH_CODE, &data_c);
	...
	async fn request_bitswap_blocks_unverified_dont_have_skips_position_in_payload_order() {
		...
		let block_cid =
			cid_for_digest(BLAKE2B_256_MULTIHASH_CODE, pezsp_crypto_hashing::sha2_256(&data));
		let response = encode_response(
