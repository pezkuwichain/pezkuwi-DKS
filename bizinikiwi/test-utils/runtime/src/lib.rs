...
pub type Address = pezsp_core::sr25519::Public;
pub type Signature = sr25519::Signature;
#[cfg(feature = "std")]
pub type Pair = pezsp_core::sr25519::Pair;
...
pub type SignedPayload = pezsp_runtime::generic::SignedPayload<RuntimeCall, TxExtension>;
...
pub type Digest = pezsp_runtime::generic::Digest;
/// A test block.
pub type Block = pezsp_runtime::generic::Block<Header, Extrinsic>;
/// A test block's header.
pub type Header = pezsp_runtime::generic::Header<BlockNumber, Hashing>;
...
impl pezsp_runtime::traits::Printable for CheckBizinikiwiCall {
	fn print(&self) {
		"CheckBizinikiwiCall".print()
	}
}

impl pezsp_runtime::traits::RefundWeight for CheckBizinikiwiCall {
	fn refund(&mut self, _weight: pezframe_support::weights::Weight) {}
}
impl pezsp_runtime::traits::ExtensionPostDispatchWeightHandler<CheckBizinikiwiCall>
	for CheckBizinikiwiCall
{
	fn set_extension_weight(&mut self, _info: &CheckBizinikiwiCall) {}
}

impl pezsp_runtime::traits::Dispatchable for CheckBizinikiwiCall {
	type RuntimeOrigin = RuntimeOrigin;
	type Config = CheckBizinikiwiCall;
	type Info = CheckBizinikiwiCall;
	type PostInfo = CheckBizinikiwiCall;
...
impl pezsp_runtime::traits::TransactionExtension<RuntimeCall> for CheckBizinikiwiCall {
	const IDENTIFIER: &'static str = "CheckBizinikiwiCall";
	type Implicit = ();
	type Pre = ();
	type Val = ();
...
impl bizinikiwi_test_pallet::Config for Runtime {}

// Required for `pezpallet_babe::Config`.
...
impl pezpallet_babe::Config for Runtime {
	...
	type EpochChangeTrigger = pezpallet_babe::SameAuthoritiesForever;
...
impl_runtime_apis! {
	...
	impl self::TestAPI<Block> for Runtime {
		...
		fn verify_ed25519(sig: ed25519::Signature, public: ed25519::Public, message: Vec<u8>) -> bool {
			pezsp_io::crypto::ed25519_verify(&sig, &message, &public)
		}
	...
	impl pezsp_consensus_aura::AuraApi<Block, AuraId> for Runtime {
		fn slot_duration() -> pezsp_consensus_aura::SlotDuration {
			pezsp_consensus_aura::SlotDuration::from_millis(1000)
		}

		fn authorities() -> Vec<AuraId> {
			BizinikiwiTest::authorities().into_iter().map(|auth| AuraId::from(auth)).collect()
		}
	}
	...
	impl pezsp_consensus_babe::BabeApi<Block> for Runtime {
		...
		fn submit_report_equivocation_unsigned_extrinsic(
			_equivocation_proof: pezsp_consensus_babe::EquivocationProof<
	...
	impl pezsp_offchain::OffchainWorkerApi<Block> for Runtime {
		fn offchain_worker(header: &<Block as BlockT>::Header) {
			...
			pezsp_io::offchain::submit_transaction(ext.encode()).unwrap();
	...
	impl pezsp_consensus_grandpa::GrandpaApi<Block> for Runtime {
		...
		fn generate_key_ownership_proof(
			...
			_authority_id: pezsp_consensus_grandpa::AuthorityId,
...
fn test_read_storage() {
	...
	pezsp_io::storage::set(KEY, b"test");

	let mut v = [0u8; 4];
	let r = pezsp_io::storage::read(KEY, &mut v, 0);
	assert_eq!(r, Some(4));
	assert_eq!(&v, b"test");

	let mut v = [0u8; 4];
	let r = pezsp_io::storage::read(KEY, &mut v, 4);
	assert_eq!(r, Some(0));
	assert_eq!(&v, &[0, 0, 0, 0]);
}
...
fn test_read_child_storage() {
	...
	pezsp_io::default_child_storage::set(STORAGE_KEY, KEY, b"test");

	let mut v = [0u8; 4];
	let r = pezsp_io::default_child_storage::read(STORAGE_KEY, KEY, &mut v, 0);
	assert_eq!(r, Some(4));
	assert_eq!(&v, b"test");

	let mut v = [0u8; 4];
	let r = pezsp_io::default_child_storage::read(STORAGE_KEY, KEY, &mut v, 8);
	assert_eq!(r, Some(0));
	assert_eq!(&v, &[0, 0, 0, 0]);
}
...
pub mod storage_key_generator {
	...
	fn concat_hashes(input: &Vec<&[u8]>) -> String {
		input.iter().map(|s| pezsp_crypto_hashing::twox_128(s)).map(hex).collect()
	}

	fn twox_64_concat(x: &[u8]) -> Vec<u8> {
		pezsp_crypto_hashing::twox_64(x).iter().chain(x.iter()).cloned().collect()
	}

	/// Generate the hashed storage keys from the raw literals. These keys are expected to be in
	/// storage with given bizinikiwi-test runtime.
	pub fn generate_expected_storage_hashed_keys(custom_heap_pages: bool) -> Vec<String> {
		...
		let keys: Vec<Vec<&[u8]>> = vec![
			...
			vec![b"BizinikiwiTest", b":__STORAGE_VERSION__:"],
			vec![b"BizinikiwiTest", b"Authorities"],
		...
		let balances_map_keys = (0..16_usize)
			...
			.map(|pubkey| {
				pezsp_crypto_hashing::blake2_128(&pubkey)
					.iter()
					.chain(pubkey.iter())
					.cloned()
					.collect::<Vec<u8>>()
	...
	pub fn get_expected_storage_hashed_keys(custom_heap_pages: bool) -> Vec<&'static str> {
		let mut res = vec![
			//BizinikiwiTest|:__STORAGE_VERSION__:
			"d98085a84eac4903081fb752c3d8ecde4e7b9012096b41c4eb3aaf947f6ea429",
			//BizinikiwiTest|Authorities
			"d98085a84eac4903081fb752c3d8ecde5e0621c4869aa60c02be9adcc98a0d1d",
...
mod tests {
	...
	use pezsp_core::{storage::well_known_keys::HEAP_PAGES, traits::CallContext};
	use pezsp_runtime::{
	...
	fn witness_backend() -> (pezsp_trie::MemoryDB<crate::Hashing>, crate::Hash) {
		let mut root = crate::Hash::default();
		let mut mdb = pezsp_trie::MemoryDB::<crate::Hashing>::default();
		{
			let mut trie =
				pezsp_trie::trie_types::TrieDBMutBuilderV1::new(&mut mdb, &mut root).build();
	...
	fn witness_backend_works() {
		...
		let backend =
			pezsp_state_machine::TrieBackendBuilder::<_, crate::Hashing>::new(db, root).build();
		let proof = pezsp_state_machine::prove_read(backend, vec![b"value3"]).unwrap();
		let client = TestClientBuilder::new().build();
		let runtime_api = client.runtime_api();
		let best_hash = client.chain_info().best_hash;
	...
	fn validate_unsigned_works() {
		...
		new_test_ext().execute_with(|| {
			let failing_calls = vec![
				bizinikiwi_test_pallet::Call::bench_call { transfer: Default::default() },
				bizinikiwi_test_pallet::Call::include_data { data: vec![] },
				bizinikiwi_test_pallet::Call::fill_block { ratio: Perbill::from_percent(50) },
			];
			let succeeding_calls = vec![
				bizinikiwi_test_pallet::Call::deposit_log_digest_item {
					log: DigestItem::Other(vec![]),
				},
				bizinikiwi_test_pallet::Call::storage_change { key: vec![], value: None },
				bizinikiwi_test_pallet::Call::read { count: 0 },
				bizinikiwi_test_pallet::Call::read_and_panic { count: 0 },
			];

			for call in failing_calls {
				assert_eq!(
					<BizinikiwiTest as pezsp_runtime::traits::ValidateUnsigned>::validate_unsigned(
	...
	#[test]
	fn check_bizinikiwi_check_signed_extension_works() {
		pezsp_tracing::try_init_simple();
		new_test_ext().execute_with(|| {
	...
	mod genesis_builder_tests {
		...
		fn build_minimal_genesis_config_works() {
			pezsp_tracing::try_init_simple();
			let default_minimal_json = r#"{"system":{},"babe":{"authorities":[],"epochConfig":{"c": [ 3, 10 ],"allowed_slots":"PrimaryAndSecondaryPlainSlots"}},"bizinikiwiTest":{"authorities":[]},"balances":{"balances":[]}}"#;
		...
		fn default_config_as_json_works() {
			...
			let expected = r#"{"system":{},"babe":{"authorities":[],"epochConfig":{"c":[1,4],"allowed_slots":"PrimaryAndSecondaryVRFSlots"}},"bizinikiwiTest":{"authorities":[]},"balances":{"balances":[],"devAccounts":null}}"#;
		...
		fn named_config_works() {
			...
			f(
				"staging",
				r#"{"balances":{"balances":[["5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty",1000000000000000],["5FLSigC9HGRKVhB9FiEo4Y3koPsNmBmLJbpXg2mp1hXcS59Y",1000000000000000]]},"bizinikiwiTest":{"authorities":["5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY","5CiPPseXPECbkjWCa6MnjNokrgYjMqmKndv2rSnekmSK2DjL"]}}"#,
			);
		}
		...
		fn build_config_from_invalid_json_fails_2() {
			...
			let r = BuildResult::decode(&mut &r[..]).unwrap();
			assert_eq!(r, Err(
				"Invalid JSON blob: unknown field `babex`, expected one of `system`, `babe`, `bizinikiwiTest`, `balances` at line 3 column 9".to_string(),
			));
		}
		...
		fn build_genesis_config_with_patch_json_works() {
			...
			pezsc_chain_spec::json_merge(&mut default_config, patch);
			...
			let authority_key_vec =
				Vec::<pezsp_core::sr25519::Public>::decode(&mut &value[..]).unwrap();
