...
use pezpallet_collective::{EnsureProportionAtLeast, Instance1};
use pezsp_runtime::{
...
pezframe_support::construct_runtime!(
	pub enum Test
	{
		...
		RootTesting: pezpallet_root_testing,
...
impl pezpallet_root_testing::Config for Test {
...
impl pezpallet_collective::Config<CouncilCollective> for Test {
	...
	type WeightInfo = ();
	type SetMembersOrigin = pezframe_system::EnsureRoot<Self::AccountId>;
...
impl Contains<RuntimeCall> for TestBaseCallFilter {
	fn contains(c: &RuntimeCall) -> bool {
		match *c {
			...
			RuntimeCall::Balances(pezpallet_balances::Call::transfer_allow_death { .. }) => true,
			RuntimeCall::Utility(_) => true,
			// For benchmarking, this acts as a noop call
			RuntimeCall::System(pezframe_system::Call::remark { .. }) => true,
			// For tests
...
use pezpallet_root_testing::Call as RootTestingCall;
use pezpallet_timestamp::Call as TimestampCall;
...
pub fn new_test_ext() -> pezsp_io::TestExternalities {
	...
	pezpallet_collective::GenesisConfig::<Test, Instance1> {
		members: vec![1, 2, 3],
	...
	new_test_ext().execute_with(|| {
		assert_err_ignore_postinfo!(
			Utility::as_derivative(
				RuntimeOrigin::signed(1),
				1,
				Box::new(RuntimeCall::Balances(pezpallet_balances::Call::transfer_keep_alive {
					dest: 2,
					value: 1
				})),
			),
...
fn batch_with_root_works() {
	new_test_ext().execute_with(|| {
		let k = b"a".to_vec();
		let call = RuntimeCall::System(pezframe_system::Call::set_storage {
...
fn batch_with_signed_filters() {
	new_test_ext().execute_with(|| {
		assert_ok!(Utility::batch(
			RuntimeOrigin::signed(1),
			vec![RuntimeCall::Balances(pezpallet_balances::Call::transfer_keep_alive {
				dest: 2,
				value: 1
			})]
		),);
...
fn empty_batch_is_normal_dispatch_class() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			pezframe_support::dispatch::DispatchClass::Normal
		);
...
fn batch_works_with_council_origin() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Council(pezpallet_collective::Event::Executed {
			proposal_hash: hash,
			result: Ok(()),
		}));
	})
...
fn batch_all_works_with_council_origin() {
	new_test_ext().execute_with(|| {
		assert_ok!(Utility::batch_all(
			RuntimeOrigin::from(pezpallet_collective::RawOrigin::Members(3, 3)),
...
fn with_weight_works() {
	new_test_ext().execute_with(|| {
		use pezframe_system::WeightInfo;
		let upgrade_code_call =
			Box::new(RuntimeCall::System(pezframe_system::Call::set_code_without_checks {
				code: vec![],
			}));
		// Weight before is max.
		assert_eq!(
			upgrade_code_call.get_dispatch_info().call_weight,
			<Test as pezframe_system::Config>::SystemWeightInfo::set_code()
		);
		assert_eq!(
			upgrade_code_call.get_dispatch_info().class,
			pezframe_support::dispatch::DispatchClass::Operational
		);
...
fn dispatch_as_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Utility::dispatch_as(
			RuntimeOrigin::root(),
			Box::new(OriginCaller::system(pezframe_system::RawOrigin::Signed(666))),
		...
		assert_ok!(Utility::dispatch_as(
			RuntimeOrigin::root(),
			Box::new(OriginCaller::system(pezframe_system::RawOrigin::Signed(777))),
