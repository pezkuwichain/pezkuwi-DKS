...
fn check_modules_error_type() {
	pezsp_io::TestExternalities::default().execute_with(|| {
		assert_eq!(
			Module1_1::fail(pezframe_system::Origin::<Runtime>::Root.into()),
		...
		assert_eq!(
			Module2::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 32,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_2::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 33,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			NestedModule3::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 34,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_3::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 6,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_4::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 3,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_5::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 4,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_6::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 1,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_7::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 2,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_8::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 12,
				error: [0; 4],
				message: Some("Something")
		...
		assert_eq!(
			Module1_9::fail(pezframe_system::Origin::<Runtime>::Root.into()),
			Err(DispatchError::Module(ModuleError {
				index: 13,
				error: [0; 4],
				message: Some("Something")
...
fn origin_codec() {
	...
	let origin = OriginCaller::system(pezframe_system::RawOrigin::None);
...
fn event_codec() {
	...
	let event = pezframe_system::Event::<Runtime>::ExtrinsicSuccess {
...
#[test]
fn call_name() {
	use pezframe_support::traits::GetCallName;
...
#[test]
fn call_metadata() {
	use pezframe_support::traits::{CallMetadata, GetCallMetadata};
	let call = RuntimeCall::Module3(module3::Call::<Runtime>::aux_4 {});
	let metadata = call.get_call_metadata();
	let expected = CallMetadata { function_name: "aux_4".into(), pezpallet_name: "Module3".into() };
...
#[test]
fn get_module_names() {
	use pezframe_support::traits::GetCallMetadata;
...
fn test_metadata() {
	...
	let pallets = vec![
		...
		PalletMetadata {
			name: "System",
			storage: None,
			calls: Some(meta_type::<pezframe_system::Call<Runtime>>().into()),
			event: Some(meta_type::<pezframe_system::Event<Runtime>>().into()),
			constants: vec![
			...
			error: Some(meta_type::<pezframe_system::Error<Runtime>>().into()),
			index: 30,
		},
		PalletMetadata {
			name: "Module1_1",
...
#[test]
fn pezpallet_in_runtime_is_correct() {
	assert_eq!(PalletInfo::index::<System>().unwrap(), 30);
	assert_eq!(PalletInfo::name::<System>().unwrap(), "System");
	assert_eq!(PalletInfo::module_name::<System>().unwrap(), "pezframe_system");
