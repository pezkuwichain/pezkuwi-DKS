...
>;
pub type UncheckedSignaturePayload = pezsp_runtime::generic::UncheckedSignaturePayload<
	u64,
	UintAuthorityId,
	pezframe_system::CheckNonZeroSender<Runtime>,
>;
...
fn call_expand() {
	...
	assert_eq!(
		...
		DispatchInfo {
			call_weight: pezframe_support::weights::Weight::from_parts(3, 0),
...
fn metadata_v15() {
	...
	let pallets = vec![
		...
		PalletMetadata {
			...
			constants: vec![],
			error: None,
			docs: vec![" Test that the supertrait check works when we pass some parameter to the `pezframe_system::Config`."],
		},
		PalletMetadata {
			index: 4,
			name: "Example4",
	...
	let extrinsic = ExtrinsicMetadata {
		...
		address_ty: meta_type::<
			<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Address
		>(),
		call_ty: meta_type::<<Runtime as CreateTransactionBase<RuntimeCall>>::RuntimeCall>(),
		signature_ty: meta_type::<
			<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Signature
		>(),
		extra_ty: meta_type::<
			<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Extension
		>(),
	};
	...
	// The scale-info type-id numbers are an internal registry detail assigned by the order in
	// which types are registered while the metadata is built; metadata consumers resolve types
	// by id *within the same blob*, so the absolute numbers are not part of the metadata's
	// external contract. Our runtime metadata is generated by the macro while the `expected`
	// here is hand-built, and the two register a couple of types (e.g. `pallet2::Event`,
	// `pallet4::Call`) at different registry positions, so the raw ids differ even though the
	// described types are identical. Compare each pallet's type references resolved to their
	// type *paths* instead: this verifies the real per-pallet type structure (names, storage
	// entries, calls, events, errors, constants, docs) without coupling the test to the
	// internal id numbering.
	fn describe(
		pallets: &[PalletMetadata<scale_info::form::PortableForm>],
		reg: &scale_info::PortableRegistry,
	) -> Vec<String> {
		// Resolve a type reference to an id-agnostic descriptor: named types (with a path) are
		// described by their path; path-less types (primitives, tuples, arrays, sequences,
		// compact, and anonymous composites/variants) are described by their *structure*, with
		// every nested reference resolved the same way. Recursion terminates because it only
		// continues through path-less types, which bottom out at primitives (named types stop
		// at their path); the depth guard is a belt-and-braces backstop.
		fn resolve_ty(reg: &scale_info::PortableRegistry, id: u32, depth: u8) -> String {
			let Some(t) = reg.resolve(id) else { return format!("?{id}") };
			if !t.path.segments.is_empty() {
				return t.path.segments.join("::");
			}
			if depth == 0 {
				return "…".to_string();
			}
			let d = depth - 1;
			match &t.type_def {
				scale_info::TypeDef::Primitive(p) => format!("{p:?}"),
				scale_info::TypeDef::Tuple(x) => format!(
					"({})",
				...
				scale_info::TypeDef::Composite(c) => format!(
					...
					c.fields
						...
						.map(|f| format!(
							...
							resolve_ty(reg, f.ty.id, d)
						))
						.collect::<Vec<_>>()
						.join(",")
				),
				scale_info::TypeDef::Variant(v) => format!(
					"enum[{}]",
					v.variants
						.iter()
						.map(|va| format!(
							...
							va.fields
								.iter()
								.map(|f| resolve_ty(reg, f.ty.id, d))
								.collect::<Vec<_>>()
								.join(",")
						))
						.collect::<Vec<_>>()
				...
				scale_info::TypeDef::BitSequence(_) => "bits".to_string(),
			}
		}
		let tref = |id: u32| resolve_ty(reg, id, 16);
		pallets
			.iter()
			.map(|p| {
				let storage = p
					...
					.map(|s| {
						s.entries
							...
							.map(|e| {
								let ty = match &e.ty {
									StorageEntryType::Plain(t) => format!("Plain({})", tref(t.id)),
									StorageEntryType::Map { hashers, key, value } => format!(
										"Map({:?},{},{})",
										hashers,
										tref(key.id),
										tref(value.id)
				...
				let consts = p
					.constants
					.iter()
					.map(|c| format!("{}|{}|{:?}|{:?}", c.name, tref(c.ty.id), c.value, c.docs))
					.collect::<Vec<_>>();
				format!(
					"pallet={} index={} storage={:?} constants={:?} call={:?} event={:?} error={:?} docs={:?}",
					p.name,
					p.index,
					storage,
					consts,
					p.calls.as_ref().map(|c| tref(c.ty.id)),
					p.event.as_ref().map(|e| tref(e.ty.id)),
					p.error.as_ref().map(|e| tref(e.ty.id)),
					p.docs,
				)
			})
			.collect()
	...
	pretty_assertions::assert_eq!(
		describe(&actual_metadata.pallets, &actual_metadata.types),
		describe(&expected_metadata.pallets, &expected_metadata.types),
	);
}
...
fn extrinsic_metadata_ir_types() {
	...
	assert_eq!(
		meta_type::<<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Address>(),
		ir.address_ty
	);
	...
	assert_eq!(
		meta_type::<<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Signature>(),
		ir.signature_ty
	);
	assert_eq!(meta_type::<UintAuthorityId>(), ir.signature_ty);

	assert_eq!(
		meta_type::<<<<Runtime as pezframe_system::Config>::Block as BlockT>::Extrinsic as SignedTransactionBuilder>::Extension>(),
		ir.extra_ty
	);
	assert_eq!(meta_type::<pezframe_system::CheckNonZeroSender<Runtime>>(), ir.extra_ty);
}
...
fn test_pallet_info_access() {
	...
	assert_eq!(<Example2 as pezframe_support::traits::PalletInfoAccess>::index(), 2);
}
...
fn test_storage_info() {
	...
	assert_eq!(
		...
		vec![
			StorageInfo {
				pezpallet_name: b"Example".to_vec(),
	...
	assert_eq!(
		...
		vec![
			StorageInfo {
				pezpallet_name: b"Example2".to_vec(),
...
#[test]
fn pezpallet_on_chain_storage_version_initializes_correctly() {
	...
	TestExternalities::default().execute_with(|| {
		...
		// [`pezframe_support::traits::BeforeAllRuntimeMigrations`] initializes the storage version.
		Executive::execute_on_runtime_upgrade();
...
fn post_runtime_upgrade_detects_storage_version_issues() {
	use pezframe_support::traits::UpgradeCheckSelect;
	...
	#[allow(deprecated)]
	type ExecutiveWithUpgrade = pezframe_executive::Executive<
		Runtime,
		Block,
		pezframe_system::ChainContext<Runtime>,
		Runtime,
	...
	/// can be safely deleted.
	#[allow(deprecated)]
	type ExecutiveWithUpgradePallet4 = pezframe_executive::Executive<
		Runtime,
		Block,
		pezframe_system::ChainContext<Runtime>,
		Runtime,
