...
pub trait StorageDoubleMap<K1: FullEncode, K2: FullEncode, V: FullCodec> {
	...
	fn remove_prefix<KArg1>(k1: KArg1, limit: Option<u32>) -> pezsp_io::KillStorageResult
	where
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			let maybe_next = if self.fetch_previous_key {
				...
				pezsp_io::default_child_storage::next_key(
...
pub trait StoragePrefixedContainer {
	...
	fn final_prefix() -> [u8; 32] {
		crate::storage::storage_prefix(Self::pezpallet_prefix(), Self::storage_prefix())
	}
}
...
pub trait StoragePrefixedMap<Value: FullCodec> {
	...
	fn remove_all(limit: Option<u32>) -> pezsp_io::KillStorageResult {
	...
	fn clear(limit: u32, maybe_cursor: Option<&[u8]>) -> pezsp_io::MultiRemovalResults {
...
pub trait StorageDecodeLength: private::Sealed + codec::DecodeLength {
	...
	fn decode_len(key: &[u8]) -> Option<usize> {
		...
		let mut data = [0u8; 5];
		let len = pezsp_io::storage::read(key, &mut data, 0)?;
	...
	StorageDoubleMapT: generator::StorageDoubleMap<K1, K2, T>,
{
	...
	>(
		...
		if current < bound {
			...
			pezsp_io::storage::append(&double_map_key, item.encode());
			Ok(())
		} else {
			Err(())
		}
...
pub fn storage_prefix(pezpallet_name: &[u8], storage_name: &[u8]) -> [u8; 32] {
	let pezpallet_hash = pezsp_io::hashing::twox_128(pezpallet_name);
	let storage_hash = pezsp_io::hashing::twox_128(storage_name);
