...
macro_rules! parameter_types {
	...
	(IMPL_STORAGE $name:ident, $type:ty, $value:expr $(, $ty_params:ident)*) => {
		...
		impl< $($ty_params),* > $name< $($ty_params),* > {
			...
			pub fn key() -> [u8; 16] {
				$crate::__private::pezsp_crypto_hashing_proc_macro::twox_128!(b":", $name, b":")
			}
...
///
/// ```
/// pezframe_support::runtime_print!("my value is {}", 3);
/// ```
#[macro_export]
	...
	($($arg:tt)+) => {
		{
			...
			$crate::__private::pezsp_io::misc::print_utf8(msg.as_bytes())
		}
	}
}
