...
impl core::fmt::Display for TryDecodeEntireStorageError {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		write!(
			f,
			"`{}::{}` key `{}` is undecodable",
			&alloc::str::from_utf8(&self.info.pezpallet_name).unwrap_or("<invalid>"),
...
fn decode_storage_info<V: Decode>(
	...
	let decode_key = |key: &[u8]| match pezsp_io::storage::get(key) {
		None => Ok(0),
		Some(bytes) => {
			let len = bytes.len();
	...
	loop {
		match next_key {
			Some(key) if key.starts_with(&info.prefix) => {
				...
				next_key = pezsp_io::storage::next_key(&key);
			},
			_ => break,
		}
	}
...
mod tests {
	...
	fn try_decode_entire_state_value_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			pezsp_io::storage::set(&Value::hashed_key(), &[0u8, 1]);
	...
	fn try_decode_entire_state_map_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			// two bytes, cannot be decoded into u32.
			pezsp_io::storage::set(&Map::hashed_key_for(2), &[0u8, 1]);
			...
			pezsp_io::storage::set(&Map::hashed_key_for(3), &[0u8, 1]);
			assert!(Map::try_decode_entire_state().is_err());
	...
	fn try_decode_entire_state_counted_map_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			pezsp_io::storage::set(&CMap::hashed_key_for(2), &[0u8]);
	...
	fn try_decode_entire_state_double_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			// two bytes, cannot be decoded into u32.
			pezsp_io::storage::set(&DMap::hashed_key_for(1, 1), &[0u8, 1]);
	...
	fn try_decode_entire_state_n_map_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			pezsp_io::storage::set(&NMap::hashed_key_for((1, 1)), &[0u8, 1]);
	...
	fn try_decode_entire_state_counted_n_map_works() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			pezsp_io::TestExternalities::new_empty().execute_with(|| {
				...
				// two bytes, cannot be decoded into u128.
				pezsp_io::storage::set(&CNMap::hashed_key_for((1, 1)), &[0u8, 1]);
	...
	fn extra_bytes_are_rejected() {
		pezsp_io::TestExternalities::new_empty().execute_with(|| {
			...
			pezsp_io::storage::set(&Map::hashed_key_for(2), &[0u8, 1, 3, 4, 5, 6]);
			assert!(Map::try_decode_entire_state().is_err());
		})
	}
