	...
	fn try_state(n: BlockNumber, targets: Select) -> Result<(), TryRuntimeError> {
		match targets {
			...
			Select::RoundRobin(len) => {
				...
				let skip: u32 =
					skip.try_into().unwrap_or_else(|_| pezsp_runtime::traits::Bounded::max_value());
				let mut result = Ok(());
			...
			Select::Only(ref pezpallet_names) => {
				let try_state_fns: &[(
					&'static str,
				...
				let mut result = Ok(());
				pezpallet_names.iter().for_each(|pezpallet_name| {
					if let Some((name, try_state_fn)) =
						try_state_fns.iter().find(|(name, _)| name.as_bytes() == pezpallet_name)
					{
						result = result.and(try_state_fn(n.clone(), targets.clone()));
					} else {
						log::warn!(
