...
pub trait StorageInstance {
	...
	fn storage_prefix_hash() -> [u8; 16] {
		pezsp_io::hashing::twox_128(Self::STORAGE_PREFIX.as_bytes())
	}

	/// Return the prefix hash of instance.
	///
	/// NOTE: This hash must be `twox_128(pezpallet_prefix())++twox_128(STORAGE_PREFIX)`.
	/// Should not impl this function by hand. Only use the default or macro generated impls.
	fn prefix_hash() -> [u8; 32] {
		let mut final_key = [0u8; 32];
		final_key[..16].copy_from_slice(&Self::pezpallet_prefix_hash());
...
where
	Base: Get<Balance>,
	Slope: Get<Balance>,
	Balance: From<u64> + pezsp_runtime::Saturating,
{
...
mod tests {
	...
	use pezsp_core::{ConstU32, ConstU64};
