...
/// (for enactment there's `EstimateNextSessionRotation`). With `pezpallet-session` this should be
...
impl<Key> KeyOwnerProofSystem<Key> for () {
	...
	type Proof = pezsp_core::Void;
	type IdentificationTuple = pezsp_core::Void;
...
/// been registered with them. The [Session module](../../pezpallet_session/index.html) is an
/// implementor.
