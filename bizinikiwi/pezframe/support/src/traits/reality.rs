...
use pezsp_runtime::{traits::Member, BoundedVec, DispatchError, DispatchResult};
...
impl<Params: Encode, RuntimeCall: Decode> Callback<Params, RuntimeCall> {
	pub const fn from_parts(pezpallet_index: u8, call_index: u8) -> Self {
		Self { pezpallet_index, call_index, phantom: PhantomData }
	}
	pub fn curry(&self, args: Params) -> Result<RuntimeCall, codec::Error> {
		(self.pezpallet_index, self.call_index, args).using_encoded(|mut d| Decode::decode(&mut d))
	}
}
