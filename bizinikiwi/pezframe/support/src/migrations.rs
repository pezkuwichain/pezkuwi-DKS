	...
	T::InCodeStorageVersion: StoreInCodeStorageVersion<T>,
{
	fn migrate(db_weight: &RuntimeDbWeight) -> Weight {
		...
		pezsp_io::storage::clear(&pezpallet_version_key(<T as PalletInfoAccess>::name()));
...
///
/// 		SomePalletToRemove: pezpallet_something = 1,
/// 		AnotherPalletToRemove: pezpallet_something_else = 2,
///
/// 		YourOtherPallets...
/// 	}
/// };
	...
	pezframe_support::traits::OnRuntimeUpgrade for RemovePallet<P, DbWeight>
{
	...
	#[cfg(feature = "try-runtime")]
	fn post_upgrade(_state: alloc::vec::Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		use crate::storage::unhashed::contains_prefixed_key;
...
/// 		System: pezframe_system = 0,
///
/// 		SomePallet: pezpallet_something = 1,
///
/// 		YourOtherPallets...
/// 	}
/// };
...
impl<P: Get<&'static str>, S: Get<&'static str>, DbWeight: Get<RuntimeDbWeight>>
	pezframe_support::traits::OnRuntimeUpgrade for RemoveStorage<P, S, DbWeight>
{
	fn on_runtime_upgrade() -> pezframe_support::weights::Weight {
...
pub trait SteppedMigration {
	...
	fn transactional_step(
		...
		with_transaction_opaque_err(move || match Self::step(cursor, meter) {
			Ok(new_cursor) => {
				cursor = new_cursor;
				pezsp_runtime::TransactionOutcome::Commit(Ok(cursor))
			},
			Err(err) => pezsp_runtime::TransactionOutcome::Rollback(Err(err)),
		})
