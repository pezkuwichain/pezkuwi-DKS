...
//! From the explicit state, Bizinikiwi expands the pezpallets with additional information
//! that is to be included in the runtime metadata. This expansion makes visible some extra
//! parts of the pezpallets, mainly the `Error` if defined. The expanded state looks like
...
/// All pezpallets that have implicit pezpallet parts (ie `System: pezframe_system`) are
...
fn construct_runtime_explicit_to_explicit_expanded(
	...
	for pezpallet in definition.pezpallets.iter().filter(|pezpallet| !pezpallet.is_expanded) {
		let pezpallet_path = &pezpallet.path;
		let pezpallet_name = &pezpallet.name;
		let pezpallet_instance =
			pezpallet.instance.as_ref().map(|instance| quote::quote!(::<#instance>));
...
/// All pezpallets have explicit definition of parts, this will expand to the runtime declaration.
fn construct_runtime_final_expansion(
	definition: ExplicitRuntimeDeclaration,
) -> Result<TokenStream2> {
	let ExplicitRuntimeDeclaration { name, pezpallets, pallets_token, where_section } = definition;
	...
	let outer_event =
		expand::expand_outer_enum(&name, &pezpallets, &scrate, expand::OuterEnumType::Event)?;
	let outer_error =
		expand::expand_outer_enum(&name, &pezpallets, &scrate, expand::OuterEnumType::Error)?;
	...
	let query = expand::expand_outer_query(&name, &pezpallets, &scrate);
	...
	let outer_config = expand::expand_outer_config(&name, &pezpallets, &scrate);
	let inherent =
	...
	let freeze_reason = expand::expand_outer_freeze_reason(&pezpallets, &scrate);
	let hold_reason = expand::expand_outer_hold_reason(&pezpallets, &scrate);
	let lock_id = expand::expand_outer_lock_id(&pezpallets, &scrate);
	let slash_reason = expand::expand_outer_slash_reason(&pezpallets, &scrate);
...
pub(crate) fn decl_all_pallets<'a>(
	...
	let all_pallets_without_system = attribute_to_names.iter().map(|(attr, names)| {
		...
		quote! {
			#attr
			/// All pezpallets included in the runtime as a nested tuple of types.
