...
///
/// Docs at `pezframe_support::CloneNoBound`.
...
///
/// Docs at `pezframe_support::DebugNoBound`.
...
///
/// Docs at `pezframe_support::PartialEqNoBound`.
...
///
/// Docs at `pezframe_support::EqNoBound`.
...
/// `pezframe_support::PartialOrdNoBound`.
...
/// Derive [`Ord`] but do no bound any generic. Docs are at `pezframe_support::OrdNoBound`.
...
/// derive `Default` but do no bound any generic. Docs are at `pezframe_support::DefaultNoBound`.
...
/// This macro is meant to be used by pezframe-support only.
...
///
/// For a full end-to-end example, see [below](#use-case-auto-derive-test-pezpallet-config-traits).
///
/// # Usage
///
...
///     type Hash = pezsp_core::hash::H256;
///     type Hashing = pezsp_runtime::traits::BlakeTwo256;
///     type AccountId = AccountId;
///     type Lookup = IdentityLookup<AccountId>;
///     type BlockHashCount = pezframe_support::traits::ConstU64<10>;
...
/// ---
///
/// Documentation for this macro can be found at `pezframe_support::pezpallet_macros::call_index`.
#[proc_macro_attribute]
...
///
/// Documentation for this macro can be found at
/// `pezframe_support::pezpallet_macros::disable_try_decode_storage`.
#[proc_macro_attribute]
...
///
/// Documentation for this macro can be found at
/// `pezframe_support::pezpallet_macros::view_functions`.
#[proc_macro_attribute]
...
/// submit such tasks via an extrinsic defined in `pezframe_system` called `do_task` which accepts
...
/// `pezframe_support::pezpallet_macros::pezpallet_section`.
#[proc_macro_attribute]
pub fn pezpallet_section(attr: TokenStream, tokens: TokenStream) -> TokenStream {
...
///
/// **Rust-Analyzer users**: See the documentation of the Rust item in
/// `pezframe_support::pezpallet_macros::import_section`.
#[import_tokens_attr {
    format!(
        "{}::macro_magic",
        match generate_access_from_frame_or_crate("pezframe-support") {
...
///
/// See the `pezpallet_parameters` for a full example.
///
/// # Arguments
///
...
/// and if valid then dispatched with the origin `pezframe_system::Origin::Authorized`.
///
/// To ensure the origin of the call is the authorization process, the call must check the origin
/// with `pezframe_system::ensure_authorized` function.
///
/// To enable the authorization process on the extrinsic, the runtime must use
/// `pezframe_system::AuthorizeCall` transaction extension in the transaction extension pipeline.
///
...
///             #[block]
///             {
///                 use pezframe_support::pezpallet_prelude::Authorize;
...
/// be done using `pezframe_system::ensure_authorized` function.
///
/// # The macro expansion
///
