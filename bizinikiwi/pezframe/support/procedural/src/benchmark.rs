...
pub fn benchmarks(
	...
	let krate = generate_access_from_frame_or_crate("pezframe-benchmarking")?;
	// gather module info
			...
			*
			...
			where T: #pezframe_system::Config,#where_clause
			{
				...
				fn run_benchmark(
					...
					let whitelisted_caller_key = <#pezframe_system::Account<
						T,
...
fn expand_benchmark(
	...
	let krate = match generate_access_from_frame_or_crate("pezframe-benchmarking") {
		Ok(ident) => ident,
		Err(err) => return err.to_compile_error().into(),
	};
	let pezframe_system = match generate_access_from_frame_or_crate("pezframe-system") {
		Ok(path) => path,
		Err(err) => return err.to_compile_error().into(),
	};
			...
			let origin = match origin {
				Expr::Cast(t) => {
					...
					quote_spanned! { origin.span() =>
						<<T as #pezframe_system::Config>::RuntimeOrigin as From<#ty>>::from(#origin);
					}
				},
				_ => quote_spanned! { origin.span() =>
					Into::<<T as #pezframe_system::Config>::RuntimeOrigin>::into(#origin);
				},
			};
