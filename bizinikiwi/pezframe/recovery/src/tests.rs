...
use pezframe::{prelude::fungible::UnbalancedHold, testing_prelude::*};
...
fn initiate_attempt_different_friend_groups_works() {
	new_test_ext().execute_with(|| {
		...
		pezframe_system::Pezpallet::<T>::set_block_number(10);
...
fn approve_attempt_works() {
	new_test_ext().execute_with(|| {
		...
		pezframe_system::Pezpallet::<T>::set_block_number(3);
		...
		pezframe_system::Pezpallet::<T>::set_block_number(4);
		assert_noop!(
		...
		hypothetically!({
			pezframe_system::Pezpallet::<T>::set_block_number(11);
			assert_ok!(Recovery::finish_attempt(signed(CHARLIE), ALICE, 0));
			assert_eq!(Recovery::inheritor(ALICE), Some(FERDIE));
			assert_eq!(Recovery::inheritance(FERDIE), vec![ALICE]);
		});
		// .. or later
		pezframe_system::Pezpallet::<T>::set_block_number(25);
		assert_ok!(Recovery::finish_attempt(signed(CHARLIE), ALICE, 0));
		assert_eq!(Recovery::inheritor(ALICE), Some(FERDIE));
		assert_eq!(Recovery::inheritance(FERDIE), vec![ALICE]);
	});
...
fn inherit_multiple_accounts_works() {
	new_test_ext().execute_with(|| {
		...
		pezframe_system::Pezpallet::<T>::set_block_number(21);
...
fn control_inherited_account_works() {
	new_test_ext().execute_with(|| {
		...
		let call_hash = call.using_encoded(<T as pezframe_system::Config>::Hashing::hash);
...
fn control_inherited_account_not_inheritor_fails() {
	new_test_ext().execute_with(|| {
		...
		let call: RuntimeCall = pezframe_system::Call::remark { remark: vec![] }.into();
...
fn cancel_attempt_works() {
	new_test_ext().execute_with(|| {
		...
		pezframe_system::Pezpallet::<T>::set_block_number(12);
		// Charlie can never cancel
		assert_noop!(Recovery::cancel_attempt(signed(CHARLIE), ALICE, 0), Error::<T>::NotCanceller);
		// Bob could cancel
		hypothetically!({
...
fn bitfield_already_voted_scenario() {
	// Simulates the approval scenario from the recovery pezpallet
...
fn inheritor_cannot_bypass_filter_via_utility_batch() {
	new_test_ext().execute_with(|| {
		...
		let batch_call: RuntimeCall =
			pezpallet_utility::Call::batch { calls: vec![slash_call] }.into();
		assert_ok!(Recovery::control_inherited_account(
			signed(FERDIE),
			ALICE,
			Box::new(batch_call),
