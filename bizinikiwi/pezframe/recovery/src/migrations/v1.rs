...
//! Multi-block migration from v0 to v1 for the recovery pezpallet.
...
impl<T: v0::MigrationConfig> SteppedMigration for MigrateV0ToV1<T> {
			...
			match cursor {
				MigrationCursor::Recoverable(last_key) => {
					...
					if let Some((account, config)) = iter.next() {
						...
						let footprint = Pezpallet::<T>::friend_group_footprint(&friend_groups);

						match T::FriendGroupsConsideration::new(&account, footprint) {
							Ok(ticket) => {
								pezpallet::FriendGroups::<T>::insert(
									&account,
				...
				MigrationCursor::ActiveRecoveries(last_key) => {
					...
					let Some((friend_groups, _)) = pezpallet::FriendGroups::<T>::get(&lost) else {
						pezframe_support::defensive!(
					...
					let held_deposit = if <T as pezpallet::Config>::Currency::hold(
				...
				MigrationCursor::Proxy(last_key) => {
					...
					let _ = pezframe_system::Pezpallet::<T>::dec_consumers(&rescuer);

					let inheritor = rescuer.clone();
					let inheritance_priority = 0u32;

					// Create inheritor ticket
					let ticket = match Pezpallet::<T>::inheritor_ticket(&inheritor) {
						Ok(ticket) => ticket,
						Err(e) => {
	...
	fn post_upgrade(
		...
		let friend_groups_count = pezpallet::FriendGroups::<T>::iter().count() as u32;
		let attempt_count = pezpallet::Attempt::<T>::iter().count() as u32;
		let inheritor_count = pezpallet::Inheritor::<T>::iter().count() as u32;
...
mod tests {
	...
	fn migration_works() {
		new_test_ext().execute_with(|| {
			...
			pezframe_system::Pezpallet::<T>::inc_consumers(&DAVE).unwrap();
			...
			assert_eq!(pezframe_system::Pezpallet::<T>::consumers(&DAVE), 1);
			...
			assert_eq!(pezpallet::FriendGroups::<T>::iter().count(), 3);

			// Check ALICE's migrated FriendGroups
			let (alice_groups, _ticket) = pezpallet::FriendGroups::<T>::get(ALICE).unwrap();
			...
			let (bob_groups, _ticket) = pezpallet::FriendGroups::<T>::get(BOB).unwrap();
			...
			let (eve_groups, _ticket) = pezpallet::FriendGroups::<T>::get(EVE).unwrap();
			...
			assert_eq!(pezpallet::Inheritor::<T>::iter().count(), 1);
			let (order, inheritor, _ticket) = pezpallet::Inheritor::<T>::get(EVE).unwrap();
			assert_eq!(inheritor, DAVE);
			assert_eq!(order, 0);

			// === Verify Attempt migration ===
			// ActiveRecovery for (ALICE, CHARLIE) should be migrated to Attempt
			assert_eq!(pezpallet::Attempt::<T>::iter().count(), 1);
			let (attempt, _ticket, deposit) = pezpallet::Attempt::<T>::get(ALICE, 0u32).unwrap();
	...
	fn migration_inserts_attempt_when_security_deposit_hold_fails() {
		new_test_ext().execute_with(|| {
			...
			pezpallet_balances::Pezpallet::<Test>::force_set_balance(
			...
			assert_eq!(
				pezpallet::Attempt::<T>::iter().count(),
				1,
	...
	fn migrated_recovery_can_be_completed() {
		...
		new_test_ext().execute_with(|| {
			...
			assert_eq!(pezpallet::FriendGroups::<T>::iter().count(), 1);
			assert_eq!(pezpallet::Attempt::<T>::iter().count(), 1);
			...
			let (groups, _) = pezpallet::FriendGroups::<T>::get(ALICE).unwrap();
			assert_eq!(groups[0].inheritor, multisig_inheritor);

			// Check the attempt state
			let (attempt, _, _) = pezpallet::Attempt::<T>::get(ALICE, 0u32).unwrap();
			...
			// === Now complete the recovery using the new pezpallet ===
			...
			pezframe_system::Pezpallet::<T>::set_block_number(15);
			...
			let (order, inheritor, _) = pezpallet::Inheritor::<T>::get(ALICE).unwrap();
			assert_eq!(inheritor, multisig_inheritor);
			assert_eq!(order, 0);

			// Attempt should be removed after finish
			assert!(pezpallet::Attempt::<T>::get(ALICE, 0u32).is_none());
		});
	}
	...
	fn migration_bumps_on_chain_storage_version() {
		new_test_ext().execute_with(|| {
			StorageVersion::new(0).put::<pezpallet::Pezpallet<T>>();
			assert_eq!(pezpallet::Pezpallet::<T>::on_chain_storage_version(), 0);
			...
			assert_eq!(pezpallet::Pezpallet::<T>::on_chain_storage_version(), 1);
		});
	}
	...
	fn migration_is_idempotent_after_completion() {
		new_test_ext().execute_with(|| {
			...
			let _guard = pezframe_support::StorageNoopGuard::new();
			let mut meter = WeightMeter::new();
	...
	fn migration_inserts_attempt_when_storage_ticket_fails() {
		new_test_ext().execute_with(|| {
			...
			let (attempt, ticket, held_deposit) = pezpallet::Attempt::<T>::get(lost, 0u32).unwrap();
			...
			use pezframe::traits::fungible::InspectHold;
			assert_eq!(
	...
	fn migration_inserts_inheritor_when_ticket_fails() {
		...
		new_test_ext().execute_with(|| {
			...
			.unwrap();
			pezframe_system::Pezpallet::<T>::inc_consumers(&rescuer).unwrap();

			v0::Proxy::<T>::insert(rescuer, lost);
			assert_eq!(pezframe_system::Pezpallet::<T>::consumers(&rescuer), 1);

			run_migration();

			assert_eq!(v0::Proxy::<T>::iter().count(), 0);
			assert_eq!(pezframe_system::Pezpallet::<T>::consumers(&rescuer), 0);

			assert_eq!(
				pezpallet::Inheritor::<T>::iter().count(),
				1,
				"Inheritor entry must survive even when ticket creation fails",
			);
			let (priority, inheritor, ticket) = pezpallet::Inheritor::<T>::get(lost).unwrap();
