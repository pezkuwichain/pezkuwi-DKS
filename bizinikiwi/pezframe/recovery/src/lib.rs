...
//! # Recovery Pezpallet
//!
//! Pezpallet Recovery allows you to have friends or family recover access to your account if you lose
...
//!
//! 1. Alice uses the recovery pezpallet to configure one or more friends groups:
...
//! 1. Day 15: *Family* finally understands Pezkuwi and initiates an attempt as well.
...
//! ## Data Structures
//!
//! The pezpallet has three storage items, see the in-code docs [`FriendGroups`], [`Attempt`] and
...
pub type FriendsOf<T> =
	BoundedVec<<T as pezframe_system::Config>::AccountId, <T as Config>::MaxFriendsPerConfig>;
pub type HashOf<T> = <T as pezframe_system::Config>::Hash;
...
pub struct FriendGroup<ProvidedBlockNumber, AccountId, Friends> {
	...
	///
	/// Uses a provided block number to avoid possible clock skew of teyrchains.
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		type Currency: MutateHold<Self::AccountId, Reason = Self::RuntimeHoldReason>
			+ Balanced<Self::AccountId>
			+ pezframe::traits::fungible::Mutate<Self::AccountId>;
		...
		/// choices are a treasury sink or `pezpallet-dap`.
	...
	impl<T: Config> Pezpallet<T> {
		...
		///
		/// The controller is not allowed to dispatch calls of the recovery pezpallet. Otherwise they
		...
		pub fn control_inherited_account(
			...
			let mut origin: T::RuntimeOrigin =
				pezframe_system::RawOrigin::Signed(recovered.clone()).into();
			// Reentrancy guard
			origin.add_filter(|c: &<T as pezframe_system::Config>::RuntimeCall| {
		...
		#[pezpallet::weight(T::WeightInfo::revoke_inheritor())]
		...
		#[pezpallet::weight(T::WeightInfo::set_friend_groups())]
		...
		#[pezpallet::weight(T::WeightInfo::initiate_attempt())]
		...
		#[pezpallet::weight(T::WeightInfo::approve_attempt())]
		...
		#[pezpallet::weight(T::WeightInfo::finish_attempt())]
		...
		#[pezpallet::weight(T::WeightInfo::cancel_attempt())]
		...
		#[pezpallet::weight(T::WeightInfo::slash_attempt())]
...
impl<T: Config> Pezpallet<T> {
	...
	pub fn attempt_of(
		...
		pezpallet::Attempt::<T>::get(lost, friend_group_index).ok_or(Error::<T>::NotAttempt)
	}
