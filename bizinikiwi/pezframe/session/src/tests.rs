...
mod externally_set_keys_tracking {
	...
	fn setup_account() {
		pezframe_system::Pezpallet::<Test>::inc_providers(&ACCOUNT);
		ValidatorAccounts::mutate(|m| {
	...
	fn assert_local_state(consumers_before: u32) {
		...
		// +1 from session's inc_consumers, +1 from pezpallet-balances hold.
	...
	fn key_rotation_succeeds_at_max_consumers() {
		new_test_ext().execute_with(|| {
			...
			while pezframe_system::Pezpallet::<Test>::can_inc_consumer(&ACCOUNT) {
				pezframe_system::Pezpallet::<Test>::inc_consumers(&ACCOUNT).unwrap();
			}
			assert!(
				!pezframe_system::Pezpallet::<Test>::can_inc_consumer(&ACCOUNT),
	...
	fn first_registration_fails_at_max_consumers() {
		new_test_ext().execute_with(|| {
			...
			pezframe_system::Pezpallet::<Test>::inc_providers(&FRESH_ACCOUNT);
			ValidatorAccounts::mutate(|m| {
				m.insert(FRESH_ACCOUNT, FRESH_ACCOUNT);
			});
			while pezframe_system::Pezpallet::<Test>::can_inc_consumer(&FRESH_ACCOUNT) {
				pezframe_system::Pezpallet::<Test>::inc_consumers(&FRESH_ACCOUNT).unwrap();
			}
