...
pub mod pezpallet {
	...
	pub enum Event<T: Config> {
		...
		/// begin with.
		NewTerm { new_members: Vec<(<T as pezframe_system::Config>::AccountId, BalanceOf<T>)> },
		...
		/// `EmptyTerm`.
		MemberKicked { member: <T as pezframe_system::Config>::AccountId },
		/// Someone has renounced their candidacy.
		Renounced { candidate: <T as pezframe_system::Config>::AccountId },
...
impl<T: Config> Pezpallet<T> {
	...
	fn do_phragmen() -> Weight {
		...
		let _ =
			pezsp_npos_elections::seq_phragmen(num_to_elect, candidate_ids, voters_and_votes, None)
...
mod tests {
	...
	pub type UncheckedExtrinsic =
		pezsp_runtime::generic::UncheckedExtrinsic<u32, RuntimeCall, u64, ()>;
	...
	impl ExtBuilder {
		...
		pub fn build_and_execute(self, test: impl FnOnce() -> ()) {
			...
			#[cfg(feature = "try-runtime")]
			ext.execute_with(|| {
				assert_ok!(<Elections as pezframe_support::traits::Hooks<u64>>::try_state(
					System::block_number()
				));
			});
		}
	...
	fn submit_candidacy(origin: RuntimeOrigin) -> pezsp_runtime::DispatchResult {
	...
	fn remove_voter_should_work() {
		ExtBuilder::default().voter_bond(8).build_and_execute(|| {
			...
			assert_eq!(balances(&2), (20, 0));
			assert_eq!(pezpallet_balances::Locks::<Test>::get(&2).len(), 0);
		});
	}
