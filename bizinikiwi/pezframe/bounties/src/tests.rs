...
use pezframe_support::{
	assert_noop, assert_ok, derive_impl, hypothetically, hypothetically_ok, parameter_types,
	pezpallet_prelude::*,
	traits::{
		fungible,
...
use pezframe_system::{pezpallet_prelude::*, EnsureSigned};
use pezsp_runtime::{
...
fn go_to_block(n: u64) {
	<Test as pezpallet_treasury::Config>::BlockNumberProvider::set_block_number(n);
...
impl pezpallet_treasury::Config for Test {
	...
	type SpendOrigin = pezframe_system::EnsureRootWithSuccess<Self::AccountId, SpendLimit>;
	type AssetKind = ();
...
impl pezpallet_treasury::Config<Instance1> for Test {
	...
	type MaxApprovals = ConstU32<100>;
	type SpendOrigin = pezframe_system::EnsureRootWithSuccess<Self::AccountId, SpendLimit1>;
	type AssetKind = ();
	type Beneficiary = Self::AccountId;
	type BeneficiaryLookup = IdentityLookup<Self::Beneficiary>;
...
type TreasuryError = pezpallet_treasury::Error<Test>;
type TreasuryError1 = pezpallet_treasury::Error<Test, Instance1>;
...
impl ExtBuilder {
	pub fn build(self) -> pezsp_io::TestExternalities {
		...
		.into();
		ext.execute_with(|| {
			<Test as pezpallet_treasury::Config>::BlockNumberProvider::set_block_number(1)
		});
		ext
	}
...
fn unused_pot_should_diminish() {
	ExtBuilder::default().build_and_execute(|| {
		let init_total_issuance = pezpallet_balances::TotalIssuance::<Test>::get();
		Balances::make_free_balance_be(&Treasury::account_id(), 101);
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), init_total_issuance + 100);

		go_to_block(2);
		assert_eq!(Treasury::pot(), 50);
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), init_total_issuance + 50);
	});
}
...
fn propose_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(0).unwrap(),
			Bounty {
				proposer: 0,
				fee: 0,
				curator_deposit: 0,
		...
		assert_eq!(
			pezpallet_bounties::BountyDescriptions::<Test>::get(0).unwrap(),
			b"1234567890".to_vec()
		);

		assert_eq!(pezpallet_bounties::BountyCount::<Test>::get(), 1);
	});
}
...
fn close_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(pezpallet_bounties::Bounties::<Test>::get(0), None);
		assert!(!pezpallet_treasury::Proposals::<Test>::contains_key(0));

		assert_eq!(pezpallet_bounties::BountyDescriptions::<Test>::get(0), None);
	});
}
...
fn close_bounty_with_additional_assets_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert!(matches!(
			pezpallet_bounties::Bounties::<Test>::get(0).unwrap().status,
			BountyStatus::Active { .. }
		));
		...
		hypothetically!({
			...
			assert!(pezframe_system::Account::<Test>::contains_key(pot));
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		for ((s, c), p) in (0..10).zip(0..10).zip(0..10) {
			hypothetically!({
				...
				pezframe_system::Account::<Test>::mutate(&pot, |a| {
					a.sufficients = s;
					a.consumers = c;
					a.providers = p;
				});
...
fn approve_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(pezpallet_bounties::BountyApprovals::<Test>::get(), vec![0]);
...
fn test_migration_v4() {
	...
	let data = vec![
		(pezpallet_bounties::BountyCount::<Test>::hashed_key().to_vec(), 10.encode().to_vec()),
		(pezpallet_bounties::Bounties::<Test>::hashed_key_for(index), bounty.encode().to_vec()),
		(pezpallet_bounties::BountyDescriptions::<Test>::hashed_key_for(index), vec![0, 0]),
		(
			pezpallet_bounties::BountyApprovals::<Test>::hashed_key().to_vec(),
...
fn approve_bounty_with_curator_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(pezpallet_bounties::BountyApprovals::<Test>::get().len(), 0);
		assert_ok!(Bounties::approve_bounty_with_curator(RuntimeOrigin::root(), 0, curator, 10));
		assert_eq!(pezpallet_bounties::BountyApprovals::<Test>::get().len(), 1);
...
fn accept_curator_sets_update_due_correctly() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Then
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(bounty_id).unwrap().status,
		...
		// Then
		assert_eq!(
			pezpallet_bounties::Bounties::<Test, Instance1>::get(bounty_id).unwrap().status,
...
fn poke_deposit_fails_for_insufficient_balance() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(pezpallet_bounties::BountyDescriptions::<Test>::get(0).unwrap(), description);
		assert_eq!(pezpallet_bounties::BountyCount::<Test>::get(), 1);
...
fn poke_deposit_fails_for_any_status_other_than_proposed() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(pezpallet_bounties::Bounties::<Test>::get(0).unwrap(), bounty);
		assert_eq!(pezpallet_bounties::BountyDescriptions::<Test>::get(0).unwrap(), description);
		assert_eq!(pezpallet_bounties::BountyCount::<Test>::get(), 1);

		// Change status to approved
		bounty.status = BountyStatus::Approved;
		pezpallet_bounties::Bounties::<Test>::insert(bounty_id, &bounty);
