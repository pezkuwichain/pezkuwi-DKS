...
use codec::Encode;
use pezframe_election_provider_support::{NposSolution, NposSolver, PerThing128, VoteWeight};
...
/// voted.
pub type VoterOf<T> = pezframe_election_provider_support::VoterOf<<T as Config>::DataProvider>;

/// Same as [`VoterOf`], but parameterized by the `MinerConfig`.
pub type MinerVoterOf<T> = pezframe_election_provider_support::Voter<
...
impl From<pezsp_npos_elections::Error> for MinerError {
...
impl<T: MinerConfig> Miner<T> {
	...
	) -> Result<(SolutionOf<T>, ElectionScore, SolutionOrSnapshotSize, TrimmingStatus), MinerError>
	{
		...
		let sorted_assignments = {
			...
			pezsp_npos_elections::reduce(&mut staked);

			// Sort the assignments by reversed voter stake. This ensures that we can efficiently
			// truncate the list.
			staked.sort_by_key(
				|pezsp_npos_elections::StakedAssignment::<T::AccountId> { who, .. }| {
			...
			let _ = pezsp_npos_elections::to_supports(&staked)
				.iter_mut()
			...
			debug_assert!({
				...
				let expected_ok: Result<
					...
					_,
				> = pezsp_npos_elections::to_supports(&staked).try_into();
				expected_ok.is_ok()
			});
	...
	pub fn trim_assignments_length(
		...
		encoded_size_of: impl Fn(&[IndexAssignmentOf<T>]) -> Result<usize, pezsp_npos_elections::Error>,
...
mod tests {
	...
	use alloc::vec;
	use codec::Decode;
	use pezframe_election_provider_support::IndexAssignment;
	use pezframe_support::{assert_noop, assert_ok, traits::OffchainWorker};
	...
	fn mine_solution_always_respects_max_backers_per_winner() {
		...
		use pezframe_election_provider_support::BoundedSupport;
