...
use pezsp_core::{hexdisplay::HexDisplay, storage::well_known_keys, H256};
use pezsp_runtime::{
...
fn set_code_with_real_wasm_blob() {
	...
	ext.execute_with(|| {
		...
		let hash =
			BlakeTwo256::hash(&bizinikiwi_test_runtime_client::runtime::wasm_binary_unwrap());
		assert_eq!(
			System::events(),
			vec![EventRecord {
				phase: Phase::Initialization,
...
fn set_code_version_3_schedules_and_applies_pending_code() {
	...
	ext.execute_with(|| {
		...
		Version::set(version);
		// Move to block 1
		crate::Pezpallet::<Test>::set_block_number(1);
		// Schedule new code
		...
		crate::Pezpallet::<Test>::maybe_apply_pending_code_upgrade();
