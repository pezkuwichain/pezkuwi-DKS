...
use pezframe_support::traits::{Authorize, PostInherents, PostTransactions, PreInherents};
...
pub mod pezpallet {
	...
	pub mod config_preludes {
		...
		use pezframe_support::{derive_impl, traits::Get};
		...
		impl DefaultConfig for TestDefaultConfig {
			...
			type BlockHashCount = TestBlockHashCount<pezframe_support::traits::ConstU32<10>>;
			type OnSetCode = ();
		...
		#[pezframe_support::register_default_impl(SolochainDefaultConfig)]
		impl DefaultConfig for SolochainDefaultConfig {
			...
			type Lookup = pezsp_runtime::traits::AccountIdLookup<Self::AccountId, ()>;

			/// The maximum number of consumers allowed on a single account. Using 128 as default.
			type MaxConsumers = pezframe_support::traits::ConstU32<128>;
			...
			type BlockHashCount = TestBlockHashCount<pezframe_support::traits::ConstU32<256>>;
	...
	pub trait Config: 'static + Eq + Clone {
		...
		///
		/// See `pezframe_executive::block_flowchart` for a in-depth explanation when it runs.
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			...
			pezsp_io::storage::set(well_known_keys::EXTRINSIC_INDEX, &0u32.encode());
		}
	}
...
impl<T: Config> Pezpallet<T> {
	...
	/// runtime migrations
	/// [`on_runtime_upgrade`](pezframe_support::traits::OnRuntimeUpgrade::on_runtime_upgrade)
	...
	pub fn initialize_intra_block_entropy(parent_hash: &T::Hash) {
		let entropy = (b"pezframe_system::initialize", parent_hash).using_encoded(blake2_256);
	...
	pub fn finalize() -> HeaderFor<T> {
		...
		let storage_root = T::Hash::decode(&mut &pezsp_io::storage::root(version)[..])
	...
	pub fn run_to_block_with<AllPalletsWithSystem>(
		...
		AllPalletsWithSystem: pezframe_support::traits::OnInitialize<BlockNumberFor<T>>
			+ pezframe_support::traits::OnFinalize<BlockNumberFor<T>>,
	{
	...
	pub fn can_set_code(code: &[u8], check_version: bool) -> CanSetCodeResult<T> {
		...
		if check_version {
			...
			let Some(new_version) = pezsp_io::misc::runtime_version(code)
...
pub fn unique(entropy: impl Encode) -> [u8; 32] {
	let mut last = [0u8; 32];
	pezsp_io::storage::read(well_known_keys::INTRABLOCK_ENTROPY, &mut last[..], 0);
	let next = (b"pezframe_system::unique", entropy, last).using_encoded(blake2_256);
	pezsp_io::storage::set(well_known_keys::INTRABLOCK_ENTROPY, &next);
	next
}
...
pub mod pezpallet_prelude {
	...
	pub type HeaderFor<T> =
		<<T as crate::Config>::Block as pezsp_runtime::traits::HeaderProvider>::HeaderT;
	...
	pub type ExtrinsicFor<T> =
		<<T as crate::Config>::Block as pezsp_runtime::traits::Block>::Extrinsic;
