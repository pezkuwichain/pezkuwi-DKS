...
pub type OffenceDetailsOf<T> = OffenceDetails<
	...
	(
		<T as pezframe_system::Config>::AccountId,
		pezsp_staking::Exposure<<T as pezframe_system::Config>::AccountId, BalanceOf<T>>,
	),
>;
...
impl<T: Config>
	pezsp_runtime::traits::Convert<
		T::AccountId,
		Option<pezsp_staking::Exposure<T::AccountId, BalanceOf<T>>>,
...
pub mod pezpallet {
	...
	use pezpallet_session::{historical, SessionManager};
	use pezpallet_staking_async_rc_client::SessionReport;
	use pezsp_runtime::{Perbill, Saturating};
	use pezsp_staking::{
	...
	pub trait Config: pezframe_system::Config {
		...
		type Fallback: pezpallet_session::SessionManager<Self::AccountId>
			+ OnOffenceHandler<
				Self::AccountId,
				(Self::AccountId, pezsp_staking::Exposure<Self::AccountId, BalanceOf<Self>>),
				Weight,
			> + pezframe_support::traits::RewardsReporter<Self::AccountId>
			+ pezpallet_authorship::EventHandler<Self::AccountId, BlockNumberFor<Self>>;
	...
	pub type QueuedOffenceOf<T> =
		(SessionIndex, rc_client::Offence<<T as pezframe_system::Config>::AccountId>);
		...
		> for Pezpallet<T>
	{
		...
		> {
			<Self as pezpallet_session::SessionManager<_>>::new_session(new_index)
				.map(|v| v.into_iter().map(|v| (v, pezsp_staking::Exposure::default())).collect())
		}
	...
	impl<T: Config> Pezpallet<T> {
		...
		fn do_end_session(end_index: u32) {
			...
			let session_report = pezpallet_staking_async_rc_client::SessionReport {
				end_index,
				validator_points,
...
mod send_queue_tests {
	use pezframe_support::hypothetically;
