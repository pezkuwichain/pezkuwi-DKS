...
//!
//! Legacy mode is used on Dicle where inflation depends on the staking ratio.
...
fn legacy_era_payout_and_mint_works() {
	ExtBuilder::default().legacy_reward_mode().build_and_execute(|| {
		...
		let pre_payout_issuance = pezpallet_balances::TotalIssuance::<Test>::get();
		assert_ok!(Staking::payout_stakers(RuntimeOrigin::signed(1337), 11, 1));

		// THEN: payout mints tokens (issuance increases).
		assert!(pezpallet_balances::TotalIssuance::<Test>::get() > pre_payout_issuance);
	});
}
...
fn legacy_to_dap_migration_flow() {
	...
	ExtBuilder::default().legacy_reward_mode().build_and_execute(|| {
		...
		let pre_legacy_issuance = pezpallet_balances::TotalIssuance::<Test>::get();
		assert_ok!(Staking::payout_stakers(RuntimeOrigin::signed(1337), 11, legacy_era));
		// Legacy payout mints — issuance increases.
		assert!(pezpallet_balances::TotalIssuance::<Test>::get() > pre_legacy_issuance);
