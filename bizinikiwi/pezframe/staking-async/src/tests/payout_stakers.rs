...
fn rewards_with_nominator_should_work() {
	ExtBuilder::default().nominate(true).session_per_era(3).build_and_execute(|| {
		...
		// In non-minting mode, rewards are minted externally (e.g. pezpallet-dap) and
...
fn test_multi_page_payout_stakers_by_page() {
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<T>::get(), pre_payout_total_issuance);
		...
		for i in 4..17 {
			...
			let pre_roll_issuance = pezpallet_balances::TotalIssuance::<T>::get();
			...
			let post_roll_issuance = pezpallet_balances::TotalIssuance::<T>::get();
			mock::make_all_reward_payment(i - 1);
			// Payout just transfers from pot, so issuance doesnt change
			assert_eq!(pezpallet_balances::TotalIssuance::<T>::get(), post_roll_issuance);
...
fn legacy_payout_ignores_pot_account_existence() {
	ExtBuilder::default().legacy_reward_mode().build_and_execute(|| {
		...
		let pre_issuance = pezpallet_balances::TotalIssuance::<Test>::get();
		assert_ok!(Staking::payout_stakers(RuntimeOrigin::signed(1337), validator, era));
		let minted = pezpallet_balances::TotalIssuance::<Test>::get() - pre_issuance;
