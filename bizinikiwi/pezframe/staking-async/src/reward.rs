...
impl<T: Config> EraRewardManager<T> {
	...
	pub(crate) fn create(era: EraIndex, kind: RewardKind) -> T::AccountId {
		...
		if pezframe_system::Pezpallet::<T>::providers(&pot_account) == 0 {
			pezframe_system::Pezpallet::<T>::inc_providers(&pot_account);
		}
		pot_account
	}
	...
	pub(crate) fn has_staker_rewards_pot(era: EraIndex) -> bool {
		...
		pezframe_system::Pezpallet::<T>::providers(&pot) > 0
	}
...
impl<T: Config> pezsp_staking::StakerRewardCalculator<BalanceOf<T>>
	...
	BalanceOf<T>: Into<u128> + From<u128>,
{
	...
	fn calculate_staker_reward(
	...
	) -> pezsp_staking::StakerRewardResult<BalanceOf<T>> {
		...
		pezsp_staking::StakerRewardResult { validator_payout, nominator_payout }
	}
}
	...
	Balance: AtLeast32BitUnsigned + Copy + Into<u128> + From<u128>,
{
	...
	let weight_u128 = if self_stake <= optimum {
		pezsp_arithmetic::helpers_128bit::sqrt(self_stake_u128)
		...
		pezsp_arithmetic::helpers_128bit::sqrt(arg)
	} else {
		let k_squared = slope_factor.square();
