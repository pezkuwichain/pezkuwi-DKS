...
//! Storage migrations for the staking-async pezpallet.
...
use pezsp_runtime::{traits::AccountIdConversion, Saturating};
	...
	for MigrateEraPotsToPool<T, S, K>
{
	fn on_runtime_upgrade() -> Weight {
		...
		for era in oldest..active_era_idx {
			...
			if pezframe_system::Pezpallet::<T>::providers(&old) == 0 {
				continue;
			}
			...
			let _ = pezframe_system::Pezpallet::<T>::dec_providers(&old);
