...
use crate::{
	self as pezpallet_staking_async,
...
use pezframe_system::{pezpallet_prelude::BlockNumberFor, EnsureRoot, EnsureSignedBy};
use pezpallet_staking_async_rc_client as rc_client;
use pezsp_core::{ConstBool, ConstU64};
use pezsp_io;
use pezsp_npos_elections::BalancingConfig;
use pezsp_runtime::{traits::Zero, BuildStorage, Weight};
use pezsp_staking::{
...
pub(crate) type AccountId = <Runtime as pezframe_system::Config>::AccountId;
pub(crate) type BlockNumber = BlockNumberFor<Runtime>;
pub(crate) type Balance = <Runtime as pezpallet_balances::Config>::Balance;
...
pub(crate) const THRESHOLDS: [pezsp_npos_elections::VoteWeight; 9] =
...
pub mod session_mock {
	...
	impl pezpallet_staking_async_rc_client::RcClientInterface for Session {
		type AccountId = AccountId;
...
impl pezpallet_dap::Config for Test {
...
pub(crate) fn default_budget() -> pezpallet_dap::BudgetAllocationMap {
...
pub(crate) fn setup_dap() {
	pezpallet_dap::BudgetAllocation::<Test>::put(default_budget());
	pezpallet_dap::LastIssuanceTimestamp::<Test>::put(session_mock::Timestamp::get());
	...
	let dap_buffer = <pezpallet_dap::Pezpallet<Test> as pezsp_staking::budget::BudgetRecipient<
		AccountId,
	>>::pot_account();
	<Balances as pezframe_support::traits::fungible::Mutate<_>>::mint_into(&dap_buffer, ed)
...
impl ExtBuilder {
	...
	fn build(self) -> pezsp_io::TestExternalities {
		...
		let _ = pezpallet_balances::GenesisConfig::<Test> { balances, ..Default::default() }
			.assimilate_storage(&mut storage);

		let _ = pezpallet_staking_async::GenesisConfig::<Test> {
...
pub(crate) fn active_era() -> EraIndex {
	pezpallet_staking_async::ActiveEra::<Test>::get().unwrap().index
}

pub(crate) fn current_era() -> EraIndex {
	pezpallet_staking_async::CurrentEra::<Test>::get().unwrap()
}
...
pub(crate) fn validator_payout_for(duration: u64) -> Balance {
	let total_inflation =
		OneTokenPerMillisecond::issue(pezpallet_balances::TotalIssuance::<Test>::get(), duration);
	// Apply budget allocation to get staker portion
	let budget = pezpallet_dap::BudgetAllocation::<Test>::get();
...
pub(crate) fn setup_incentive_with_budget(staker_pct: u32, incentive_pct: u32) {
	...
	pezpallet_dap::BudgetAllocation::<Test>::put(build_budget(&entries));
}
