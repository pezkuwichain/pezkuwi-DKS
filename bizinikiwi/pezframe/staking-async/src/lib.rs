...
//!
//! This pezpallet supports two reward modes, controlled by [`Config::DisableMinting`]:
//!
//! ### Non-minting mode (`DisableMinting = true`)
//!
//! Staking does **not** mint tokens. It expects an external source (e.g. `pezpallet-dap`) to
...
//!
//! This mode is kept for Dicle/non-polkadot runtime's compatibility where inflation depends on the
//! staking ratio.
//!
//! ### Switching modes
//!
...
pub use pezsp_staking::EraPayout;
...
pub enum RewardPot {
	/// General pot: funded by an external source (e.g. pezpallet-dap).
...
where
	...
	S: Get<pezframe_support::PalletId>,
{
	fn pot_account(pot: RewardPot) -> AccountId {
...
impl<AccountId, P> pezsp_staking::budget::BudgetRecipient<AccountId> for StakerRewardRecipient<P>
where
...
impl<AccountId, P> pezsp_staking::budget::BudgetRecipient<AccountId>
