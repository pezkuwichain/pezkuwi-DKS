...
#[cfg(feature = "xcm-sender")]
use pezframe_support::storage::transactional::with_transaction_opaque_err;
...
#[cfg(feature = "xcm-sender")]
use pezsp_runtime::{traits::Convert, TransactionOutcome};
use pezsp_runtime::{traits::OpaqueKeys, Perbill};
use pezsp_staking::SessionIndex;
// XCM imports are only used by the optional XCMSender helper struct for runtimes, not by the
// pezpallet's public API. The pezpallet only uses the abstract SendToRelayChain trait.
//
// TODO: Consider relocating `staking-async` pezpallets to `polkadot/pezpallets/` or
// `cumulus/pezpallets/`. These pezpallets are Pezkuwi-specific (AH↔RC communication) and leak XCM
// types into FRAME, which historically has been chain-agnostic. Alternatively, the `XCMSender`
// helper could be moved to runtime level, keeping this pezpallet XCM-agnostic through the
...
///
/// This is the standard pattern for system teyrchain → relay chain messages where
/// the relay chain trusts the teyrchain origin and doesn't charge fees.
#[cfg(feature = "xcm-sender")]
...
/// Type alias for balance used in this pezpallet.
pub type BalanceOf<T> = <<T as pezpallet::Config>::Currency as FunInspect<
...
///
/// Note: This trait intentionally avoids XCM types in its signature to keep the pezpallet
...
pub trait SendToRelayChain {
	...
	///
	/// The keys are forwarded to `pezpallet-staking-async-ah-client::set_keys_from_ah` on the RC.
	...
	///
	/// The request is forwarded to `pezpallet-staking-async-ah-client::purge_keys_from_ah` on the RC.
	///
	/// The relay chain uses `UnpaidExecution`, so no fees are charged there. Instead, the total
	/// fee (delivery + remote execution cost) is charged on AssetHub.
	///
...
pub struct SessionReport<AccountId> {
	...
	/// message with `leftover = true` comes in. The client pezpallets should handle this queuing.
	///
...
// NOTE: to have the pezpallet fully XCM-agnostic, XCMSender should be moved out (to a new or existing
	...
	ToXcm: Convert<Message, Xcm<()>>,
{
			...
			+ core::ops::Add<Output = Balance>,
	{
		...
		//
		// For teyrchain→relay chain messages, delivery fees are returned as a single
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		/// - 0 = export immediately when received from staking pezpallet
		...
		/// in `pezpallet-staking-async`. The XCM message is sent when BOTH conditions met:
		...
		/// The session keys type that must match the Relay Chain's `pezpallet_session::Config::Keys`.
		///
	...
	impl<T: Config> Pezpallet<T> {
		...
		/// `pezpallet-session::purge_keys` which allows anyone to call it.
		///
