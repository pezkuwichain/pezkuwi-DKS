...
//! Benchmarking setup for pezpallet-staking-async-rc-client.
...
/// Wrapper pezpallet for benchmarking.
pub struct Pezpallet<T: Config>(crate::Pezpallet<T>);

/// Configuration trait for benchmarking `pezpallet-staking-async-rc-client`.
///
...
pub trait Config: crate::Config {
	...
	/// - Is registered as a validator in the staking pezpallet
...
mod benchmarks {
	...
	fn set_keys() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		crate::Pezpallet::<T>::set_keys(RawOrigin::Signed(stash), keys, proof, None);
	...
	fn purge_keys() -> Result<(), BenchmarkError> {
		...
		#[extrinsic_call]
		crate::Pezpallet::<T>::purge_keys(RawOrigin::Signed(caller), None);
