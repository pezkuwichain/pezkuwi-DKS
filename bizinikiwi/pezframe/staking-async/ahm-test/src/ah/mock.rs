...
use pezpallet_staking_async::{ActiveEra, CurrentEra, Forcing, PotAccountProvider};
...
use pezsp_staking::{budget::BudgetRecipient, SessionIndex};
...
pub fn roll_next() {
	...
	// Re-init pezframe-system, as execute would do. This resets the block weight usage counter, as we
...
pub(crate) fn roll_until_next_active(mut end_index: SessionIndex) -> Vec<AccountId> {
	...
	while pezpallet_staking_async::session_rotation::Rotator::<Runtime>::planned_era() == active_era
	{
	...
	while !pezpallet_staking_async_rc_client::OutgoingValidatorSet::<T>::exists() {
		roll_next();
...
pub type BlockWeights = <Runtime as pezframe_system::Config>::BlockWeights;
...
pezframe::deps::pezsp_runtime::impl_opaque_keys! {
	pub struct RCSessionKeys {
		pub other: pezframe::deps::pezsp_runtime::testing::UintAuthorityId,
	}
}
...
impl pezframe_support::traits::Get<bool> for DisableMintingMode {
	fn get() -> bool {
...
impl pezsp_staking::budget::IssuanceCurve<Balance> for OneTokenPerMillisecond {
...
impl pezpallet_dap::Config for Runtime {
	...
	type BudgetRecipients = (
		...
		pezpallet_staking_async::StakerRewardRecipient<pezpallet_staking_async::SequentialTest>,
		pezpallet_staking_async::ValidatorIncentiveRecipient<
			pezpallet_staking_async::SequentialTest,
		>,
	);
	type Time = MockTime;
...
/// Asset transactor that uses pezpallet_balances for the native asset.
...
/// Mock XCM executor that performs real fee charging using pezpallet_balances.
...
impl pezpallet_staking_async_rc_client::SendToRelayChain for DeliverToRelay {
	...
	fn set_keys(
		...
		if let Some(mut local_queue) = LocalQueue::get() {
			...
			shared::in_rc(|| {
				let origin = crate::rc::RuntimeOrigin::root();
				pezpallet_staking_async_ah_client::Pezpallet::<crate::rc::Runtime>::set_keys_from_ah(
					origin,
					stash,
					keys.clone(),
				)
	...
	fn purge_keys(
		...
		if let Some(mut local_queue) = LocalQueue::get() {
			...
			shared::in_rc(|| {
				let origin = crate::rc::RuntimeOrigin::root();
				pezpallet_staking_async_ah_client::Pezpallet::<crate::rc::Runtime>::purge_keys_from_ah(
					origin, stash,
				)
				.unwrap();
			});
...
impl ExtBuilder {
	...
	pub fn build(self) -> TestState {
		...
		use pezframe_support::pezsp_runtime::traits::AccountIdConversion;
...
pub(crate) fn staker_reward_key() -> pezsp_staking::budget::BudgetKey {
	<pezpallet_staking_async::StakerRewardRecipient<pezpallet_staking_async::SequentialTest> as
		BudgetRecipient<AccountId>>::budget_key()
}

pub(crate) fn validator_incentive_key() -> pezsp_staking::budget::BudgetKey {
	<pezpallet_staking_async::ValidatorIncentiveRecipient<pezpallet_staking_async::SequentialTest> as
		BudgetRecipient<AccountId>>::budget_key()
}

pub(crate) fn buffer_key() -> pezsp_staking::budget::BudgetKey {
	<pezpallet_dap::Pezpallet<Runtime> as BudgetRecipient<AccountId>>::budget_key()
}

pub(crate) fn build_budget(
	entries: &[(pezsp_staking::budget::BudgetKey, u32)],
) -> pezpallet_dap::BudgetAllocationMap {
	let mut budget = pezpallet_dap::BudgetAllocationMap::new();
...
pub(crate) fn setup_dap() {
	pezpallet_dap::BudgetAllocation::<Runtime>::put(build_budget(&[
		(staker_reward_key(), 50),
		(buffer_key(), 50),
	]));

	pezpallet_dap::LastIssuanceTimestamp::<Runtime>::put(MockTime::get());

	// Fund general pots with ED to keep them alive.
	let general_staker = pezpallet_staking_async::SequentialTest::pot_account(
		pezpallet_staking_async::RewardPot::General(
			pezpallet_staking_async::RewardKind::StakerRewards,
		),
	);
	Balances::mint_into(&general_staker, 1).unwrap();

	let general_incentive = pezpallet_staking_async::SequentialTest::pot_account(
		pezpallet_staking_async::RewardPot::General(
			pezpallet_staking_async::RewardKind::ValidatorSelfStake,
		),
	);
...
fn end_session_and_assert_idle(activate: bool, expect_export: bool) {
	...
	let old_era_points = pezpallet_staking_async::ErasRewardPoints::<T>::get(&active_era);
	...
	let last_session_end_index =
		pezpallet_staking_async_rc_client::LastSessionReportEndingIndex::<T>::get()
			.unwrap_or_default();
	let end_index = last_session_end_index + 1;

	// end the session..
	assert_ok!(pezpallet_staking_async_rc_client::Pezpallet::<T>::relay_session_report(
		RuntimeOrigin::root(),
		SessionReport {
			end_index,
	...
	assert_eq!(
		...
		pezpallet_staking_async::Event::SessionRotated {
	...
	let updated_era_points = pezpallet_staking_async::ErasRewardPoints::<T>::get(&active_era);
...
impl pezframe_support::traits::InstanceFilter<RuntimeCall> for ProxyType {
			...
			ProxyType::Staking => matches!(
				...
				RuntimeCall::Staking(..)
					| RuntimeCall::RcClient(
						pezpallet_staking_async_rc_client::Call::set_keys { .. }
							| pezpallet_staking_async_rc_client::Call::purge_keys { .. }
					)
			),
		}
	}
...
impl pezpallet_proxy::Config for Runtime {
	...
	type CallHasher = pezframe::deps::pezsp_runtime::traits::BlakeTwo256;
