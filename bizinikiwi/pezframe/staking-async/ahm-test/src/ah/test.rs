...
use pezframe_election_provider_support::Weight;
...
fn on_receive_session_report() {
	ExtBuilder::default().local_queue().build().execute_with(|| {
		...
		// Validator is queued in session pezpallet for 1 session. We wait to activate.
		end_session_with(false, AssertSessionType::IdleNoExport);
...
fn validator_set_send_fail_retries() {
	ExtBuilder::default().local_queue().build().execute_with(|| {
		...
		// assert validator set queued in rc client pezpallet
...
fn on_offence_current_era_instant_apply() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let dap_buffer =
				<pezpallet_dap::Pezpallet<Runtime> as pezsp_staking::budget::BudgetRecipient<
					AccountId,
				>>::pot_account();
...
mod poll_operations {
	use super::*;
	use pezpallet_election_provider_multi_block::verifier::{Status, Verifier};
	...
	fn full_election_cycle_with_occasional_out_of_weight_completes() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert_eq!(pezpallet_staking_async::NextElectionPage::<T>::get(), None);
			roll_next();
			assert_eq!(
				election_events_since_last_call(),
			...
			assert_eq!(pezpallet_staking_async::NextElectionPage::<T>::get(), Some(1));
			...
			assert_eq!(pezpallet_staking_async::NextElectionPage::<T>::get(), Some(0));
			...
			assert_eq!(
				...
				vec![StakingEvent::Unexpected(
					pezpallet_staking_async::UnexpectedKind::PagedElectionOutOfWeight {
						page: 0,
	...
	fn slashing_processing_while_election() {
		...
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert!(pezpallet_staking_async::NextElectionPage::<T>::get().is_none());
...
mod session_keys {
	...
	use codec::Encode;
	use pezframe_support::{assert_noop, pezpallet_prelude::DispatchError};
	...
	fn set_keys_success() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert_ok!(rc_client::Pezpallet::<T>::set_keys(
	...
	fn set_keys_not_validator() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert_noop!(
				rc_client::Pezpallet::<T>::set_keys(
	...
	fn set_keys_via_staking_proxy() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert_ok!(pezpallet_proxy::Pezpallet::<T>::add_proxy(
			...
			assert_ok!(pezpallet_proxy::Pezpallet::<T>::proxy(
	...
	fn purge_keys_success() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			assert_ok!(rc_client::Pezpallet::<T>::purge_keys(
				RuntimeOrigin::signed(validator),
				None,
			));
	...
	/// This matches the original pezpallet-session behavior and enables validators to purge their
	...
	fn purge_keys_xcm_send_fails() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			// THEN: XcmSendFailed error is returned
			assert_noop!(
				rc_client::Pezpallet::<T>::purge_keys(RuntimeOrigin::signed(validator), None),
				rc_client::Error::<T>::XcmSendFailed
			);
		});
	}
	...
	fn purge_keys_fails_with_insufficient_balance() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			// THEN: XcmSendFailed error is returned (fee charging failure causes XCM send to fail)
			assert_noop!(
				rc_client::Pezpallet::<T>::purge_keys(RuntimeOrigin::signed(account), None),
				rc_client::Error::<T>::XcmSendFailed
			);
		});
	}
	...
	fn purge_keys_max_fee_scenarios() {
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			hypothetically!({
				assert_noop!(
					rc_client::Pezpallet::<T>::purge_keys(
						RuntimeOrigin::signed(account),
						Some(total_fee - 1),
					),
					rc_client::Error::<T>::FeesExceededMax
		...
		ExtBuilder::default().local_queue().build().execute_with(|| {
			...
			hypothetically!({
				...
				assert_noop!(
					...
					DispatchError::Token(pezframe::runtime::prelude::TokenError::FundsUnavailable)
				);
			});
	...
	fn purge_keys_on_rc_does_not_release_ah_deposit() {
		...
		shared::in_rc(|| {
			assert_ok!(pezpallet_session::Pezpallet::<rc::Runtime>::purge_keys(
				rc::RuntimeOrigin::signed(validator),
			));
			let next_keys = pezpallet_session::NextKeys::<rc::Runtime>::get(validator);
...
impl pezframe_support::traits::Get<u64> for LastInflationTs {
	fn get() -> u64 {
...
impl pezframe_support::traits::Get<pezpallet_dap::BudgetAllocationMap> for InitialBudget {
	fn get() -> pezpallet_dap::BudgetAllocationMap {
...
fn legacy_to_dap_era_payout_e2e() {
	...
	ExtBuilder::default().local_queue().build().execute_with(|| {
		...
		pezpallet_dap::LastIssuanceTimestamp::<T>::kill();
		let pot_before_migration = Balances::total_balance(&general_staker);

		// Run the migration.
		use pezframe_support::traits::UncheckedOnRuntimeUpgrade;
		let _ = pezpallet_dap::migrations::InnerMigrateV1ToV2::<
			T,
			LastInflationTs,
			InitialBudget,
			pezframe_support::traits::ConstU64<{ 6 * 12_000 }>,
		>::on_runtime_upgrade();

		// Migration seeded the budget and anchored `LastIssuanceTimestamp` at `now`.
		assert!(!pezpallet_dap::BudgetAllocation::<T>::get().is_empty());
		assert_eq!(pezpallet_dap::LastIssuanceTimestamp::<T>::get(), before_migration);
		...
		let _ = staking_events_since_last_call();
		assert_ok!(staking_async::Pezpallet::<T>::payout_stakers(
		...
		pezpallet_dap::BudgetAllocation::<T>::put(build_budget(&[
