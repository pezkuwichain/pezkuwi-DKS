...
pub mod pezpallet_reward_point_filler {
	...
	pub mod pezpallet {
			...
			T::AccountId: From<[u8; 32]>,
		{
			fn on_initialize(_: BlockNumberFor<T>) -> Weight {
				let current =
					pezpallet_staking_async_ah_client::ValidatorPoints::<T>::iter().count() as u32;
				if let Some(deficit) = T::FillValidatorPointsTo::get().checked_sub(current) {
					for index in 0..deficit {
						...
						pezpallet_staking_async_ah_client::ValidatorPoints::<T>::insert(
...
impl_runtime_weights!(pezpallet_staking_async_rc_runtime_constants);
...
impl pezpallet_beefy::Config for Runtime {
	...
	type MaxAuthorities = MaxAuthorities;
	type MaxNominators = <Self as pezpallet_babe::Config>::MaxNominators;
...
impl pezsp_runtime::traits::Convert<AccountId, Option<AccountId>> for IdentityValidatorIdeOf {
...
impl<I: pezpallet_session::SessionManager<AccountId>> pezpallet_session::SessionManager<AccountId>
	for MaybeUsePreviousValidatorsElse<I>
{
	fn end_session(end_index: SessionIndex) {
		<I as pezpallet_session::SessionManager<_>>::end_session(end_index);
	}
	fn new_session(new_index: SessionIndex) -> Option<Vec<AccountId>> {
		...
		let actual_session_manager =
			<I as pezpallet_session::SessionManager<_>>::new_session(new_index);

		match actual_session_manager {
			Some(new_used) if !force_use_previous => Some(new_used),
			Some(_new_ignored) => {
				let current_validators = pezpallet_session::Validators::<Runtime>::get();
	...
	fn new_session_genesis(new_index: SessionIndex) -> Option<Vec<AccountId>> {
		<I as pezpallet_session::SessionManager<_>>::new_session_genesis(new_index)
	}
	fn start_session(start_index: SessionIndex) {
		<I as pezpallet_session::SessionManager<_>>::start_session(start_index);
	}
}
...
impl<S: Get<Perbill>> DisablingStrategy<Runtime> for AlwaysDisableForSlashGreaterThan<S> {
	fn decision(
		...
		let offender_index = pezpallet_session::Validators::<Runtime>::get()
			.iter()
...
impl pezpallet_staking_async_ah_client::Config for Runtime {
...
impl pezpallet_election_provider_multi_phase::Config for Runtime {
	...
	type SignedDepositBase = pezpallet_election_provider_multi_phase::GeometricDepositBase<
		Balance,
		SignedFixedDeposit,
...
parameter_types! {
	...
	pub const BondingDuration: pezsp_staking::EraIndex = 3;
...
parameter_types! {
	pub const BagThresholds: &'static [pezsp_npos_elections::VoteWeight] = &THRESHOLDS;
}
...
parameter_types! {
	...
	pub const FriendGroupsHoldReason: RuntimeHoldReason = RuntimeHoldReason::Recovery(pezpallet_recovery::HoldReason::FriendGroupsStorage);
	pub const AttemptHoldReason: RuntimeHoldReason = RuntimeHoldReason::Recovery(pezpallet_recovery::HoldReason::AttemptStorage);
	pub const InheritorHoldReason: RuntimeHoldReason = RuntimeHoldReason::Recovery(pezpallet_recovery::HoldReason::InheritorStorage);
}
	...
	fn filter(&self, c: &RuntimeCall) -> bool {
		match self {
			...
			ProxyType::NonTransfer => matches!(
				...
				RuntimeCall::Recovery(pezpallet_recovery::Call::control_inherited_account{..}) |
				RuntimeCall::Recovery(pezpallet_recovery::Call::initiate_attempt{..}) |
				RuntimeCall::Recovery(pezpallet_recovery::Call::approve_attempt{..}) |
				RuntimeCall::Recovery(pezpallet_recovery::Call::finish_attempt{..}) |
...
impl pezpallet_staking_async_preset_store::Config for Runtime {}
...
mod runtime {
	...
	// Teyrchains pezpallets. Start indices at 40 to leave room.
...
pezsp_api::impl_runtime_apis! {
	...
	impl pezkuwi_primitives::runtime_api::TeyrchainHost<Block> for Runtime {
		...
		fn max_relay_parent_session_age() -> u32 {
			teyrchains_staging_runtime_api_impl::max_relay_parent_session_age::<Runtime>()
		}
		...
		fn ancestor_relay_parent_info(
			...
			teyrchains_staging_runtime_api_impl::ancestor_relay_parent_info::<Runtime>(session_index, relay_parent)
		}
	}
	...
	impl pezframe_benchmarking::Benchmark<Block> for Runtime {
		...
		> {
			...
			// Trying to add benchmarks directly to some pezpallets caused cyclic dependency issues.
			...
			parameter_types! {
				...
				pub AssetHubParaId: ParaId = pezpallet_staking_async_rc_runtime_constants::system_teyrchain::ASSET_HUB_ID.into();
		...
		fn build_state(config: Vec<u8>) -> pezsp_genesis_builder::Result {
			...
			match PresetStore::preset().unwrap().as_str() {
				"real-m" => {
					...
					assert_eq!(
						pezpallet_session::Validators::<Runtime>::get().len(), 4,
				...
				"real-s" => {
					...
					assert_eq!(
						pezpallet_session::Validators::<Runtime>::get().len(), 2,
				...
				_ => {
					panic!("unrecognized min nominator bond in genesis config: {}",
						pezpallet_staking::MinNominatorBond::<Runtime>::get()
					);
				}
			}
			res
