...
//! # Staking Async Teyrchain Runtime
...
use xcm_config::{
	...
	TrustBackedAssetsPalletLocation, XcmOriginToTransactDispatchOrigin, ZagrosLocation,
};
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: alloc::borrow::Cow::Borrowed("staking-async-teyrchain"),
	impl_name: alloc::borrow::Cow::Borrowed("staking-async-teyrchain"),
	authoring_version: 1,
...
construct_runtime!(
	pub enum Runtime
	{
		...
		// Governance.
		Preimage: pezpallet_preimage = 90,
		Scheduler: pezpallet_scheduler = 91,
		ConvictionVoting: pezpallet_conviction_voting = 92,
		Referenda: pezpallet_referenda = 93,
		Origins: pezpallet_custom_origins = 94,
		Whitelist: pezpallet_whitelist = 95,
		Treasury: pezpallet_treasury = 96,
		AssetRate: pezpallet_asset_rate = 97,

		// Balances.
		Vesting: pezpallet_vesting = 100,
...
mod benches {
	pezframe_benchmarking::define_benchmarks!(
		...
		[pezpallet_election_provider_multi_block_verifier, MultiBlockElectionVerifier]
		[pezpallet_election_provider_multi_block_unsigned, MultiBlockElectionUnsigned]
		[pezpallet_election_provider_multi_block_signed, MultiBlockElectionSigned]
...
impl_runtime_apis! {
	...
	impl pezframe_support::view_functions::runtime_api::RuntimeViewFunction<Block> for Runtime {
		fn execute_view_function(
			...
			input: Vec<u8>,
		) -> Result<Vec<u8>, pezframe_support::view_functions::ViewFunctionDispatchError> {
	...
	impl xcm_runtime_pezapis::fees::XcmPaymentApi<Block> for Runtime {
		...
		fn query_weight_to_asset_fee(weight: Weight, asset: VersionedAssetId) -> Result<u128, XcmPaymentApiError> {
			let native_asset = xcm_config::ZagrosLocation::get();
	...
	impl pezpallet_staking_async_runtime_api::StakingApi<Block, Balance, AccountId> for Runtime {
		...
		fn dispatch_benchmark(
			...
			impl pezpallet_xcm::benchmarking::Config for Runtime {
				...
				fn reserve_transferable_asset_and_dest() -> Option<(Asset, Location)> {
					Some((
						...
						// AH can reserve transfer native token to some random teyrchain.
			...
			parameter_types! {
				...
				pub TrustedReserve: Option<(Location, Asset)> = Some(
					(
						...
						Asset::from((xcm_config::bridging::to_pezkuwichain::RocLocation::get(), 1000000000000 as u128))
					)
				);
			}
			...
			impl pezpallet_xcm_benchmarks::generic::Config for Runtime {
				...
				fn transact_origin_and_runtime_call() -> Result<(Location, RuntimeCall), BenchmarkError> {
					Ok((ZagrosLocation::get(), pezframe_system::Call::remark_with_event { remark: vec![] }.into()))
				}
		...
		fn build_state(config: Vec<u8>) -> pezsp_genesis_builder::Result {
			...
			match PresetStore::preset().unwrap().as_str() {
				...
				"fake-hez" => {
					...
					crate::staking::enable_hez_preset(true);
				},
				_ => {
