...
use xcm_builder::{
	...
	FungibleAdapter, FungiblesAdapter, GlobalConsensusTeyrchainConvertsFor, HashedDescription,
...
pub type LocationToAccountId = (
	...
	// Different global consensus teyrchain sovereign account.
...
impl xcm_executor::Config for XcmConfig {
	...
	type Weigher = WeightInfoBounds<
		crate::weights::xcm::AssetHubNextZagrosXcmWeight<RuntimeCall>,
		RuntimeCall,
		MaxInstructions,
	>;
	type Trader = (
...
parameter_types! {
	...
	pub const AuthorizeAliasHoldReason: crate::RuntimeHoldReason = crate::RuntimeHoldReason::PezkuwiXcm(pezpallet_xcm::HoldReason::AuthorizeAlias);
}
...
pub mod bridging {
	...
	pub mod to_ethereum {
		...
		parameter_types! {
			...
			/// Set up exporters configuration.
			/// `Option<Asset>` represents static "base fee" which is used for total delivery fee calculation.
			pub BridgeTable: pezsp_std::vec::Vec<NetworkExportTableItem> = pezsp_std::vec![
				NetworkExportTableItem::new(
					EthereumNetwork::get().into(),
					Some(pezsp_std::vec![Junctions::Here]),
					SiblingBridgeHub::get(),
					Some((
						XcmBridgeHubRouterFeeAssetId::get(),
			...
			pub EthereumBridgeTable: pezsp_std::vec::Vec<NetworkExportTableItem> = pezsp_std::vec::Vec::new().into_iter()
