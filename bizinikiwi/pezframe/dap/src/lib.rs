...
//! # Dynamic Allocation Pool (DAP) Pezpallet
//!
...
//!   [`pezsp_staking::budget::BudgetRecipient`]s according to a governance-updatable
...
use pezsp_runtime::{traits::Zero, BoundedBTreeMap, Perbill, SaturatedConversion, Saturating};
use pezsp_staking::budget::{BudgetKey, BudgetRecipientList, IssuanceCurve};

pub use pezpallet::*;

pub use pezsp_dap::DAP_PALLET_ID;
...
pub mod pezpallet {
	...
	use pezframe_support::{pezsp_runtime::traits::AccountIdConversion, traits::StorageVersion};
	...
	pub trait Config: pezframe_system::Config<RuntimeEvent: From<Event<Self>>> {
		...
		/// The pezpallet ID used to derive the buffer account.
		#[pezpallet::constant]
		...
		/// Time provider (typically `pezpallet_timestamp`).
		///
	...
	impl<T: Config> Pezpallet<T> {
		...
		#[pezpallet::weight(T::WeightInfo::set_budget_allocation())]
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn staging_account() -> T::AccountId {
			pezsp_dap::DAP_PALLET_ID.into_sub_account_truncating(pezsp_dap::DAP_STAGING_ACCOUNT_ID)
		}
	...
	impl<T: Config> Pezpallet<T> {
		...
		fn check_budget_allocation() -> Result<(), pezsp_runtime::TryRuntimeError> {
			let allocation = BudgetAllocation::<T>::get();
...
///
/// For pezpallets still using the legacy `Currency` trait (e.g. `pezpallet_referenda`),
...
/// # Example
/// ```ignore
/// type Slash = pezpallet_dap::DapLegacyAdapter<Runtime, Balances>;
/// ```
	...
	C: Currency<T::AccountId, Balance = BalanceOf<T>>,
{
	fn on_nonzero_unbalanced(amount: LegacyNegativeImbalance<T::AccountId, C>) {
		let staging = Pezpallet::<T>::staging_account();
		let numeric_amount = amount.peek();
...
impl<T: Config> pezsp_staking::budget::BudgetRecipient<T::AccountId> for Pezpallet<T> {
