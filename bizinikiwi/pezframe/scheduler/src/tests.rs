...
fn scheduling_with_preimages_works() {
	new_test_ext().execute_with(|| {
		...
		let hash = <Test as pezframe_system::Config>::Hashing::hash_of(&call);
...
fn schedule_retry_fails_when_retry_target_block_is_full(named: bool) {
	...
	new_test_ext().execute_with(|| {
		...
		let events = pezframe_system::Pezpallet::<Test>::events();
		let retry_failed_event: <Test as pezframe_system::Config>::RuntimeEvent =
			Event::RetryFailed { task: (4, 0), id: None }.into();
		assert!(
...
fn scheduler_v3_anon_basic_works() {
	use pezframe_support::traits::schedule::v3::Anon;
	new_test_ext().execute_with(|| {
		let call =
			RuntimeCall::Logger(LoggerCall::log { i: 42, weight: Weight::from_parts(10, 0) });
...
fn scheduler_v3_named_basic_works() {
	use pezframe_support::traits::schedule::v3::Named;
...
fn scheduler_v3_named_cancel_without_name_works() {
	use pezframe_support::traits::schedule::v3::{Anon, Named};
	new_test_ext().execute_with(|| {
		let call =
			RuntimeCall::Logger(LoggerCall::log { i: 42, weight: Weight::from_parts(10, 0) });
		let bound = Preimage::bound(call).unwrap();
...
fn postponed_task_is_still_available() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Scheduler::schedule(
			...
			None,
			128,
			Box::new(RuntimeCall::from(pezframe_system::Call::remark {
