...
//! `pezframe_system::Config::BaseCallFilter` for all origin types (except root which will get no
//! filter).
//!
...
//!       index.
//!
//! [`on_initialize`]: pezframe_support::traits::Hooks::on_initialize
...
pub type CallOrHashOf<T> =
	MaybeHashed<<T as Config>::RuntimeCall, <T as pezframe_system::Config>::Hash>;

pub type BoundedCallOf<T> =
	Bounded<<T as Config>::RuntimeCall, <T as pezframe_system::Config>::Hashing>;
...
impl<T: Config> Pezpallet<T> {
	...
	pub fn migrate_v1_to_v4() -> Weight {
		...
		#[allow(deprecated)]
		pezframe_support::storage::migration::remove_storage_prefix(
			Self::name().as_bytes(),
			b"StorageVersion",
			&[],
		);
