...
use pezpallet_delegated_staking::Event as DelegatedStakingEvent;

use pezsp_runtime::{bounded_btree_map, traits::Zero, Perbill};
use pezsp_staking::{Agent, StakingInterface};

/// Helper to assert the current active era.
fn assert_active_era(expected: u32) {
	assert_eq!(
		pezpallet_staking_async::ActiveEra::<Runtime>::get().map(|e| e.index),
		Some(expected)
	);
}
...
fn pool_chill_e2e() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Staking::set_staking_configs(
			RuntimeOrigin::root(),
			pezpallet_staking_async::ConfigOp::Set(55), // minimum nominator bond
...
fn pool_slash_e2e() {
	new_test_ext().execute_with(|| {
		...
		pezpallet_staking_async::slashing::do_slash::<Runtime>(
			&POOL1_BONDED,
			30,
			&mut Default::default(),
			&mut Default::default(),
...
fn pool_members_not_slashed_when_nominators_not_slashable() {
	new_test_ext().execute_with(|| {
		...
		let pool_stake_before =
			pezpallet_staking_async::Ledger::<Runtime>::get(&POOL1_BONDED).unwrap().active;
