...
use pezframe_system::RawOrigin as RuntimeOrigin;
...
type CurrencyOf<T> = <T as pezpallet_nomination_pools::Config>::Currency;
...
// balance.
fn create_pool_account<T: pezpallet_nomination_pools::Config>(
	n: u32,
	balance: BalanceOf<T>,
	...
	if let Some(c) = commission {
		let pool_id = pezpallet_nomination_pools::LastPoolId::<T>::get();
	...
	let pool_account = pezpallet_nomination_pools::BondedPools::<T>::iter()
...
fn migrate_to_transfer_stake<T: Config>(pool_id: PoolId) {
	...
	PoolMembers::<T>::iter()
		...
		.for_each(|(member_acc, member)| {
			...
			<T as pezpallet_nomination_pools::Config>::Currency::transfer(
				&member_acc,
				&pool_acc,
				member_balance,
...
#[allow(unused)]
struct ListScenario<T: pezpallet_nomination_pools::Config> {
...
impl<T: Config> ListScenario<T> {
	...
	pub(crate) fn new(
		...
		is_increase: bool,
			...
			pezpallet_nomination_pools::MaxPools::<T>::get().unwrap_or(0) >= 3,
...
mod benchmarks {
	...
	fn withdraw_unbonded_kill(s: Linear<0, MAX_SPANS>) {
		...
		// up when unbonding.
		let reward_account = Pools::<T>::generate_reward_account(1);
		assert!(pezframe_system::Account::<T>::contains_key(&reward_account));
		...
		assert!(!RewardPools::<T>::contains_key(&1));
		assert!(!PoolMembers::<T>::contains_key(&depositor));
		assert!(!pezframe_system::Account::<T>::contains_key(&pool_account));
		assert!(!pezframe_system::Account::<T>::contains_key(&reward_account));
	...
	#[benchmark]
	fn set_metadata(
		n: Linear<1, { <T as pezpallet_nomination_pools::Config>::MaxMetadataLen::get() }>,
	) {
		// Create a pool
		let (depositor, _pool_account) =
	...
	#[benchmark]
	fn update_roles() {
		let first_id = pezpallet_nomination_pools::LastPoolId::<T>::get() + 1;
		let (root, _) =
			create_pool_account::<T>(0, Pools::<T>::depositor_min_bond() * 2u32.into(), None);
		...
		assert_eq!(
			pezpallet_nomination_pools::BondedPools::<T>::get(first_id).unwrap().roles,
			pezpallet_nomination_pools::PoolRoles {
				depositor: root,
