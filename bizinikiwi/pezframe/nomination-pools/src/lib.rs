...
//! [pezsp_staking::StakingInterface::bonding_duration]`, it can can cash it's points out of the
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		/// types similar to that of Pezkuwi and Dicle, in the absence of severe slashing (or
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn update_roles(
			...
			let mut bonded_pool = match ensure_root(origin.clone()) {
				...
				Err(pezsp_runtime::traits::BadOrigin) => {
					let who = ensure_signed(origin)?;
					let bonded_pool =
...
impl<T: Config> Pezpallet<T> {
	...
	pub fn fully_unbond(
		origin: pezframe_system::pezpallet_prelude::OriginFor<T>,
		member: T::AccountId,
	) -> DispatchResult {
