...
use crate as pezpallet_membership;
use crate::{mock::*, *};

use pezsp_runtime::{bounded_vec, traits::BadOrigin, BuildStorage};

use pezframe_support::{assert_noop, assert_ok, assert_storage_noop, traits::StorageVersion};
...
fn genesis_build_panics_with_duplicate_members() {
	pezpallet_membership::GenesisConfig::<Test> {
...
fn migration_v4() {
	new_test_ext().execute_with(|| {
		...
		let old_pezpallet_name = "OldMembership";
		let new_pezpallet_name =
			<Test as pezframe_system::Config>::PalletInfo::name::<Membership>().unwrap();
		...
		crate::migrations::v4::pre_migrate::<Membership, _>(old_pezpallet_name, new_pezpallet_name);
		crate::migrations::v4::migrate::<Test, Membership, _>(
			old_pezpallet_name,
			new_pezpallet_name,
		);
		crate::migrations::v4::post_migrate::<Membership, _>(
			old_pezpallet_name,
			new_pezpallet_name,
		);
	});
