...
//! Tests for pezpallet-fast-unstake.
...
use pezpallet_staking::{CurrentEra, RewardDestination};
...
mod on_idle {
	...
	fn validators_cannot_bail() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert!(pezpallet_staking::EraInfo::<T>::get_paged_exposure(
...
fn dicle_estimate() {
	...
	.ref_time() as f32;
	let on_idle = crate::weights::BizinikiwiWeight::<T>::on_idle_check(1000, 64).ref_time() as f32;
...
fn pezkuwi_estimate() {
	...
	.ref_time() as f32;
	let on_idle = crate::weights::BizinikiwiWeight::<T>::on_idle_check(300, 64).ref_time() as f32;
	dbg!(block_time, on_idle, on_idle / block_time);
}
