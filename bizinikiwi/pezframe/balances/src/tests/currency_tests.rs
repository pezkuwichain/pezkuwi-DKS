...
use pezframe_system::Event as SysEvent;
use pezsp_runtime::traits::DispatchTransaction;

const ID_1: LockIdentifier = *b"1       ";
const ID_2: LockIdentifier = *b"2       ";

pub const CALL: &<Test as pezframe_system::Config>::RuntimeCall =
...
fn account_should_be_reaped() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			assert!(!pezframe_system::Account::<Test>::contains_key(&1));
		});
}
...
fn reap_failed_due_to_provider_and_consumer() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			assert_eq!(System::inc_providers(&1), pezframe_system::IncRefStatus::Existed);
...
fn lock_should_work_reserve() {
	ExtBuilder::default()
		...
		.monied(true)
		.build_and_execute_with(|| {
			pezpallet_transaction_payment::NextFeeMultiplier::<Test>::put(
...
fn reward_should_work() {
	ExtBuilder::default().monied(true).build_and_execute_with(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 120);
	});
}
...
fn slashing_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(1), 69);
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 70);
	});
}
...
fn withdrawing_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 100);
	});
}
...
fn slashing_incomplete_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 22);
	});
}
...
fn slashing_incomplete_reserved_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(1), 0);
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 69);
	});
}
...
fn account_removal_on_free_too_low() {
	ExtBuilder::default().existential_deposit(100).build_and_execute_with(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 220);
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), 130);
	});
}
...
fn burn_must_work() {
	ExtBuilder::default().monied(true).build_and_execute_with(|| {
		...
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), init_total_issuance - 10);
		drop(imbalance);
		assert_eq!(pezpallet_balances::TotalIssuance::<Test>::get(), init_total_issuance);
	});
}
...
fn operations_on_dead_account_should_not_change_state() {
	...
	ExtBuilder::default().existential_deposit(1).build_and_execute_with(|| {
		assert!(!pezframe_system::Account::<Test>::contains_key(&1337));
