...
use pezkuwi_subxt::backend::legacy::rpc_methods::TransactionStatus;
use pezkuwi_subxt_signer::bip39::core::pin::Pin;
...
impl EthRpcServer for EthRpcServerImpl {
	...
	async fn send_raw_transaction(&self, transaction: Bytes) -> RpcResult<H256> {
		...
		let call = pezkuwi_subxt_client::tx().revive().eth_transact(transaction.0);
