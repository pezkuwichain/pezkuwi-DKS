...
use crate::{
	...
	client::{BizinikiwiBlock, BizinikiwiBlockNumber, runtime_api::RuntimeApi},
...
type EventDetails = pezkuwi_subxt::events::EventDetails<SrcChainConfig>;

/// Outcome of decoding a single pezpallet-revive event.
enum ReviveEvent {
	Revert,
	Log(Log),
}

/// Decode a single event detail into a [`ReviveEvent`], or `None` if it is not a pezpallet-revive
/// event we care about.
fn decode_revive_event(
	...
	if event.pezpallet_name() != ContractEmitted::PALLET {
		return None;
	}
...
impl ReceiptExtractor {
	...
	pub fn is_before_first_evm_block(&self, block_number: BizinikiwiBlockNumber) -> bool {
	...
	pub fn set_first_evm_block(&self, block_number: BizinikiwiBlockNumber) {
	...
	pub fn first_evm_block(&self) -> Option<BizinikiwiBlockNumber> {
		let val = self.first_evm_block.load(Ordering::Acquire);
...
mod tests {
	...
	use pezpallet_revive::evm::{Account, TransactionLegacyUnsigned, TransactionUnsigned};
	...
	impl EventsBuilder {
		...
		fn push_event(
			mut self,
			phase: pezframe_system::Phase,
			event: pezpallet_revive::Event<Runtime>,
		) -> Self {
	...
	fn extract_revive_events_decodes_contract_emitted_log() {
		...
		let events = EventsBuilder::new()
			.push_event(
				pezframe_system::Phase::ApplyExtrinsic(5),
				pezpallet_revive::Event::ContractEmitted {
					contract,
					data: data.clone(),
					topics: topics.clone(),
				},
	...
	fn extract_revive_events_skips_irrelevant_events() {
		...
		let empty_contract_emitted = pezpallet_revive::Event::ContractEmitted {
			contract: H160::zero(),
			data: vec![],
			topics: vec![],
		};
		let revert = pezpallet_revive::Event::EthExtrinsicRevert {
			dispatch_error: pezsp_runtime::DispatchError::Other("skipped-phase revert"),
		};
		let events = EventsBuilder::new()
			.push_event(pezframe_system::Phase::Finalization, empty_contract_emitted.clone())
			.push_event(pezframe_system::Phase::Initialization, revert.clone())
			.push_event(pezframe_system::Phase::ApplyExtrinsic(5), empty_contract_emitted)
			.push_event(pezframe_system::Phase::ApplyExtrinsic(5), revert)
			.build();
	...
	fn extract_revive_events_accumulates_per_extrinsic() {
		...
		let emitted_by = |contract: H160| pezpallet_revive::Event::ContractEmitted {
			contract,
			data: vec![],
			topics: vec![],
		};
		let events = EventsBuilder::new()
			.push_event(
				pezframe_system::Phase::ApplyExtrinsic(0),
				emitted_by(H160::from([0xaa; 20])),
			)
			.push_event(
				pezframe_system::Phase::ApplyExtrinsic(0),
				emitted_by(H160::from([0xbb; 20])),
			)
			.push_event(
				pezframe_system::Phase::ApplyExtrinsic(1),
				pezpallet_revive::Event::EthExtrinsicRevert {
					dispatch_error: pezsp_runtime::DispatchError::Other("tx-1 revert"),
				},
			)
			.push_event(
				pezframe_system::Phase::ApplyExtrinsic(2),
				emitted_by(H160::from([0xcc; 20])),
			)
			.build();
