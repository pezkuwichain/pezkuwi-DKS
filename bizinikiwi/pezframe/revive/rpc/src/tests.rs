...
use crate::{
	...
	pezkuwi_subxt_client::{
		self, SrcChainConfig, src_chain::runtime_types::pezpallet_revive::primitives::Code,
	},
};
...
use pezkuwi_subxt_signer::eth::Keypair;
...
use pezpallet_revive_fixtures::{Callee, Counter, TwoSlots};
...
impl SharedResources {
	...
	fn provider() -> impl Provider {
		let secret_key = pezkuwi_subxt_signer::eth::dev::alith().secret_key();
...
async fn test_receipt_mixed_revert_and_logs_same_block() -> anyhow::Result<()> {
	...
	let deploy = |name, fixture_type| {
		...
		async move {
			let (bytes, _) =
				pezpallet_revive_fixtures::compile_module_with_type(name, fixture_type)?;
	...
	let revert_contract =
		deploy("ok_trap_revert", pezpallet_revive_fixtures::FixtureType::Rust).await?;
	let event_contract = deploy("MultiEvent", pezpallet_revive_fixtures::FixtureType::Solc).await?;
...
async fn test_runtime_api_dry_run_addr_works() -> anyhow::Result<()> {
	...
	let payload = pezkuwi_subxt_client::apis().revive_api().instantiate(
...
async fn get_evm_block_from_storage(
	...
	// `StorageValue` borrows the entry, so the entry needs a binding of its own.
	let storage_at = node_client.storage().at(block_hash);
	let entry = storage_at.entry(query)?;
	let Ok(block) = entry.fetch().await else {
...
async fn test_subscribe_logs() -> anyhow::Result<()> {
	...
	let (bytes, _) = pezpallet_revive_fixtures::compile_module_with_type(
		"SimpleReceiver",
	...
	// Assert
	let event_signature = H256(pezsp_io::hashing::keccak_256(b"Received(address,uint256)"));
...
async fn test_estimate_gas_of_contract_with_consume_all_gas() -> anyhow::Result<()> {
	// Arrange
	let code = pezpallet_revive_fixtures::compile_module_with_type(
		"ContractWithConsumeAllGas",
		pezpallet_revive_fixtures::FixtureType::Resolc,
	)?
	.0;
	let client = Arc::new(SharedResources::client().await);
	let account = Account::default();
...
/// Test that deploys and calls the Fibonacci contract via Bizinikiwi APIs works
async fn test_fibonacci_call_via_runtime_api() -> anyhow::Result<()> {
	use pezpallet_revive::precompiles::alloy::sol_types::SolCall;
	use pezpallet_revive_fixtures::Fibonacci;
	...
	// Deploy the Fibonacci contract via Bizinikiwi API
	log::trace!(target: LOG_TARGET, "Deploying Fibonacci contract via Substrate API");
	let dry_run_result = node_client
		.runtime_api()
		.at_latest()
		.await?
		.call(pezkuwi_subxt_client::apis().revive_api().instantiate(
			pezkuwi_subxt::utils::AccountId32(origin),
			0u128, // value
			None,  // gas_limit
			None,  // storage_deposit_limit
			pezkuwi_subxt_client::src_chain::runtime_types::pezpallet_revive::primitives::Code::Upload(
				bytes.clone(),
			),
	...
	let events = node_client
		...
		.sign_and_submit_then_watch_default(
			&pezkuwi_subxt_client::tx().revive().instantiate_with_code(
				0u128,                   // value
			...
			&pezkuwi_subxt_signer::sr25519::dev::alice(),
		)
		.await?
	...
	let instantiated_event = events
		.find_first::<pezkuwi_subxt_client::revive::events::Instantiated>()?
	...
	let call_payload = pezkuwi_subxt_client::apis().revive_api().call(
		pezkuwi_subxt::utils::AccountId32(origin),
		contract_address,
		0u128, // value
		None,  // gas_limit
...
-> anyhow::Result<(Client, mpsc::Receiver<GapFillRequest>)> {
	use pezsc_cli::{RPC_DEFAULT_MAX_REQUEST_SIZE_MB, RPC_DEFAULT_MAX_RESPONSE_SIZE_MB};
...
async fn test_state_override_code_empty_to_evm() -> anyhow::Result<()> {
	...
	let (code, _) = pezpallet_revive_fixtures::compile_module_with_type(
		"Callee",
		pezpallet_revive_fixtures::FixtureType::SolcRuntime,
	)?;
...
async fn deploy_contract(
	...
	fixture_name: &str,
	fixture_type: pezpallet_revive_fixtures::FixtureType,
	constructor_args: &[u8],
) -> anyhow::Result<AlloyAddress> {
	let (bytecode, _) =
		pezpallet_revive_fixtures::compile_module_with_type(fixture_name, fixture_type)?;
...
async fn test_state_override_code_evm_to_evm() -> anyhow::Result<()> {
	...
	let from = AlloyAddress::from(Account::default().address().0);
	let target =
		deploy_contract(&provider, "Counter", pezpallet_revive_fixtures::FixtureType::Solc, &[])
			.await?;
...
async fn test_state_override_code_pvm_to_evm() -> anyhow::Result<()> {
	...
	let from = AlloyAddress::from(Account::default().address().0);
	let target =
		deploy_contract(&provider, "Counter", pezpallet_revive_fixtures::FixtureType::Resolc, &[])
			.await?;
