...
use crate::{
	...
	pezkuwi_subxt_client::{self, SrcChainConfig, revive::calls::types::EthTransact},
};
...
impl From<TransactionStatus<BizinikiwiBlockHash>> for SubmitError {
	fn from(status: TransactionStatus<BizinikiwiBlockHash>) -> Self {
		match status {
...
pub(crate) struct GapFillRequest {
	pub from_inclusive: BizinikiwiBlockNumber,
	pub to_inclusive: BizinikiwiBlockNumber,
}
...
impl SubscriptionGapQueue {
	...
	pub fn detect_and_queue(&self, current: BizinikiwiBlockNumber, last: BizinikiwiBlockNumber) {
...
async fn chain_id(api: &OnlineClient<SrcChainConfig>) -> Result<u64, ClientError> {
	let query = pezkuwi_subxt_client::constants().revive().chain_id().unvalidated();
...
async fn max_block_weight(api: &OnlineClient<SrcChainConfig>) -> Result<Weight, ClientError> {
	let query = pezkuwi_subxt_client::constants().system().block_weights().unvalidated();
...
impl Client {
	...
	async fn advance_sync_head(&self, block_number: BizinikiwiBlockNumber, hash: H256) {
		if !self.is_archive
		...
		Fut: std::future::Future<Output = Result<(), ClientError>> + Send,
	{
		...
		let mut last_finalized_seen: Option<BizinikiwiBlockNumber> = None;
	...
	) -> Result<StreamOfResults<TransactionStatus<BizinikiwiBlockHash>>, ClientError> {
	...
	pub async fn submit(
		&self,
		call: pezkuwi_subxt::tx::DefaultPayload<EthTransact>,
	) -> Result<TransactionStatus<BizinikiwiBlockHash>, ClientError> {
	...
	> {
		let signed_block: Option<
			pezsp_runtime::generic::SignedBlock<
				pezsp_runtime::generic::Block<
					pezsp_runtime::generic::Header<u32, pezsp_runtime::traits::BlakeTwo256>,
					pezsp_runtime::OpaqueExtrinsic,
				>,
