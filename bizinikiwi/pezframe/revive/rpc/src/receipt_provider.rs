...
/// This is an abstraction on top of [`BizinikiwiBlock`] that can't be mocked in tests.
...
impl<B: BlockInfoProvider> ReceiptProvider<B> {
	...
	async fn restore_first_evm_block(&self) -> Result<(), ClientError> {
		...
		let has_evm_hash = |block_number: BizinikiwiBlockNumber| async move {
...
mod tests {
	...
	use pezpallet_revive::evm::{BlockTag, ReceiptInfo, TransactionSigned};
	...
	async fn is_before_earliest_block_edge_cases() {
		...
		assert!(!provider.is_before_earliest_block(&BlockNumberOrTag::BlockTag(
			pezpallet_revive::evm::BlockTag::Latest
		)));
	}
