...
use crate::{
	BlockInfoProvider,
	client::{BizinikiwiBlockNumber, Client, ClientError, GapFillRequest},
};
use pezpallet_revive::evm::H256;
use tokio::sync::mpsc;
...
pub struct SyncCheckpoint {
	pub block_number: BizinikiwiBlockNumber,
	pub block_hash: Option<H256>,
}

impl SyncCheckpoint {
	/// Create a checkpoint with a known block hash.
	pub fn new(block_number: BizinikiwiBlockNumber, block_hash: H256) -> Self {
		Self { block_number, block_hash: Some(block_hash) }
	}

	/// Create a checkpoint with only a block number (no hash).
	pub fn from_number(block_number: BizinikiwiBlockNumber) -> Self {
...
struct BackwardSyncRange {
	from: BizinikiwiBlockNumber,
	to: BizinikiwiBlockNumber,
...
impl Client {
	...
	async fn sync_backward_fresh(
		&self,
		latest_finalized: BizinikiwiBlockNumber,
	) -> Result<(), ClientError> {
	...
	async fn sync_backward_range(
		...
		let mut last_synced: Option<(BizinikiwiBlockNumber, H256)> = None;
		let at_checkpoint =
		...
		let loop_result: Result<(), ClientError> = loop {
			...
			let ethereum_hash = match self
				...
				.eth_block_hash(pezpallet_revive::evm::U256::from(block_number))
				.await
			{
				Ok(h) => h,
				Err(err) => {
