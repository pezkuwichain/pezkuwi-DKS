...
use crate::{
	...
	Pezpallet as Contracts, RuntimeCosts, TrieId,
...
use pezsp_io::{crypto::secp256k1_ecdsa_recover_compressed, hashing::blake2_256};
use pezsp_runtime::{
...
pub trait PrecompileExt: sealing::Sealed {
	...
	fn charge(&mut self, weight: Weight) -> Result<ChargedAmount, DispatchError> {
		self.pezframe_meter_mut().charge_weight_token(RuntimeCosts::Precompile(weight))
	}
	...
	fn charge_or_halt<Tok: Token<Self::T>>(
		...
		self.pezframe_meter_mut().charge_or_halt(token)
	}
	...
	#[deprecated(
		note = "Renamed to `pezframe_meter`; this alias will be removed in future versions"
	)]
	fn gas_meter(&self) -> &FrameMeter<Self::T>;

	/// Get a mutable reference to the nested resource meter of the frame.
	#[deprecated(
		note = "Renamed to `pezframe_meter_mut`; this alias will be removed in future versions"
	)]
	fn gas_meter_mut(&mut self) -> &mut FrameMeter<Self::T>;

	/// Get an immutable reference to the nested resource meter of the frame.
	fn pezframe_meter(&self) -> &FrameMeter<Self::T>;

	/// Get a mutable reference to the nested resource meter of the frame.
	fn pezframe_meter_mut(&mut self) -> &mut FrameMeter<Self::T>;
	...
	E: Executable<T>,
{
	...
	fn new_frame<S: State>(
		...
		let (account_id, contract_info, executable, delegate, entry_point) = match pezframe_args {
		...
		let frame = Frame {
			...
			account_id,
			entry_point,
			pezframe_meter: meter.new_nested(call_resources)?,
			allows_reentry: true,
			read_only,
	...
	fn push_frame(
		...
		let meter = &mut frame.pezframe_meter;
		...
		let do_transaction = || -> ExecResult {
			...
			let frame = if entry_point == ExportedFunction::Constructor {
				...
				if !is_pvm {
					...
					// deposit to: use the pezpallet's own account as a sentinel owner
					...
					let mut module = match &self.origin {
						...
						Origin::Root => {
							crate::ContractBlob::<T>::from_evm_runtime_code_with_deposit(
								data,
								crate::Pezpallet::<T>::account_id(),
								Zero::zero(),
							)?
						},
					};
					module.store_code(&self.exec_config, &mut frame.pezframe_meter)?;
				...
				frame.pezframe_meter.charge_contract_deposit_and_transfer(
			...
			Ok((success, output)) => {
				if_tracing(|tracer| {
					...
					let gas_consumed = if is_first_frame {
						...
						pezframe_meter.eth_gas_consumed()
					};

					let gas_consumed: u64 = gas_consumed.try_into().unwrap_or(u64::MAX);
					let weight_consumed = pezframe_meter.weight_consumed();
	...
	fn pop_frame(&mut self, persist: bool) {
		...
		if let Some(mut frame) = frame {
			...
			if !persist {
				prev.pezframe_meter.absorb_weight_meter_only(frame.pezframe_meter);
				return;
			}
			...
			prev.pezframe_meter.absorb_all_meters(
				frame.pezframe_meter,
				account_id,
				contract.as_mut(),
			);
			...
			if !persist {
				self.transaction_meter
					.absorb_weight_meter_only(mem::take(&mut self.first_frame.pezframe_meter));
				return;
			}

			let mut contract = self.first_frame.contract_info.as_contract();
			self.transaction_meter.absorb_all_meters(
				mem::take(&mut self.first_frame.pezframe_meter),
	...
	fn do_terminate(
		...
		// If root created this contract we need to use the pezpallet account_id because root has no
		// account.
		let origin: Origin<T> = match origin {
			Origin::Signed(o) => Origin::Signed(o.clone()),
			Origin::Root => Origin::from_account_id(crate::Pezpallet::<T>::account_id()),
		};
	...
	E: Executable<T>,
{
	fn delegate_call(
		...
		*self.last_frame_output_mut() = Default::default();
		...
		top_frame.pezframe_meter.apply_pending_storage_changes(&mut contract_info);
		...
		*self.last_frame_output_mut() = Default::default();
		...
		let executable = {
			let executable = match &mut code {
				...
				Code::Existing(hash) => {
					let executable = E::from_storage(*hash, self.pezframe_meter_mut())?;
	...
	E: Executable<T>,
{
		...
		*self.last_frame_output_mut() = Default::default();
		...
		let try_call = || {
			...
			let cached_info = self
				...
				.and_then(|f| match &f.contract_info {
					CachedContract::Cached(contract) => {
						...
						f.pezframe_meter.apply_pending_storage_changes(&mut contract_with_pending);
			...
			if let Some(executable) = self.push_frame(
				...
				let snapshot = if_tracing(|_| top_frame!(self).pezframe_meter.snapshot());
				...
				if_tracing(|t| {
					...
					let (gas_used, weight_delta) =
						top_frame!(self).pezframe_meter.delta_since(snapshot);
					match result {
	...
	fn code_hash(&self, address: &H160) -> H256 {
		if let Some(code) = <AllPrecompiles<T>>::code(address.as_fixed_bytes()).or_else(|| {
			...
			return pezsp_io::hashing::keccak_256(code).into();
		}
	...
	fn gas_meter(&self) -> &FrameMeter<Self::T> {
		&self.top_frame().pezframe_meter
	}

	#[inline]
	fn gas_meter_mut(&mut self) -> &mut FrameMeter<Self::T> {
		&mut self.top_frame_mut().pezframe_meter
	}
	...
	fn terminate_caller(&mut self, beneficiary: &H160) -> Result<(), DispatchError> {
		...
		Self::transfer(
			...
			<Contracts<T>>::evm_balance(&contract_address),
			Preservation::Preserve,
			&mut top_frame_mut!(self).pezframe_meter,
			&self.exec_config,
		)?;
	...
	fn gas_left(&self) -> u64 {
		...
		frame.pezframe_meter.eth_gas_left().unwrap_or_default().saturated_into::<u64>()
	}
	...
	fn charge_storage(&mut self, diff: &Diff) -> DispatchResult {
		assert!(self.has_contract_info());
		self.top_frame_mut().pezframe_meter.record_contract_storage_changes(diff)
	}
}
