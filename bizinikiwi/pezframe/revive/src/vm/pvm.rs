...
use crate::{
	Code, Config, Error, LOG_TARGET, Pezpallet, ReentrancyProtection, RuntimeCosts, SENTINEL,
...
use pezpallet_revive_uapi::{CallFlags, ReturnErrorCode, ReturnFlags, StorageFlags};
...
macro_rules! charge_gas {
	($runtime:expr, $costs:expr) => {{ $runtime.ext.pezframe_meter_mut().charge_weight_token($costs) }};
}
...
impl<'a, E: Ext, M: ?Sized + Memory<E::T>> Runtime<'a, E, M> {
	...
	fn adjust_gas(&mut self, charged: ChargedAmount, actual_costs: RuntimeCosts) {
		self.ext.pezframe_meter_mut().adjust_weight(charged, actual_costs);
	}
...
impl<'a, E: Ext, M: ?Sized + Memory<E::T>> FrameTraceInfo for Runtime<'a, E, M> {
	fn gas_left(&self) -> u64 {
		let meter = self.ext.pezframe_meter();
...
impl<'a, E: Ext> PreparedCall<'a, E> {
	pub fn call(mut self) -> ExecResult {
		...
		let sync_result =
			self.runtime.ext().pezframe_meter_mut().sync_from_executor(self.instance.gas());
