...
//! keyed by the pezpallet's own account (the holder of that deposit). The native currency itself
//! stays where it is.
//!
...
impl<T: Config> SteppedMigration for Migration<T> {
	...
	fn id() -> Self::Identifier {
		MigrationId { pezpallet_id: *PALLET_MIGRATIONS_ID, version_from: 3, version_to: 4 }
	}
	...
	#[cfg(feature = "try-runtime")]
	fn post_upgrade(prev: Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		use crate::deposit_payment::Deposit;
		...
		for (owner, expected) in per_owner {
			let got = NativeDepositOf::<T>::get(&pezpallet_account, &owner);
			assert_eq!(
				got, expected,
				"v4: NativeDepositOf[pezpallet][{owner:?}] = {got:?}, expected {expected:?}",
			);
		}
...
pub(crate) mod old {
	...
	#[storage_alias]
	pub(crate) type DeletionQueue<T: Config> = StorageMap<Pezpallet<T>, Twox64Concat, u32, TrieId>;
}
...
impl<T: Config> Migration<T> {
	...
	fn step_1_code_upload(last: Option<H256>) -> Option<Cursor> {
		...
		let pezpallet_account = Pezpallet::<T>::account_id();
		NativeDepositOf::<T>::mutate(&pezpallet_account, info.owner(), |entitlement| {
...
mod tests {
	...
	use pezframe_support::traits::fungible::{
	...
	fn seed_code_upload(hash: H256, owner: AccountId32, deposit: u128) {
		let pezpallet_account = Pezpallet::<Test>::account_id();
		let ed = <Test as Config>::Currency::minimum_balance();
		<Test as Config>::Currency::mint_into(&pezpallet_account, ed).unwrap();
		<Test as Config>::Currency::mint_into(&pezpallet_account, deposit).unwrap();
	...
	fn phase_one_populates_native_deposit_for_code_upload() {
		ExtBuilder::default().genesis_config(None).build().execute_with(|| {
			...
			assert_eq!(
				NativeDepositOf::<Test>::get(&pezpallet_account, &owner_a),
				1_500,
				"owner_a sum of code deposits"
			);
			assert_eq!(
				NativeDepositOf::<Test>::get(&pezpallet_account, &owner_b),
				2_000,
	...
	fn phase_two_burns_native_and_mints_pgas_on_contracts() {
		ExtBuilder::default().genesis_config(None).build().execute_with(|| {
			...
			use pezframe_support::traits::tokens::fungibles::{Inspect, InspectHold};
			...
			use pezframe_support::traits::tokens::fungibles::InspectFreeze;
			assert_eq!(
	...
	fn eoa_accounts_are_skipped() {
		...
		use pezframe_support::traits::tokens::fungibles::InspectHold;

		ExtBuilder::default().genesis_config(None).build().execute_with(|| {
			let _ = <Test as Config>::Currency::mint_into(&ALICE, Pezpallet::<Test>::min_balance());
			let _ = <Test as Config>::Currency::mint_into(&BOB, Pezpallet::<Test>::min_balance());
			AccountInfoOf::<Test>::insert(
				ALICE_ADDR,
