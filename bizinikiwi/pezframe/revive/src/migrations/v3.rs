...
//!
//! Iterates `pezframe_system::Account` and for each account:
...
impl<T: Config> SteppedMigration for Migration<T> {
	...
	fn id() -> Self::Identifier {
		MigrationId { pezpallet_id: *PALLET_MIGRATIONS_ID, version_from: 2, version_to: 3 }
	}
	...
	fn pre_upgrade() -> Result<Vec<u8>, pezsp_runtime::TryRuntimeError> {
		...
		use codec::Encode;
		let unmapped: u32 = pezframe_system::Account::<T>::iter_keys()
	...
	fn post_upgrade(prev: Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		...
		let still_unmapped: u32 = pezframe_system::Account::<T>::iter_keys()
			.filter(|id| !T::AddressMapper::is_mapped(id))
			.count() as u32;
		assert_eq!(
			still_unmapped, 0,
		...
		for (account_id, _) in pezframe_system::Account::<T>::iter() {
			assert!(
...
fn migrate_to_v3() {
	...
	use pezframe_support::{traits::fungible::Mutate, weights::WeightMeter};
	use pezsp_core::H160;
