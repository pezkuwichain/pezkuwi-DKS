...
impl<T: Config> PrimitivePrecompile for Bn128Add<T> {
	...
	fn call(
		...
		env.pezframe_meter_mut().charge_weight_token(RuntimeCosts::Bn128Add)?;
...
impl<T: Config> PrimitivePrecompile for Bn128Mul<T> {
	...
	fn call(
		...
		env.pezframe_meter_mut().charge_weight_token(RuntimeCosts::Bn128Mul)?;
...
impl<T: Config> PrimitivePrecompile for Bn128Pairing<T> {
	...
	fn call(
		...
		let ret_val = if input.is_empty() {
			env.pezframe_meter_mut().charge_weight_token(RuntimeCosts::Bn128Pairing(0))?;
			U256::one()
		} else {
