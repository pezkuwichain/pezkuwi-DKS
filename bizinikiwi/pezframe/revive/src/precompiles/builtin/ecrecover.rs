...
impl<T: Config> PrimitivePrecompile for EcRecover<T> {
	...
	fn call(
		...
		env.pezframe_meter_mut().charge_weight_token(RuntimeCosts::EcdsaRecovery)?;
		...
		let data = match pezsp_io::crypto::secp256k1_ecdsa_recover(&sig, &msg) {
			Ok(pubkey) => {
				let mut address = pezsp_io::hashing::keccak_256(&pubkey);
