...
use pezsp_runtime::{AccountId32, DispatchResult, Saturating};
...
mod test {
	...
	fn auto_mapper_maps_on_new_account() {
		ExtBuilder::default().build().execute_with(|| {
			...
			assert!(!<Test as Config>::AddressMapper::is_mapped(&EVE));
			// Funding a new account triggers pezframe_system's OnNewAccount hook
			<Test as Config>::Currency::set_balance(&EVE, 1_000_000);
			assert!(<Test as Config>::AddressMapper::is_mapped(&EVE));
			// no deposit taken
			assert_eq!(
	...
	fn auto_mapper_unmaps_on_killed_account() {
		ExtBuilder::default().build().execute_with(|| {
			...
			// Killing the account triggers pezframe_system's OnKilledAccount hook
	...
	fn unmap_account_dispatchable_blocked_when_auto_map_enabled() {
		...
		ExtBuilder::default().build().execute_with(|| {
			...
			assert_noop!(
				Pezpallet::<Test>::unmap_account(RuntimeOrigin::signed(EVE)),
	...
	fn batch_map_accounts_empty_pays_yes() {
		ExtBuilder::default().build().execute_with(|| {
			let info =
				Pezpallet::<Test>::batch_map_accounts(RuntimeOrigin::signed(ALICE), alloc::vec![])
					.unwrap();
	...
	fn batch_map_accounts_all_eth_derived_pays_yes() {
		ExtBuilder::default().build().execute_with(|| {
			...
			let info = Pezpallet::<Test>::batch_map_accounts(
	...
	fn batch_map_accounts_pays_no_when_mostly_unmapped() {
		ExtBuilder::default().build().execute_with(|| {
			...
			let info =
				Pezpallet::<Test>::batch_map_accounts(RuntimeOrigin::signed(ALICE), accounts)
					.unwrap();
	...
	fn batch_map_accounts_rejects_nonexistent_accounts() {
		ExtBuilder::default().build().execute_with(|| {
			...
			assert!(
				!pezframe_system::Pezpallet::<Test>::account_exists(&unknown),
