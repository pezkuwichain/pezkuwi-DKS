...
use crate::{
	AccountIdOf, AddressMapper, BalanceOf, CallOf, Config, LOG_TARGET, Pezpallet, Zero,
	evm::{
		CreateCallMode,
...
where
	E: EthExtra,
	Self: Encode,
	<E::Config as pezframe_system::Config>::Nonce: TryFrom<U256>,
		...
		if !self.0.is_signed() {
			if let Some(crate::Call::eth_transact { payload }) = self.0.function.is_sub_type() {
				log::trace!(
					...
					pezsp_core::hexdisplay::HexDisplay::from(&pezsp_crypto_hashing::blake2_256(&self.encode())),
				);
...
pub trait EthExtra {
	...
	///
	/// Use [`pezsp_runtime::traits::InvalidVersion`] if no other versions should be supported.
	...
	fn get_eth_extension(
		nonce: <Self::Config as pezframe_system::Config>::Nonce,
	...
	>
	where
		<Self::Config as pezframe_system::Config>::Nonce: TryFrom<U256>,
...
mod test {
	...
	use pezsp_runtime::traits::{self, Checkable, DispatchTransaction, LookupError};
	...
	fn eth_pre_dispatch_weight_matches_check_weight_booking() {
		...
		ExtBuilder::default().build().execute_with(|| {
			let reported =
				Pezpallet::<Test>::eth_pre_dispatch_weight(signed_transaction.signed_payload())
					.unwrap();
			let info = <Test as Config>::FeeInfo::dispatch_info(&call);
			let expected = pezframe_system::calculate_consumed_extrinsic_weight::<CallOf<Test>>(
				&<Test as pezframe_system::Config>::BlockWeights::get(),
				&info,
	...
	/// that if we were to run this transaction on pezpallet-revive that it would run and also produce
