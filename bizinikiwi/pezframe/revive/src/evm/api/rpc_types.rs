...
///
/// 1. **New RPC, old runtime (trailing bytes are ignored):** The `pezsp_api` runtime API argument
...
///    discard the rest. This is intentional behavior in `pezsp_api` — see the generated code in
///    `bizinikiwi/primitives/api/proc-macro/src/impl_runtime_apis.rs`.
///
...
/// - This pattern relies on `pezsp_api` continuing to use `Decode::decode` rather than `decode_all`.
///   If that ever changes, a new runtime API method would be needed instead.
///
/// ## Constraints on runtime API placement
///
/// The trailing-bytes trick described in point 1 above only works because `pezsp_api` discards
...
/// on `pezsp_api`'s use of `Decode::decode` (not `decode_all`) to silently discard trailing bytes
