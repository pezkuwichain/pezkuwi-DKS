...
//! Contains the fee types that need to be configured for `pezpallet-transaction-payment`.
...
use pezsp_arithmetic::{FixedPointOperand, SignedRounding};
use pezsp_runtime::{
...
///
/// The reason for this type is to avoid coupling the rest of pezpallet_revive to
/// pezpallet_transaction_payment. This way we bundle all the trait bounds in once place.
pub struct Info<Address, Signature, Extra>(PhantomData<(Address, Signature, Extra)>);

/// A trait that exposes all the transaction payment details to `pezpallet_revive`.
///
/// This trait is sealed. Use [`Info`].
pub trait InfoT<T: Config>: seal::Sealed {
	/// Check that the fee configuration of the chain is valid.
	///
	/// This is being called by the pezpallets `integrity_check`.
	fn integrity_test() {}
	...
	/// Add some additional fee to the `pezpallet_transaction_payment` credit.
...
where
	...
	<E::Config as pezframe_system::Config>::RuntimeCall:
		...
		SuppressedDrop<Inner = CreditOf<E::Config>>,
{
	...
	fn fixed_fee(encoded_len: u32) -> BalanceOf<E::Config> {
		Self::weight_to_fee(
			&<E::Config as pezframe_system::Config>::BlockWeights::get()
