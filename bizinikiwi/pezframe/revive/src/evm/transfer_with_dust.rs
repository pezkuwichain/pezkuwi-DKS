...
mod tests {
	...
	use pezsp_runtime::{DispatchError, traits::Zero};
	...
	fn burn_with_dust_redirects_to_on_burn() {
		...
		} in test_cases.into_iter()
		{
			ExtBuilder::default().build().execute_with(|| {
				...
				assert_eq!(
					Pezpallet::<Test>::evm_balance(&ALICE_ADDR),
					Pezpallet::<Test>::convert_native_to_evm(expected_balance),
	...
	fn burn_with_dust_fails_on_insufficient_balance() {
		ExtBuilder::default().build().execute_with(|| {
			...
			assert_eq!(
				Pezpallet::<Test>::evm_balance(&ALICE_ADDR),
				Pezpallet::<Test>::convert_native_to_evm(BalanceWithDust::new_unchecked::<Test>(
					10, 0
				)),
			);
		});
