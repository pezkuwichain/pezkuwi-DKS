...
use crate::{
	...
	pezsp_runtime::TransactionOutcome,
...
use pezframe_system::{
	Pezpallet as System, ensure_signed,
...
pub use codec;
use pezframe_support::traits::tokens::Precision;
...
pub use pezsp_core::{H160, H256, U256};
pub use pezsp_crypto_hashing::keccak_256;
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		///
		/// Requires `pezframe_system::Config::OnNewAccount` and `OnKilledAccount` to be set
	...
	pub enum Error<T> {
		...
		/// or other pezpallets that allow creating locks over held balance.
	...
	/// A reason for the pezpallet revive placing a freeze on PGAS funds.
	#[pezpallet::composite_enum]
	...
	/// - `holder`: account on which the deposit is held (a contract, or the pezpallet's own account
	...
	impl<T: Config> Pezpallet<T> {
		...
		#[pezpallet::weight(<T as Config>::WeightInfo::call().saturating_add(*weight_limit))]
		pub fn call(
			origin: OriginFor<T>,
			dest: H160,
		...
		#[pezpallet::weight(<T as Config>::WeightInfo::batch_map_accounts(accounts.len().saturated_into::<u32>()))]
		pub fn batch_map_accounts(
			...
			for account_id in accounts
				...
				.filter(|&a| pezframe_system::Pezpallet::<T>::account_exists(a))
			{
...
impl<T: Config> Pezpallet<T> {
	...
	///
	/// This matches the weight contribution that `pezframe_system::CheckWeight` would add for the
		...
		CallOf<T>: SetWeightLimit,
	{
		...
		Ok(pezframe_system::calculate_consumed_extrinsic_weight::<CallOf<T>>(
			&T::BlockWeights::get(),
			&info,
		...
		CallOf<T>: SetWeightLimit,
	{
		...
		// we don't support priority fee for now as the tipping system in pezpallet-transaction-payment
		...
		let total_cost_wei = Pezpallet::<T>::convert_native_to_evm(total_cost);
...
/// The address used to call the runtime's pezpallets dispatchables
///
/// Note:
...
/// This macro wraps substrate's `impl_runtime_apis!` and implements `pezpallet_revive` runtime APIs
		...
		impl_runtime_apis! {
			...
			impl pezpallet_revive::ReviveApi<Block, AccountId, Balance, Nonce, BlockNumber, __ReviveMacroMoment> for $Runtime
			{
				...
				fn max_extrinsic_weight_in_gas() -> $crate::U256 {
					$crate::Pezpallet::<Self>::evm_max_extrinsic_weight_in_gas()
				}
				...
				fn eth_estimate_gas(
					...
					$crate::Pezpallet::<Self>::eth_estimate_gas(tx, config)
				}

				fn eth_pre_dispatch_weight(
					tx: Vec<u8>,
				) -> Result<$crate::Weight, $crate::EthTransactError> {
					$crate::Pezpallet::<Self>::eth_pre_dispatch_weight(tx)
				}
