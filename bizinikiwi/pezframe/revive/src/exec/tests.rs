...
use super::*;
use crate::{
	AddressMapper, Error, Pezpallet, ReentrancyProtection,
...
fn balance_too_low() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let result = MockStack::transfer(
			...
			&from,
			&dest,
			Pezpallet::<Test>::convert_native_to_evm(100u128).as_u64().into(),
			Preservation::Preserve,
			&mut meter,
			&ExecConfig::new_bizinikiwi_tx(),
		);
...
fn code_hash_returns_proper_values() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		pezframe_system::Account::<Test>::insert(
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let result = MockStack::run_instantiate(
				...
				executable,
				&mut meter,
				Pezpallet::<Test>::convert_native_to_evm(100u128),
				vec![],
				Some(&[0; 32]),
				&ExecConfig::new_bizinikiwi_tx(),
			)
...
fn block_hash_returns_proper_values() {
	...
	ExtBuilder::default().build().execute_with(|| {
		pezframe_system::BlockHash::<Test>::insert(
			...
			<tests::Test as pezframe_system::Config>::Hash::from([1; 32]),
		);
		pezframe_system::BlockHash::<Test>::insert(
			&BlockNumberFor::<Test>::from(1u32),
			<tests::Test as pezframe_system::Config>::Hash::default(),
		);
		pezframe_system::BlockHash::<Test>::insert(
		...
		pezframe_system::BlockHash::<Test>::insert(
			...
			<tests::Test as pezframe_system::Config>::Hash::from([2; 32]),
		);
		pezframe_system::BlockHash::<Test>::insert(
