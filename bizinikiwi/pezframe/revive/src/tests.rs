...
pub type Block = pezsp_runtime::generic::Block<Header<u64, BlakeTwo256>, UncheckedExtrinsic>;
...
pub mod test_utils {
	...
	pub fn place_contract(address: &AccountIdOf<Test>, code_hash: pezsp_core::H256) {
	...
	pub fn get_code_deposit(code_hash: &pezsp_core::H256) -> BalanceOf<Test> {
		crate::CodeInfoOf::<Test>::get(code_hash).unwrap().deposit()
	}
	pub fn lockup_deposit(code_hash: &pezsp_core::H256) -> BalanceOf<Test> {
...
impl FindAuthor<<Test as pezframe_system::Config>::AccountId> for Test {
	fn find_author<'a, I>(_digests: I) -> Option<<Test as pezframe_system::Config>::AccountId>
	where
