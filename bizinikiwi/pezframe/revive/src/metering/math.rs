...
pub mod substrate_execution {
	...
	/// Create a transaction-level (root) meter for Bizinikiwi-style execution.
	///
	...
	pub fn new_root<T: Config>(
		...
		Ok(TransactionMeter {
			...
			// ignore max total gas for Bizinikiwi executions
	...
	///
	/// This produces a pezframe-local meter that enforces the resource limits for
	...
	/// Compute the remaining ethereum-gas-equivalent for a Bizinikiwi-style transaction.
	///
	...
	/// Compute the total consumed gas (signed) for Bizinikiwi-style execution.
	///
	...
	/// Compute the gas (signed) during the lifetime of this meter for Bizinikiwi-style execution.
