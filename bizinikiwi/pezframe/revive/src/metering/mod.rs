...
use pezsp_runtime::{DispatchError, DispatchResult, FixedU128, SaturatedConversion};
...
/// Nested state for pezframe-level resource metering.
///
...
pub struct ResourceMeter<T: Config, S: State> {
	...
	///
	/// `max_total_gas` is only required for Ethereum execution, it is always zero for Bizinikiwi
	/// executions.
	...
	/// Ethereum or Bizinikiwi execution mode.
...
pub enum TransactionLimits<T: Config> {
	...
	/// Bizinikiwi execution mode: the transaction specifies a weight limit and a storage deposit
	/// limit
...
impl<T: Config> TransactionMeter<T> {
	...
	/// - A bizinikiwi-style meter with explicit weight and deposit limits
	...
	/// Convenience constructor for bizinikiwi-style weight+deposit limits.
