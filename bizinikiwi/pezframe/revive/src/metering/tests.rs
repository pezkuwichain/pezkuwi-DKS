...
use pezsp_runtime::{FixedU128, Weight};
...
fn max_storage_deposit_reported_for_unfunded_dry_run(
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let run_in_rollback = |build: &dyn Fn() -> _| {
			with_transaction(|| {
				TransactionOutcome::Rollback(Ok::<_, pezsp_runtime::DispatchError>(build()))
			})
			.unwrap()
		};
		...
		let unfunded = run_in_rollback(&|| {
			crate::Pezpallet::<Test>::prepare_dry_run(&CHARLIE);
			builder::bare_call(caller_addr)
...
fn catch_constructor_test() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let first_estimate = crate::Pezpallet::<Test>::dry_run_eth_transact(
			GenericTransaction {
		...
		let second_estimate = crate::Pezpallet::<Test>::dry_run_eth_transact(
			GenericTransaction {
				from: Some(ALICE_ADDR),
				to: Some(test_address),
...
fn dry_run_bounded_execution_runs_out_of_gas() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let result = crate::Pezpallet::<Test>::dry_run_eth_transact(
			GenericTransaction {
				from: Some(ALICE_ADDR),
				to: Some(addr),
