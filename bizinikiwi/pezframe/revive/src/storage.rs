...
impl<T: Config> ContractInfo<T> {
	...
	pub fn write(
		...
		pezframe_meter: Option<&mut FrameMeter<T>>,
		take: bool,
		...
		self.write_raw(&hashed_key, new_value.as_deref(), pezframe_meter, take)
	}
	...
	fn write_raw(
		...
		if let Some(pezframe_meter) = pezframe_meter {
			...
			pezframe_meter.record_contract_storage_changes(&diff)?;
		}
	...
	pub fn load_code_hash(account: &AccountIdOf<T>) -> Option<pezsp_core::H256> {
