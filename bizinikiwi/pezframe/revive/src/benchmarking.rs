...
#[benchmarks(
	where
		T: Config,
		<T as Config>::RuntimeCall: From<pezframe_system::Call<T>>,
		<T as pezframe_system::Config>::Hash: pezframe_support::traits::IsType<H256>,
...
mod benchmarks {
	...
	fn deletion_queue_per_native_deposit_key(k: Linear<0, 1024>) -> Result<(), BenchmarkError> {
		use pezframe_benchmarking::v2::account;
	...
	fn eth_substrate_call(c: Linear<0, { 100 * 1024 }>) -> Result<(), BenchmarkError> {
		...
		let dispatchable = pezframe_system::Call::remark { remark: vec![] }.into();
		#[extrinsic_call]
	...
	fn remove_code() -> Result<(), BenchmarkError> {
		...
		assert_eq!(uploaded.deposit, T::Currency::total_balance_on_hold(&pezpallet_account));
		assert!(<Contract<T>>::code_exists(&hash));
		#[extrinsic_call]
		_(origin, hash);
		// removing the code should have unreserved the deposit
		assert_eq!(T::Currency::total_balance_on_hold(&pezpallet_account), 0u32.into());
	...
	fn seal_code_hash() {
		...
		let len = <pezsp_core::H256 as MaxEncodedLen>::max_encoded_len() as u32;
		...
		assert_ok!(result);
		assert_eq!(
			<pezsp_core::H256 as Decode>::decode(&mut &memory[..]).unwrap(),
	...
	fn weight_left() {
		...
		let weight_left_before = ext.pezframe_meter().weight_left().unwrap();
		let result;
		#[block]
		{
			result = run_builtin_precompile(
		...
		let weight_left_after = ext.pezframe_meter().weight_left().unwrap();
	...
	#[benchmark(pov_mode = Measured)]
	fn seal_balance_of() {
		let len = <pezsp_core::U256 as MaxEncodedLen>::max_encoded_len();
	...
	fn sha2_256(n: Linear<0, { limits::code::BLOB_BYTES }>) {
		...
		assert_eq!(pezsp_io::hashing::sha2_256(&input).to_vec(), result.unwrap().data);
	}
	...
	fn seal_hash_keccak_256(n: Linear<0, { limits::code::BLOB_BYTES }>) {
		...
		assert_eq!(pezsp_io::hashing::keccak_256(&memory[32..]), &memory[0..32]);
		assert_ok!(result);
	}
	...
	fn hash_blake2_256(n: Linear<0, { limits::code::BLOB_BYTES }>) {
		...
		let truth: [u8; 32] = pezsp_io::hashing::blake2_256(&input);
	...
	fn hash_blake2_128(n: Linear<0, { limits::code::BLOB_BYTES }>) {
		...
		let truth: [u8; 16] = pezsp_io::hashing::blake2_128(&input);
	...
	fn seal_ecdsa_to_eth_address() {
		let key_type = pezsp_core::crypto::KeyTypeId(*b"code");
		let pub_key_bytes = pezsp_io::crypto::ecdsa_generate(key_type, None).0;
	...
	fn instr(r: Linear<0, 10_000>) {
		...
		const MEMORY_SIZE: u64 = pezsp_core::MAX_POSSIBLE_ALLOCATION as u64;
	...
	fn v3_migration_step() {
		...
		let _ = pezframe_system::Account::<T>::clear(u32::MAX, None);

		let account = account::<T::AccountId>("target", 0, 0);
		T::Currency::mint_into(&account, Pezpallet::<T>::min_balance())
	...
	fn v4_code_upload_step() {
		...
		let pezpallet_account = Pezpallet::<T>::account_id();
		T::Currency::mint_into(&pezpallet_account, Pezpallet::<T>::min_balance()).unwrap();
		T::Currency::mint_into(&pezpallet_account, deposit).unwrap();
		T::Currency::hold(
			&HoldReason::CodeUploadDepositReserve.into(),
			&pezpallet_account,
			deposit,
		...
		assert_eq!(
			NativeDepositOf::<T>::get(&pezpallet_account, &owner),
			deposit + deposit,
	...
	fn v4_contract_step() {
		...
		for byte in [0x41u8, 0x42u8] {
			...
			T::Currency::mint_into(&contract_account, Pezpallet::<T>::min_balance()).unwrap();
	...
	fn create_signed_transaction<T: Config>(
		...
		let hashed_payload = pezsp_io::hashing::keccak_256(&unsigned_tx.unsigned_payload());
