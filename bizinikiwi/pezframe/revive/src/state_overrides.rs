...
use crate::{
	AccountInfoOf, AccountType, CodeInfoOf, Config, EthTransactError, LOG_TARGET, Pezpallet,
	PristineCode,
...
///
/// Delegates to [`Pezpallet::set_evm_balance`] which handles the native currency conversion and dust
/// accounting.
fn apply_balance_override<T: Config>(
	address: &H160,
	balance: U256,
) -> Result<(), EthTransactError> {
	Pezpallet::<T>::set_evm_balance(address, balance).map_err(|err| {
...
///
/// Mutates the `pezframe_system` account nonce directly, which is where pezpallet-revive reads nonces
	...
	T::Nonce: TryFrom<U256>,
{
	...
	pezframe_system::Account::<T>::mutate(&account_id, |account| {
		account.nonce = nonce;
	});
	Ok(())
}
...
fn apply_code_override<T: Config>(address: &H160, code: Vec<u8>) -> Result<(), EthTransactError> {
	...
	<AccountInfoOf<T>>::try_mutate(address, |account| -> Result<(), DispatchError> {
		match account {
			...
			_ => {
				let nonce = pezframe_system::Pezpallet::<T>::account_nonce(&account_id);
