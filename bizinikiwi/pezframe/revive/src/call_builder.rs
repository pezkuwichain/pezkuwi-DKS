...
use pezpallet_revive_fixtures::bench as bench_fixtures;
	...
	T: Config,
{
	...
	pub fn with_index(
		...
		Self::with_caller(pezframe_benchmarking::account("instantiator", index, 0), module, data)
	}
	...
	pub fn with_caller(
		...
		#[cfg(feature = "runtime-benchmarks")]
		pezframe_benchmarking::benchmarking::add_to_whitelist(
			pezframe_system::Account::<T>::hashed_key_for(&caller).into(),
		);
	...
	pub fn code_exists(hash: &pezsp_core::H256) -> bool {
	...
	pub fn code_removed(hash: &pezsp_core::H256) -> bool {
		!<PristineCode<T>>::contains_key(hash) && !<CodeInfoOf<T>>::contains_key(&hash)
	}
}
