...
use crate::{
	BalanceOf, Code, Config, Error, EthBlockBuilderFirstValues, GenesisConfig, Origin, Pezpallet,
...
use pezpallet_revive_fixtures::{Fibonacci, FixtureType, NestedCounter, compile_module_with_type};
...
fn call_tracing_records_consumption_for_nested_transfer_to_eoa() {
	...
	use pezpallet_revive_fixtures::Caller;
	use pezsp_core::H160;
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let _ = Pezpallet::<Test>::set_evm_balance(&caller, 100_000_000_000u128.into());
		...
		assert!(Pezpallet::<Test>::evm_balance(&eoa) >= 1_000_000.into());
...
fn upload_evm_runtime_code_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let uploaded_blob = Pezpallet::<Test>::try_upload_code(
			deployer,
			runtime_code.clone(),
...
fn upload_and_remove_code_works_for_evm() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		// Upload the code.
		assert_ok!(Pezpallet::<Test>::upload_code(RuntimeOrigin::signed(ALICE), code, 1000u128));
...
fn upload_fails_if_evm_bytecode_disabled() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_err!(
			Pezpallet::<Test>::upload_code(RuntimeOrigin::signed(ALICE), code, 1000u128),
...
fn execution_tracing_works() {
	...
	use pezpallet_revive_fixtures::{Callee, Caller};
