...
use pezframe_support::assert_err_ignore_postinfo;

use alloy_core::sol_types::{SolCall, SolInterface};
use pezframe_support::traits::{fungible::Mutate, Get};
use pezpallet_revive_fixtures::{compile_module_with_type, Caller, FixtureType, Host};
use pretty_assertions::assert_eq;
use test_case::test_case;

fn convert_to_free_balance(total_balance: u128) -> U256 {
	let existential_deposit_planck =
		<Test as pezpallet_balances::Config>::ExistentialDeposit::get() as u128;
...
fn extcodecopy_works(caller_type: FixtureType, callee_type: FixtureType) {
	use pezpallet_revive_fixtures::{HostEvmOnly, HostEvmOnly::HostEvmOnlyCalls};
...
fn blockhash_works(fixture_type: FixtureType) {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		{
			let block_hash = [1; 32];
			pezframe_system::BlockHash::<Test>::insert(
				&crate::BlockNumberFor::<Test>::from(block_number_to_test),
				<Test as pezframe_system::Config>::Hash::from(&block_hash),
			);
			let result = builder::bare_call(addr)
				.data(
...
fn transient_storage_works(fixture_type: FixtureType) {
	use pezpallet_revive_fixtures::HostTransientMemory;
