...
use pezpallet_revive_fixtures::{Callee, Caller, FixtureType, Host, compile_module_with_type};
...
fn mocked_code_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let expected_hash = pezsp_io::hashing::keccak_256(&MOCK_CODE);
...
fn instantiate_from_constructor_works() {
	use pezpallet_revive_fixtures::CallerWithConstructor::*;
...
fn root_call_can_create_and_destroy_in_next_block(
	...
	use crate::{
		AccountInfo, HoldReason, Pezpallet,
		address::AddressMapper,
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let pezpallet_upload_hold_before = get_balance_on_hold(&upload_hold, &pezpallet_account);
		...
		if caller_type == FixtureType::Solc {
			assert_eq!(
				get_balance_on_hold(&upload_hold, &pezpallet_account),
				pezpallet_upload_hold_before
			);
		}
	});
}
...
fn root_call_can_create_and_destroy_in_same_tx(caller_type: FixtureType, callee_type: FixtureType) {
	use crate::{
		AccountInfo, HoldReason, Pezpallet, address::AddressMapper, test_utils::DJANGO_ADDR,
		tests::test_utils::get_balance_on_hold,
	};
	use alloy_core::primitives::Address;
	use pezpallet_revive_fixtures::NestedDeployer::{
		NestedDeployerCalls, deployAndDestroyChildCall,
	};
