...
/// system consumer. Burning the ED directly is rejected by the underlying balance pezpallet
...
/// [`FreezeReason::PGasMinBalance`]. Pezpallet-assets' `reducible_balance` returns
...
/// against the [`PGasDeposit`] backend. Surfaces any issue from the pezpallet account not
...
fn code_upload_and_remove_with_pgas() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let pezpallet_account = crate::Pezpallet::<Test>::account_id();
			...
			assert_eq!(
				...
				deposit,
				"deposit held in PGAS on the pezpallet account",
			);
			assert_eq!(
				NativeDepositOf::<Test>::get(&pezpallet_account, &ALICE),
				0,
