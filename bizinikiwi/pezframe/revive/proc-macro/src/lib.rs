...
//! Procedural macros used in `pezpallet-revive`.
//!
...
fn expand_functions(def: &EnvDef) -> TokenStream2 {
	let impls = def.host_funcs.iter().map(|f| {
		...
		let wrapped_body_with_trace = {
			...
			quote! {
				...
				let result = (|| #body)();
				::log::trace!(target: "runtime::revive::strace", #trace_fmt_str, #( #trace_fmt_args, )* result, self.ext.pezframe_meter().weight_consumed());
	...
	quote! {
		...
		// Write gas from pezpallet-revive into polkavm after leaving the host function.
		let gas = self.ext.pezframe_meter_mut().sync_to_executor();
