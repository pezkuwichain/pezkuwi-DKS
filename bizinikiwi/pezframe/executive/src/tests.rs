...
//! Test the `pezframe-executive` crate.
...
mod custom {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pezpallet<T> {
		...
		fn on_runtime_upgrade() -> Weight {
			pezsp_io::storage::set(super::TEST_KEY, "module".as_bytes());
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn calculate_storage_root(_origin: OriginFor<T>) -> DispatchResult {
			let root = pezsp_io::storage::root(pezsp_runtime::StateVersion::V1);
...
impl pezframe_system::ExtensionsWeightInfo for MockExtensionsWeights {
...
impl pezpallet_balances::WeightInfo for BalancesWeights {
...
impl pezframe_support::traits::Get<pezsp_version::RuntimeVersion> for RuntimeVersion {
	fn get() -> pezsp_version::RuntimeVersion {
		RuntimeVersionTestValues::get().clone()
	}
}

parameter_types! {
	pub static RuntimeVersionTestValues: pezsp_version::RuntimeVersion =
		Default::default();
}
...
type UncheckedXt = pezsp_runtime::generic::UncheckedExtrinsic<
	u64,
	RuntimeCall,
...
impl OnRuntimeUpgrade for CustomOnRuntimeUpgrade {
	...
	fn post_upgrade(_state: Vec<u8>) -> Result<(), TryRuntimeError> {
		assert_eq!(&pezsp_io::storage::get(TEST_KEY_2).unwrap()[..], *b"try_runtime_upgrade_works");
		Ok(())
	}
}
...
impl PreInherents for MockedSystemCallbacks {
	fn pre_inherents() {
		...
		pezframe_support::storage::unhashed::put(b":pre_inherent", b"0");
	}
}
...
impl PostInherents for MockedSystemCallbacks {
	fn post_inherents() {
		...
		// Change the storage to modify the root hash:
		pezframe_support::storage::unhashed::put(b":post_inherent", b"0");
	}
}
...
impl PostTransactions for MockedSystemCallbacks {
	fn post_transactions() {
		...
		// Change the storage to modify the root hash:
		pezframe_support::storage::unhashed::put(b":post_transaction", b"0");
	}
}
...
impl MockedSystemCallbacks {
	...
	fn reset() {
		...
		pezframe_support::storage::unhashed::kill(b":pre_inherent");
		pezframe_support::storage::unhashed::kill(b":post_inherent");
		pezframe_support::storage::unhashed::kill(b":post_transaction");
	}
}
...
fn tx_ext(nonce: u64, fee: Balance) -> TxExtension {
	(
		...
		pezpallet_transaction_payment::ChargeTransactionPayment::from(fee),
...
fn balance_transfer_dispatch_works() {
	...
	pezpallet_balances::GenesisConfig::<Runtime> { balances: vec![(1, 211)], ..Default::default() }
	...
	let weight = xt.get_dispatch_info().total_weight()
		+ <Runtime as pezframe_system::Config>::BlockWeights::get()
			...
			.base_extrinsic;
	let fee: Balance =
		<Runtime as pezpallet_transaction_payment::Config>::WeightToFee::weight_to_fee(&weight);
	let mut t = pezsp_io::TestExternalities::new(t);
	t.execute_with(|| {
...
fn new_test_ext(balance_factor: Balance) -> pezsp_io::TestExternalities {
	let mut t = pezframe_system::GenesisConfig::<Runtime>::default().build_storage().unwrap();
...
fn new_test_ext_v0(balance_factor: Balance) -> pezsp_io::TestExternalities {
	let mut t = pezframe_system::GenesisConfig::<Runtime>::default().build_storage().unwrap();
	pezpallet_balances::GenesisConfig::<Runtime> {
		balances: vec![(1, 111 * balance_factor)],
		..Default::default()
	...
	.assimilate_storage(&mut t)
	.unwrap();
	(t, pezsp_runtime::StateVersion::V0).into()
}
...
fn block_import_works_inner(mut ext: pezsp_io::TestExternalities, state_root: H256) {
	ext.execute_with(|| {
...
fn block_weight_limit_enforced() {
	...
	let transfer_weight =
			<<Runtime as pezpallet_balances::Config>::WeightInfo as pezpallet_balances::WeightInfo>::transfer_allow_death();
	...
	let block_weights = <Runtime as pezframe_system::Config>::BlockWeights::get();
...
fn block_weight_and_size_is_stored_per_tx() {
	...
	let transfer_weight = <<Runtime as pezpallet_balances::Config>::WeightInfo as pezpallet_balances::WeightInfo>::transfer_allow_death();
...
fn can_not_pay_for_tx_fee_on_full_lock() {
	...
	t.execute_with(|| {
		...
		.unwrap();
		let xt = UncheckedXt::new_signed(
			RuntimeCall::System(pezframe_system::Call::remark { remark: vec![1u8] }),
			1,
			1.into(),
			tx_ext(0, 0),
		);
...
fn runtime_upgraded_should_work() {
	new_test_ext(1).execute_with(|| {
		...
		RuntimeVersionTestValues::mutate(|v| {
			*v = pezsp_version::RuntimeVersion { spec_version: 1, ..Default::default() }
		});
		assert!(Executive::runtime_upgraded());

		RuntimeVersionTestValues::mutate(|v| {
			*v = pezsp_version::RuntimeVersion {
				spec_version: 1,
				spec_name: "test".into(),
				..Default::default()
			}
...
fn last_runtime_upgrade_was_upgraded_works() {
	...
	for (spec_version, spec_name, c_spec_version, c_spec_name, result) in test_data {
		let current = pezsp_version::RuntimeVersion {
...
fn custom_runtime_upgrade_is_called_before_modules() {
	new_test_ext(1).execute_with(|| {
		...
		assert_eq!(&pezsp_io::storage::get(TEST_KEY).unwrap()[..], *b"module");
		assert_eq!(pezsp_io::storage::get(CUSTOM_ON_RUNTIME_KEY).unwrap(), true.encode());
		assert_eq!(
...
fn custom_runtime_upgrade_is_called_when_using_execute_block_trait() {
	...
	RuntimeVersionTestValues::mutate(|v| {
		*v = pezsp_version::RuntimeVersion { spec_version: 0, ..Default::default() }
	});
...
fn offchain_worker_works_as_expected() {
	new_test_ext(1).execute_with(|| {
		let parent_hash = pezsp_core::H256::from([69u8; 32]);
...
fn try_execute_block_works() {
	...
	new_test_ext(1).execute_with(|| {
		Executive::try_execute_block(
			...
			true,
			true,
			pezframe_try_runtime::TryStateSelect::All,
		)
		.unwrap();
	});
}
...
fn try_runtime_upgrade_works() {
	...
	new_test_ext(1).execute_with(|| {
		...
		assert!(&pezsp_io::storage::get(TEST_KEY_2).is_none());
...
fn max_transaction_depth_is_respected() {
	...
	let mut call = RuntimeCall::System(pezframe_system::Call::remark { remark: vec![1, 2, 3] });
