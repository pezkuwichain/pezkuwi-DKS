...
//!                    +-----+----+                              +------+---+
//!                    |          |                              |          |
//! pezpallet-do-election |          |                              |          | pezpallet-needs-election
//!                    |          |                              |          |
//!                    |          |                              |          |
//!                    +-----+----+                              +------+---+
//!                          |                                          ^
...
//!
//! ```rust
//! # use pezframe_election_provider_support::{*, data_provider};
//! # use pezsp_npos_elections::{Support, Assignment};
...
//!
//! 		fn duration() -> <Self as pezframe_election_provider_support::ElectionProvider>::BlockNumber { todo!() }
//!
//! 		fn start() -> Result<(), <Self as pezframe_election_provider_support::ElectionProvider>::Error> { todo!() }
//!
...
pub trait NposSolver {
	...
	type AccountId: pezsp_npos_elections::IdentifierT;
	...
	for QuickDirtySolver<AccountId, Accuracy>
{
	...
	fn solve(
		...
		use pezsp_std::collections::btree_map::BTreeMap;
...
/// A wrapper for [`pezsp_npos_elections::seq_phragmen`] that implements [`NposSolver`]. See the
/// documentation of [`pezsp_npos_elections::seq_phragmen`] for more info.
	...
	NposSolver for SequentialPhragmen<AccountId, Accuracy, Balancing>
{
	type AccountId = AccountId;
	type Accuracy = Accuracy;
	type Error = pezsp_npos_elections::Error;
	fn solve(
		winners: usize,
		targets: Vec<Self::AccountId>,
		voters: Vec<(
		...
		pezsp_npos_elections::seq_phragmen(winners, targets, voters, Balancing::get())
	}
...
/// A wrapper for [`pezsp_npos_elections::phragmms()`] that implements [`NposSolver`]. See the
/// documentation of [`pezsp_npos_elections::phragmms()`] for more info.
	...
	NposSolver for PhragMMS<AccountId, Accuracy, Balancing>
{
	...
	fn solve(
		...
		pezsp_npos_elections::phragmms(winners, targets, voters, Balancing::get())
	}
...
/// A bounded vector of supports. Bounded equivalent to [`pezsp_npos_elections::Supports`].
#[derive(
...
impl<AccountId, Bound: Get<u32>> TryFrom<pezsp_npos_elections::Support<AccountId>>
	for BoundedSupport<AccountId, Bound>
{
	type Error = &'static str;
	fn try_from(s: pezsp_npos_elections::Support<AccountId>) -> Result<Self, Self::Error> {
...
///
/// A [`BoundedSupports`] is a set of [`pezsp_npos_elections::Supports`] which are bounded in two
...
impl<AccountId, BOuter: Get<u32>, BInner: Get<u32>> pezsp_npos_elections::EvaluateSupport
	for BoundedSupports<AccountId, BOuter, BInner>
{
	fn evaluate(&self) -> pezsp_npos_elections::ElectionScore {
		pezsp_npos_elections::evaluate_support(self.iter().map(|(_, s)| s))
	}
}

impl<AccountId, BOuter: Get<u32>, BInner: Get<u32>> pezsp_std::ops::DerefMut
	for BoundedSupports<AccountId, BOuter, BInner>
{
...
impl<AccountId, BOuter: Get<u32>, BInner: Get<u32>> pezsp_std::ops::Deref
	for BoundedSupports<AccountId, BOuter, BInner>
{
	...
	for BoundedSupports<AccountId, BOuter, BInner>
{
	...
	type IntoIter = pezsp_std::vec::IntoIter<Self::Item>;
