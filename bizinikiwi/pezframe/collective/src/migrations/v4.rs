...
>(
	...
	if new_pezpallet_name == old_pezpallet_name {
		log::info!(
			target: LOG_TARGET,
			"New pezpallet name is equal to the old pezpallet name. No migration needs to be done.",
		);
		return Weight::zero();
	}
...
pub fn pre_migrate<P: GetStorageVersion + PalletInfoAccess, N: AsRef<str>>(old_pezpallet_name: N) {
	let old_pezpallet_name = old_pezpallet_name.as_ref();
	let new_pezpallet_name = <P as PalletInfoAccess>::name();
...
///
/// Panics if anything goes wrong.
pub fn post_migrate<P: GetStorageVersion + PalletInfoAccess, N: AsRef<str>>(old_pezpallet_name: N) {
	let old_pezpallet_name = old_pezpallet_name.as_ref();
	let new_pezpallet_name = <P as PalletInfoAccess>::name();
