...
use crate as pezpallet_collective;
...
use pezframe_system::{EnsureRoot, EventRecord, Phase};
use pezsp_core::{ConstU128, H256};
use pezsp_runtime::{
	testing::Header,
...
pezframe_support::construct_runtime!(
	pub enum Test
	{
		...
		Collective: pezpallet_collective::<Instance1>,
		CollectiveMajority: pezpallet_collective::<Instance2>,
		DefaultCollective: pezpallet_collective,
...
parameter_types! {
	...
	pub const ProposalHoldReason: RuntimeHoldReason =
		RuntimeHoldReason::Collective(pezpallet_collective::HoldReason::ProposalSubmission);
}
...
impl ExtBuilder {
	...
	pub fn build(self) -> pezsp_io::TestExternalities {
		let mut ext: pezsp_io::TestExternalities = RuntimeGenesisConfig {
			...
			// balances: pezpallet_balances::GenesisConfig::default(),
			balances: pezpallet_balances::GenesisConfig {
				balances: vec![(1, 100), (2, 200)],
				..Default::default()
			},
			collective: pezpallet_collective::GenesisConfig {
				members: self.collective_members,
				phantom: Default::default(),
			},
			collective_majority: pezpallet_collective::GenesisConfig {
...
fn make_proposal(value: u64) -> RuntimeCall {
	RuntimeCall::System(pezframe_system::Call::remark_with_event {
...
fn default_max_proposal_weight() -> Weight {
	pezsp_runtime::Perbill::from_percent(80) * BlockWeights::get().max_block
}
...
fn close_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			System::events(),
			vec![
				record(RuntimeEvent::Balances(pezpallet_balances::Event::Held {
					reason: <Test as pezpallet_balances::Config>::RuntimeHoldReason::Collective(
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			vec![
				...
				record(RuntimeEvent::Balances(pezpallet_balances::Event::Released {
					reason: <Test as pezpallet_balances::Config>::RuntimeHoldReason::Collective(
						HoldReason::ProposalSubmission,
					),
					who: 5,
...
fn genesis_build_panics_with_too_many_members() {
	...
	pezpallet_collective::GenesisConfig::<Test> {
...
fn kill_proposal_with_deposit() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			System::events(),
			vec![
				record(RuntimeEvent::Balances(pezpallet_balances::Event::BurnedHeld {
					reason: <Test as pezpallet_balances::Config>::RuntimeHoldReason::Collective(
						HoldReason::ProposalSubmission,
					),
					who: 1,
