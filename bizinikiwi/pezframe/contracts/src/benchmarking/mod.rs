...
use pezframe_support::{
	self, assert_ok,
	pezpallet_prelude::StorageVersion,
	storage::child,
...
use pezpallet_contracts_uapi::{CallFlags, ReturnErrorCode};
use pezsp_runtime::traits::{Bounded, Hash};
...
mod benchmarks {
	...
	fn seal_hash_sha2_256(n: Linear<0, { code::max_pages::<T>() * 64 * 1024 }>) {
		...
		assert_eq!(pezsp_io::hashing::sha2_256(&memory[32..]), &memory[0..32]);
		assert_ok!(result);
	}
	...
	fn seal_hash_blake2_256(n: Linear<0, { code::max_pages::<T>() * 64 * 1024 }>) {
		...
		assert_eq!(pezsp_io::hashing::blake2_256(&memory[32..]), &memory[0..32]);
		assert_ok!(result);
	}
	...
	fn seal_hash_blake2_128(n: Linear<0, { code::max_pages::<T>() * 64 * 1024 }>) {
		...
		assert_eq!(pezsp_io::hashing::blake2_128(&memory[16..]), &memory[0..16]);
		assert_ok!(result);
	}
	...
	fn seal_ecdsa_recover() {
		let message_hash = pezsp_io::hashing::blake2_256("Hello world".as_bytes());
		let key_type = pezsp_core::crypto::KeyTypeId(*b"code");
		let signature = {
			let pub_key = pezsp_io::crypto::ecdsa_generate(key_type, None);
			let sig = pezsp_io::crypto::ecdsa_sign_prehashed(key_type, &pub_key, &message_hash)
				.expect("Generates signature");
