...
pub type SeedOf<T> = <T as pezframe_system::Config>::Hash;
pub type ExecResult = Result<ExecReturnValue, ExecError>;

/// A type that represents a topic of an event. At the moment a hash is used.
pub type TopicOf<T> = <T as pezframe_system::Config>::Hash;
...
mod tests {
	...
	impl MockLoader {
		...
		fn insert(
			...
			Loader::mutate(|loader| {
				...
				let hash = <Test as pezframe_system::Config>::Hash::from_low_u64_be(loader.counter);
				loader.counter += 1;
				loader.map.insert(
					hash,
					MockExecutable {
	...
	fn call_runtime_works() {
		let code_hash = MockLoader::insert(Call, |ctx, _| {
			let call = RuntimeCall::System(pezframe_system::Call::remark_with_event {
		...
		ExtBuilder::default().build().execute_with(|| {
			...
			let remark_hash = <Test as pezframe_system::Config>::Hashing::hash(b"Hello World");
			assert_eq!(
				System::events(),
				vec![
					EventRecord {
						phase: Phase::Initialization,
						event: MetaEvent::System(pezframe_system::Event::Remarked {
							sender: BOB,
							hash: remark_hash
						}),
						topics: vec![],
	...
	fn call_runtime_filter() {
		let code_hash = MockLoader::insert(Call, |ctx, _| {
			use pezframe_system::Call as SysCall;
			use pezpallet_balances::Call as BalanceCall;
			use pezpallet_utility::Call as UtilCall;
		...
		TestFilter::set_filter(|call| match call {
			RuntimeCall::Balances(pezpallet_balances::Call::transfer_allow_death { .. }) => false,
			_ => true,
		});
		...
		ExtBuilder::default().build().execute_with(|| {
			...
			let remark_hash = <Test as pezframe_system::Config>::Hashing::hash(b"Hello");
			assert_eq!(
				System::events(),
				vec![
					EventRecord {
					...
					EventRecord {
						phase: Phase::Initialization,
						event: MetaEvent::Utility(pezpallet_utility::Event::ItemCompleted),
						topics: vec![],
					},
					EventRecord {
						phase: Phase::Initialization,
						event: MetaEvent::Utility(pezpallet_utility::Event::BatchInterrupted {
							index: 1,
