...
pezframe_support::construct_runtime!(
	pub enum Test
	{
		...
		Randomness: pezpallet_insecure_randomness_collective_flip,
...
pub mod test_utils {
	...
	pub fn contract_info_storage_deposit(
		addr: &<Test as pezframe_system::Config>::AccountId,
	) -> BalanceOf<Test> {
...
parameter_types! {
	pub static UploadAccount: Option<<Test as pezframe_system::Config>::AccountId> = None;
	pub static InstantiateAccount: Option<<Test as pezframe_system::Config>::AccountId> = None;
}

pub struct EnsureAccount<T, A>(core::marker::PhantomData<(T, A)>);
impl<T: Config, A: pezsp_core::Get<Option<crate::AccountIdOf<T>>>>
	EnsureOrigin<<T as pezframe_system::Config>::RuntimeOrigin> for EnsureAccount<T, A>
where
	<T as pezframe_system::Config>::AccountId: From<AccountId32>,
{
	type Success = T::AccountId;

	fn try_origin(o: T::RuntimeOrigin) -> Result<Self::Success, T::RuntimeOrigin> {
		let who = <pezframe_system::EnsureSigned<_> as EnsureOrigin<_>>::try_origin(o.clone())?;
...
impl ExtBuilder {
	...
	pub fn build(self) -> pezsp_io::TestExternalities {
		...
		pezpallet_balances::GenesisConfig::<Test> { balances: vec![], ..Default::default() }
			...
			vec![
				...
				EventRecord {
					...
					event: RuntimeEvent::Contracts(
						pezpallet_contracts::Event::StorageDepositTransferredAndHeld {
							from: ALICE,
							to: addr.clone(),
		...
		pretty_assertions::assert_eq!(
			...
			vec![
				...
				EventRecord {
					phase: Phase::Initialization,
					event: RuntimeEvent::Contracts(
						pezpallet_contracts::Event::StorageDepositTransferredAndReleased {
							from: addr.clone(),
							to: ALICE,
							amount: info_deposit,
						}
...
fn gas_estimation_for_subcalls() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let runtime_call = RuntimeCall::Dummy(pezpallet_dummy::Call::overestimate_pre_charge {
...
fn gas_estimation_call_runtime() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		let call = RuntimeCall::Dummy(pezpallet_dummy::Call::overestimate_pre_charge {
...
fn call_runtime_reentrancy_guarded() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		// Call pezpallet_contracts call() dispatchable
...
fn sr25519_verify() {
	...
	ExtBuilder::default().existential_deposit(50).build().execute_with(|| {
		...
		// Signed here rather than pasted in. A hard-coded signature is a wire-format constant:
		// it verifies only under the context it was produced with, and this chain's signing
		// context is deliberately not upstream's. The pasted pair verified under `b"substrate"`
		// and this test began failing the moment the sovereign context was put back.
		let pair = <pezsp_core::sr25519::Pair as pezsp_core::Pair>::from_string("//Alice", None)
			.expect("//Alice is a valid seed; qed");
		let public_key: [u8; 32] = <pezsp_core::sr25519::Pair as pezsp_core::Pair>::public(&pair).0;

		let call_with = |message: &[u8; 11], signed: &[u8; 11]| {
			let signature: [u8; 64] =
		...
		assert_return_code!(call_with(b"hello world", b"hello world"), RuntimeReturnCode::Success);

		// A signature over a different message must not verify.
		assert_return_code!(
			call_with(b"hello worlD", b"hello world"),
...
fn failed_deposit_charge_should_roll_back_call() {
	...
	let execute = || {
		ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
			...
			let transfer_proxy_call = RuntimeCall::Proxy(pezpallet_proxy::Call::proxy {
				real: ALICE,
	...
	ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
		...
		assert_eq!(
			...
			vec![
				EventRecord {
					phase: Phase::Initialization,
					event: RuntimeEvent::Balances(pezpallet_balances::Event::Held {
						reason: <Test as Config>::RuntimeHoldReason::Contracts(
	...
	ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
		...
		assert_eq!(
			...
			vec![
				...
				EventRecord {
					phase: Phase::Initialization,
					event: RuntimeEvent::Balances(pezpallet_balances::Event::Released {
						reason: <Test as Config>::RuntimeHoldReason::Contracts(
							HoldReason::CodeUploadDepositReserve,
						),
						who: ALICE,
...
fn set_code_extrinsic() {
	...
	ExtBuilder::default().existential_deposit(100).build().execute_with(|| {
		...
		assert_eq!(
			...
			vec![EventRecord {
				phase: Phase::Initialization,
				event: RuntimeEvent::Contracts(pezpallet_contracts::Event::ContractCodeUpdated {
					contract: addr.clone(),
					new_code_hash,
...
fn read_only_call_cannot_transfer() {
	...
	ExtBuilder::default().existential_deposit(200).build().execute_with(|| {
		...
		assert_err_ignore_postinfo!(
			builder::call(addr_caller)
				.data(
					(addr_callee, pezpallet_contracts_uapi::CallFlags::READ_ONLY.bits(), 100u64)
						.encode()
				)
				.build(),
