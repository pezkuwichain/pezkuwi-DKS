...
type CodeHash<T> = <T as pezframe_system::Config>::Hash;
...
type EventRecordOf<T> =
	EventRecord<<T as pezframe_system::Config>::RuntimeEvent, <T as pezframe_system::Config>::Hash>;
...
/// `pezpallet-contracts` and any given contract.
///
/// Since those types are configurable they can vary between
/// chains all using `pezpallet-contracts`. Hence we need a mechanism to communicate those types
...
pub struct Environment<T: Config> {
	...
	hash: EnvironmentType<<T as pezframe_system::Config>::Hash>,
	hasher: EnvironmentType<<T as pezframe_system::Config>::Hashing>,
...
/// This is used to communicate the available APIs in pezpallet-contracts.
///
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		/// This is applied in **addition** to [`pezframe_system::Config::BaseCallFilter`].
		...
		/// ```
		/// use pezpallet_contracts::migration::v10;
		/// # struct Runtime {};
		/// # struct Currency {};
