...
fn transaction_payment_rejects_reduced_to_zero_in_native_asset() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let fee = pezpallet_transaction_payment::Pezpallet::<Runtime>::compute_fee(
				len as u32, &info, tip,
			);
			...
			let result = ext.validate_only(
				...
				&info,
				len,
				pezsp_runtime::transaction_validity::TransactionSource::External,
				0,
			);
...
fn transaction_payment_rejects_reduced_to_zero_in_asset() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let fee_in_native = pezpallet_transaction_payment::Pezpallet::<Runtime>::compute_fee(
				len as u32, &info, tip,
			);
...
fn post_dispatch_ok_when_asset_account_blocked_post_withdraw() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let pool_account = <<Runtime as pezpallet_asset_conversion::Config>::PoolLocator
				as pezpallet_asset_conversion::PoolLocator<_, _, _>>::pool_address(
				&NativeOrWithId::Native,
				&NativeOrWithId::WithId(asset_id),
			)
			.unwrap();
...
fn post_dispatch_ok_when_native_account_has_no_free_balance() {
	use pezframe_support::traits::ReservableCurrency;
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let _ = pezframe_system::Pezpallet::<Runtime>::inc_providers(&caller);
...
fn post_dispatch_ok_when_pool_asset_dusted_post_withdraw() {
	use pezpallet_asset_conversion::PoolLocator;
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let pool_account =
				<<Runtime as pezpallet_asset_conversion::Config>::PoolLocator as PoolLocator<
					_,
					_,
					_,
