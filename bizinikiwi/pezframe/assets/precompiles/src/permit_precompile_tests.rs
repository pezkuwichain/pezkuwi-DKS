...
//!
//! `permit_tests.rs` exercises `permit::Pezpallet` directly in isolation. The
...
fn hardhat_account_id() -> u64 {
	<Test as pezpallet_revive::Config>::AddressMapper::to_account_id(&HARDHAT_ACCOUNT_0)
}
...
fn sign_permit(
	...
	let nonce = permit::Pezpallet::<Test>::nonce(&asset_addr, &HARDHAT_ACCOUNT_0);
	let value_bytes: [u8; 32] = value.to_be_bytes();
	let deadline_bytes: [u8; 32] = deadline.to_be_bytes();

	let digest = permit::Pezpallet::<Test>::permit_digest(
		&asset_addr,
		PERMIT_TOKEN_NAME,
		&HARDHAT_ACCOUNT_0,
		&spender,
	...
	let key_type = pezsp_core::crypto::KeyTypeId(*b"prmt");
	let pub_key = pezsp_io::crypto::ecdsa_generate(key_type, Some(HARDHAT_ACCOUNT_0_SEED.to_vec()));
	let sig = pezsp_io::crypto::ecdsa_sign_prehashed(key_type, &pub_key, &digest)
...
fn raw_permit(
	...
	r: [u8; 32],
	s: [u8; 32],
) -> pezpallet_revive::ContractResult<pezpallet_revive::ExecReturnValue, u128> {
...
/// matching the given pezpallet error variant. Use for the
...
/// pezpallet error) cannot silently keep the test green if the failure
/// surface changes.
fn assert_permit_dispatch_err<E>(
	result: pezpallet_revive::ContractResult<pezpallet_revive::ExecReturnValue, u128>,
	expected: E,
) where
	E: Into<pezsp_runtime::DispatchError>,
{
...
fn assert_no_contract_event_from(contract: H160) {
	let any = System::events().iter().any(|er| {
		matches!(
			&er.event,
			RuntimeEvent::Revive(pezpallet_revive::Event::ContractEmitted { contract: c, .. }) if *c == contract,
		)
	});
...
fn permit_setup(prefix: u16) -> PermitSetup {
	...
	let spender_addr =
		<Test as pezpallet_revive::Config>::AddressMapper::to_address(&spender_account);
...
fn permit_set_and_revoke(asset_index: u16) {
	...
	new_test_ext().execute_with(|| {
		...
		let deposit: u128 = <Test as pezpallet_assets::Config>::ApprovalDeposit::get();

		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&setup.asset_addr, &HARDHAT_ACCOUNT_0),
			U256::zero()
		);
...
/// `pezpallet_revive`'s contract events ARE rolled back by
...
fn permit_rollback_does_not_increment_nonce() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_permit_dispatch_err(result, pezpallet_assets::Error::<Test>::AssetNotLive);
...
/// But both pezpallet-assets entry points gate on `AssetStatus::Live` as
...
fn permit_rejects_when_owner_lacks_deposit_balance() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_permit_dispatch_err(result, pezpallet_balances::Error::<Test>::InsufficientBalance);
		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&setup.asset_addr, &HARDHAT_ACCOUNT_0),
			U256::zero(),
...
fn permit_signature_bound_to_verifying_contract(sign_prefix: u16, submit_prefix: u16) {
	new_test_ext().execute_with(|| {
		...
		{
			crate::pezpallet::Pezpallet::<Test>::insert_asset_mapping(&setup.asset_id)
		...
		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&asset_addr_signed, &HARDHAT_ACCOUNT_0),
			U256::zero()
		);
		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&asset_addr_submitted, &HARDHAT_ACCOUNT_0),
			U256::zero()
		);
	});
}
...
fn permit_rejects_zero_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&setup.asset_addr, &H160::zero()),
			U256::zero(),
		);
		assert_eq!(
			permit::Pezpallet::<Test>::nonce(&setup.asset_addr, &HARDHAT_ACCOUNT_0),
...
/// inadvertent flip to `<=`. Not covered at the pezpallet level.
#[test]
fn permit_accepts_deadline_at_current_timestamp() {
	...
	new_test_ext().execute_with(|| {
		...
		pezpallet_timestamp::Pezpallet::<Test>::set_timestamp(now_seconds * 1_000);
...
/// permit-pezpallet error variants the dispatcher maps to a revert string.
...
/// through pezframe_support storage writes that bypass pezpallet-revive's
...
fn permit_staticcall_is_rejected() {
	...
	new_test_ext().execute_with(|| {
		...
		let (init_code, _) = pezpallet_revive_fixtures::compile_module_with_type(
			"Caller",
			pezpallet_revive_fixtures::FixtureType::Solc,
		)
		.expect("Caller fixture must be compiled");
		let caller_addr = pezpallet_revive::Pezpallet::<Test>::bare_instantiate(
...
/// would not be caught by tests that read `permit::Pezpallet::nonce`
/// storage directly.
#[test]
fn nonces_via_precompile() {
	new_test_ext().execute_with(|| {
		...
		let read_nonce = |asset_addr: H160| -> AlloyU256 {
			...
			let bytes = pezpallet_revive::Pezpallet::<Test>::bare_call(
