...
use pezpallet_revive::{precompiles::TransactionLimits, Code, ExecConfig};
use pezsp_core::H160;
...
fn deposit_event_charges_data_byte_length() {
	use pezpallet_revive::precompiles::Token;
	...
	new_test_ext().execute_with(|| {
		...
		let expected = <() as pezpallet_assets::WeightInfo>::transfer().saturating_add(
...
/// `domain_separator()` (e.g. a lazy-init inside `pezpallet_assets::name()`), which would
...
fn domain_separator_is_staticcall_compatible(asset_index: u16) {
	new_test_ext().execute_with(|| {
		...
		let expected =
			permit::Pezpallet::<Test>::compute_domain_separator(&asset_addr, b"Static Token");
		assert_eq!(
			&ret.output[..],
			expected.as_bytes(),
...
fn transfer_and_transfer_from_revert_on_overflow() {
	...
	new_test_ext().execute_with(|| {
		...
		let assert_reverts_with = |caller: u64, data: Vec<u8>, label: &str| {
			let exec = pezpallet_revive::Pezpallet::<Test>::bare_call(
...
fn transfer_from_decrements_normally_after_max_approve(asset_index: u16) {
	...
	new_test_ext().execute_with(|| {
		...
		let spender_addr = <Test as pezpallet_revive::Config>::AddressMapper::to_address(&spender);
		let recipient_addr =
			<Test as pezpallet_revive::Config>::AddressMapper::to_address(&recipient);
