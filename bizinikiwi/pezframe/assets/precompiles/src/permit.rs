...
//! ERC20Permit pezpallet for signature-based approvals (EIP-2612).
//!
//! This pezpallet stores permit-related state (nonces) and provides EIP-712
...
use alloc::vec::Vec;
use pezframe_support::{pezpallet_prelude::*, traits::UnixTime};
...
///
/// TODO: Replace usages with `pezsp_core::ecdsa::is_signature_normalized` once
/// paritytech/pezkuwi-sdk#5841 lands.
...
pub mod pezpallet {
	...
	#[pezpallet::config]
	pub trait Config: pezframe_system::Config + pezpallet_timestamp::Config {
	...
	/// Error types for the permit pezpallet.
	#[pezpallet::error]
	pub enum Error<T> {
	...
	impl<T: Config> Pezpallet<T> {
		...
		///
		/// TODO: Replace with `pezsp_core::ecdsa::is_signature_normalized` once
		/// paritytech/pezkuwi-sdk#5841 lands.
		...
		fn do_verify_permit(
			...
			// gives us seconds regardless of pezpallet_timestamp's internal resolution
			// (which stores milliseconds, converted via `Duration::from_millis` in
			// pezpallet_timestamp's `UnixTime` implementation).
			let now_seconds = <pezpallet_timestamp::Pezpallet<T> as UnixTime>::now().as_secs();
