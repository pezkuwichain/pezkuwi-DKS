...
//! Migrations for `pezpallet-assets-precompiles`.

use crate::{foreign_assets::pezpallet, weights::WeightInfo};
...
/// Locations) and sequential u32 indices in `pezpallet_assets_precompiles` for all existing foreign
/// assets.
///
...
/// The pezpallet maintains a bidirectional mapping:
...
/// # Type Parameters
///
/// - `T`: The runtime configuration implementing both `pezpallet_assets::Config<I>` and
///   `pezpallet::Config`
/// - `I`: The pezpallet_assets instance identifier (e.g., `ForeignAssetsInstance`)
...
/// pub type Migrations = (
///     // ... other migrations ...
///     pezpallet_assets_precompiles::MigrateForeignAssetPrecompileMappings<Runtime, ForeignAssetsInstance, ()>,
/// );
/// ```
///
/// # Safety
...
where
	T: pezpallet_assets::Config<I>
		+ pezpallet::Config<ForeignAssetId = <T as pezpallet_assets::Config<I>>::AssetId>,
	I: 'static,
	W: WeightInfo,
{
	type Cursor = <T as pezpallet_assets::Config<I>>::AssetId;
	type Identifier = MigrationId<32>;

	fn id() -> Self::Identifier {
		MigrationId {
			pezpallet_id: *PRECOMPILE_MAPPINGS_MIGRATION_ID,
			version_from: 0,
			version_to: 1,
		}
	}
	...
	fn step(
		...
		loop {
			...
			if pezpallet::Pezpallet::<T>::asset_index_of(&asset_id).is_none() {
				match pezpallet::Pezpallet::<T>::insert_asset_mapping(&asset_id) {
	...
	fn pre_upgrade() -> Result<alloc::vec::Vec<u8>, pezsp_runtime::TryRuntimeError> {
		...
		for (asset_id, _) in pezpallet_assets::Asset::<T, I>::iter() {
			if pezpallet::Pezpallet::<T>::asset_index_of(&asset_id).is_none() {
	...
	fn post_upgrade(state: alloc::vec::Vec<u8>) -> Result<(), pezsp_runtime::TryRuntimeError> {
		...
		let unmapped_assets: alloc::vec::Vec<<T as pezpallet_assets::Config<I>>::AssetId> =
			Decode::decode(&mut &state[..])
				.map_err(|_| pezsp_runtime::TryRuntimeError::Other("Failed to decode state"))?;

		let mut migrated = 0u64;

		for asset_id in &unmapped_assets {
			// Check that a mapping now exists for this asset
			match pezpallet::Pezpallet::<T>::asset_index_of(asset_id) {
				Some(index) => {
					// Verify reverse mapping is consistent
					match pezpallet::Pezpallet::<T>::asset_id_of(index) {
				...
				None => {
					...
					return Err(pezsp_runtime::TryRuntimeError::Other("Asset not migrated"));
				},
			}
		}
	...
	W: WeightInfo,
{
	...
	fn peek_next_asset(
		maybe_last_key: Option<&<T as pezpallet_assets::Config<I>>::AssetId>,
	) -> Option<<T as pezpallet_assets::Config<I>>::AssetId> {
