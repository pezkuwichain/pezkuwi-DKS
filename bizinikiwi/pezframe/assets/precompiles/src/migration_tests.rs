...
use crate::{foreign_assets::pezpallet, migration::MigrateForeignAssetPrecompileMappings};
...
use pezpallet_assets::AssetDetails;
...
#[cfg(feature = "runtime-benchmarks")]
impl pezpallet_assets::BenchmarkHelper<Location, ()> for LocationBenchmarkHelper {
	fn create_asset_id_parameter(id: u32) -> Location {
		Location::new(1, [Junction::Teyrchain(id)])
	}
...
mod runtime {
	...
	pub type ForeignAssets = pezpallet_assets<Instance1>;
	#[runtime::pezpallet_index(22)]
	pub type AssetsPrecompiles = crate::foreign_assets;
}

pub type ForeignAssetsInstance = pezpallet_assets::Instance1;
...
impl pezpallet_assets::Config<ForeignAssetsInstance> for Test {
	...
	type StringLimit = pezframe_support::traits::ConstU32<50>;
	type Freezer = ();
	type Extra = ();
	type WeightInfo = ();
...
fn create_asset_details(
	owner: <Test as pezframe_system::Config>::AccountId,
) -> AssetDetails<
	<Test as pezpallet_assets::Config<ForeignAssetsInstance>>::Balance,
	<Test as pezframe_system::Config>::AccountId,
	<Test as pezpallet_balances::Config>::Balance,
> {
	AssetDetails {
		owner,
		issuer: owner,
		...
		sufficients: 0,
		approvals: 0,
		status: pezpallet_assets::AssetStatus::Live,
	}
}
...
fn migration_populates_precompile_mappings() {
	new_test_ext().execute_with(|| {
		...
		// Create foreign asset locations (simulating assets from sibling teyrchains)
		let sibling_para_1000_asset = Location::new(1, [Junction::Teyrchain(1000)]);
		let sibling_para_2000_asset = Location::new(1, [Junction::Teyrchain(2000)]);
		let sibling_para_3000_asset = Location::new(1, [Junction::Teyrchain(3000)]);
		...
		// Insert foreign assets directly into pezpallet_assets storage without creating precompile
		// mappings
		for asset_location in &test_assets {
			pezpallet_assets::Asset::<Test, ForeignAssetsInstance>::insert(
		...
		for asset_location in &test_assets {
			assert!(
				pezpallet_assets::Asset::<Test, ForeignAssetsInstance>::contains_key(
					asset_location
				),
				"Asset should exist in pezpallet_assets storage"
			);
		}
		...
		for asset_location in &test_assets {
			assert!(
				pezpallet::Pezpallet::<Test>::asset_index_of(asset_location).is_none(),
		...
		for asset_location in &test_assets {
			...
			let stored_index = pezpallet::Pezpallet::<Test>::asset_index_of(asset_location);
			assert!(
			...
			let stored_location = pezpallet::Pezpallet::<Test>::asset_id_of(index);
			assert_eq!(
				stored_location,
...
fn migration_skips_already_mapped_assets() {
	new_test_ext().execute_with(|| {
		...
		let asset_with_mapping = Location::new(1, [Junction::Teyrchain(1111)]);
		let asset_without_mapping = Location::new(1, [Junction::Teyrchain(2222)]);
		...
		let pre_mapped_index =
			pezpallet::Pezpallet::<Test>::insert_asset_mapping(&asset_with_mapping).unwrap();

		run_migration_to_completion();

		// The pre-mapped asset should keep its original index
		assert_eq!(
			pezpallet::Pezpallet::<Test>::asset_index_of(&asset_with_mapping),
			Some(pre_mapped_index),
			"Pre-existing mapping should be preserved"
		);
		assert_eq!(
			pezpallet::Pezpallet::<Test>::asset_id_of(pre_mapped_index),
		...
		assert!(
			pezpallet::Pezpallet::<Test>::asset_index_of(&asset_without_mapping).is_some(),
...
fn migration_respects_weight_limits() {
	new_test_ext().execute_with(|| {
		...
		let assets: Vec<Location> = (1000..1005u32)
			.map(|para_id| Location::new(1, [Junction::Teyrchain(para_id)]))
			.collect();
		...
		for asset_location in &assets {
			assert!(
				pezpallet::Pezpallet::<Test>::asset_index_of(asset_location).is_some(),
...
fn migration_id_is_consistent() {
	...
	assert_eq!(id1.pezpallet_id, id2.pezpallet_id, "Pezpallet ID should be consistent");
	...
	assert_eq!(
		&id1.pezpallet_id, b"foreign-asset-precompile-mapping",
		"Migration ID should have correct pezpallet_id"
	);
...
fn migration_assigns_sequential_indices() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(pezpallet::Pezpallet::<Test>::next_asset_index(), 0);

		run_migration_to_completion();

		// NextAssetIndex should be 5 after migrating 5 assets
		assert_eq!(pezpallet::Pezpallet::<Test>::next_asset_index(), 5);

		// Collect all assigned indices
		let mut assigned_indices: Vec<u32> = assets
			.iter()
			.map(|loc| pezpallet::Pezpallet::<Test>::asset_index_of(loc).unwrap())
			.collect();
