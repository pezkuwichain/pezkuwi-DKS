...
use pezpallet_revive::precompiles::H160;
...
fn nonce_starts_at_zero() {
	new_test_ext().execute_with(|| {
		...
		let nonce = permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner);
...
fn nonce_increments() {
	new_test_ext().execute_with(|| {
		...
		let nonce1 =
			permit::Pezpallet::<Test>::increment_nonce(&verifying_contract, &owner).unwrap();
		...
		let nonce_read = permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner);
...
fn nonces_are_independent_per_verifying_contract() {
	new_test_ext().execute_with(|| {
		...
		permit::Pezpallet::<Test>::increment_nonce(&contract_1, &owner).unwrap();
		permit::Pezpallet::<Test>::increment_nonce(&contract_1, &owner).unwrap();

		assert_eq!(permit::Pezpallet::<Test>::nonce(&contract_1, &owner), U256::from(2));
		assert_eq!(permit::Pezpallet::<Test>::nonce(&contract_2, &owner), U256::zero());
	});
}
...
fn nonces_are_independent_per_owner() {
	new_test_ext().execute_with(|| {
		...
		permit::Pezpallet::<Test>::increment_nonce(&verifying_contract, &owner1).unwrap();
		permit::Pezpallet::<Test>::increment_nonce(&verifying_contract, &owner1).unwrap();

		assert_eq!(permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner1), U256::from(2));
		assert_eq!(permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner2), U256::zero());
	});
}
...
fn domain_separator_is_computed() {
	new_test_ext().execute_with(|| {
		...
		let separator =
			permit::Pezpallet::<Test>::compute_domain_separator(&verifying_contract, name);
...
fn domain_separators_differ_per_verifying_contract() {
	new_test_ext().execute_with(|| {
		...
		let separator1 = permit::Pezpallet::<Test>::compute_domain_separator(&contract_1, name);
		let separator2 = permit::Pezpallet::<Test>::compute_domain_separator(&contract_2, name);
...
fn domain_separators_differ_per_token_name() {
	new_test_ext().execute_with(|| {
		...
		let separator1 =
			permit::Pezpallet::<Test>::compute_domain_separator(&verifying_contract, b"Token A");
		let separator2 =
			permit::Pezpallet::<Test>::compute_domain_separator(&verifying_contract, b"Token B");
...
fn permit_digest_is_deterministic() {
	new_test_ext().execute_with(|| {
		...
		let digest1 = permit::Pezpallet::<Test>::permit_digest(
			&verifying_contract,
			name,
			&owner,
			&spender,
		...
		let digest2 = permit::Pezpallet::<Test>::permit_digest(
			&verifying_contract,
			name,
			&owner,
			&spender,
...
fn ecrecover_with_valid_signature() {
	new_test_ext().execute_with(|| {
		...
		let message_hash = pezsp_io::hashing::keccak_256(b"test message");
		...
		let result = permit::Pezpallet::<Test>::ecrecover(&message_hash, v, &r, &s);
...
fn ecrecover_fails_with_invalid_v() {
	new_test_ext().execute_with(|| {
		...
		let result = permit::Pezpallet::<Test>::ecrecover(&digest, v, &r, &s);
		assert!(matches!(result, Err(permit::pezpallet::Error::<Test>::InvalidVValue)));
	});
}
...
fn ecrecover_rejects_high_s_value() {
	new_test_ext().execute_with(|| {
		...
		let result = permit::Pezpallet::<Test>::ecrecover(&digest, v, &r, &s);
		assert!(matches!(result, Err(permit::pezpallet::Error::<Test>::SignatureSValueTooHigh)));
	});
}
...
fn verify_permit_fails_with_expired_deadline() {
	new_test_ext().execute_with(|| {
		...
		let result = permit::Pezpallet::<Test>::verify_permit(
			&verifying_contract,
			name,
			&owner,
			&spender,
		...
		assert!(matches!(result, Err(permit::pezpallet::Error::<Test>::PermitExpired)));
	});
}
...
fn verify_permit_does_not_increment_nonce() {
	new_test_ext().execute_with(|| {
		...
		let initial_nonce = permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner);

		// Call verify_permit multiple times
		for _ in 0..3 {
			let _ = permit::Pezpallet::<Test>::verify_permit(
				&verifying_contract,
				name,
				&owner,
				&spender,
		...
		let final_nonce = permit::Pezpallet::<Test>::nonce(&verifying_contract, &owner);
...
fn use_permit_succeeds_with_valid_signature() {
	new_test_ext().execute_with(|| {
		...
		let initial_nonce = permit::Pezpallet::<Test>::nonce(&p.verifying_contract, &p.owner);
		assert_eq!(initial_nonce, U256::zero(), "initial nonce should be zero");

		// First use_permit should succeed
		let result = permit::Pezpallet::<Test>::use_permit(
		...
		let nonce_after = permit::Pezpallet::<Test>::nonce(&p.verifying_contract, &p.owner);
...
fn use_permit_rejects_replay_of_consumed_permit() {
	new_test_ext().execute_with(|| {
		...
		let first_result = permit::Pezpallet::<Test>::use_permit(
			&p.verifying_contract,
			p.name,
			&p.owner,
			&p.spender,
		...
		let nonce = permit::Pezpallet::<Test>::nonce(&p.verifying_contract, &p.owner);
		...
		let replay_result = permit::Pezpallet::<Test>::use_permit(
			&p.verifying_contract,
			p.name,
			&p.owner,
			&p.spender,
		...
		assert!(
			matches!(replay_result, Err(permit::pezpallet::Error::<Test>::SignerMismatch)),
		...
		let nonce_after_replay = permit::Pezpallet::<Test>::nonce(&p.verifying_contract, &p.owner);
		assert_eq!(
			nonce_after_replay,
			U256::one(),
...
fn use_permit_rejects_multiple_replay_attempts() {
	new_test_ext().execute_with(|| {
		...
		let final_nonce = permit::Pezpallet::<Test>::nonce(&p.verifying_contract, &p.owner);
		assert_eq!(
			final_nonce,
			U256::one(),
...
fn permit_typehash_is_correct() {
	let computed = pezsp_io::hashing::keccak_256(
