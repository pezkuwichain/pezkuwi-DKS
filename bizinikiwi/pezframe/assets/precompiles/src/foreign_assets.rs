...
use pezpallet_assets::AssetsCallback;

pub use pezpallet::*;

const LOG_TARGET: &str = "pezpallet_foreign_assets";

pub struct ForeignAssetId<T, I = ()>(PhantomData<(T, I)>);
impl<T: Config, I> AssetsCallback<T::AssetId, T::AccountId> for ForeignAssetId<T, I>
where
	T: Config<ForeignAssetId = T::AssetId> + pezpallet_assets::Config<I>,
	I: 'static,
{
	fn created(id: &T::AssetId, _: &T::AccountId) -> Result<(), ()> {
		Pezpallet::<T>::insert_asset_mapping(id).map(|_| ())
	}

	fn destroyed(id: &T::AssetId) -> Result<(), ()> {
		Pezpallet::<T>::remove_asset_mapping(id);
		Ok(())
	}
}
