...
//! Benchmarks for `pezpallet_assets_precompiles`.
//!
//! All benchmarks are registered under `foreign_assets::Pezpallet` so that a single
//! `pezframe-omni-bencher --pezpallet=pezpallet_assets_precompiles` run generates one
//! `weights.rs` containing every weight function.

#![cfg(feature = "runtime-benchmarks")]

use crate::{
	foreign_assets::pezpallet::{Config, Pezpallet},
...
#[benchmarks(
	where
		// Migration bounds
		T: pezpallet_assets::Config<T::AssetsInstance, AssetId = <T as Config>::ForeignAssetId>,
		// Permit bounds
		T: crate::permit::Config,
		<T as pezpallet_assets::Config<T::AssetsInstance>>::Balance: From<u32>,
		<T as pezpallet_assets::Config<T::AssetsInstance>>::AssetIdParameter: From<<T as pezpallet_assets::Config<T::AssetsInstance>>::AssetId>,
)]
mod benchmarks {
	use super::*;
	use pezpallet_assets::BenchmarkHelper;
	...
	fn migrate_foreign_asset_step() {
		...
		let _ = pezpallet_assets::Asset::<T, T::AssetsInstance>::clear(u32::MAX, None);

		// Create one asset in pezpallet_assets storage.
		let caller: T::AccountId = whitelisted_caller();
		let caller_lookup = <T as pezframe_system::Config>::Lookup::unlookup(caller);
		let asset_id_param = <T as pezpallet_assets::Config<T::AssetsInstance>>::BenchmarkHelper::create_asset_id_parameter(42);
		let asset_id: <T as pezpallet_assets::Config<T::AssetsInstance>>::AssetId =
			asset_id_param.clone().into();

		pezpallet_assets::Pezpallet::<T, T::AssetsInstance>::force_create(
		...
		Pezpallet::<T>::remove_asset_mapping(&asset_id);

		// Verify no precompile mapping exists yet.
		assert!(Pezpallet::<T>::asset_index_of(&asset_id).is_none());
		...
		assert!(Pezpallet::<T>::asset_index_of(&asset_id).is_some());
	...
	fn nonces() {
		...
		#[block]
		{
			result = crate::permit::Pezpallet::<T>::nonce(&verifying_contract, &owner);
		}
	...
	fn domain_separator() {
		...
		{
			result =
				crate::permit::Pezpallet::<T>::compute_domain_separator(&verifying_contract, name);
		}
		assert_ne!(result, pezsp_core::H256::zero());
	}
	...
	fn use_permit() {
		...
		// triggers `OnTimestampSet` callbacks (e.g. pezpallet_babe slot validation)
		// that require additional setup not relevant to this benchmark.
		let timestamp: <T as pezpallet_timestamp::Config>::Moment =
			1_704_067_200_000u64.try_into().unwrap_or_default();
		pezpallet_timestamp::Now::<T>::put(timestamp);

		// Compute EIP-712 digest using runtime's chain_id.
		let nonce = U256::zero();
		let digest = crate::permit::Pezpallet::<T>::permit_digest(
			&verifying_contract,
			TEST_TOKEN_NAME,
			&owner,
			&spender,
		...
		// Sign with Hardhat account #0 private key via pezsp_io host function.
		...
		let pub_key = pezsp_io::crypto::ecdsa_generate(
			key_type,
		...
		#[block]
		{
			crate::permit::Pezpallet::<T>::use_permit(
				&verifying_contract,
				TEST_TOKEN_NAME,
				&owner,
				&spender,
		...
		assert_eq!(crate::permit::Pezpallet::<T>::nonce(&verifying_contract, &owner), U256::one());
	}
