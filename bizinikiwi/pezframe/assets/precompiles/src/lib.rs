...
use pezframe_support::traits::fungibles::metadata::Inspect as MetadataInspect;
use pezpallet_assets::{weights::WeightInfo as _, Call, Config, TransferFlags};
...
use pezsp_runtime::traits::{UniqueSaturatedInto, Zero};
...
pub use foreign_assets::{pezpallet, pezpallet::Config as ForeignAssetsConfig, ForeignAssetId};
pub use migration::MigrateForeignAssetPrecompileMappings;
pub use permit::pezpallet::Config as PermitConfig;
...
where
	Runtime: pezpallet_assets::Config<Instance>
		+ pezpallet::Config<ForeignAssetId = <Runtime as pezpallet_assets::Config<Instance>>::AssetId>
		+ pezpallet_revive::Config,
{
	type AssetId = <Runtime as pezpallet_assets::Config<Instance>>::AssetId;
	fn asset_id_from_address(addr: &[u8; 20]) -> Result<Self::AssetId, Error> {
		let bytes: [u8; 4] = addr[0..4].try_into().expect("slice is 4 bytes; qed");
		let index = u32::from_be_bytes(bytes);
		pezpallet::Pezpallet::<Runtime>::asset_id_of(index)
...
where
	...
	Runtime: crate::Config<Instance> + pezpallet_revive::Config + permit::Config,
	...
	alloy::primitives::U256: TryFrom<<Runtime as Config<Instance>>::Balance>,
{
	...
	fn deposit_event(env: &mut impl Ext<T = Runtime>, event: IERC20Events) -> Result<(), Error> {
		...
		env.pezframe_meter_mut().charge_weight_token(RuntimeCosts::DepositEvent {
	...
	fn approve(
		...
		use pezframe_support::traits::fungibles::approvals::Inspect as ApprovalsInspect;
		...
		let current = pezpallet_assets::Pezpallet::<Runtime, Instance>::allowance(
			asset_id.clone(),
			&owner_account,
			&spender_account,
		);
		...
		if new_amount.is_zero() {
			if !current.is_zero() {
				// Revoke: use the pezpallet's cancel logic to remove the approval and
				// unreserve the deposit.
				pezpallet_assets::Pezpallet::<Runtime, Instance>::do_cancel_approval(
					&asset_id,
					&owner_account,
					&spender_account,
				)?;
	...
	pub(crate) fn permit(
		...
		let transaction_outcome = pezframe_support::storage::with_transaction(|| {
			let result = (|| {
				// Use the permit - this validates deadline, signature, and increments nonce
				permit::Pezpallet::<Runtime>::use_permit(
					&verifying_contract,
					&pezpallet_assets::Pezpallet::<Runtime, Instance>::name(asset_id.clone()),
					&owner_h160,
					&spender_h160,
					&value_bytes,
					&deadline_bytes,
				...
				.map_err(|e| {
					let msg = match e {
						permit::pezpallet::Error::PermitExpired => "Permit expired",
						permit::pezpallet::Error::InvalidSignature => "Invalid signature",
						permit::pezpallet::Error::SignerMismatch => "Signer does not match owner",
						permit::pezpallet::Error::SignatureSValueTooHigh => {
							"Signature s value too high (malleability)"
						},
						permit::pezpallet::Error::InvalidVValue => "Invalid signature v value",
						permit::pezpallet::Error::NonceOverflow => "Nonce overflow",
						permit::pezpallet::Error::InvalidOwner => "Invalid owner address",
						permit::pezpallet::Error::InvalidSpender => "Invalid spender address",
					};
				...
				use pezframe_support::traits::fungibles::approvals::Inspect as ApprovalsInspect;
				let owner_account =
					<Runtime as pezpallet_revive::Config>::AddressMapper::to_account_id(
						&owner_h160,
					);
				let spender_account =
					<Runtime as pezpallet_revive::Config>::AddressMapper::to_account_id(
			...
			match result {
				Ok(actual_weight) => {
					pezframe_support::storage::TransactionOutcome::Commit(Ok(actual_weight))
				},
				Err(e) => {
					log::trace!(target: pezframe_support::LOG_TARGET, "Call to permit failed: {e:?}");
					pezframe_support::storage::TransactionOutcome::Rollback(Err(e))
				},
			}
		});
	...
	fn nonces(
		...
		let owner_h160: H160 = call.owner.into_array().into();
		let nonce = permit::Pezpallet::<Runtime>::nonce(&verifying_contract, &owner_h160);

		// Convert pezsp_core::U256 to alloy U256
	...
	fn domain_separator(
		...
		let token_name = pezpallet_assets::Pezpallet::<Runtime, Instance>::name(asset_id);

		let separator = permit::Pezpallet::<Runtime>::compute_domain_separator(
			&verifying_contract,
			&token_name,
		);
	...
	fn name(
		...
		let metadata = pezpallet_assets::Pezpallet::<Runtime, Instance>::get_metadata(asset_id)
