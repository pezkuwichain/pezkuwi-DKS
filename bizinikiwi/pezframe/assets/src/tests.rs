...
fn freezer_should_work() {
	build_and_execute(|| {
		...
		// (https://github.com/pezkuwichain/pezkuwi-DKS/pull/4530/commits/2ab35354d86904c035b21a2229452841b79b0457)
...
fn balance_conversion_should_work() {
	build_and_execute(|| {
		use pezframe_support::traits::tokens::ConversionToAssetBalance;
...
fn querying_name_symbol_and_decimals_should_work() {
	build_and_execute(|| {
		use pezframe_support::traits::fungibles::metadata::Inspect;
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
		assert_ok!(Assets::force_set_metadata(
			RuntimeOrigin::root(),
			0,
...
fn querying_allowance_should_work() {
	build_and_execute(|| {
		use pezframe_support::traits::fungibles::approvals::{Inspect, Mutate};
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
		assert_ok!(Assets::mint(RuntimeOrigin::signed(1), 0, 1, 100));
		Balances::make_free_balance_be(&1, 2);
...
fn querying_roles_should_work() {
	build_and_execute(|| {
		use pezframe_support::traits::fungibles::roles::Inspect;
		assert_ok!(Assets::force_create(RuntimeOrigin::root(), 0, 1, true, 1));
