	...
	for IncreaseIssuanceWithEvent<T, I>
{
	fn handle(asset_id: <T as Config<I>>::AssetId, amount: <T as Config<I>>::Balance) {
		fungibles::IncreaseIssuance::<T::AccountId, Pezpallet<T, I>>::handle(
			asset_id.clone(),
			amount,
		);
		Pezpallet::<T, I>::deposit_event(Event::BurnedDebt { asset_id, amount });
	}
}
...
{
	fn handle(asset_id: <T as Config<I>>::AssetId, amount: <T as Config<I>>::Balance) {
		fungibles::DecreaseIssuance::<T::AccountId, Pezpallet<T, I>>::handle(
			asset_id.clone(),
			amount,
		);
		Pezpallet::<T, I>::deposit_event(Event::BurnedCredit { asset_id, amount });
	}
}
