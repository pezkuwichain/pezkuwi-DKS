...
benchmarks_instance_pallet! {
	...
	migration_v2_foreign_asset_set_reserve_weight {
		...
		Pezpallet::<T, I>::unchecked_update_reserves(asset_id, bounded_reserves).unwrap();
	}
	verify {
	...
	get_metadata {
		...
		let _ = Pezpallet::<T, I>::get_metadata(asset_id.clone().into());
	} verify {
		let metadata = Pezpallet::<T, I>::get_metadata(asset_id.into());
