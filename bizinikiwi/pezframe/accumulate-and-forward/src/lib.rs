...
//! # Accumulate-and-Forward Pezpallet
//!
//! Intercepts configurable token inflows (transaction fees, dust removal, coretime revenue) on
//! system teyrchains and gathers them in a local accumulation account for periodic forwarding
//! to a configurable destination.
//!
//! ## Usage
//!
//! - **Fees**: Use [`DealWithFeesSplit`] to split fees between accumulation and other handlers
//! - **Burns/Revenue**: Use the pezpallet as `OnUnbalanced<CreditOf>` handler (e.g., dust removal,
//!   coretime revenue)
//! Note: Direct calls to `pezpallet_balances::Pezpallet::burn()` extrinsic are not redirected to
...
use pezsp_runtime::{traits::BlockNumberProvider, Percent, Saturating};

pub use pezpallet::*;

/// Trait for forwarding accumulated funds to a configured destination.
///
/// Implementations carry all message-construction and dispatch logic, keeping this pezpallet
...
pub mod pezpallet {
	...
	pub trait Config: pezframe_system::Config {
		...
		/// The pezpallet ID used to derive the accumulation account.
		type PalletId: Get<PalletId>;

		/// The implementation responsible for forwarding accumulated funds to the destination.
		/// Message construction and dispatch logic lives here, keeping this pezpallet free of
		...
		/// Block number provider. Use `RelaychainDataProvider` on teyrchains so that
		/// `TransferPeriod` is expressed in relay chain blocks, keeping the cadence stable.
		type BlockNumberProvider: BlockNumberProvider;

		/// Weight information for the pezpallet's operations.
	...
	impl<T: Config> Pezpallet<T> {
		/// Get the accumulation account derived from the pezpallet ID.
		///
	...
	fn on_unbalanceds(mut fees_then_tips: impl Iterator<Item = CreditOf<T>>) {
		if let Some(fees) = fees_then_tips.next() {
			...
			if !accumulated_percent.is_zero() {
				<Pezpallet<T> as OnUnbalanced<_>>::on_unbalanced(split.0);
			}
...
/// forwarding.
///
/// For pezpallets still using the legacy `Currency` trait (e.g. `pezpallet_identity`), use
/// [`LegacyAdapter`] instead.
impl<T: Config> OnUnbalanced<CreditOf<T>> for Pezpallet<T> {
...
///
/// Cannot be implemented directly on `Pezpallet<T>` because the compiler cannot prove that
...
///
/// Will be removed once all consumer pezpallets migrate to fungible traits.
///
/// # Example
/// ```ignore
/// type Slashed = pezpallet_accumulate_and_forward::LegacyAdapter<Runtime, Balances>;
/// ```
