...
use pezsp_runtime::{BuildStorage, DispatchError, TokenError};
...
fn can_create_pool() {
	new_test_ext().execute_with(|| {
		let asset_account_deposit: u128 =
			<mock::Test as pezpallet_assets::Config<Instance1>>::AssetAccountDeposit::get();
		let user = 1;
...
fn can_add_liquidity() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(pezpallet_account, token_1.clone()), 10000);
		assert_eq!(balance(pezpallet_account, token_2.clone()), 10);
		...
		let pezpallet_account = <Test as Config>::PoolLocator::address(&pool_id).unwrap();
		assert_eq!(balance(pezpallet_account, token_1.clone()), 10000);
		assert_eq!(balance(pezpallet_account, token_3.clone()), 10);
...
fn add_tiny_liquidity_directly_to_pool_address() {
	new_test_ext().execute_with(|| {
		...
		// token_1.clone()
		let pezpallet_account =
			<Test as Config>::PoolLocator::address(&(token_1.clone(), token_2.clone())).unwrap();
		assert_ok!(Balances::force_set_balance(RuntimeOrigin::root(), pezpallet_account, 1000));
...
fn quote_price_returns_none_swap_fails() {
	new_test_ext().execute_with(|| {
		...
		assert!(Pezpallet::<Test>::get_amount_in(&1001, &reserve_in, &reserve_out).is_ok());
...
fn can_swap_with_native() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(pezpallet_account, token_1.clone()), liquidity1 - expect_receive);
		assert_eq!(balance(pezpallet_account, token_2.clone()), liquidity2 + input_amount);
	});
}
...
fn check_no_panic_when_try_swap_close_to_empty_pool() {
	new_test_ext().execute_with(|| {
		...
		let pezpallet_account = <Test as Config>::PoolLocator::address(&pool_id).unwrap();
		assert_eq!(balance(pezpallet_account, token_1.clone()), liquidity1);
		assert_eq!(balance(pezpallet_account, token_2.clone()), liquidity2);
		...
		assert_eq!(balance(pezpallet_account, token_1.clone()), 708);
		assert_eq!(balance(pezpallet_account, token_2.clone()), 15);
		...
		let token_1_left = balance(pezpallet_account, token_1.clone());
		let token_2_left = balance(pezpallet_account, token_2.clone());
...
fn can_swap_tokens_for_exact_tokens() {
	new_test_ext().execute_with(|| {
		...
		let pezpallet_account = <Test as Config>::PoolLocator::address(&pool_id).unwrap();
		let before1 = balance(pezpallet_account, token_1.clone()) + balance(user, token_1.clone());
		let before2 = balance(pezpallet_account, token_2.clone()) + balance(user, token_2.clone());
		...
		assert_eq!(balance(pezpallet_account, token_1.clone()), liquidity1 + expect_in);
		assert_eq!(balance(pezpallet_account, token_2.clone()), liquidity2 - exchange_out);
		...
		assert_eq!(
			before1,
			balance(pezpallet_account, token_1.clone()) + balance(user, token_1.clone())
		);
		assert_eq!(
			before2,
			balance(pezpallet_account, token_2.clone()) + balance(user, token_2.clone())
		);
	});
}
...
fn swap_exact_tokens_for_tokens_in_multi_hops() {
	new_test_ext().execute_with(|| {
		...
		let pezpallet_account1 = <Test as Config>::PoolLocator::address(&pool_id1).unwrap();
		let pezpallet_account2 = <Test as Config>::PoolLocator::address(&pool_id2).unwrap();

		assert_eq!(balance(user, token_1.clone()), base1 + ed - input_amount);
		assert_eq!(balance(pezpallet_account1, token_1.clone()), liquidity1 + input_amount);
		assert_eq!(balance(pezpallet_account1, token_2.clone()), liquidity2 - expect_out2);
		assert_eq!(balance(pezpallet_account2, token_2.clone()), liquidity2 + expect_out2);
		assert_eq!(balance(pezpallet_account2, token_3.clone()), liquidity3 - expect_out3);
...
fn swap_tokens_for_exact_tokens_in_multi_hops() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(pezpallet_account1, token_1.clone()), liquidity1 + expect_in1);
		assert_eq!(balance(pezpallet_account1, token_2.clone()), liquidity2 - expect_in2);
		assert_eq!(balance(pezpallet_account2, token_2.clone()), liquidity2 + expect_in2);
		assert_eq!(balance(pezpallet_account2, token_3.clone()), liquidity3 - exchange_out3);
...
fn new_test_ext_with_genesis_pools(
	...
	pezpallet_assets::GenesisConfig::<Test, Instance1> {
