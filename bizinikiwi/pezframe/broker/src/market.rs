...
//!
//! Contains the [`Market`] trait — an abstraction that allows `pezpallet-broker` to work with any
...
use pezframe_support::{weights::WeightMeter, Parameter};
...
pub trait Market<RelayBlockNumber, Balance, AccountId> {
	...
	///
	/// This function is called from the `on_initialize` hook by `pezpallet-broker`.
	///
	/// ### Parameters
...
///
/// When `pezpallet-broker` calls [`Market::tick`], it receives a list of [`TickAction`]s
...
/// relies on `pezpallet-broker` to execute them.
pub enum TickAction<AccountId, Balance, RelayBlockNumber> {
	...
	/// Process the auto renewals which are stored in `pezpallet-broker`.
	ProcessAutoRenewals {
	...
	///
	/// This allows `pezpallet-broker` to handle sale boundary transitions.
	SaleRotated {
