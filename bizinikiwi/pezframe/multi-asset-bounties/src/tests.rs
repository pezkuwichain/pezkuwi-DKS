...
use pezsp_runtime::{traits::Dispatchable, TokenError};

type UtilityCall = pezpallet_utility::Call<Test>;
...
fn fund_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(parent_bounty_id).unwrap(),
			Bounty {
				asset_kind,
				value,
				metadata,
		...
		assert_eq!(
			pezpallet_bounties::CuratorDeposit::<Test>::get(parent_bounty_id, None::<BountyIndex>),
			None
		);
	});
}
...
fn fund_bounty_fails() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// When/Then
		let invalid_metadata: <Test as pezframe_system::Config>::Hash = [1u8; 32].into();
		assert_noop!(
			Bounties::fund_bounty(
				RuntimeOrigin::root(),
				Box::new(asset_kind),
...
fn fund_child_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			pezpallet_bounties::ChildBounties::<Test>::get(s.parent_bounty_id, s.child_bounty_id)
				.unwrap(),
			ChildBounty {
		...
		assert_eq!(
			pezpallet_bounties::TotalChildBountiesPerParent::<Test>::get(s.parent_bounty_id),
			1
		);
		assert_eq!(pezpallet_bounties::ChildBountiesPerParent::<Test>::get(s.parent_bounty_id), 1);
		assert_eq!(
			pezpallet_bounties::ChildBountiesValuePerParent::<Test>::get(s.parent_bounty_id),
			s.child_value
		);
...
fn check_status_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(s.parent_bounty_id).unwrap(),
			Bounty {
		...
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(s.parent_bounty_id).unwrap().status,
		...
		assert_eq!(pezpallet_bounties::Bounties::<Test>::iter().count(), 4 - 1);
		assert_eq!(pezpallet_bounties::Bounties::<Test>::get(s.parent_bounty_id), None);
		assert_eq!(pezpallet_bounties::ChildBountiesPerParent::<Test>::get(s.parent_bounty_id), 0);
		assert_eq!(
			pezpallet_bounties::TotalChildBountiesPerParent::<Test>::get(s.parent_bounty_id),
			0
		);
		...
		assert_eq!(pezpallet_bounties::Bounties::<Test>::iter().count(), 6 - 1 - 1);
		assert_eq!(pezpallet_bounties::Bounties::<Test>::get(s.parent_bounty_id), None);
		assert_eq!(pezpallet_bounties::ChildBountiesPerParent::<Test>::get(s.parent_bounty_id), 0);
		assert_eq!(
			pezpallet_bounties::TotalChildBountiesPerParent::<Test>::get(s.parent_bounty_id),
		...
		assert_eq!(
			pezpallet_bounties::ChildBounties::<Test>::iter_prefix(s.parent_bounty_id).count(),
			1
		);
...
fn unprivileged_caller_cannot_unassign_active_child_curator_when_parent_not_active() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Balances::reserved_balance(&s.child_curator), s.child_curator_deposit);
		assert!(pezpallet_bounties::CuratorDeposit::<Test>::get(
			s.parent_bounty_id,
			Some(s.child_bounty_id)
		)
		.is_some());
...
fn multi_asset_bounty_accounts_differ_from_legacy_bounty_accounts() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Old derivation (what pezpallet-bounties uses with &str)
		...
		// Old derivation (what pezpallet-child-bounties uses with &str)
...
fn increase_value_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			pezpallet_bounties::Bounties::<Test>::get(s.parent_bounty_id).unwrap().value,
			new_value
		);
