...
pub mod pezpallet {
	...
	type DebtOf<T> =
		fungible::Debt<<T as pezframe_system::Config>::AccountId, <T as Config>::Currency>;
	type ReceiptRecordOf<T> =
		ReceiptRecord<<T as pezframe_system::Config>::AccountId, BlockNumberFor<T>, BalanceOf<T>>;
