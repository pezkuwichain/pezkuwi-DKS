...
fn bond_with_no_staked_value() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(pezpallet_balances::Holds::<Test>::get(&1)[0].amount, 5);
			...
			assert_ok!(Staking::withdraw_unbonded(RuntimeOrigin::signed(1), 0));
			assert!(Staking::ledger(1.into()).is_err());
			assert_eq!(pezpallet_balances::Holds::<Test>::get(&1).len(), 0);
		});
}
...
fn six_session_delay() {
	ExtBuilder::default().initialize_first_session(false).build_and_execute(|| {
		...
		// pezpallet-session is delaying session by one, thus the next session to plan is +2.
...
fn test_multi_page_payout_stakers_by_page() {
	...
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		let pre_payout_total_issuance = pezpallet_balances::TotalIssuance::<Test>::get();
		...
		for era in 0..15 {
			assert_eq!(ClaimedRewards::<Test>::get(era, &11), Vec::<pezsp_staking::Page>::new());
		}
...
fn offences_weight_calculated_correctly() {
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		let zero_offence_weight =
			<Test as pezframe_system::Config>::DbWeight::get().reads_writes(4, 1);
		assert_eq!(
		...
		let n_offence_unapplied_weight = <Test as pezframe_system::Config>::DbWeight::get()
			.reads_writes(4, 1) +
			<Test as pezframe_system::Config>::DbWeight::get().reads_writes(4, 5) +
			<Test as pezframe_system::Config>::DbWeight::get().reads_writes(2, 2);
		...
		let one_offence_unapplied_weight =
			<Test as pezframe_system::Config>::DbWeight::get().reads_writes(4, 1)
		 +
			<Test as pezframe_system::Config>::DbWeight::get().reads_writes(rw, rw)
			// One `slash_cost`
			+ <Test as pezframe_system::Config>::DbWeight::get().reads_writes(6, 5)
...
fn on_finalize_weight_is_nonzero() {
	ExtBuilder::default().build_and_execute(|| {
		let on_finalize_weight = <Test as pezframe_system::Config>::DbWeight::get().reads(1);
...
mod election_data_provider {
	...
	fn targets_2sec_block() {
		...
		while <Test as Config>::WeightInfo::get_npos_targets(validators).all_lt(Weight::from_parts(
			2u64 * pezframe_support::weights::constants::WEIGHT_REF_TIME_PER_SECOND,
			u64::MAX,
		)) {
			validators += 1;
		}
...
fn test_validator_exposure_is_backward_compatible_with_non_paged_rewards_payout() {
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		assert!(<Staking as pezsp_staking::StakingInterface>::is_exposed_in_era(&11, &1));
		// nominators are exposed
		for i in 10..15 {
			let who: AccountId = 1000 + i;
			assert!(<Staking as pezsp_staking::StakingInterface>::is_exposed_in_era(&who, &1));
		}
...
mod staking_interface {
	use pezframe_support::storage::with_storage_layer;
	use pezsp_staking::StakingInterface;
...
mod staking_unchecked {
	use pezsp_staking::{Stake, StakingInterface, StakingUnchecked};
...
mod ledger {
	use super::*;
	use pezsp_staking::StakingAccount;
...
mod getters {
	...
	use pezsp_staking::{EraIndex, Exposure, IndividualExposure, Page, SessionIndex};
...
mod hold_migration {
	...
	use pezsp_staking::{Stake, StakingInterface};
