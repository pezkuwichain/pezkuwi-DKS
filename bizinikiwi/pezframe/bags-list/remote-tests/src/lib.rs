...
pub trait RuntimeT<I: 'static>:
	pezpallet_staking_async::Config + pezpallet_bags_list::Config<I> + pezframe_system::Config
{
}
impl<
		I: 'static,
		T: pezpallet_staking_async::Config + pezpallet_bags_list::Config<I> + pezframe_system::Config,
	> RuntimeT<I> for T
{
}
...
pub fn display_and_check_bags<Runtime: RuntimeT<Instance1>>(
	...
	let min_nominator_bond = <pezpallet_staking_async::MinNominatorBond<Runtime>>::get();
		...
		<Runtime as pezpallet_bags_list::Config<Instance1>>::BagThresholds::get()
	{
		...
		for id in bag.std_iter().map(|node| node.std_id().clone()) {
			...
			let vote_weight_thresh_u64: u64 = (*vote_weight_thresh)
				...
				.map_err(|_| "runtime must configure score to at most u64 to use this test")
				.unwrap();
			let vote_weight_as_balance: pezpallet_staking_async::BalanceOf<Runtime> =
