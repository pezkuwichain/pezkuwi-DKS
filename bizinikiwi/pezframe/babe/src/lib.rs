...
pub use pezsp_consensus_babe::AuthorityId;
...
/// for epoch changes (e.g. pezpallet-session) is used.
				...
				if let Some(signature) = pre_digest.vrf_signature() {
					let randomness: Option<BabeRandomness> = Authorities::<T>::get()
						...
						.and_then(|(authority, _)| {
							...
							// down the runtime.
							debug_assert!({
								use pezsp_core::crypto::VrfPublic;
...
impl<T: Config> Pezpallet<T> {
	...
	/// manager logic like `pezpallet-session`.
	///
	...
	pub fn enact_epoch_change(
		...
		let epoch_index = pezsp_consensus_babe::epoch_index(
			CurrentSlot::<T>::get(),
			GenesisSlot::<T>::get(),
			T::EpochDuration::get(),
		);
	...
	pub fn current_epoch_start() -> Slot {
		pezsp_consensus_babe::epoch_start_slot(
			EpochIndex::<T>::get(),
			GenesisSlot::<T>::get(),
			T::EpochDuration::get(),
		)
	...
	pub fn next_epoch() -> Epoch {
		...
		let start_slot = pezsp_consensus_babe::epoch_start_slot(
			next_epoch_index,
			GenesisSlot::<T>::get(),
			T::EpochDuration::get(),
		);
...
pub mod migrations {
	...
	impl<T: BabePalletPrefix> pezframe_support::traits::StorageInstance for __OldNextEpochConfig<T> {
		fn pezpallet_prefix() -> &'static str {
			T::pezpallet_prefix()
		}
