...
//! [`pezframe_support::traits::EnqueueMessage::enqueue_message`]`(m, o)`.
//!
...
//! [`pezframe_support::traits::ProcessMessage::Origin`].
...
impl<T: Config> Pezpallet<T> {
	...
	fn process_message_payload(
		...
		let mut id = pezsp_io::hashing::blake2_256(message);
...
/// Provides a [`pezsp_core::Get`] to access the `MEL` of a [`codec::MaxEncodedLen`] type.
