...
pub mod pezpallet {
	...
	use pezframe_system::{self, pezpallet_prelude::*};
	use pezsp_core::{
	...
	impl<T: Config> MigrationTask<T> {
		...
		fn migrate_child(&mut self) -> Result<(), Error<T>> {
			use pezsp_io::default_child_storage as child_io;
			let (maybe_current_child, child_root) = match (&self.progress_child, &self.progress_top)
			{
				...
				_ => {
					...
					pezframe_support::defensive!("cannot migrate child key.");
					return Ok(());
				},
			};
			...
			let maybe_current_top = match &self.progress_top {
				Progress::LastKey(last_top) => {
					let maybe_top: Option<BoundedVec<u8, T::MaxKeyLen>> =
						if let Some(next) = pezsp_io::storage::next_key(last_top) {
							Some(next.try_into().map_err(|_| Error::<T>::KeyTooLong)?)
						} else {
							None
						};
				...
				Progress::Complete => {
					// defensive: there must be an ongoing top migration.
					pezframe_support::defensive!("cannot migrate top key.");
					return Ok(());
				},
			};

			if let Some(current_top) = maybe_current_top.as_ref() {
				let added_size = if let Some(data) = pezsp_io::storage::get(current_top) {
					pezsp_io::storage::set(current_top, &data);
					data.len() as u32
				} else {
					Zero::zero()
				};
	...
	pub trait Config: pezframe_system::Config {
		...
		/// A [`pezframe_support::storage::StorageNMap`] for example can have an arbitrary number of
		...
		/// - [`pezframe_support::storage::StorageValue`]: 32 byte
		/// - [`pezframe_support::storage::StorageMap`]: 64 byte
	...
	impl<T: Config> Pezpallet<T> {
		...
		pub fn migrate_custom_top(
			...
			for key in &keys {
				if let Some(data) = pezsp_io::storage::get(key) {
					dyn_size = dyn_size.saturating_add(data.len() as u32);
					pezsp_io::storage::set(key, &data);
				}
			}
	...
	impl<T: Config> Pezpallet<T> {
		...
		fn dynamic_weight(items: u32, size: u32) -> pezframe_support::pezpallet_prelude::Weight {
			...
			<T as pezframe_system::Config>::DbWeight::get()
				.reads_writes(1, 1)
		...
		fn transform_child_key(root: &Vec<u8>) -> Option<&[u8]> {
			use pezsp_core::storage::{ChildType, PrefixedStorageKey};
...
mod benchmarks {
	...
	mod inner_benchmarks {
		...
		fn continue_migrate() -> Result<(), BenchmarkError> {
			...
			#[extrinsic_call]
			_(
				pezframe_system::RawOrigin::Signed(caller.clone()),
				null,
				0,
		...
		fn migrate_custom_top_success() -> Result<(), BenchmarkError> {
			let null = MigrationLimits::default();
			let caller: T::AccountId = pezframe_benchmarking::whitelisted_caller();
			let stash = set_balance_for_deposit::<T>(&caller, null.item);
			#[extrinsic_call]
			migrate_custom_top(
				pezframe_system::RawOrigin::Signed(caller.clone()),
		...
		fn migrate_custom_top_fail() -> Result<(), BenchmarkError> {
			...
			// migrated.
			pezsp_io::storage::set(b"foo", vec![1u8; 33].as_ref());
			#[block]
			{
				assert!(StateTrieMigration::<T>::migrate_custom_top(
					pezframe_system::RawOrigin::Signed(caller.clone()).into(),
					vec![b"foo".to_vec()],
					1,
				)
				.is_ok());
		...
		fn migrate_custom_child_fail() -> Result<(), BenchmarkError> {
			...
			// for tests, we need to make sure there is _something_ in storage that is being
			// migrated.
			pezsp_io::default_child_storage::set(b"top", b"foo", vec![1u8; 33].as_ref());
		...
		fn process_top_key(v: Linear<1, { 4 * 1024 * 1024 }>) -> Result<(), BenchmarkError> {
			...
			#[block]
			{
				let data = pezsp_io::storage::get(KEY).unwrap();
				pezsp_io::storage::set(KEY, &data);
				let _next = pezsp_io::storage::next_key(KEY);
		...
		impl_benchmark_test_suite!(
			StateTrieMigration,
			crate::mock::new_test_ext(pezsp_runtime::StateVersion::V0, true, None, None),
			crate::mock::Test
		);
	}
}
...
mod mock {
	use super::*;
	use crate as pezpallet_state_trie_migration;
	use alloc::{vec, vec::Vec};
	use pezframe_support::{derive_impl, parameter_types, traits::Hooks, weights::Weight};
	use pezframe_system::{EnsureRoot, EnsureSigned};
	use pezsp_core::{
		storage::{ChildInfo, StateVersion},
		H256,
	};
	use pezsp_runtime::{traits::Header as _, BuildStorage, StorageChild};
	...
	pezframe_support::construct_runtime!(
		pub enum Test
		{
			...
			StateTrieMigration: pezpallet_state_trie_migration,
		}
	);
	...
	impl pezpallet_state_trie_migration::Config for Test {
	...
	pub fn new_test_ext(
		...
		let minimum_size = pezsp_core::storage::TRIE_VALUE_NODE_THRESHOLD as usize + 1;
		let mut custom_storage = pezsp_core::storage::Storage {
			top: vec![
...
mod test {
	...
	use pezsp_runtime::{bounded_vec, traits::Bounded, StateVersion};
	...
	fn halts_if_top_key_too_long() {
		...
		new_test_ext(StateVersion::V0, true, Some(bad_top_keys), None).execute_with(|| {
			...
			pezframe_support::assert_ok!(StateTrieMigration::continue_migrate(
	...
	fn signed_migrate_works() {
		new_test_ext(StateVersion::V0, true, None, None).execute_with(|| {
			...
			pezframe_support::assert_err!(
				StateTrieMigration::continue_migrate(
					RuntimeOrigin::signed(1),
					MigrationLimits { item: 5, size: pezsp_runtime::traits::Bounded::max_value() },
					Bounded::max_value(),
					MigrationProcess::<Test>::get()
				),
			...
			pezframe_support::assert_err_ignore_postinfo!(
				StateTrieMigration::continue_migrate(
					RuntimeOrigin::signed(1),
					MigrationLimits { item: 5, size: 100 },
					100,
	...
	fn custom_migrate_top_works() {
		let correct_witness = 3 + pezsp_core::storage::TRIE_VALUE_NODE_THRESHOLD * 3 + 1 + 2 + 3;
		new_test_ext(StateVersion::V0, true, None, None).execute_with(|| {
			pezframe_support::assert_ok!(StateTrieMigration::migrate_custom_top(
				RuntimeOrigin::signed(1),
	...
	fn custom_migrate_child_works() {
		new_test_ext(StateVersion::V0, true, None, None).execute_with(|| {
			pezframe_support::assert_ok!(StateTrieMigration::migrate_custom_child(
				RuntimeOrigin::signed(1),
...
pub(crate) mod remote_tests {
		...
		Block::Header: serde::de::DeserializeOwned,
	{
		let mut ext = remote_externalities::Builder::<Block>::new()
			.mode(mode)
			.overwrite_state_version(pezsp_core::storage::StateVersion::V0)
			.build()
			.await
			.unwrap();
		...
		ext.state_version = pezsp_core::storage::StateVersion::V1;

		let status =
			bizinikiwi_state_trie_migration_rpc::migration_status(&ext.as_backend()).unwrap();
		assert!(
...
mod remote_tests_local {
	...
	type Extrinsic = pezsp_runtime::testing::TestXt<MockCall, ()>;
