...
//! ## Overall idea
//!
//! `pezpallet_election_provider_multi_phase` provides the basic ability for NPoS solutions to be
...
use pezsp_npos_elections::{EvaluateSupport, VoteWeight};
use pezsp_runtime::{
...
pub struct Continue<T>(pezsp_std::marker::PhantomData<T>);
...
pub struct IfSolutionQueuedElse<T, Queued, NotQueued>(
	pezsp_std::marker::PhantomData<(T, Queued, NotQueued)>,
);

/// A `Get` impl for `Phase::Done`
pub struct GetDone<T>(pezsp_std::marker::PhantomData<T>);
...
pub struct GetSigned<T>(pezsp_std::marker::PhantomData<T>);
...
pub mod pezpallet {
	...
	pub(crate) struct Snapshot<T>(pezsp_std::marker::PhantomData<T>);
	impl<T: Config> Snapshot<T> {
		...
		fn write_storage_with_pre_allocate<E: Encode>(key: &[u8], data: E) -> T::Hash {
			...
			pezsp_io::storage::set(key, &buffer);
...
impl<T: Config> Pezpallet<T> {
	...
	fn fallback_for_page(page: PageIndex) -> Result<BoundedSupportsOf<Self>, ElectionError<T>> {
		use pezframe_election_provider_support::InstantElectionProvider;
	...
	BlockNumberFor<T>: From<u32>,
{
	...
	pub(crate) fn roll_until_before_matches(criteria: impl FnOnce() -> bool + Copy) {
		use pezframe_support::storage::TransactionOutcome;
		loop {
	...
	fn funded_account(seed: &'static str, index: u32) -> T::AccountId {
		use pezframe_benchmarking::whitelist;
		use pezframe_support::traits::fungible::{Inspect, Mutate};
		let who: T::AccountId = pezframe_benchmarking::account(seed, index, 777);
		whitelist!(who);
