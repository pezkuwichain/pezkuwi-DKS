...
use pezframe_support::{
	...
	pezpallet_prelude::{StorageDoubleMap, ValueQuery, *},
	traits::{
		tokens::{
...
#[cfg_attr(test, derive(pezframe_support::PartialEqNoBound, pezframe_support::EqNoBound))]
...
impl<T: Config> crate::types::SignedInterface for Pezpallet<T> {
			...
			let did_eject = if let Some(_) = sorted_scores.iter().position(|(x, _)| x == who) {
				...
				if sorted_scores.is_full() {
					...
					if insert_idx > remove_idx {
						...
						pezsp_std::mem::swap(&mut sorted_scores[remove_idx], &mut record);
						// slicing safety note:
