...
#[derive(
	pezframe_support::DebugNoBound, pezframe_support::EqNoBound, pezframe_support::PartialEqNoBound,
)]
...
impl<T: MinerConfig> From<pezsp_npos_elections::Error> for MinerError<T> {
...
pub type PageSupportsOfMiner<T> = pezframe_election_provider_support::BoundedSupports<
...
impl<T: MinerConfig> pezframe_support::traits::Get<u32> for MaxWinnersFinal<T> {
	fn get() -> u32 {
...
pub type FullSupportsOfMiner<T> = pezframe_election_provider_support::BoundedSupports<
	<T as MinerConfig>::AccountId,
	MaxWinnersFinal<T>,
...
pub struct OffchainWorkerMiner<T: Config>(pezsp_std::marker::PhantomData<T>);
...
impl<T: Config> OffchainWorkerMiner<T> {
	...
	fn submit_call(call: Call<T>) -> Result<(), OffchainMinerError<T>> {
		...
		pezframe_system::offchain::SubmitTransaction::<T, Call<T>>::submit_transaction(xt)
			.map(|_| {
				sublog!(
					debug,
					"unsigned::ocw-miner",
...
mod trimming {
	...
	use pezframe_election_provider_support::TryFromUnboundedPagedSupports;
