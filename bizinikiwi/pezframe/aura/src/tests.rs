...
use pezsp_runtime::{Digest, DigestItem, TryRuntimeError};
...
fn try_state_validates_timestamp_slot_consistency() {
	build_ext(vec![0, 1, 2, 3]).execute_with(|| {
		...
		pezpallet_timestamp::Now::<Test>::put(12u64);
		assert_eq!(
			Aura::do_try_state(),
...
fn integrity_test_passes_with_valid_config() {
	...
	build_ext(vec![0, 1, 2, 3]).execute_with(|| {
		...
		<Aura as Hooks<pezframe_system::pezpallet_prelude::BlockNumberFor<Test>>>::integrity_test();
	});
}
...
fn increase_slot_duration() {
	build_ext(vec![0, 1, 2, 3]).execute_with(|| {
		...
		pezpallet_timestamp::Now::<Test>::put(100u64);
		pezpallet::CurrentSlot::<Test>::put(Slot::from(50u64));
		...
		assert_eq!(u64::from(pezpallet::CurrentSlot::<Test>::get()), 25);
		...
		assert_eq!(pezpallet::CurrentSlot::<Test>::get(), slot);
...
fn decrease_slot_duration() {
	build_ext(vec![0, 1, 2, 3]).execute_with(|| {
		...
		pezpallet_timestamp::Now::<Test>::put(100u64);
		pezpallet::CurrentSlot::<Test>::put(Slot::from(25u64));
		...
		assert_eq!(u64::from(pezpallet::CurrentSlot::<Test>::get()), 50);
