...
pub mod pezpallet {
	...
	impl<T: Config<I>, I: 'static> Hooks<SystemBlockNumberFor<T>> for Pezpallet<T, I> {
		...
		fn try_state(_: SystemBlockNumberFor<T>) -> Result<(), pezsp_runtime::TryRuntimeError> {
			Self::do_try_state()?;
			Ok(())
		}
	}
...
impl<T: Config<I>, I: 'static> Pezpallet<T, I> {
	...
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_proposals() -> Result<(), pezsp_runtime::TryRuntimeError> {
	...
	/// > spend.valid_from.
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_spends() -> Result<(), pezsp_runtime::TryRuntimeError> {
...
impl<R> pezsp_runtime::traits::TypedGet for TreasuryAccountId<R>
where
	R: crate::Config,
{
