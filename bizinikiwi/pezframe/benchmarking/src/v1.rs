...
macro_rules! to_origin {
	...
	($origin:expr, $origin_type:ty) => {
		<<T as pezframe_system::Config>::RuntimeOrigin as From<$origin_type>>::from($origin)
	};
}
			...
			where T: pezframe_system::Config, $( $where_clause )*
		{
			...
			fn run_benchmark(
				...
				let whitelisted_caller_key =
					<pezframe_system::Account::<T> as $crate::__private::storage::StorageMap<_,_>>::hashed_key_for(
...
/// ```
///
/// When called inside the `benchmarks` macro of the `pezpallet_example_basic` as
///
/// ```rust,ignore
/// benchmarks! {
...
/// `pezsp_io::TestExternalities`, or some other type with a similar interface.
///
...
///
/// ```ignore
/// add_benchmark!(params, batches, pezpallet_balances, Balances);
/// add_benchmark!(params, batches, pezpallet_session, SessionBench::<Runtime>);
/// add_benchmark!(params, batches, pezframe_system, SystemBench::<Runtime>);
/// ...
/// ```
///
...
/// add_benchmark!(params, batches, pezpallet_collective, Council); // pezpallet_collective_council.rs
/// add_benchmark!(params, batches, pezpallet_collective, TechnicalCommittee); // pezpallet_collective_technical_committee.rs
/// ```
///
...
/// add_benchmark!(params, batches, pezpallet_collective, Council2); // pezpallet_collective_council_2.rs
/// ```
#[macro_export]
macro_rules! add_benchmark {
	( $params:ident, $batches:ident, $name:path, $location:ty ) => {
		let pezpallet_string = stringify!($name).replace(" ", "").into_bytes();
...
macro_rules! list_benchmark {
	( $list:ident, $extra:ident, $name:path, $location:ty ) => {
		...
		$list.push(pezpallet_benchmarks)
	};
}
