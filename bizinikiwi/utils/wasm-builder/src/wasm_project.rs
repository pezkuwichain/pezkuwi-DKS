	...
	if !module.custom_sections().any(|cs| cs.name() == "runtime_version") {
		println!(
			"Couldn't find the `runtime_version` section. \
				  Please ensure that you are using the `pezsp_version::runtime_version` attribute macro!"
		);
		process::exit(1);
	}
}
...
fn build_bloaty_blob(
	...
	rustflags.push_str(" --cfg bizinikiwi_runtime ");
...
fn try_compress_blob(compact_blob_path: &Path, out_name: &str) -> Option<WasmBinary> {
	use pezsp_maybe_compressed_blob::CODE_BLOB_BOMB_LIMIT;
	...
	if let Some(compressed) =
		pezsp_maybe_compressed_blob::compress_strongly(&data, CODE_BLOB_BOMB_LIMIT)
	{
