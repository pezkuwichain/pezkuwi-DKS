...
/// Shared body of the threshold generators. Takes the two things the runtime is needed
/// for — its spec name and its existential vote weight — so the same code serves both
/// the classic staking pallet and staking-async.
fn generate_thresholds_inner(
	spec_name: impl core::fmt::Display,
	existential_weight: VoteWeight,
	n_bags: usize,
	output: &Path,
	total_issuance: u128,
	...
	writeln!(buf, "//! for the {} runtime.", spec_name,)?;
...
/// Generate bag thresholds for a runtime using the classic `pezpallet_staking`.
pub fn generate_thresholds<T: pezpallet_staking::Config>(
	...
	generate_thresholds_inner(
		<T as pezframe_system::Config>::Version::get().spec_name,
		existential_weight::<T>(total_issuance, minimum_balance),
		n_bags,
		output,
		total_issuance,
		minimum_balance,
...
/// Same, for a runtime on `pezpallet_staking_async` — which is where staking lives once it
/// has moved to the Asset Hub. The two pallets expose the same `CurrencyToVote`.
pub fn generate_thresholds_async<T: pezpallet_staking_async::Config>(
	n_bags: usize,
	output: &Path,
	total_issuance: u128,
	minimum_balance: u128,
	...
	use pezsp_staking::currency_to_vote::CurrencyToVote;
	let existential_weight = <T as pezpallet_staking_async::Config>::CurrencyToVote::to_vote(
		minimum_balance.try_into().map_err(|_| "bad minimum_balance").unwrap(),
		total_issuance.try_into().map_err(|_| "bad total_issuance").unwrap(),
	);
	generate_thresholds_inner(
		<T as pezframe_system::Config>::Version::get().spec_name,
		existential_weight,
