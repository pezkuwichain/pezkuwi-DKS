...
//!
//! An equivalent of `pezsp_io::TestExternalities` that can load its state from a remote substrate
	...
	B::Header: DeserializeOwned,
{
	...
	async fn init_remote_client(&mut self) -> Result<()> {
		...
		online_config.pezpallets.iter().for_each(|p| {
			online_config
				.hashed_prefixes
			...
			.count() == 0
		{
			info!(
				target: LOG_TARGET,
				"since no prefix is filtered, the data for all pezpallets will be downloaded"
			);
...
mod remote_tests {
	use super::test_prelude::*;
	use pezframe_support::storage::KeyPrefixIterator;
	...
	async fn child_keys_are_loaded() {
		...
		let child_info = pezsp_core::storage::ChildInfo::new_default(b"test_child");
	...
	async fn can_build_few_pallet() {
		...
		Builder::<Block>::new()
			.mode(Mode::Online(OnlineConfig {
				transport_uris: vec![endpoint().clone()],
				pezpallets: vec!["Proxy".to_owned(), "Multisig".to_owned()],
				child_trie: false,
				..Default::default()
			}))
			.build()
	...
	async fn can_create_child_snapshot() {
		...
		Builder::<Block>::new()
			.mode(Mode::Online(OnlineConfig {
				transport_uris: vec![endpoint().clone()],
				state_snapshot: Some(SnapshotConfig::new(CACHE)),
				pezpallets: vec!["Crowdloan".to_owned()],
				child_trie: true,
				..Default::default()
			}))
			.build()
	...
	async fn can_build_big_pallet() {
		...
		Builder::<Block>::new()
			.mode(Mode::Online(OnlineConfig {
				transport_uris: vec![endpoint().clone()],
				pezpallets: vec!["Staking".to_owned()],
				child_trie: false,
				..Default::default()
			}))
			.build()
	...
	async fn bridge_hub_pezkuwi_storage_root_matches() {
		init_logger();

		// Use multiple RPC providers for load distribution
		let endpoints = vec![
			"wss://bridge-hub-pezkuwi-rpc.n.dwellir.com",
	...
	async fn builder_fails_with_invalid_transport_uris() {
		...
		let result = Builder::<Block>::new()
			.mode(Mode::Online(OnlineConfig {
				transport_uris: vec!["http://try-runtime.pezkuwichain.io:443".to_string()],
				pezpallets: vec!["Proxy".to_owned()],
				..Default::default()
			}))
			.build()
		...
		let result = Builder::<Block>::new()
			.mode(Mode::Online(OnlineConfig {
				transport_uris: vec![
					"http://try-runtime.pezkuwichain.io:443".to_string(),
					"https://try-runtime.pezkuwichain.io:443".to_string(),
					"garbage".to_string(),
				],
				pezpallets: vec!["Proxy".to_owned()],
				..Default::default()
