...
mod tests {
	...
	async fn cannot_init_client_with_invalid_uri() {
		...
		assert!(Client::new("http://try-runtime.pezkuwichain.io:443").await.is_none());
		assert!(Client::create_ws_client("http://try-runtime.pezkuwichain.io:443")
			.await
			.is_err());

		assert!(Client::new("https://try-runtime.pezkuwichain.io:443").await.is_none());
		assert!(Client::create_ws_client("https://try-runtime.pezkuwichain.io:443")
			.await
			.is_err());

		// Invalid URIs/garbage
		assert!(Client::new("wsss://try-runtime.pezkuwichain.io:443").await.is_none());
		assert!(Client::create_ws_client("wsss://try-runtime.pezkuwichain.io:443")
			.await
			.is_err());
