...
use pezsp_runtime::{traits::Block as BlockT, StateVersion};
use std::{
	fs,
	path::{Path, PathBuf},
};
...
pub(crate) const DEFAULT_WS_ENDPOINT: &str = "wss://try-runtime.pezkuwichain.io:443";
...
pub struct OnlineConfig<H> {
	...
	/// The pezpallets to scrape. These values are hashed and added to `hashed_prefix`.
