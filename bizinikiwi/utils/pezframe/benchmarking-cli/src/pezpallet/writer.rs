...
pub(crate) fn process_storage_results(
	...
	let skip_storage_info = StorageInfo {
		pezpallet_name: b"Skipped".to_vec(),
	...
	let benchmark_override = StorageInfo {
		pezpallet_name: b"Benchmark".to_vec(),
		...
		for (key, reads, writes, whitelisted) in &result.keys {
			...
			let override_pov_mode = match key_info {
				Some(_) => {
					...
					pov_modes.get(&(pezpallet_name.clone(), storage_name.clone())).or(
						// .. or for the storage prefix?
						pov_modes.get(&(pezpallet_name.clone(), "ALL".to_string())).or(
			...
			if !is_prefix_identified {
				match key_info {
					Some(_) => {
						let comment = format!(
							"Storage: `{}::{}` (r:{} w:{})",
							pezpallet_name, storage_name, reads, writes,
						);
				...
				match key_info {
					Some(key_info) => {
						match worst_case_pov(
							...
							Some(new_pov) => {
								let comment = format!(
									"Proof: `{pezpallet_name}::{storage_name}` (`max_values`: {:?}, `max_size`: {:?}, added: {}, mode: `{:?}`)",
									key_info.max_values,
									key_info.max_size,
									new_pov,
									used_pov_mode,
							...
							None => {
								let comment = format!(
									...
									pezpallet_name, storage_name, key_info.max_values, key_info.max_size,
									used_pov_mode,
								);
								comments.push(comment);
							},
...
mod test {
	...
	fn test_storage_info() -> Vec<StorageInfo> {
		vec![StorageInfo {
			pezpallet_name: b"bounded".to_vec(),
	...
	fn pov_mode_mel_constant_works() {
		...
		let storage_info = vec![StorageInfo {
			pezpallet_name: b"scheduler".to_vec(),
